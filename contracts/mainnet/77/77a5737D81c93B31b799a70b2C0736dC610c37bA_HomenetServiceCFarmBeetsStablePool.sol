...
abstract contract HomenetService is IMCeler {
        ...
        returns (uint256 fee)
    {
        ...
        bytes32 rewardMessageId = keccak256(abi.encode(3, block.timestamp, block.number, address(this), block.chainid, rewardMessageNonce));

        rewardMessageNonce++;

        bytes memory outgoingMessage = Serializer.createRewardMessage(rewardMessageId, rewardAmount);
...
contract HomenetServiceCFarmBeetsStablePool is HomenetService {
    ...
    modifier onlyQuoteToken(address quoteTokenBridged) {
        require(quoteTokenBridged == quoteToken, "QUOTE_TOKEN");
        _;
    }
    ...
    function investImplementation(address quoteTokenInvested, uint256 bridgedQuoteTokenAmount, QueueEntry[] memory incomingQueue)
        internal
        onlyQuoteToken(quoteTokenInvested)
        returns (QueueEntry[] memory outgoingQueue)
    {
        require(bridgedQuoteTokenAmount > 0, "ZERO0");
        require(IERC20(quoteToken).balanceOf(address(this)) >= bridgedQuoteTokenAmount, "EMPTY_BRIDGED");
    ...
    function onMessage(bytes memory incomingMessage)
        ...
        internal
    {
        (bytes32 roundtripId, QueueEntry[] memory incomingQueue) = Serializer.parseWithdrawMessage(incomingMessage);

        (QueueEntry[] memory outgoingQueue, uint256 quoteTokenAmount) = withdrawImplementation(incomingQueue);

        bytes memory outgoingMessage = Serializer.createWithdrawResponseMessage(roundtripId, quoteTokenAmount, outgoingQueue);
        sendMessageWithTransfer(quoteToken, quoteTokenAmount, outgoingMessage);
    }

    function onMessageWithTransfer(bytes memory incomingMessage, address token, uint256 tokenAmount)
        override
        internal
    {
        require(tokenAmount > 0, "ZERO0");

        (bytes32 roundtripId, QueueEntry[] memory incomingQueue) = Serializer.parseInvestMessage(incomingMessage);

        QueueEntry[] memory outgoingQueue = investImplementation(token, tokenAmount, incomingQueue);

        bytes memory outgoingMessage = Serializer.createInvestResponseMessage(roundtripId, outgoingQueue);
