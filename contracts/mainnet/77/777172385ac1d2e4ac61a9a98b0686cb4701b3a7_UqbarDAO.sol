...
contract UqbarDAO is Ownable {
  ...
  */
  ...
  mapping(uint256 => mapping(address => bool)) public votes;  // map atTime to votes
  ...
  event Proposal(uint256 proposedAtTime, uint256 newTotalSupply);
  event Voter(address voter, uint256 proposalId, Vote vote);
  event Inflation(uint quorumVotesNeeded, uint256 proposedAtTime, uint256 newSupply);
  ...
  function newInflationProposal(uint256 _newTotalSupply) external onlyOwner {
    ...
    require(block.timestamp != proposal.atTime, "Wait 1 block before submitting a new proposal");

    // overwrite current proposal
    proposal.atTime = block.timestamp;
    proposal.newTotalSupply = _newTotalSupply;
    proposal.aye = 0;
    proposal.nay = 0;
    proposal.open = true;

    emit Proposal(block.timestamp, _newTotalSupply);
  }
  ...
  function vote(Vote _vote) external {
    ...
    require(block.timestamp < proposal.atTime + votingPeriod, "Voting period elapsed");
    require(uqAlloc[msg.sender] > 0, "Must own Uqbar allocation to vote");
    require(!votes[proposal.atTime][msg.sender], "Must not have voted");

    votes[proposal.atTime][msg.sender] = true;
  ...
  function executeProposal() external onlyOwner {
    require(block.timestamp > proposal.atTime + votingPeriod, "Voting period not elapsed");
    ...
    emit Inflation(quorum, proposal.atTime, totalSupply);
  }
}