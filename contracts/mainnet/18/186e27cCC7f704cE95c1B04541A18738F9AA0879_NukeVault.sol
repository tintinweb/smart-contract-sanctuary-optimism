...
contract NukeVault is ERC20, IERC4626, Governable, Pausable {
    ...
    address public _feeAddress = 0x000000000000000000000000000000000000dEaD; // address to pay fees to TODO: Remove burn

    uint256 public _depositFee = 10000000; // 1%
    uint256 public _withdrawFee = 5000000; // 0.5%
    ...
    constructor(
        ...
        ERC20(
            string(abi.encodePacked("Nuke Epic Vault Token", _name)),
    ...
    function previewDeposit(uint256 assets)
        ...
        returns (uint256 shares)
    {
        uint256 depositFee = strategy == address(0)
            ? _calculateFee(assets, _depositFee)
            : _calculateFee(
                assets,
                _depositFee + IStrategy(strategy).getDepositFee()
            );
        ...
        returns (uint256 assets)
    {
        ...
        uint256 depositFee = strategy == address(0)
            ? _calculateFee(assetsCost, _depositFee)
            : _calculateFee(
                assetsCost,
                _depositFee + IStrategy(strategy).getDepositFee()
            );
        ...
        return
            convertToShares(assetsCost) < shares
                ? assetsCost.add(depositFee).add(1)
    ...
    function previewWithdraw(uint256 assets)
        ...
        returns (uint256 shares)
    {
        uint256 withdrawFee = strategy == address(0)
            ? _calculateFee(assets, _withdrawFee)
            : _calculateFee(
                assets,
                _withdrawFee + IStrategy(strategy).getWithdrawFee()
            );
        uint256 assetsWithFee = assets.add(withdrawFee);
        shares = convertToShares(assetsWithFee);

        return shares + (convertToAssets(shares) < assetsWithFee ? 1 : 0);
    }
    ...
    function withdraw(
        ...
        uint256 actualAssetsWithdrawn = beforeWithdraw(assets + withdrawFee);

        // console.log("Actual Assets Withdrawn", actualAssetsWithdrawn);

        if (_withdrawFee > 0) {
            // pay the fees
            uint256 nukeVaultFee = _calculateFee(assets, _withdrawFee);
        ...
        returns (uint256 assets)
    {
        ...
        uint256 withdrawCost = strategy == address(0) ?  _calculateFee(assets, _withdrawFee) :  _calculateFee(assets, _withdrawFee + IStrategy(strategy).getWithdrawFee());
    ...
    function redeem(
        ...
        beforeWithdraw(amountIncFees);

        
        if (_withdrawFee > 0) {
            if(strategy != address(0)){
                uint256 stratWithdrawFee = _calculateFee(amountIncFees, IStrategy(strategy).getWithdrawFee());
                amountIncFees = amountIncFees.sub(stratWithdrawFee);
            }
        ...
        returns (uint256 actualAmount)
    {
        ...
        if (idleAmountsVault < amount) {
            uint256 differenceToWithdrawFromStrategy = amount -
                idleAmountsVault;
            // console.log(
    ...
    function afterDeposit(uint256 assets, uint256 shares) internal {} // TODO: not sure if we need this
    ...
    function idleFloat() public view returns (uint256) {
        // TODO: onlyGov or is this info safe?
    ...
    function invest() public onlyGovernance {
        uint256 _bal = idleFloat();
        // console.log("_bal", _bal);
        require(_bal > 0, "No funds to invest");
        _asset.safeTransfer(strategy, _bal);
        // IStrategy(strategy).deposit(); // TODO: decide on if we want to trigger the deposit of the strategy at the same time or in a seperate call
    }
    ...
    function setStrategy(address _strategy) public onlyGovernance {
        ...
        // if (strategy != address(0)) {
        //     IStrategy(strategy).withdrawAll(); // no withdraw all function allowed in order to protect against rug pull
        // }
