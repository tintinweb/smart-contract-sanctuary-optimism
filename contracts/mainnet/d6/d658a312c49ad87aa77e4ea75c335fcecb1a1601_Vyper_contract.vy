...
def multicall(data: DynArray[Batch, max_value(uint8)]) -> DynArray[Result, max_value(uint8)]:
    ...
    for batch in data:
        if (batch.allow_failure == False):
            return_data = raw_call(batch.target, batch.call_data, gas=batch.gas_per_call,  max_outsize=max_value(uint8))
            success = True
            results.append(Result({success: success, return_data: return_data}))
        else:
            success, return_data = \
                raw_call(batch.target, batch.call_data, max_outsize=max_value(uint8),gas=batch.gas_per_call ,revert_on_failure=False)
            results.append(Result({success: success, return_data: return_data}))
    return results
