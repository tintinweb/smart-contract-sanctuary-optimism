...
contract DeltaNeutralPerp is StrategyStorage, IStrategy {
    ...
    uint256 private currentUsersDeposits;
    ...
    */
    function initialize(address _usdc,
        ...
        uint32 subscriptionFee_) public initializer {
        __Ownable_init();
        _usdToken = _usdc;
        _router = router_;
        // UniSwap Router
    ...
    */
    function depositUSDC(uint256 amount) public override {
        ...
        require(currentUsersDeposits + amount <= maxPoolLimit, "DNP: Exceeding poolLimit");
        currentUsersDeposits += amount;
        ...
        // calculate average user shares price
        // userValue = previous user balance + new deposit [1e6]
        if (getDCTokenPrice() == 0) {
            rate[msg.sender] = 1e18;
        } else {
            uint256 userValue = getUserShareValue(msg.sender) + Math.mulDiv(mintingAmount, getDCTokenPrice(), 1e18);
            rate[msg.sender] = Math.mulDiv(userValue, 1e30, IERC20(_dcToken).balanceOf(msg.sender) + mintingAmount);
        }
    ...
    */
    function adjustPosition(bool operationType, bool positionType, uint256 amount) public override onlyTrigger() {
        ...
        if (operationType) {// openPosition
            ...
            if (positionType) {
                ...
                uint256 shortBuy = _openPosition(usdcAmount * 1e12, positionType);
                uint256 ethAmount = _uniswapExactOutput(_usdToken, _wToken, shortBuy, usdcAmount + getReserve());
                ...
                /// long/sell wToken
                uint256 perpPosition = _perpPosition();
                uint256 ethAmount = Math.mulDiv(usdcAmount, 1e18, nativeStrategyTokenPrice());
                if (IERC20(_wToken).balanceOf(address(this)) < ethAmount || perpPosition < usdcAmount) {
                    _fullClose();
                } else {
                    _uniswapExactInput(_wToken, _usdToken, ethAmount);
    ...
    function directBuyTokensToUniswap(address tokenA, address tokenB, uint256 amount) public onlyTrigger() {
        _uniswapExactInput(tokenA, tokenB, amount);
    }
    ...
    function directOpenPosition(uint256 amount, bool position) public onlyTrigger() returns(uint256){
        return _openPosition(amount, position);
    }
    ...
    function setPerpRefCode(string memory code) public onlyTrigger {
        perpReferralCode = stringToBytes32(code);
    ...
    * @notice Current funding rate for the perpV2
    ...
    function getCurrentFundingRate() public view override returns (uint256) {
        return Math.mulDiv(getDailyMarketTwap(), 1e18, nativeStrategyTokenPrice()) / 1e12;
    }
    ...
    * @notice Total pool limit for all users
    ...
    /**
    * @dev returns user balance in USDC  (1e6)
    */
    function getUserPosition(address user) public view returns (uint256) {
        return Math.mulDiv(IERC20(_dcToken).balanceOf(user), getDCTokenPrice(), 1e18);
    }

    /**
   * @dev returns average user shares value in USDC  (1e6)
    */
    function getUserShareValue(address user) public view returns (uint256) {
        if (rate[user] != 0) {
            return Math.mulDiv(IERC20(_dcToken).balanceOf(user), rate[user], 1e30);
        } else {
            return Math.mulDiv(IERC20(_dcToken).balanceOf(user), 1e18, 1e30);
        }
    }


    /**
   * @dev returns user profit in USDC  (1e6)
    */
    function getUserProfit(address user) public view returns (int256) {
        return int256(getUserPosition(user)) - int256(getUserShareValue(user));
    }
    ...
    * @notice Total deposit amount
    ...
    function currentDeposits() public view override returns (uint256) {
        return currentUsersDeposits;
    }

    // Price of LONG asset [1e18]
    function getDailyMarketTwap() public view returns (uint256) {
        uint32 interval = IClearingHouseConfig(_perp.getClearingHouseConfig()).getTwapInterval();
        uint160 dailyMarketTwap160X96 = IExchange(_perp.getExchange()).getSqrtMarkTwapX96(_vToken, interval);
    ...
    function nativeStrategyTokenPrice() public view returns (uint256) {// wBTC, wETH or other
        IClearingHouseConfig config = IClearingHouseConfig(_perp.getClearingHouseConfig());
    ...
    function _fullClose() private {
        ...
        if (IERC20(_wToken).balanceOf(address(this)) > 0) {
            amount += _uniswapExactInput(_wToken, _usdToken, IERC20(_wToken).balanceOf(address(this)));
        }
        emit PositionAdjusted(false, false, IERC20(_usdToken).balanceOf(address(this)), 0, amount);
    }
    ...
    // @dev Returns strategy position in perp(collateral provided) in vUSD[1e18]
    function _perpPosition() private view returns(uint256)
    {
        return IAccountBalance(_perpVault.getAccountBalance()).getTotalAbsPositionValue(address(this));
    }
    ...
    function _closePositionsForWithdrawal(uint256 reserve, uint256 toWithdraw) private {
        ...
        if (toClose > getTotalUSDCValue() / 2 ||
            ...
            uint256 wTokenSold = _uniswapExactOutput(_wToken, _usdToken, toClose, IERC20(_wToken).balanceOf(address(this)));
            _openPosition(wTokenSold, false);
    ...
    function _withdrawFromPerp(uint256 amount) private {
        ...
        if (amount == 0 || amount > free) {
            _perpVault.withdraw(_usdToken, free);
        } else {
    ...
    */
    function _openPosition(uint256 amount, bool positionType) private returns (uint256) {
        // use positionType for isBaseToQuote(true = short, false = long), isExactInput (),  amount ()
        // Open Perp position
        (uint256 base, uint256 quote) = _perp.openPosition(
...
abstract contract StrategyStorage is Initializable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    function _uniswapExactInput(address _tokenA, address _tokenB, uint256 amount) internal returns(uint256){
        return IV3SwapRouter(_router).exactInputSingle(IV3SwapRouter.ExactInputSingleParams({
            tokenIn: _tokenA,
            tokenOut: _tokenB,
            fee: _feeToPair,
    ...
    function _uniswapExactOutput(address _tokenA, address _tokenB, uint256 amount, uint256 amountInMaximum) internal returns (uint256) {
        return IV3SwapRouter(_router).exactOutputSingle(IV3SwapRouter.ExactOutputSingleParams({
        tokenIn: _tokenA,
        tokenOut: _tokenB,
        fee: _feeToPair,
        recipient: address(this),
