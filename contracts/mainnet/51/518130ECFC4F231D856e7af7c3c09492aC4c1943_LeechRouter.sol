    ...
    ILeechRouter
{
        ...
        checkWhitelist(signature, maxBlockNumber)
    {
        ...
        if (depositMinAmount[address(depositToken)] == 0 ||
            amount < depositMinAmount[address(depositToken)]) 
        revert ("Token is not allowed or amount is too low.");
        // Get active strategy
        ...
        checkWhitelist(signature, maxBlockNumber)
    {
        ...
        if (chainId == activeStrat.chainId) revert CrosschainError();
        // Check income token
        address bridgedToken = path[path.length - 1];

        if (address(depositToken) != bridgedToken) {
            // Transfer token here for the swap
            depositToken.safeTransferFrom(msg.sender, address(this), amount);
            // Make swap and get swapped amounts
            uint256[] memory swapedAmounts = _swap(amount, path);
            // Update deposit amount after swap to base token
            amount = swapedAmounts[swapedAmounts.length - 1];
            // Transfer base token to the transtorder
            IERC20(bridgedToken).safeTransfer(address(transporter), amount);
        } else {
            // Send token from user to the bridge transporter

            IERC20(bridgedToken).safeTransferFrom(
                msg.sender,
                address(transporter),
                amount
            );
    ...
    function withdraw(
        ...
        address[] calldata token1ToTokenOut,
        bytes memory data
    ) external payable enabled(msg.sender) whenNotPaused {
        ...
        if (chainId == activeStrat.chainId) {
            ...
            uint256 withdrew = IStrategyMasterchefFarmV2(
                activeStrat.strategyAddress
            ).withdraw(
                    msg.sender,
                    shares,
            ...
            IERC20(tokenOut).safeTransfer(msg.sender, withdrew);
        ...
        zeroAddr(user)
    {
        ...
        uint256 tokenOutAmount = IStrategyMasterchefFarmV2(
            activeStrat.strategyAddress
        ).withdraw(user, shares, token0ToTokenOut, token1ToTokenOut, "");
        ...
        zeroAddr(_strategy.strategyAddress)
    {
        ...
        if (_strategy.chainId == chainId) {
            ...
            IERC20(bridgedToken).safeTransfer(address(transporter), withdrawAmount);

            if (address(baseToken) != bridgedToken) {
                uint256[] memory swapedAmounts = _swap(withdrawAmount, path);
                withdrawAmount = swapedAmounts[swapedAmounts.length - 1];
            }
    ...
    function quotePotentialWithdraw(
        ...
        amountOut = IStrategyMasterchefFarmV2(stratAddress)
            .quotePotentialWithdraw(shares, token0toTokenOut, token1toTokenOut, data, 0, 0);
    }
