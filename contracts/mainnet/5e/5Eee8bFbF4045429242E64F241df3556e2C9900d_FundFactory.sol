...
contract FundFactory is Initializable, UUPSUpgradeable, OwnableUpgradeable, IFundFactory {
    ...
    function newFund(FundInfo calldata fundInfo) public override returns(uint256) {
        ...
        QHToken itoken = new QHToken(address(feeder), address(interaction));
        QHTrade trade = new QHTrade(triggerServer, usdt, manager);
...
contract QHToken is ERC20, IToken, Warded {
   ...
   // ITKN stands for Invest Token
    constructor (address interaction, address feeder) ERC20("Defunds Token", "ITKN") {
        wards[interaction] = 1;
        wards[feeder] = 1;
    }
...
//        return Math.mulDiv(ethBalance, etherPrice, 1e18) + usdtBalance;
    ...
    //FIXME Secure with wards for manager or trigger
