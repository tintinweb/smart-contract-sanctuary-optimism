...
contract NFTEAirdrop is Ownable, ReentrancyGuard {
    event Airdrop(
        address holder,
        uint256 amount
    );
    ...
    uint256 public airdropAmount = 3643.15 ether;

    address private signer;
    address private safe;

    constructor(address _signer, address _safe, IERC20 _token) {
        signer = _signer;
        safe = _safe;
        token = IERC20(_token);
    }
    ...
    nonReentrant
    {
        ...
        require(!claimed[msg.sender], "You already claimed the airdrop");
        require(allowance >= airdropAmount, "Vault has been emptied");

        claimed[msg.sender] = true;
        token.transferFrom(safe, msg.sender, airdropAmount);
