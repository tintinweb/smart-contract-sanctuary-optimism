/**
 *Submitted for verification at Optimistic.Etherscan.io on 2022-09-13
*/
       ...
       for (uint i; i < activeVoteLps.length;) {
            ...
            for (uint j; j < rewardsGauge.bribeTokens.length;) {
                ...
                if (tokenBal > 0 && rewardsGauge.bribeTokens[j] != address(want)) {
                    uint256[] memory amounts = router.swapExactTokensForTokens(tokenBal, 0, routes[rewardsGauge.bribeTokens[j]], address(this), block.timestamp);
                    unchecked {rewardAmt += amounts[amounts.length - 1];}
                }
                unchecked { ++j; }
            }
            ...
            for (uint k; k < rewardsGauge.feeTokens.length;) {
                ...
                if (tokenBal > 0 && rewardsGauge.feeTokens[k] != address(want)) {
                    uint256[] memory amounts = router.swapExactTokensForTokens(tokenBal, 0, routes[rewardsGauge.feeTokens[k]], address(this), block.timestamp);
                    unchecked {rewardAmt += amounts[amounts.length - 1];}
                }
                unchecked { ++k; }
            }
        ...
        uint256 emissionsGap = totalWant() - rewardAmt - totalSupply();
        if (emissionsGap > 0) {
            if (rewardAmt > 0) {
                emissionsGap = emissionsGap > rewardAmt ? rewardAmt : emissionsGap;
                _mint(address(beVeloRewardPool), emissionsGap);
                beVeloRewardPool.notifyRewardAmount(emissionsGap);
                rewardAmt -= emissionsGap;
            }
        }
