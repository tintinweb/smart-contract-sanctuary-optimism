...
contract TestDepositAddress {
    function lockDeposit(address tokenAddress, bytes32 salt) external {
        address depositHandlerAddress = _getCreate2Address(salt, keccak256(abi.encodePacked(type(DepositHandler).creationCode)));
        // codehash is 0x0 when the deposit handler is not deployed or at the end of tx processing after being destroyed.
        // However, it is NOT 0x0 during the tx processing where it is deployed even though it is destroyed.
        if (
            depositHandlerAddress.codehash != 0xc5d2460186f7233c927e7db2dcc703c0e500b653ca82273b7bfad8045d85a470 &&
            depositHandlerAddress.codehash != bytes32(0)
        ) revert('Failed');
        ...
        if (!success || (returnData.length != uint256(0) && !abi.decode(returnData, (bool)))) revert('Failed');
