...
// File: contracts/BIFI/zaps/crossChainZaps/BeefyReceiver.sol
...
interface IBeefyZap {
    ...
    function beefIn(address _beefyVault, address _inputToken, uint256 _tokenInAmount, bytes calldata _token0, bytes calldata _token1, WantType _type) external;
}

contract BeefyReceiver is Ownable {
    ...
    function sgReceive(
        ...
        TradBridgeTransfer memory data = abi.decode(payload, (TradBridgeTransfer));
        try this._sgReceive{value: msg.value} (
            ...
            if (_token == sgNative) {
                (bool sent,) = data.user.call{value: msg.value}("");
                require(sent, "Failed to send Ether");
            } else {
                IERC20(_token).safeTransfer(data.user, amountLD);
            }
            emit Error();
        }
    }
    ...
    function _sgReceive(
        ...
        if (_token == sgNative) {
            _token = wNative;
            IStargateEthVault(sgNative).withdraw(amountLD);
        }

        _zapIn(data.vault, _token, amountLD, data.token0, data.token1, data.wantType);
    ...
    function _zapIn(address _vault, address _token, uint256 _amount, bytes memory _token0, bytes memory _token1, IBeefyZap.WantType _type) private {
        _approveTokenIfNeeded(_token, zap);
        IBeefyZap(zap).beefIn(_vault, _token, _amount, _token0, _token1, _type);
        address want = IBeefyVault(_vault).want();

        if (_type != IBeefyZap.WantType.WANT_TYPE_SINGLE) {
            address[] memory path = new address[](3);
            path[0] = IUniswapV2Pair(want).token0();
            path[1] = IUniswapV2Pair(want).token1();
            path[2] = _token;
            _returnAssets(path);
        } else {
    ...
    function _returnAssets(address[] memory _tokens) private {
        ...
        for (uint256 i; i < _tokens.length; i++) {
            ...
            if (balance > 0) {
                if (_tokens[i] == wNative) {
                    IWrappedNative(wNative).withdraw(balance);
