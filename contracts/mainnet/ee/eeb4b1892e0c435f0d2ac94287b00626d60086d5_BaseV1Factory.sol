...
interface IBaseV1Factory {
    ...
    function barbarianMaker() external returns (address);
...
contract BaseV1Fees {
    ...
    function claimFeesFor(address recipient, uint amount0, uint amount1) external returns (uint256 claimed0, uint256 claimed1) {
        ...
        // send 25% to barbarianMaker
        address barbarianMaker = IBaseV1Factory(factory).barbarianMaker();
        if (barbarianMaker != address(0x0)) {
            if (amount0 > 0) _safeTransfer(token0, barbarianMaker, amount0 / 4);
            if (amount1 > 0) _safeTransfer(token1, barbarianMaker, amount1 / 4);
            counter--;
        }
...
contract BaseV1Pair {
    ...
    function barbarianMaker() external returns (address) {
        return IBaseV1Factory(factory).barbarianMaker();
    }
...
contract BaseV1Factory {
    ...
    function setBarbarianMaker(address _barbarianMaker) external {
        require (msg.sender == owner);
        barbarianMaker = _barbarianMaker;
    }
