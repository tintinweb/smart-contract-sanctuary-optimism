...
// File @openzeppelin/contracts-upgradeable/security/Pau[email protected]
...
// File contracts/VeloVoteFarmerUsdc.sol
...
contract VeloVoteFarmerUsdc is UUPSUpgradeable, AccessControlEnumerableUpgradeable, PausableUpgradeable {
    ...
    struct RewardInfo {
        ...
        uint256 totalReceivedUsdc; // How much the above was worth in velo
    }
    ...
    function harvest() external onlyRole(MANAGER) {
        _removeRevokedNfts();
        increaseDurationAll();
        uint256 unclaimedUsdc = IERC20Upgradeable(USDC).balanceOf(address(this));
        claimFees();
        swapRewardsAndChargeFee(unclaimedUsdc); // into usdc
        _attributeRewards();
        rebase();
    }
    ...
    function withdraw(uint256 _withdrawTokenId, address _owner) external onlyRole(MANAGER) {
        ...
        _claimTo(_withdrawTokenId, _owner);
    ...
    function claim(uint256 _tokenId, address _owner) external onlyRole(MANAGER) {
        _requireOwnerOf(_tokenId, _owner);
        _claimTo(_tokenId, _owner);
    }

    function claimAll(address _owner) external onlyRole(MANAGER) {
        for (uint256 i; i < tokenIds.length; i = _uncheckedInc(i)) {
            if (IVeloVotingEscrow(VEVELO).ownerOf(tokenIds[i]) == _owner) {
                _claimTo(tokenIds[i], _owner);
            }
        }
    }
    ...
    /// @notice For each token, try to swap to USDC
    /// @dev To prepare for the incoming compounding, should store the amount of velo gotten
    function swapRewardsAndChargeFee(uint256 unclaimedUsdc) public onlyRole(MANAGER) {
        uint256 rewardBal;
        uint256 feeToCharge;
        for (uint256 i; i < rewards.length(); i = _uncheckedInc(i)) {
            rewardBal = IERC20Upgradeable(rewards.at(i)).balanceOf(address(this));
            if (rewards.at(i) == USDC) {
                rewardBal -= unclaimedUsdc;
            } else {
                uint256 usdcBalBefore = IERC20Upgradeable(USDC).balanceOf(address(this));
                _swap(rewards.at(i), USDC, rewardBal);
                rewardBal = IERC20Upgradeable(USDC).balanceOf(address(this)) - usdcBalBefore;
            }
            uint256 fee = rewardBal * totalFee / PERCENT_DIVISOR;
            tokenToRewardInfo[rewards.at(i)].totalReceivedUsdc = rewardBal - fee;
            feeToCharge += fee;
        }

        if (feeToCharge != 0) {
            IERC20Upgradeable(USDC).safeTransfer(treasury, feeToCharge);
        }
    }
    ...
    function _attributeRewards() internal onlyRole(MANAGER) {
        ...
        for (uint256 i; i < tokenIds.length; i = _uncheckedInc(i)) {
            for (uint256 j = 0; j < rewards.length(); j = _uncheckedInc(j)) {
                ...
                //total amuount of reward expressed in rewardToken
                totalReward = tokenToRewardInfo[rewards.at(j)].totalReceived;
                // total amount of usdc gotten
                totalRewardUsdc = tokenToRewardInfo[rewards.at(j)].totalReceivedUsdc;
                if (totalReward != 0) {
                    tokenIdToInfo[tokenIds[i]].rewardDebt += (veNftRewardShare * totalRewardUsdc) / totalReward;
                }
            }
    ...
    function resetBalancesAll() external onlyRole(MANAGER) {
        for (uint256 i; i < rewards.length(); i = _uncheckedInc(i)) {
            ...
            tokenToRewardInfo[rewards.at(i)].totalReceivedUsdc = 0;
        }
    }
    ...
    function synchronize() external onlyRole(MANAGER) {
        ...
        for (uint256 i; i < rewardsArray.length; i = _uncheckedInc(i)) {
            ...
            swapPath[rewardsArray[i]][USDC] = manager.getSwapPath(rewardsArray[i],USDC);
        }
    ...
    function pendingRewards(uint256 _tokenId) public view returns (uint256) {
        return tokenIdToInfo[_tokenId].rewardDebt;
    }

    function userPendingRewards(address _user) external view returns (uint256) {
        uint256 total;
        for (uint256 i; i < tokenIds.length; i = _uncheckedInc(i)) {
            if (IVeloVotingEscrow(VEVELO).ownerOf(tokenIds[i]) == _user) {
                total += tokenIdToInfo[tokenIds[i]].rewardDebt;
            }
        }
        return total;
    }
    ...
    function _removeRevokedNfts() internal {
        for (uint256 i; i < tokenIds.length;  i = _uncheckedInc(i)) {
            ...
            if ((!IVeloVotingEscrow(VEVELO).isApprovedOrOwner(address(this), tokenIds[i])) || lockEnd < block.timestamp) {
    ...
    function _claimTo(uint256 _tokenId, address _user) internal {
        uint256 amount = tokenIdToInfo[_tokenId].rewardDebt;
        tokenIdToInfo[_tokenId].rewardDebt = 0;
        IERC20Upgradeable(USDC).safeTransfer(_user, amount);
    }
