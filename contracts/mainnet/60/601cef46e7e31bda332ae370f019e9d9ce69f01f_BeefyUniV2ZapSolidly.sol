...
contract BeefyUniV2ZapSolidly {
    ...
    function _getSwapAmount(IUniswapV2Pair pair, uint256 investmentA, uint256 reserveA, uint256 reserveB, address tokenA, address tokenB) private view returns (uint256 swapAmount) {
        ...
        uint256 stableSwapAmount;
        if (pair.stable()) {
            stableSwapAmount = _getStableSwap(pair, investmentA, halfInvestment, tokenA, tokenB);
        }
        ...
        swapAmount = pair.stable() ? stableSwapAmount : investmentA.sub(Babylonian.sqrt(halfInvestment * halfInvestment * nominator / denominator));
    }
    ...
    function _getStableSwap(IUniswapV2Pair pair, uint256 investmentA, uint256 halfInvestment, address tokenA, address tokenB) private view returns (uint256 swapAmount) {
        (uint out,) = router.getAmountOut(halfInvestment, tokenA, tokenB);
                ...
                uint ratio = out * 1e18 / halfInvestment * amountB / amountA;
