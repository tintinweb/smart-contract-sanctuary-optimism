...
/// @title Worldcoin Launch NFT
/// @author Miguel Piedrafita <[email protected]>
/// @notice Commemorative NFTs for the team behind the launch of Worldcoin
contract LaunchNFT is ERC721 {
	error InvalidGater();
	...
	/// @dev EIP-712 types for a signature that can be used to claim a stamp.
	bytes32 public constant CLAIM_HASH = keccak256('Claim(address to)');

	constructor(address _minter, string memory _tokenURI) {
		...
		DOMAIN_SEPARATOR = keccak256(
			abi.encode(
				...
				keccak256(bytes('Worldcoin Launch NFT')),
				keccak256(bytes('1')),
				block.chainid,
				address(this)
			)
	...
	/// @notice Mint a new NFT to the given address if the provided signature is valid
	/// @param to The address to mint the NFT to
	/// @param v The v value of the signature
	/// @param r The r value of the signature
	/// @param s The s value of the signature
	function claim(address to, uint8 v, bytes32 r, bytes32 s) public {
		bytes32 digest = keccak256(
			abi.encode('\x19\x01', DOMAIN_SEPARATOR, keccak256(abi.encode(CLAIM_HASH, address(to))))
		);

		if (ecrecover(digest, v, r, s) != minter) {
	...
	/// @notice Update the address allowed to mint NFTs
	/// @param _minter The new address allowed to mint NFTs
	function updateMinterAddr(address _minter) public {
		if (msg.sender != minter || _minter == address(0)) {
			revert InvalidGater();
		}
