...
contract PoolRTHM {
	...
	address public rthm; // RTHM address
	...
	function deposit(uint256 amount) external {
		...
		IERC20(rthm).safeTransferFrom(msg.sender, address(this), amount);
	...
	function withdraw(uint256 amount) external {
		...
		IERC20(rthm).safeTransfer(msg.sender, amount);
	...
	function _updateRewards() internal {
		...
		for (uint256 i = 0; i < length; i++) {
			...
			address rewardsContract = IRouter(router).getRthmRewards(currency);
