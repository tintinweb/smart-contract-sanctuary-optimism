/**
 *Submitted for verification at Optimistic.Etherscan.io on 2022-11-28
*/
 ...
 * @title OpxPrediction
 */
contract OpxPrediction is Ownable, Pausable, ReentrancyGuard {
    ...
    address public fPriceFeed; // address of fasr price feed
     ...
     * @dev Callable by admin
     */
    function setFPriceFeed(address _fPriceFeed) external  onlyAdmin {
        require(_fPriceFeed != address(0), "Cannot be zero address");
        fPriceFeed = _fPriceFeed;
    }
    ...
    function _getPriceFromOracle() internal view returns (uint80, int256) {
        ...
        (, int256 oraclePrice, , uint256 oracleTimestamp, ) = oracle.latestRoundData();

        uint256 fPrice = IFastPriceFeed(fPriceFeed).getPrice(WETH, uint256(oraclePrice), true) / 1e22;
        uint256 fTimestamp = IFastPriceFeed(fPriceFeed).lastUpdatedAt();
        int256 price = oraclePrice;
        uint256 timestamp = oracleTimestamp;
        if (oracleTimestamp < fTimestamp){
            price = int256(fPrice);
