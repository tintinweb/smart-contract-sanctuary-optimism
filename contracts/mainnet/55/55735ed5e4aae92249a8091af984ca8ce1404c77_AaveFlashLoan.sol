    ...
    AccountBalance
{
    ...
    function flashLoan(
        ...
        bytes memory params = abi.encode(_positions);
    ...
    function executeOperation(
        ...
        Position[] memory _positions = abi.decode(params, (Position[]));

        address _asset;
        uint256 _amount;
        console.log(_positions[0]._openPosition.baseToken, "_positions");

        for (uint i = 0; i < assets.length; i++) {
            _asset = assets[i];
            _amount = amounts[i];
            uint amountOwing = _amount.add(premiums[i]);
            IERC20(_asset).approve(vault, _amount);
            _deposit(_asset, _amount);
            _makeProfit(_positions);
            console.log("LINE 90");
            ...
            console.log(balance, "==", amountOwing, "CCCCCCCCC");
    ...
    function _makeProfit(Position[] memory _positions) internal {
        
        for (uint256 index = 0; index < _positions.length; index++) {
            // postions2[_positions[0],_positions[1]]

            IClearingHouse.OpenPositionParams memory _openPosition = _positions[
                index
            ]._openPosition;
            IClearingHouse.ClosePositionParams
                memory _closePosition = _positions[index]._closePosition;
             _openPositions(_openPosition);
            //deplzay
            if(block.timestamp+100> block.timestamp){

             (uint256 base, uint256 quote)=_closePositions(_closePosition);
             console.log(base,"base",quote,"quote");
            }
            // _getTotalPositionValue(address(this), _openPosition.baseToken);
            // _quitMarket(address(this),_openPosition.baseToken);
        }
    }

    function SetModules(address _vault, address _clearingHouse)
        public
        onlyOwner
...
pragma solidity ^0.8.0;
import "interfaces/IPerp/IClearingHouse.sol";
import "interfaces/IPerp/IVault.sol";
import "interfaces/IPerp/IAccountBalance.sol";
import "hardhat/console.sol";

abstract contract ClearingHouse {
    address public clearingHouse;
    struct Position {
        IClearingHouse.OpenPositionParams _openPosition;
        IClearingHouse.ClosePositionParams _closePosition;
    }

    function _openPositions(IClearingHouse.OpenPositionParams memory params)
        internal
        returns (uint256 base, uint256 quote)
    {
        IClearingHouse.OpenPositionParams memory updateParams = params;
        updateParams.deadline = block.timestamp + 900;
        IClearingHouse _clearingHouse = IClearingHouse(clearingHouse);
        (base, quote) = _clearingHouse.openPosition(updateParams);
        // console.log("close", base, quote);

        return (base, quote);
    }

    function _closePositions(IClearingHouse.ClosePositionParams memory params)
        internal
        returns (uint256 base, uint256 quote)
    {
        IClearingHouse.ClosePositionParams memory updateParams = params;
        // updateParams.deadline = block.timestamp + 900;
        IClearingHouse _clearingHouse = IClearingHouse(clearingHouse);
        (base, quote) = _clearingHouse.closePosition(updateParams);
        // console.log("open", base, quote);
        return (base, quote);
    }

    // function _quitMarket(address trader, address baseToken) internal {
    //     IClearingHouse _clearingHouse = IClearingHouse(clearingHouse);
    //     _clearingHouse.cancelAllExcessOrders(trader, baseToken);
    // }
}
...
abstract contract Vault {
    ...
    function _deposit(address _asset, uint256 _amount) internal {
        IVault(vault).deposit(_asset, _amount);
    }

    function _withdraw(address _asset, uint256 _amount) internal {
        int256 freeCollateral = IVault(vault).getAccountValue(address(this));
        console.log(uint(freeCollateral), "free");
       uint256 settlementX10_S = IVault(vault).getRepaidSettlementByCollateral(_asset, uint(_amount));
        console.log(settlementX10_S,"settlementX10_S");
        IVault(vault).withdraw(_asset, uint(freeCollateral));
...
abstract contract AccountBalance {
    address public accountBalance;

    //----------------------------------------------------------------AccountBalance----------------------------------------------------------------

    function _getTotalPositionSize(address trader, address baseToken)
        internal
        view
    {
        int256 totalPositionSize = IAccountBalance(accountBalance)
            .getTotalPositionSize(trader, baseToken);
        console.log(uint(totalPositionSize), "_totalPositionSize");
    }

    function _getTotalPositionValue(address trader, address baseToken)
        internal
        view
    {
        int256 totalPositionValue = IAccountBalance(accountBalance)
            .getTotalPositionValue(trader, baseToken);
        console.log(uint(totalPositionValue), "_totalPositionSizevalue");
    }
}
