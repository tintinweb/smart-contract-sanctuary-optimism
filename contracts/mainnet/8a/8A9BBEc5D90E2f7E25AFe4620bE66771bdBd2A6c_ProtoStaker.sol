...
contract ProtoStaker is Initializable {
  ...
  function stake(address payable account, uint256 inEXA, uint256 minEXA, uint256 keepETH) internal {
    ...
    if (msg.value - inETH != 0) account.safeTransferETH(msg.value - inETH);
  }
  ...
  function stakeBalance(Permit calldata permit, uint256 minEXA, uint256 keepETH) external payable {
    exa.safePermit(permit.owner, address(this), permit.value, permit.deadline, permit.v, permit.r, permit.s);
    exa.safeTransferFrom(permit.owner, address(this), permit.value);
    stake(permit.owner, permit.value, minEXA, keepETH);
  }

  function stakeRewards(ClaimPermit calldata permit, uint256 minEXA, uint256 keepETH) external payable {
    if (permit.assets.length != 1 || address(permit.assets[0]) != address(exa)) {
      return payable(permit.owner).safeTransferETH(msg.value);
    }
    (, uint256[] memory claimedAmounts) = rewardsController.claim(rewardsController.allMarketsOperations(), permit);
    if (claimedAmounts[0] == 0) return payable(permit.owner).safeTransferETH(msg.value);
    exa.safeTransfer(address(this), claimedAmounts[0]);
    stake(payable(permit.owner), claimedAmounts[0], minEXA, keepETH);
  }
...
interface IPoolFactory {
  ...
  function getPool(EXA exa, ERC20 weth, bool stable) external view returns (IPool);
}
