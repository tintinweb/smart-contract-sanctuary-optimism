...
import {ISolidlyRouter} from "../../interfaces/Velodrome.sol";
...
contract VeloZapper is ZapperBase {
    using SafeERC20 for ERC20;
    using SafeERC20 for IERC20;

    address public constant VELOLPTOKEN_ADDRESS =
        address(0xd16232ad60188B68076a235c65d692090caba155);
    address public constant VELOROUTER_ADDRESS =
        address(0xa132DAB612dB5cB9fC9Ac426A0Cc215A3423F9c9);
    address public constant VELOTOKEN_ADDRESS =
    ...
    // ERC20 private constant DAI_TOKEN = ERC20(DAI_ADDRESS); // TODO: might do this later in order to save gas..., but we are on optimism so...
    address private constant CURVE_FACTORY_DEPOSIT_ADDRESS =
        0x167e42a1C7ab4Be03764A2222aAC57F5f6754411;

    // mapping(address => bool) private approvedTokens; // so... maybe it's a security risk to be able to accept any token, someone could make a malicious token I guess... gonna leave it hardcoded for now
    // tokensToDeposit: this order is important since funds to deposit needs to be in the same order, and this order is used when adding liq to the pool
    address[4] private tokensToDeposit = [
        SUSD_ADDRESS,
        DAI_ADDRESS,
        USDC_ADDRESS,
        USDT_ADDRESS
    ...
    function doApprovals() public override onlyGovernance {
        IERC20(USDC_ADDRESS).safeApprove(VELOROUTER_ADDRESS, type(uint256).max);
        IERC20(SUSD_ADDRESS).safeApprove(VELOROUTER_ADDRESS, type(uint256).max);
    ...
    {
        uint256[2] memory fundsToDeposit = [uint256(0), uint256(0)];
        ...
        uint256 amountOutUSDC = ISwapRouter03(UNISWAPROUTER_V3_ADDRESS)
            .exactInputSingle(
                IV3SwapRouter.ExactInputSingleParams(
                    WETH_ADDRESS,
                    USDC_ADDRESS,
        ...
        balanceOfWeth = OptimismWETH(WETH_ADDRESS).balanceOf(address(this));

        uint256 amountOutSUSD = ISwapRouter03(UNISWAPROUTER_V3_ADDRESS)
            .exactInputSingle(
                IV3SwapRouter.ExactInputSingleParams(
                    WETH_ADDRESS,
                    SUSD_ADDRESS,
        ...
        fundsToDeposit[0] = amountOutUSDC;
        fundsToDeposit[1] = amountOutSUSD;

        // TODO: Worth to define tokensToDeposit as state variable? we are only reading from it, maybe it's to expensive to iniate in memory, or hardcode with if's instead... do gas tests
        ...
        ISolidlyRouter(VELOROUTER_ADDRESS).addLiquidity(
            USDC_ADDRESS,
            SUSD_ADDRESS,
            true,
            fundsToDeposit[0],
