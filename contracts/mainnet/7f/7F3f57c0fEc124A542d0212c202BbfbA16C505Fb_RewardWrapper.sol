...
/**
 * @title RewardWrapper
 * @notice RewardWrapper is an upgradeable contract that fulfills orders on behalf of users and rewards eligible
 *         orders with OP tokens. This contract cannot fulfill any order that is not a sell order
 *         paid in ETH. Note that future versions of this contract must be upgrade safe:
 *         https://docs.openzeppelin.com/upgrades-plugins/1.x/writing-upgradeable
 */
contract RewardWrapper is
    IRewardWrapper,
    PausableUpgradeable,
    ...
    IERC1155Receiver
{
    ...
    /**
     * @notice Emitted when a buyer receives OP tokens for a fulfilled order. Note that the 
     *
     * @param rewardReceiver   Buyer who is receiving the OP tokens.
     * @param rewardAmountInOP Amount of OP token rewarded.
     * @param campaignId       Campaign ID.
     * @param campaignString   Campaign string.
     */
    event RewardSent(
    ...
    /**
     * @notice Emitted when the contract owner withdraws ETH from this contract.
     *
     * @param caller Address of the contract owner.
     * @param amount Amount of ETH withdrawn.
     */
    event ETHWithdrawn(address indexed caller, uint256 amount);

    /**
     * @notice Emitted when the RewardDeriver address is set.
     *
     * @param rewardDeriver The new RewardDeriver address.
     */
    event RewardDeriverSet(address indexed rewardDeriver);
     ...
     */
    IRewardDeriver public rewardDeriver;

    /**
     * @notice Maps a campaign ID to the total amount of OP token reward sent for the campaign.
     */
    mapping(bytes32 => uint256) internal rewardSentByCampaignInOP;

    /**
     * @notice Prevents this implementation contract from being initialized directly so that
     *         initialization most occur through the proxy. Further explanation:
     ...
     *
     * @param _seaport Seaport contract.
     * @param _rewardDeriver RewardDeriver contract.
     * @param _marketplaceOwner Address of the owner of this contract.
     */
    function initialize(
        Seaport _seaport,
        IRewardDeriver _rewardDeriver,
        address _marketplaceOwner
    ) external initializer {
        seaport = _seaport;
        rewardDeriver = _rewardDeriver;
     ...
     */
    function fulfillOrder(Order calldata order) external payable whenNotPaused nonReentrant {
        require(
            order.parameters.offer.length == 1,
            "RewardWrapper: order must have one offer item"
        );

        RewardInfo[] memory rewardInfoArray = rewardDeriver.getRewardInOP(order, msg.sender);
        
        uint256 totalRewardAmountInOP = 0;
        for (uint256 i = 0; i < rewardInfoArray.length; i++) {
            RewardInfo memory rewardInfo = rewardInfoArray[i];
            if (rewardInfo.rewardAmountInOP > 0) {
                bytes32 campaignId = keccak256(bytes(rewardInfo.campaignString));
                emit RewardSent(
                    msg.sender,
                    rewardInfo.rewardAmountInOP,
                    campaignId,
                    rewardInfo.campaignString
                );
                rewardSentByCampaignInOP[campaignId] += rewardInfo.rewardAmountInOP;
                totalRewardAmountInOP += rewardInfo.rewardAmountInOP;
            }
        }

        if (totalRewardAmountInOP > 0) {
            // Send the OP token reward to the buyer. This call should never revert because we check
            // for the two necessary conditions in { RewardDeriver.getRewardInOP } prior to calling
            // this function:
            // 1. The owner of this contract has a sufficient balance of OP token.
            // 2. The RewardWrapper has a sufficient allowance of OP token.
            IERC20Upgradeable(OP_TOKEN).safeTransferFrom(owner(), msg.sender, totalRewardAmountInOP);
        }

        bool fulfilled = seaport.fulfillOrder{ value: msg.value }(order, bytes32(0));
        require(fulfilled, "RewardWrapper: order cannot be fulfilled");

        OfferItem memory offerItem = order.parameters.offer[0];
        if (offerItem.itemType == ItemType.ERC721) {
            ...
            require(
                ...
                "RewardWrapper: ERC1155 start amount must equal end amount"
            );
    ...
    function fulfillAvailableOrders(
        ...
        require(
            orders.length == maximumFulfilled,
            "RewardWrapper: order length must equal maximum fulfilled"
        );

        (bool[] memory availableOrders, ) = seaport.fulfillAvailableOrders{ value: msg.value }(
            orders,
            offerFulfillments,
        ...
        uint256 totalRewardAmountInOP = 0;
        // Initialize the buyer's refund. The price of all successful orders will be subtracted from
        // this amount.
        ...
        for (uint256 i = 0; i < orders.length; i++) {
            ...
            if (availableOrders[i] == true) {
                // Subtract the consideration amount from the remaining ETH.
                for (uint256 j = 0; j < order.parameters.consideration.length; j++) {
                    refund -= order.parameters.consideration[j].startAmount;
                }

                RewardInfo[] memory rewardInfoArray = rewardDeriver.getRewardInOP(order, msg.sender);
                
                for (uint256 k = 0; k < rewardInfoArray.length; k++) {
                    RewardInfo memory rewardInfo = rewardInfoArray[k];
                    if (rewardInfo.rewardAmountInOP > 0) {
                        bytes32 campaignId = keccak256(bytes(rewardInfo.campaignString));
                        emit RewardSent(
                            msg.sender,
        ...
        if (refund > 0) {
            (bool success, ) = payable(msg.sender).call{ value: refund }(new bytes(0));
            require(success, "RewardWrapper: call to refund buyer failed");
        }
    }
    ...
    function getRewardInOP(Order memory _order, address _recipient)
        ...
        returns (uint256)
    {
        RewardInfo[] memory rewardInfoArray = rewardDeriver.getRewardInOP(_order, _recipient);
        ...
        return totalRewardAmountInOP;
    }

    function getRewardSentByCampaignInOP(string memory _campaignId) external view returns (uint256) {
        return rewardSentByCampaignInOP[keccak256(bytes(_campaignId))];
    }

    /**
     * @notice Allows the contract owner to withdraw all ETH in this contract.
     */
    function withdrawETH() external onlyOwner {
        uint256 amount = address(this).balance;
        // solhint-disable-next-line avoid-low-level-calls
        (bool success, ) = payable(msg.sender).call{ value: amount }(new bytes(0));
        require(success, "RewardWrapper: call to ChugSplashManager failed");
        emit ETHWithdrawn(msg.sender, amount);
    }

    /**
     * @notice Allows the owner to set the RewardDeriver address.
     *
     * @param _rewardDeriver The new RewardDeriver address.
     */
    function setRewardDeriver(IRewardDeriver _rewardDeriver) external onlyOwner {
        rewardDeriver = _rewardDeriver;
        emit RewardDeriverSet(address(_rewardDeriver));
    }
    ...
    /**
     * @notice inheritdoc IERC1155Receiver
     */
    ...
    /**
     * @notice Allows this contract to receive ETH from Seaport if there is any leftover amount
