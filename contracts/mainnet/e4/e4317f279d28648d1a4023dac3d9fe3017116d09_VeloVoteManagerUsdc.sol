...
// File contracts/interfaces/IVeloVoteFarmerUsdc.sol
...
/// @dev Vote for velodrome pairs, get rewards and grow veVelo
interface IVeloVoteFarmerUsdc is IVeloVoteFarmer {
    function claim(uint256 _tokenId, address _owner) external;
    function claimAll(address _owner) external;
    function pendingRewards(uint256 _tokenId) external view returns (uint256);
    function userPendingRewards(address _user) external view returns (uint256);
}
...
// File contracts/VeloVoteManagerUsdc.sol
...
contract VeloVoteManagerUsdc is UUPSUpgradeable, AccessControlEnumerableUpgradeable, PausableUpgradeable {
    using EnumerableSetUpgradeable for EnumerableSetUpgradeable.AddressSet;
    /* ========== STATE VARIABLES ========== */
    /// @dev Role
    ...
    function deposit(uint256 _tokenId) external whenNotPaused {
        ...
        IVeloVoteFarmerUsdc(strategy).deposit(_tokenId, msg.sender);
        tokenIdToStrat[_tokenId] = strategy;
    }

    function withdraw(uint256 _tokenId) external {
        address strategy = tokenIdToStrat[_tokenId];
        IVeloVoteFarmerUsdc(strategy).withdraw(_tokenId, msg.sender);
    ...
    function autoLock(uint256 _tokenId, bool _enable) external {
        address strategy = tokenIdToStrat[_tokenId];
        IVeloVoteFarmerUsdc(strategy).autoLock(_tokenId, _enable, msg.sender);
    }

    function claim(uint256 _tokenId) external {
        address strategy = tokenIdToStrat[_tokenId];
        IVeloVoteFarmerUsdc(strategy).claim(_tokenId, msg.sender);
    }

    function claimAll() external {
        for (uint256 i; i < strategies.length(); i = _uncheckedInc(i)) {
            IVeloVoteFarmerUsdc(strategies.at(i)).claimAll(msg.sender);
        }
    }
    ...
    function vote(uint256 start, uint256 end) external {
        ...
        for (uint256 i = start; i < end; i = _uncheckedInc(i)) {
            IVeloVoteFarmerUsdc(strategies.at(i)).vote();
        }
    }
    ...
    function harvest(uint256 start, uint256 end) external {
        ...
        // Harvest
        for (uint256 i = start; i < end; i = _uncheckedInc(i)) {
            IVeloVoteFarmerUsdc(strategies.at(i)).harvest();
        }
    }
    ...
    function increaseDurationAll(uint256 start, uint256 end) external {
        ...
        //Increase Duration
        for (uint256 i = start; i < end; i = _uncheckedInc(i)) {
            IVeloVoteFarmerUsdc(strategies.at(i)).increaseDurationAll();
        }
    }
    ...
    function resetBalancesAll(uint256 start, uint256 end) external {
        ...
        // Reset balances
        for (uint256 i = start; i < end; i = _uncheckedInc(i)) {
            IVeloVoteFarmerUsdc(strategies.at(i)).resetBalancesAll();
        }
    }


    /* ========== ADMIN ========== */
    function checkTokenId(uint256 _tokenId) external {
        _atLeastRole(KEEPER);
        address strategy = tokenIdToStrat[_tokenId];
        for (uint256 i; i < IVeloVoteFarmer(strategy).tokenIdsLength(); i = _uncheckedInc(i)) {
            if (IVeloVoteFarmer(strategy).tokenIds(i) == _tokenId) {
                return;
            }
        }
        // Reaching this point means the token is no longer managed
        tokenIdToStrat[_tokenId] = address(0);
    }
    ...
    function selectDepositStrategy() public view returns (address depositStrategy, bool strategiesFull) {
        ...
        for (uint256 i; i < stratLen; i = _uncheckedInc(i)) {
            if (IVeloVoteFarmerUsdc(strategies.at(i)).tokenIdsLength() < nftCap) {
                depositStrategy = strategies.at(i);
                if ((i == stratLen - 1) && (IVeloVoteFarmerUsdc(strategies.at(i)).tokenIdsLength() == nftCap - 1)) {
                    strategiesFull = true;
                }
                break;
            }
    ...
    function _deployStrategy() internal returns (address strategy) {
        ...
        IVeloVoteFarmerUsdc(strategy).initialize();
    ...
    function _synchronizeStrategies(uint256 start, uint256 end) internal {
        _verifySlice(start, end);
        for(uint256 i = start; i < end; i = _uncheckedInc(i)) {
            IVeloVoteFarmerUsdc(strategies.at(i)).synchronize();
        }
    }
    ...
    function pendingRewards(uint256 _tokenId) external view returns(uint256) {
        address strategy = tokenIdToStrat[_tokenId];
        return IVeloVoteFarmerUsdc(strategy).pendingRewards(_tokenId);
    }

    function userPendingRewards(address _user) external view returns(uint256) {
        uint256 total;
        for (uint256 i; i < strategies.length(); i = _uncheckedInc(i)) {
            total += IVeloVoteFarmerUsdc(strategies.at(i)).userPendingRewards(_user);
        }
        return total;
    }
    ...
    function pause(uint256 start, uint256 end) external {
        ...
        _pause();
        for (uint256 i = start; i < end; i = _uncheckedInc(i)) {
            IVeloVoteFarmerUsdc(strategies.at(i)).pause();
        }
    }
    ...
    function unpause(uint256 start, uint256 end) external {
        ...
        _unpause();
        for (uint256 i = start; i < end; i = _uncheckedInc(i)) {
            IVeloVoteFarmerUsdc(strategies.at(i)).unpause();
        }
    }
}