...
interface Gauge:
    ...
    def reward_data(_reward_token: address) -> (address, uint256): view
...
def deposit_reward_token(_reward_token: address):
    ...
    @dev Any user can forward deposited reward tokens to a gauge, after calling
        `allow`.
    """
    depositor: address = ZERO_ADDRESS
    period_finish: uint256 = 0
    depositor, period_finish = Gauge(GAUGE).reward_data(_reward_token)
    assert period_finish <= block.timestamp and depositor == self
