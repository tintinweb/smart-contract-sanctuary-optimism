...
contract TherundownConsumer is ChainlinkClient {
    ...
    function storeGamesCreated(uint256 _idx) external view returns (GameCreate memory) {
        GameCreate memory game = abi.decode(games[_idx], (GameCreate));
        return game;
    }

    function storeGamesResolved(uint256 _idx) external view returns (GameResolve memory) {
