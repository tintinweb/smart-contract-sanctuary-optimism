...
import { ICommonVaultStruct } from "./interface/ICommonVaultStruct.sol";
import { IKtbArbitrageurEvent } from "./interface/IKtbArbitrageurEvent.sol";
import { KtbArbitrageurStorage } from "./storage/KtbArbitrageurStorage.sol";
import { KtbArbitrageurDispatcher } from "./KtbArbitrageurDispatcher.sol";

contract KtbArbitrageur is
    IKtbArbitrageurEvent,
    KtbArbitrageurDispatcher,
    SafeOwnable,
    ReentrancyGuardUpgradeable,
    KtbArbitrageurStorage
{
    using PerpMath for int256;
    using SafeMath for uint256;

    address public constant ONE_INCH_AGGREGATION_ROUTER_V5 = address(0x1111111254EEB25477B68fb85Ed929f73A960582);
    address public constant USDC_USD_CHAINLINK_AGGREGATOR = address(0x16a9FA2FDa030272Ce99B29CF780dFA30361E0f3);
    ...
    /// @dev If we consider USDC/USD in the range of [1.003, 0.997] is pegged,
    /// we should set _maxUsdcDepeggedDeviation to 0.003e8 = 300000
    function setMaxUsdcDepeggedDeviation(uint256 maxUsdcDepeggedDeviationArg) external onlyOwner {
        // KA_IDDU: invalid depegged deviation USDC
        require(maxUsdcDepeggedDeviationArg < 1e8, "KA_IDDU");
        _maxUsdcDepeggedDeviation = maxUsdcDepeggedDeviationArg;
        emit UpdateMaxUsdcDepeggedDeviation(_maxUsdcDepeggedDeviation);
    }

    function setWhitelistedCaller(address whitelistedCallerArg, bool enable) external onlyOwner {
        // KA_SV: same value
        require(_whitelistedCallerMap[whitelistedCallerArg] != enable, "KA_SV");

        _whitelistedCallerMap[whitelistedCallerArg] = enable;

        emit UpdateWhitelistedCaller(whitelistedCallerArg, enable);
    }
    ...
    function arbitrage(
        ...
        // KA_CAC: caller is a contract
        require(tx.origin == msgSender, "KA_CAC");

        return _arbitrageFrom(msgSender, kantabanVault, tokenIn, tokenOut, amountIn, arbitrageType, callData);
    }

    function deleverage(address kantabanVault, uint256 reducedPositionSizeAbs)
        external
        nonReentrant
        returns (int256 pnl)
    {
        // KA_IC: Invalid Caller
        require(_whitelistedCallerMap[_msgSender()], "KA_IC");
        return ICommonVault(kantabanVault).deleverage(reducedPositionSizeAbs);
    }

    /// @dev this function is for quoter using, will always revert, do not interact it on-chain
    function revertSwapOnKtbFrom(
        address arbitrageur,
        address kantabanVault,
        address tokenIn,
        address tokenOut,
        ...
        _transferTokenIn(tokenIn, arbitrageur, amountIn);
        uint256 amountOut = _swapOnKtb(kantabanVault, tokenIn, tokenOut, amountIn);

        // 0x40 is the free memory pointer address
        // amountOut is uint256 which is 32 bytes
        assembly {
            let ptr := mload(0x40)
    ...
    function revertArbitrageFrom(
        ...
        // 0x40 is the free memory pointer address
        // profit is uint256 which is 32 bytes
        assembly {
            let ptr := mload(0x40)
            mstore(ptr, profit)
            revert(ptr, 32)
    ...
    function isWhitelistedCaller(address caller) external view returns (bool) {
        return _whitelistedCallerMap[caller];
    }

    function getMaxUsdcDepeggedDeviation() external view returns (uint256) {
        return _maxUsdcDepeggedDeviation;
    }
    ...
    function _arbitrageFrom(
        ...
        _requireUsdcIsPegged();

        _transferTokenIn(tokenIn, arbitrageur, amountIn);

        uint256 amountOut;
        if (arbitrageType == ArbitrageType.SWAP_KTB_THEN_COMBO) {
            uint256 amountOutFromKtb = _swapOnKtb(kantabanVault, tokenIn, tokenOut, amountIn);
            amountOut = _swapCombo(amountOutFromKtb, callData);
        } else if (arbitrageType == ArbitrageType.SWAP_COMBO_THEN_KTB) {
            uint256 amountOutFromCombo = _swapCombo(amountIn, callData);
            amountOut = _swapOnKtb(kantabanVault, tokenOut, tokenIn, amountOutFromCombo);
        } else if (arbitrageType == ArbitrageType.SWAP_1INCH_THEN_KTB) {
            uint256 amountOutFromOneInch = _swapOnOneInch(tokenIn, tokenOut, amountIn, callData);
            amountOut = _swapOnKtb(kantabanVault, tokenOut, tokenIn, amountOutFromOneInch);
        } else if (arbitrageType == ArbitrageType.SWAP_KTB_THEN_1INCH) {
            uint256 amountOutFromKtb = _swapOnKtb(kantabanVault, tokenIn, tokenOut, amountIn);
            amountOut = _swapOnOneInch(tokenOut, tokenIn, amountOutFromKtb, callData);
        } else {
            // KA_UAT: unknown arbitrage type
            revert("KA_UAT");
        }

        // KA_NP: no profit
        require(amountOut > amountIn, "KA_NP");

        // return funds to msg.sender
        SafeERC20.safeTransfer(IERC20(tokenIn), arbitrageur, amountIn);
        ...
        emit ArbitrageWithType(kantabanVault, tokenIn, tokenOut, amountIn, arbitrageType, profit);

        return profit;
    }

    function _swapCombo(uint256 amountIn, bytes calldata comboData) internal virtual returns (uint256) {
        // comboData: selector(4bytes) + tokenIn(32bytes) + tokenOut(32bytes)
        uint256 length = comboData.length / 68;

        // KA_LZ: length is zero
        require(length > 0, "KA_LZ");

        callBySwapCombo = 2;

        for (uint8 i = 0; i < length; i++) {
            // callData: selector(4bytes) + tokenIn(32bytes) + tokenOut(32bytes) + amountIn(32bytes)
            bytes memory callData = new bytes(100);

            assembly {
                // copy 68 bytes from callData to comboData[0:68]
                calldatacopy(add(callData, 0x20), add(comboData.offset, mul(i, 0x44)), 0x44)
                // store amountIn to callData[68:100]
                mstore(add(callData, 0x64), amountIn)
            }

            // Because we use proxy pattern, ADDRESS_THIS is our impl address
            (bool success, bytes memory result) = ADDRESS_THIS.delegatecall(callData);
            // KA_DCF: delegatecall failed
            require(success, "KA_DCF");
    ...
    function _swapOnKtb(
        ...
        return
            ICommonVault(kantabanVault).swapExactInput(
                ICommonVaultStruct.SwapExactInputParams({
                    tokenIn: tokenIn,
                    tokenOut: tokenOut,
                    recipient: address(this),
                    deadline: block.timestamp,
                    amountIn: amountIn,
                    amountOutMinimum: 0, // we require(amountOut > amountIn) in arbitrage(), should be fine to set to 0
                    sqrtPriceLimitX96OnPerp: 0
                })
            );
    }
    ...
    function _swapOnOneInch(
        ...
        IERC20(tokenIn).approve(ONE_INCH_AGGREGATION_ROUTER_V5, amountIn);

        uint256 tokenOutBalanceBefore = IERC20(tokenOut).balanceOf(address(this));

        (bool success, ) = ONE_INCH_AGGREGATION_ROUTER_V5.call{ value: 0 }(oneInchTxData);
        if (!success) {
            // KA_OSF: 1inch Swap Fail, for instance, due to slippage
            revert("KA_OSF");
        }
    ...
    function _transferTokenIn(
        ...
        // KA_IBA: inconsistent balance amount, to prevent from deflationary tokens
        require(balanceNow > balanceBefore && (balanceNow - balanceBefore) == amount, "KA_IBA");
    }
    ...
    function _requireUsdcIsPegged() internal view {
        AggregatorV3Interface chainlinkAggregator = AggregatorV3Interface(USDC_USD_CHAINLINK_AGGREGATOR);
        uint8 decimals = chainlinkAggregator.decimals();

        // KA_DC: Decimals Changed
        require(decimals == 8, "KA_DC");

        (, int256 price, , , ) = chainlinkAggregator.latestRoundData();

        // KA_IP: invalid price
        require(price >= 0, "KA_IP");

        // we expect USDC/USD = 1 = 1e8 (Chainlink aggregator decimals is 8)
        // KA_UD: USDC Depegged
        require((1e8 - price).abs() <= _maxUsdcDepeggedDeviation, "KA_UD");
    }

    function _onlyCallBySwapCombo() internal view override {
        // KA_OCSC: only call by swap combo
        require(callBySwapCombo == 2, "KA_OCSC");
    }
}
...
import { ICurveRegistry } from "./interface/ICurveRegistry.sol";
import { ICurveFactory } from "./interface/ICurveFactory.sol";
import { ICurve3Pool } from "./interface/ICurve3Pool.sol";
import { ICurve4Pool } from "./interface/ICurve4Pool.sol";
import { IMummyRouter } from "./interface/IMummyRouter.sol";

abstract contract KtbArbitrageurDispatcher {
    address public constant UNISWAP_V3_ROUTER = address(0xE592427A0AEce92De3Edee1F18E0157C05861564);
    address public constant MUMMY_ROUTER = address(0x68d1CA32Aee9a73534429D8376743Bf222ff1870);
    address public constant CURVE_3POOL = address(0x1337BedC9D22ecbe766dF105c9623922A27963EC);
    address public constant CURVE_REGISTRY = address(0xC5cfaDA84E902aD92DD40194f0883ad49639b023);
    address public constant CURVE_4POOL = address(0x061b87122Ed14b9526A813209C8a59a633257bAb);
    address public constant CURVE_FACTORY = address(0x2db0E83599a91b508Ac268a6197b8B14F5e72840);

    function swapOnMummy(
        address tokenIn,
        address tokenOut,
        uint256 amountIn
    ) public returns (uint256) {
        _onlyCallBySwapCombo();
        ...
        IMummyRouter(MUMMY_ROUTER).swap(path, amountIn, 0, address(this));

        return IERC20(tokenOut).balanceOf(address(this));
    }

    function swapOnCurve3Pool(
        address tokenIn,
        address tokenOut,
        uint256 amountIn
    ) public returns (uint256) {
        ...
        (int128 fromIndex, int128 toIndex, ) = ICurveRegistry(CURVE_REGISTRY).get_coin_indices(
            CURVE_3POOL,
            tokenIn,
            tokenOut
        );
        return ICurve3Pool(CURVE_3POOL).exchange(fromIndex, toIndex, amountIn, 0);
    }

    function swapOnCurve4Pool(
        address tokenIn,
        address tokenOut,
        uint256 amountIn
    ) public returns (uint256) {
        _onlyCallBySwapCombo();
        IERC20(tokenIn).approve(CURVE_4POOL, amountIn);
        (int128 fromIndex, int128 toIndex, ) = ICurveFactory(CURVE_FACTORY).get_coin_indices(
            CURVE_4POOL,
            tokenIn,
            tokenOut
        );
        return ICurve4Pool(CURVE_4POOL).exchange_underlying(fromIndex, toIndex, amountIn, 0);
    }

    function swapOnUniswapV3Pool100(
        address tokenIn,
        address tokenOut,
        uint256 amountIn
    ) public returns (uint256) {
        return _swapOnUniswapV3Pool(tokenIn, tokenOut, uint24(100), amountIn);
    }

    function swapOnUniswapV3Pool500(
        address tokenIn,
        address tokenOut,
        uint256 amountIn
    ) public returns (uint256) {
        return _swapOnUniswapV3Pool(tokenIn, tokenOut, uint24(500), amountIn);
    }

    function swapOnUniswapV3Pool3000(
        address tokenIn,
        address tokenOut,
        uint256 amountIn
    ) public returns (uint256) {
        return _swapOnUniswapV3Pool(tokenIn, tokenOut, uint24(3000), amountIn);
    }
    ...
    function _onlyCallBySwapCombo() internal view virtual;
}
...
import { KtbArbitrageur } from "./KtbArbitrageur.sol";

contract KtbArbitrageurQuoter {
    KtbArbitrageur public immutable ktbArbitrageur;

    constructor(KtbArbitrageur ktbArbitrageurArg) {
        ktbArbitrageur = ktbArbitrageurArg;
    }

    function quoteSwapOnKtbFrom(
        address arbitrageur,
        address kantabanVault,
        address tokenIn,
        address tokenOut,
        ...
        try
            ktbArbitrageur.revertSwapOnKtbFrom(arbitrageur, kantabanVault, tokenIn, tokenOut, amountIn)
    ...
    function quoteArbitrageFrom(
        address arbitrageur,
        address kantabanVault,
        address tokenIn,
        address tokenOut,
        uint256 amountIn,
        KtbArbitrageur.ArbitrageType arbitrageType,
        bytes calldata oneInchTxData
    ) external returns (uint256 profit) {
        try
            ktbArbitrageur.revertArbitrageFrom(
                arbitrageur,
                kantabanVault,
                tokenIn,
                tokenOut,
    ...
    function _parseRevertReason(bytes memory reason) private pure returns (uint256 amount) {
        if (reason.length != 32) {
            if (reason.length < 68) revert("Unexpected Error");

            // if reason.length >= 68, the reason is the revert message, skip the first 4 bytes of signature hash,
            // then decode the actual revert message
            // solhint-disable-next-line no-inline-assembly
            assembly {
        ...
        // if reason.length is 32, the reason is the amountOut
...
interface IKtbArbitrageurEvent {
    enum ArbitrageType {
        SWAP_KTB_THEN_COMBO,
        SWAP_COMBO_THEN_KTB,
        SWAP_1INCH_THEN_KTB,
    ...
    event UpdateMaxUsdcDepeggedDeviation(uint256 maxUsdcDepeggedDeviation);

    event UpdateWhitelistedCaller(address caller, bool enable);
}
...
/// @notice For future upgrades, do not change KtbArbitrageurStorageV1. Create a new
/// contract which implements KtbArbitrageurStorageV1 and following the naming convention
/// KtbArbitrageurStorageVX.
abstract contract KtbArbitrageurStorage {
    // we only apply this to deleverage(),
    // arbitrage() can still be called by any EOA
    mapping(address => bool) internal _whitelistedCallerMap;
    uint256 internal _maxUsdcDepeggedDeviation;
    uint8 callBySwapCombo;
}