...
contract StrategyAlphaHomora is BaseStrategy {
    ...
    constructor(
        ...
        IERC20(USDC).approve(address(integration), MAX_INT);
        IERC20(stableToken).approve(address(integration), MAX_INT);
        IERC20(volatileToken).approve(address(integration), MAX_INT);
   ...
   function requireAmmCheckPass() view internal {
    ...
    function _decreasePosition(uint256 gradient) internal returns (uint256) {
        ...
        requireAmmCheckPass();
        {
            ...
            amountWithdrawn = IERC20(stableToken).balanceOf(address(this))-stableBalance0;
...
contract UniswapV3SpellIntegrationOp is BaseIntegration, Ownable {
  ...
  function withdrawOther(address _asset) external onlyOwner returns (uint256 balance) {
    ...
    IERC20(_asset).safeTransfer(owner(), balance);
  }
  ...
  function _volatilePrice(uint _positionId) public view returns (uint) {
    ...
    uint stableDecimals = uint(10)**ERC20(stableToken).decimals();
    address volatileToken = stableToken==pool.token0()?pool.token1():pool.token0();
    uint volatileDecimals = uint(10)**ERC20(volatileToken).decimals();
  ...
  function _convertToStable(uint _positionId, uint slippage) internal returns (uint) {
    ...
    uint volatileBalance = IERC20(volatileToken).balanceOf(address(this));
    ...
    uint stableBalance = IERC20(stableToken).balanceOf(address(this));
    return stableBalance;
  }
  ...
  function _getPositionBalance(uint _positionId) public view returns (uint) {
    ...
    address volatileToken = stableToken==posInfo.token0?posInfo.token1:posInfo.token0;
    volatileDecimals = uint(10)**ERC20(volatileToken).decimals();
...
abstract contract BaseStrategy is Pausable, SettAccessControl {
    ...
    address public constant USDC = 0x7F5c764cBc14f9669B88837ca1490cCa17c31607; //MAINNET AVALANCHE
