...
contract RangoAcross is IRangoAcross, BaseProxyContract {
    ...
    function initialize(address _weth, address[] calldata _addresses) public initializer {
        BaseProxyStorage storage baseStorage = getBaseProxyContractStorage();
    ...
    function addAcrossSpokePools(address[] calldata _addresses) public onlyOwner {
        addAcrossSpokePoolsInternal(_addresses);
    }
    ...
    function acrossBridge(
        ...
        require(acrossSpokePools[request.spokePoolAddress], "Requested spokePool address not whitelisted");
        if (fromToken == ETH) {
            ...
            approve(fromToken, request.spokePoolAddress, amount);
        }
