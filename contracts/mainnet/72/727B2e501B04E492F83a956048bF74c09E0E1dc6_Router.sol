...
contract Router {
	...
	mapping(address => uint256) private rthmShare; // currency => bps

	mapping(address => address) poolRewards; // currency => contract
	mapping(address => address) rthmRewards; // currency => contract
	...
	function getRthmShare(address currency) external view returns(uint256) {
		return rthmShare[currency];
	}
	...
	function getRthmRewards(address currency) external view returns(address) {
		return rthmRewards[currency];
	}
	...
	function setRthmShare(address currency, uint256 share) external onlyOwner {
		rthmShare[currency] = share;
	}
	...
	function setRthmRewards(address currency, address _contract) external onlyOwner {
		rthmRewards[currency] = _contract;
	}
...
interface IRouter {
    ...
    function rthmPool() external view returns (address);
