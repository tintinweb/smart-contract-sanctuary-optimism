...
contract FlashDownRecipient is IFlashLoanRecipient {
    ...
    uint public constant FEE_PCT = 5;
   ...
   struct FlashDownParams {
    ...
    function requestFlashLoan(FlashDownParams calldata flashDownParams,
     ...
     function receiveFlashLoan(
        ...
        // Convert them back to debtToken to repay flashloan & incur fee
        uint256 myFee = swapParams[0].amountIn * this.FEE_PCT() / 10000;
        swapParams[0].amountIn -= myFee;
        ...
        require(leftOver >= myFee, 'Insufficient balance to pay app fee!');
        if(leftOver != myFee) {
            IERC20(flashDownParams.lendAsset).transfer(flashDownParams.onBehalfOf, leftOver - myFee);
        }
