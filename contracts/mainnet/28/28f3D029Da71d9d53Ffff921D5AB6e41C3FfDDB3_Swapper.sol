...
contract Swapper {
  ...
  /// @notice Swaps `address(this).balance` ETH for EXA and sends it to `account`.
  ...
  function swap(address payable account, uint256 minEXA, uint256 keepETH) external {
    if (keepETH > address(this).balance) return account.safeTransferETH(address(this).balance);

    uint256 inETH = address(this).balance - keepETH;
    uint256 outEXA = pool.getAmountOut(inETH, weth);
    if (outEXA < minEXA) return account.safeTransferETH(address(this).balance);
    ...
    try pool.swap(amount0Out, amount1Out, account, "") {
      ...
      account.safeTransferETH(address(this).balance);
    }
  }
