...
contract Interaction is Initializable, UUPSUpgradeable, OwnableUpgradeable, WardedLivingUpgradeable, IInteraction {
    ...
    function newFund(uint256 fundId,
        ...
        feeder.newFund(fundId, manager, minStakingAmount, minWithdrawalAmount, itoken);
    ...
    function tvl(uint256 fundId, uint256 tradeTvl) public override view returns (int256) {
        ITrade trader = ITrade(funds[fundId].trade);
        return feeder.tvl(fundId, tradeTvl) + int256(trader.tvl());
    }
...
//        uint256 amountOut = Math.mulDiv(amount, tvl, tokenSupply);
    ...
    function drip(uint256 fundId, uint256 tradeTvl) external override live auth {
        ...
        int256 pending = feeder.tvl(fundId, tradeTvl);
        console.log("Interact-Pending:");
        console.logInt(pending);
        console.logInt(tvl(fundId, tradeTvl));

        ITrade trader = ITrade(funds[fundId].trade);
        uint256 traderTvl = trader.tvl();
        console.log("trader tvl:", tradeTvl);
    ...
    function withdrawMultiple(uint256 fundId, address[] calldata users, uint256 tradeTvl) external override auth {
        ...
        feeder.withdrawMultiple(fundId, users, tradeTvl);
    }
    ...
    function tokenRate(uint256 fundId, uint256 tradeTvl) public override view returns (uint256) {
        int256 tvl = tvl(fundId, tradeTvl);
        if (tvl < 0) {
            tvl = 0;
        }
    ...
    function quoteTokens(uint256 fundId, uint256 tradeTvl, uint256 usdt) external override view returns (uint256) {
        return usdt * 1e18 / tokenRate(fundId, tradeTvl);
    }

    function userTVL(uint256 fundId, uint256 tradeTvl, address user) external override view returns (uint256) {
        return Math.mulDiv(funds[fundId].itoken.balanceOf(user), tokenRate(fundId, tradeTvl), 1e18);
    }

    function pendingTvl(uint256 fundId, uint256 tradeTvl) external view returns (int256) {
        return feeder.tvl(fundId, tradeTvl);
    }
