...
contract Roulette is RrpRequesterV0 {
    ...
    function checkIfNumberWon() external returns (uint256) {
        if (userToCurrentBet[msg.sender] == 0) revert NoBet();
        if (!userBetANumber[msg.sender]) revert NoBet();
        uint256 _spin = userToSpinCount[msg.sender];
        if (!spinIsComplete[_spin]) revert SpinNotComplete();
        if (spinResult[_spin] == 37) {
            (bool sent, ) = msg.sender.call{
                value: userToCurrentBet[msg.sender]
            }("");
            if (!sent) revert ReturnFailed();
        } else {}
        if (userToNumber[msg.sender] == spinResult[_spin] % 37) {
            (bool sent, ) = msg.sender.call{
                value: userToCurrentBet[msg.sender] * 35
            }("");
            if (!sent) revert HouseBalanceTooLow();
        } else {
            (bool sent, ) = sponsorWallet.call{
                value: userToCurrentBet[msg.sender] / 10
            }("");
            if (!sent) revert TransferToSponsorWalletFailed();
            (bool sent2, ) = deployer.call{
                value: userToCurrentBet[msg.sender] / 50
            }("");
            if (!sent2) revert TransferToDeployerWalletFailed();
        }
        userToCurrentBet[msg.sender] = 0;
        userBetANumber[msg.sender] = false;
    ...
    /// @return userToSpinCount[msg.sender] spin count for this msg.sender, to enter in spinRouletteWheel()
    function betOneThird(uint256 _oneThirdBet)
        external
        payable
        returns (uint256)
    {
    ...
    /// @dev unsuccessful bet sends 10% to sponsor wallet to ensure future fulfills, 2% to deployer, rest kept by house
    function checkIfThirdWon() external returns (uint256) {
        if (userToCurrentBet[msg.sender] == 0) revert NoBet();
        if (!userBetThird[msg.sender]) revert NoBet();
        uint256 _spin = userToSpinCount[msg.sender];
        if (!spinIsComplete[_spin]) revert SpinNotComplete();
        ...
        if (userToThird[msg.sender] == 1 && _thirdResult == 1) {
            (bool sent, ) = msg.sender.call{
                value: userToCurrentBet[msg.sender] * 3
            }("");
            if (!sent) revert HouseBalanceTooLow();
        } else if (userToThird[msg.sender] == 2 && _thirdResult == 2) {
            (bool sent, ) = msg.sender.call{
                value: userToCurrentBet[msg.sender] * 3
            }("");
            if (!sent) revert HouseBalanceTooLow();
        } else if (userToThird[msg.sender] == 3 && _thirdResult == 3) {
            (bool sent, ) = msg.sender.call{
                value: userToCurrentBet[msg.sender] * 3
            }("");
            if (!sent) revert HouseBalanceTooLow();
    ...
    /// @dev unsuccessful bet sends 10% to sponsor wallet to ensure future fulfills, 2% to deployer, rest kept by house
    function checkIfEvenOddWon() external returns (uint256) {
        if (userToCurrentBet[msg.sender] == 0) revert NoBet();
        if (!userBetEvenOdd[msg.sender]) revert NoBet();
        uint256 _spin = userToSpinCount[msg.sender];
        if (!spinIsComplete[_spin]) revert SpinNotComplete();
        uint256 _result = spinResult[_spin] % 37;
        if (spinResult[_spin] == 37) {
        ...
        if (_result == 0) {
            ...
            if (!sent) revert TransferToSponsorWalletFailed();
        } else if (userToEven[msg.sender] && (_result % 2 == 0)) {
            (bool sent, ) = msg.sender.call{
                value: userToCurrentBet[msg.sender] * 2
            }("");
            if (!sent) revert HouseBalanceTooLow();
        } else if (!userToEven[msg.sender] && _result % 2 != 0) {
            (bool sent, ) = msg.sender.call{
        ...
        userBetEvenOdd[msg.sender] = false;
        userToCurrentBet[msg.sender] = 0;
        emit WinningNumber(_spin, spinResult[_spin] % 37);
        return (spinResult[_spin] % 37);
    }
    ...
    /// @dev unsuccessful bet sends 10% to sponsor wallet to ensure future fulfills, 2% to deployer, rest kept by house
    function checkIfColorWon() external returns (uint256) {
        if (userToCurrentBet[msg.sender] == 0) revert NoBet();
        if (!userBetAColor[msg.sender]) revert NoBet();
        uint256 _spin = userToSpinCount[msg.sender];
        if (!spinIsComplete[_spin]) revert SpinNotComplete();
        uint256 _result = spinResult[_spin] % 37;
        if (spinResult[_spin] == 37) {
            ...
            if (userToColor[msg.sender] && blackSpin[_spin]) {
                (bool sent, ) = msg.sender.call{
                    value: userToCurrentBet[msg.sender] * 2
                }("");
                if (!sent) revert HouseBalanceTooLow();
            } else if (
                !userToColor[msg.sender] && !blackSpin[_spin] && _result != 0
            ) {
                (bool sent, ) = msg.sender.call{
                    value: userToCurrentBet[msg.sender] * 2
                }("");
        ...
        userBetAColor[msg.sender] = false;
        userToCurrentBet[msg.sender] = 0;
        emit WinningNumber(_spin, spinResult[_spin] % 37);
        return (spinResult[_spin] % 37);
    }
    ...
    function spinRouletteWheel(uint256 _spinCount) external {
