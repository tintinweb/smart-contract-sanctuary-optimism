...
contract Swapper {
  ...
  /// @notice Swaps WETH allowance for EXA and sends it to `account`.
  ...
  function swap(address payable account, uint256 minEXA, uint256 keepETH) external {
    uint256 amountWETH = weth.allowance(msg.sender, address(this));
    keepETH = keepETH <= amountWETH ? keepETH : amountWETH;
    uint256 inWETH = amountWETH - keepETH;
    uint256 outEXA = pool.getAmountOut(inWETH, weth);
    if (outEXA < minEXA || inWETH == 0) {
      weth.safeTransferFrom(msg.sender, account, inWETH);
      weth.safeTransferFrom(msg.sender, address(this), keepETH);
      weth.withdraw(keepETH);
      return account.safeTransferETH(keepETH);
    }

    weth.safeTransferFrom(msg.sender, address(pool), inWETH);
