    ...
    function adjustPosition(bool operationType, bool positionType, uint256 amount) public override onlyTrigger() {
        ...
        if (operationType) {// openPosition
            ...
            if (positionType) {
                ...
                uint256 freeCollateral = _perpVault.getFreeCollateral(address(this));
                if (freeCollateral < usdcAmount) {
    ...
    * @notice Total deposit amount
    ...
    function _closePositionsForWithdrawal(uint256 reserve, uint256 toWithdraw) private {
        ...
        if (toClose > getTotalUSDCValue() / 2 ||
            ...
            // If we have too much inside perp vault just withdraw from there
            uint256 perpPosition = IAccountBalance(_perpVault.getAccountBalance()).getTotalAbsPositionValue(address(this));
            uint256 freeCollateral = _perpVault.getFreeCollateral(address(this));
            uint256 ethValue = Math.mulDiv(IERC20(_wToken).balanceOf(address(this)), nativeStrategyTokenPrice(), 1e18);
            if (perpPosition > ethValue && freeCollateral > toClose) {
                _withdrawFromPerp(toClose);
            } else {
                _uniswapExactOutput(_wToken, _usdToken, toClose, IERC20(_wToken).balanceOf(address(this)));
