...
contract jointLPHolderVelo is Ownable {
    ...
    constructor (
        ...
        farm = IGauge(_farm);
        router = ISolidlyRouter01(_router);
        weth = router.weth();
        rewardTokens.push(IERC20(_rewardToken));

        IERC20(_rewardToken).approve(_router, uint256(-1));
        IERC20(address(lp)).approve(_farm, uint256(-1));

    }

    function name() external view returns (string memory) {
        return "JointLPHolderVelo";
    }
    ...
    function pendingRewards() public view returns(uint256) { 
        return(farm.earned(address(rewardTokens[0]), address(this)));
    }
    ...
    function countLpPooled() public view returns (uint256) {
        uint256 _amount = farm.balanceOf(address(this));
        return _amount;
    }
    ...
    function _depositToFarm() internal {
        ...
        if(lpAmt > 0) { 
            farm.depositAll(farmPid); /// deposit LP tokens to farm
        }
    ...
    function _harvestInternal() internal {
        ...
        farm.getReward(address(this), tokens);
        for (uint256 i = 0; i < rewardTokens.length; i++){
