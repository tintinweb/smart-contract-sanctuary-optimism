...
import "src/adapters/ProviderAwareOracle.sol";

import { console2 } from "forge-std/console2.sol";

contract VelodromeV2TWAP is ProviderAwareOracle {
    struct TwapConfig {
        address pairAddress;
        uint8 decimals;
    }
    ...
    function _fetchPrice(address asset) private view returns (uint256 amountOut) {
        TwapConfig memory twap = twaps[asset];
        IPairV2 pair = IPairV2(twap.pairAddress);

        uint8 decimals = twap.decimals; // 18

        //Granularity 5 = last 5 points = 2.5 hours @30min window.

        if (decimals > TOKEN_DECIMALS) {
            uint256 _tokenMissingDecimals = decimals - TOKEN_DECIMALS; // 12
            amountOut = pair.quote(asset, PRECISION, 5) * (10 ** _tokenMissingDecimals);
        } else {
            uint256 _tokenMissingDecimals = TOKEN_DECIMALS - decimals;
            amountOut = pair.quote(asset, PRECISION, 5) / (10 ** _tokenMissingDecimals);
        }
    ...
    function initializeOracle(address asset, bool isStable) external onlyOwner {
        ...
        // console2.log("token 0: %s", token0);
        // console2.log("token 1: %s", token1);
        // console2.log("is stable: %s", isStable);
...
interface IPairV2 {
    ...
    function quote(address tokenIn, uint256 amountIn, uint granularity) external view returns (uint256 amountOut);
}
