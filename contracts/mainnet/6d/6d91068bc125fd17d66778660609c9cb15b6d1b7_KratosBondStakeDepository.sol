...
contract KratosBondStakeDepository is Ownable {
    ...
    address public immutable Kratos; // intermediate reward token from treasury
    address public immutable sKratos; // token given as payment for bond
    ...
    struct Terms {
        ...
        uint32 vestingTerm; // in timestamp
    ...
    struct Bond {
        uint gonsPayout; // sKratos gons remaining to be paid
        uint KratosPayout; //Kratos amount at the moment of bond
        uint32 vesting; // Second left to vest
    ...
    function deposit( 
        ...
        IERC20( Kratos ).approve( stakingManager, payout );

        IStakingManager(stakingManager).stake( payout, address(this) );
        /* ---------------------------------------------------------- */
        
        uint stakeGons=ISKratos(sKratos).gonsForBalance(payout);

        // depositor info is stored
        _bondInfo[ _depositor ] = Bond({ 
            gonsPayout: _bondInfo[ _depositor ].gonsPayout.add( stakeGons ),
            KratosPayout: _bondInfo[ _depositor ].KratosPayout.add( payout ),
    ...
    function redeem( address _recipient, bool _stake) external returns ( uint ) {        
        ...
        uint _amount = ISKratos(sKratos).balanceForGons(info.gonsPayout);
        emit BondRedeemed( _recipient, _amount, 0 ); // emit bond data
        IERC20( sKratos ).transfer( _recipient, _amount ); // pay user everything due
        return _amount;
    }
    ...
    /**
     *  @notice return bond info with latest sKratos balance calculated from gons
     *  @param _depositor address
     *  @return payout uint
    ...
    function bondInfo(address _depositor) public view returns ( uint payout,uint32 vesting,uint32 lastTime,uint pricePaid ) {
        ...
        payout=ISKratos(sKratos).balanceForGons(info.gonsPayout);
    ...
    function pendingPayoutFor( address _depositor ) external view returns ( uint pendingPayout_ ) {
        ...
        uint payout = ISKratos(sKratos).balanceForGons(_bondInfo[ _depositor ].gonsPayout);
