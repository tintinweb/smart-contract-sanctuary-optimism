...
contract CFarmVelodrome is CFarm {
    ...
    function initialize(address _homenetService, address _router, address _gauge, address tokenA, address tokenB, bool _isStable, address _tokenMain)
        public
        initializer
    {
        __CFarm_initialize(_homenetService);

        pair = IPairFactory(IUniswapV2Router02(_router).factory()).getPair(tokenA, tokenB, _isStable);
        ...
        IERC20Upgradeable(pair).safeApprove(gauge, 2**256-1);
    }
    ...
    {
        uint256 tokenMainAmountToSwap = 0;

        {
            bool isMainToken0 = tokenMain == IUniswapV2Pair(pair).token0();
            (uint256 reserve0, uint256 reserve1,) = IUniswapV2Pair(pair).getReserves();
            (uint256 reserveMain, uint256 reserveOther) = isMainToken0 ? (reserve0, reserve1) : (reserve1, reserve0);
            ...
            uint256 tokenMainBalance = IERC20Upgradeable(tokenMain).balanceOf(address(this));
        ...
        (uint addLiquidityQuoteTokenMain, uint addLiquidityQuoteTokenOther,) = IRouter(router).quoteAddLiquidity(
            tokenMain,
            tokenOther,
            isStable,
            IERC20Upgradeable(tokenMain).balanceOf(address(this)),
            IERC20Upgradeable(tokenOther).balanceOf(address(this))
        );
        ...
        (,, lpAmount) = IRouter(router).addLiquidity(
            ...
            addLiquidityQuoteTokenMain,
            addLiquidityQuoteTokenOther,

            addLiquidityQuoteTokenMain * SLIPPAGE / 100,
            addLiquidityQuoteTokenOther * SLIPPAGE / 100,
        ...
        IGauge(gauge).deposit(lpAmount, 0);
        ...
        returns (uint256 tokenMainAmount)
    {
        ...
        (uint minAmountTokenMain, uint minAmountTokenOther) = IRouter(router).quoteRemoveLiquidity(
            tokenMain,
            tokenOther,
            isStable,
            lpAmount
        ...
        (uint256 removedLiquidityMainAmount, uint256 removedLiquidityOtherAmount) = IRouter(router).removeLiquidity(
            tokenMain,
            tokenOther,
            ...
            minAmountTokenMain * SLIPPAGE / 100,
            minAmountTokenOther * SLIPPAGE / 100,

            address(this),
            block.timestamp
        );

        uint[] memory amounts = IRouter(router).swapExactTokensForTokensSimple(
            removedLiquidityOtherAmount,
            0,
            tokenOther,
            tokenMain,
            isStable,
        ...
        tokenMainAmount = removedLiquidityMainAmount + amounts[1];
    ...
    function harvest(address[] calldata rewardTokensList)
        public
        onlyHomenetService
    {
        IGauge(gauge).getReward(address(this), rewardTokensList);

        for (uint i=0; i<rewardTokensList.length; i++) {
            IERC20Upgradeable rewardToken = IERC20Upgradeable(rewardTokensList[i]);
    ...
    function setGauge(address _gauge)
        ...
        onlyOwner
    {
        IERC20Upgradeable(pair).safeApprove(gauge, 0);
        gauge = _gauge;
        IERC20Upgradeable(pair).safeApprove(gauge, 2**256-1);
    }
}
...
import "./lib/Serializer.sol";
import "./SwapHelperOptimism.sol";
import "./interfaces/ISwapHelper.sol";
import "./HomenetService.sol";
import "./CFarmVelodrome.sol";

uint256 constant PERCENT_PRECISION = 10000;

contract HomenetServiceCFarmVelodrome is HomenetService {
    using SafeERC20Upgradeable for IERC20Upgradeable;
        ...
        initializer
    {
        ...
        IERC20Upgradeable(CFarmVelodrome(cFarm).tokenOther()).safeApprove(CFarmVelodrome(cFarm).router(), 2**256-1);
    }
    ...
    // for now we only care for VELO, so this is left as an example
    /*
    function getRewardTokensList()
        internal
        returns (address[] memory rewardTokensList)
    {
        CFarmVelodrome _cFarm = CFarmVelodrome(cFarm);

        IGauge _gauge = IGauge(_cFarm.gauge());

        uint rewardTokensListLength = _gauge.rewardsListLength();
        rewardTokensList = new address[](rewardTokensListLength);

        for (uint i=0; i<rewardTokensListLength; i++) {
            rewardTokensList[i] = _gauge.rewards(i);
        }
    }
    */
        ...
        returns (uint256 ardnRewardAmount)
    {
        ...
        address[] memory rewardTokensList = new address[](1);
        rewardTokensList[0] = address(velo);
        _cFarm.harvest(rewardTokensList);

        uint256 amount = velo.balanceOf(address(this));
        ...
        if (amount > 0) {
            velo.safeTransfer(swapHelper, amount);
            ISwapHelper(swapHelper).swap(address(velo), _cFarm.tokenMain(), cFarm);
            _cFarm.addLiquidityAndStake(false);
        }

        CollectTokens._collect(address(velo), protocolFeeCollector);
    ...
    {
        address tokenMain = CFarmVelodrome(cFarm).tokenMain();
        ...
        (uint256 lpAmount, uint256 cFarmAmount) = CFarmVelodrome(cFarm).addLiquidityAndStake(true);
        ...
        returns (QueueEntry[] memory outgoingQueue, uint256 quoteTokenAmount)
    {
        ...
        uint256 tokenMainAmount = CFarmVelodrome(cFarm).unstakeAndRemoveLiquidity(cFarmAmount);
...
interface IStablePool {
    ...
    function name() external view returns (string memory);
    function nonces(address owner) external view returns (uint256);
    function onExitPool(bytes32 poolId, address sender, address recipient, uint256[] calldata balances, uint256 lastChangeBlock, uint256 protocolSwapFeePercentage, bytes calldata userData) external returns (uint256[] memory, uint256[] memory);
    function onJoinPool(bytes32 poolId, address sender, address recipient, uint256[] calldata balances, uint256 lastChangeBlock, uint256 protocolSwapFeePercentage, bytes calldata userData) external returns (uint256[] memory, uint256[] memory);
    function permit(address owner, address spender, uint256 value, uint256 deadline, uint8 v, bytes32 r, bytes32 s) external;
    function queryExit(bytes32 poolId, address sender, address recipient, uint256[] calldata balances, uint256 lastChangeBlock, uint256 protocolSwapFeePercentage, bytes calldata userData) external returns (uint256 bptIn, uint256[] memory amountsOut);
    function queryJoin(bytes32 poolId, address sender, address recipient, uint256[] calldata balances, uint256 lastChangeBlock, uint256 protocolSwapFeePercentage, bytes calldata userData) external returns (uint256 bptOut, uint256[] memory amountsIn);
    function setAssetManagerPoolConfig(address token, bytes calldata poolConfig) external;
...
import "./interfaces/velodrome/IRouter.sol";
import "./interfaces/ISwapHelper.sol";
import "./interfaces/beets/IBalancerVault.sol";
import "./interfaces/beets/IStablePool.sol";
...
address constant VELO = 0x3c8B650257cFb5f272f799F5e2b4e65093a11a05;
address constant DOLA = 0x8aE125E8653821E851F12A49F7765db9a9ce7384;

address constant BBUSDPLUS = 0x88D07558470484c03d3bb44c3ECc36CAfCF43253; // FIXME do we need it?
address constant BEETS_VAULT = 0xBA12222222228d8Ba445958a75a0704d566BF2C8;

address constant VELODROME_ROUTER = 0x9c12939390052919aF3155f41Bf4160Fd3666A6f;

contract SwapHelperOptimism is ISwapHelper {
        ...
        returns (uint256)
    {
        ...
        if (address(to) == BBUSDPLUS || address(from) == BBUSDPLUS) {
            return swapViaBeets(from, to, amount, recipient);
        }

        coinFromContract.safeIncreaseAllowance(VELODROME_ROUTER, amount);

        if (
            ((from == USDC || from == SUSD) && (to == USDC || to == SUSD)) ||
            ((from == USDC || from == DOLA) && (to == USDC || to == DOLA)) ||
            (from == VELO && to == USDC)
        ) {
            uint[] memory amounts = IRouter(VELODROME_ROUTER).swapExactTokensForTokensSimple(
                amount,
                0,
                from,
                to,
        ...
        if (from == VELO && to == SUSD) {
            ...
            uint[] memory amounts = IRouter(VELODROME_ROUTER).swapExactTokensForTokens(
                amount,
                0,
                routes,
                recipient,
    ...
    function swapViaBeets(address from, address to, uint256 amount, address recipient)
        internal
        returns (uint256)
    {
        IStablePool pool = IStablePool(BBUSDPLUS);
        ...
        uint256 balanceBefore = IERC20(to).balanceOf(recipient);

        // bbusd doesn't support increaseAllowance because who knows why.
        if (IERC20(from).allowance(address(this), BEETS_VAULT) < amount) {
            IERC20(from).safeApprove(BEETS_VAULT, 0);
            IERC20(from).safeApprove(BEETS_VAULT, amount);
        }

        IBalancerVault(BEETS_VAULT).swap(singleSwap, fundManagement, 0, block.timestamp);

        uint256 balanceAfter = IERC20(to).balanceOf(recipient);
