...
// Interfaces
import {IEconames} from "./interface/IEconames.sol";

// Libraries
import {Lib_ENSNamehash} from "./lib/Lib_ENSNamehash.sol";

/// @title Econames is a NFT contract to resolve names to address as an ENS L2 Resolver contract
contract Econames is
    Initializable,
    ERC721Upgradeable,
    OwnableUpgradeable,
    IEconames
{
    ...
    /// @notice base tokenURI
    string private baseTokenURI;

    /// @notice counter of token IDs
    CountersUpgradeable.Counter private tokenIds;

    /// @notice top-level-domain for econames
    string public tld;

    /// @notice amount of tokens charged per registration
    uint256 public registrationFee;

    /// @notice Address of the token used to pay for registrations
    IERC20Upgradeable public token;

    /// @notice mapping of tokenIDs to name
    mapping(uint256 => string) public nameOf;

    /// @notice mapping of an address to its primary tokenID which is tied to a econame, in the event that the address has multiple tokens
    mapping(address => uint256) public primaryNameOf;

    /// @notice mapping of namehashes to tokenIDs
    ...
    function register(
        ...
        bytes32 namehash = Lib_ENSNamehash.namehash(bytes(name));

        require(tokenIdOf[namehash] == 0, "name is already registered");

        nameOf[tokenId] = name;
        tokenIdOf[namehash] = tokenId;
    ...
    function resolve(string memory _name) external view returns (address) {
        bytes32 namehash = Lib_ENSNamehash.namehash(bytes(_name));
    ...
    /// @notice checks if `str` is an alpha-numeric string
    /// @param _str string to check
    /// @return isAlphaNumeric return true if string is alpha-numeric
    function _isAlphaNumeric(string memory _str) private pure returns (bool) {
    ...
    function _strlen(string memory _s) private pure returns (uint256) {
        uint256 len;
        uint256 i = 0;
...
interface IEconames is IERC721Upgradeable {
    ...
    /// @notice Register an Econame and collects a fee
    /// @param owner of the new Econame
    /// @param name used for the new Econame
    ...
    /// @notice total supply of econames
