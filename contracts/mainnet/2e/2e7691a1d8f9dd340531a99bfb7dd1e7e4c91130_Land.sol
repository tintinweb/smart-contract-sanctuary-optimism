/**
 *Submitted for verification at Optimistic.Etherscan.io on 2022-12-21
*/
...
contract Land is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 48h property pools
    uint256 public constant INITIAL_PROPERTY_POOL_DISTRIBUTION = 100000 ether;
    // Initial distribution for the day 2-5 LAND-WCRO LP -> LAND pool
    uint256 public constant INITIAL_LAND_POOL_DISTRIBUTION = 200000 ether;
    // Distribution for shares wallet
    uint256 public constant INITIAL_SHARE_WALLET_DISTRIBUTION = 30000 ether;
    ...
    /**
     * @notice Constructs the LAND ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("Landshare Token", "LAND") {
        // Mints 1 LAND to contract creator for initial pool setup
    ...
    function _getLandPrice() internal view returns (uint256 _landPrice) {
        try IOracle(landOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Land: failed to fetch LAND price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _landPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_landPrice >= taxTiersTwaps[tierId]) {
    ...
    function setLandOracle(address _landOracle) public onlyOperatorOrTaxOffice {
        require(_landOracle != address(0), "oracle address cannot be 0 address");
        landOracle = _landOracle;
    }
    ...
    /**
     * @notice Operator mints LAND to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of LAND to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentLandPrice = _getLandPrice();
            currentTaxRate = _updateTaxRate(currentLandPrice);
            if (currentLandPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        address _landPool,
        address _shareWallet
    ) external onlyOperator {
        require(!rewardPoolDistributed, "only can distribute once");
        require(_propertyPool != address(0), "!_propertyPool");
        require(_landPool != address(0), "!_landPool");
        require(_shareWallet != address(0), "!_shareWallet");
        rewardPoolDistributed = true;
        _mint(_propertyPool, INITIAL_PROPERTY_POOL_DISTRIBUTION);
        _mint(_landPool, INITIAL_LAND_POOL_DISTRIBUTION);
        _mint(_shareWallet, INITIAL_SHARE_WALLET_DISTRIBUTION);
    }
