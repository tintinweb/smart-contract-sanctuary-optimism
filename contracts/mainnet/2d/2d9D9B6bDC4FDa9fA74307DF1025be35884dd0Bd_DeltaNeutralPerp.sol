...
contract DeltaNeutralPerp is StrategyStorage {
    ...
    int256 private _depositsSinceLastCharge;
    int256 private _withdrawalsSinceLastCharge;
    int256 private _lastTotal;
    ...
    uint32 internal constant _DAILY_SECONDS = 60 * 60 * 24;
    ...
    uint256 private _lastPfChargeTime;
    ...
    */
    function depositUSDC(uint256 amount) public {
        require(amount != 0, "DeltaNeutralPerp: zero amount");
        require(_allowList[_msgSender()], "DeltaNeutralPerp: You are not allowed to deposit");
        require(_depositAmount[_msgSender()] + amount <= _depositLimit, "DeltaNeutralPerp: Exceeding deposit limit");

        uint256 tvl = getTotalUSDCValue();
        uint256 mintingAmount = amount * 1e12; // user get token amount equal to USDC amount
        if (tvl > 0) {
            uint256 share = FullMath.mulDiv(amount, 1e18, tvl);
            mintingAmount = FullMath.mulDiv(share, IdcToken(_dcToken).totalSupply(), 1e18);
        }
        IERC20(_usdToken).transferFrom(_msgSender(), address(this), amount);

        IdcToken(_dcToken).mint(_msgSender(), mintingAmount);
        _depositsSinceLastCharge += int256(amount);
    ...
    */
    function withdrawUSDC(uint256 amount) public {
        require(amount != 0, "DeltaNeutralPerp: zero amount");
        require(amount <= IdcToken(_dcToken).balanceOf(_msgSender()), "DeltaNeutralPerp: insufficient tokens");
        uint256 poolShare = FullMath.mulDiv(amount, 1e6, IdcToken(_dcToken).totalSupply());
        uint256 result = FullMath.mulDiv(getTotalUSDCValue(), poolShare, 1e6);
        IdcToken(_dcToken).transferFrom(_msgSender(), address(this), amount);
        IdcToken(_dcToken).burn(amount);
        if (getReserve() < result) {
            _closePositionsForWithdrawal(getReserve(), result);
        }
        _withdrawalsSinceLastCharge += int256(result);
        if (result > _depositAmount[_msgSender()]) {
            _depositAmount[_msgSender()] = 0;
        } else {
            _depositAmount[_msgSender()] -= result;
        }
        IERC20(_usdToken).transfer(_msgSender(), result);
    ...
    */
    function adjustPosition(bool operationType, bool positionType, uint256 amount) public onlyTrigger() {
        require(amount != 0, "DeltaNeutralPerp: zero amount");
        uint256 wTokenToReserveProportion = FullMath.mulDiv(_RESERVE_VALUE, 100, _W_TOKEN_VALUE);
        uint256 wTokenAmountUSDC = FullMath.mulDiv(IERC20(_wToken).balanceOf(address(this)),
                                                    _nativeStrategyTokenPrice(0), 1e30);
        uint256 strategyReserve = FullMath.mulDiv(wTokenAmountUSDC, wTokenToReserveProportion, 100);

        if (wTokenAmountUSDC > 0 && operationType && positionType) {
            require(getReserve() - amount <= strategyReserve, "DeltaNeutralPerp: Proportion break");
        }
        ...
        if (operationType) { // openPosition
            ...
            /// short/buy wToken
            if(positionType) {
                require(getReserve() >= amount, "DeltaNeutralPerp: Insufficient reserve");
                ...
                /// long/sell wToken
                IERC20(_wToken).approve(_router, usdcAmountForUniswap);
            ...
            _closePosition(0); /// full position
            _withdrawFromPerp(amount);
            _buyTokensToUniswap(_wToken, _usdToken, IERC20(_wToken).balanceOf(address(this)));
            emit PositionAdjusted(operationType, positionType, IERC20(_usdToken).balanceOf(address(this)), 0);
        }
    }
    ...
    function chargePf() public onlyTrigger() {
        uint256 blockTime = block.timestamp;
        require(blockTime - _lastPfChargeTime > _DAILY_SECONDS, "DeltaNeutralPerp: pf already gathered");
        int256 income = int256(getTotalUSDCValue()) -
        _depositsSinceLastCharge - _lastTotal + _withdrawalsSinceLastCharge;
        _lastTotal = int256(getTotalUSDCValue());
        _lastPfChargeTime = blockTime;
        if (income <= 0) {
            return;
        }
        uint256 pfUSD = uint256(income) * _performanceFee;
        uint256 pf = pfUSD * IdcToken(_dcToken).totalSupply() / (getTotalUSDCValue() - pfUSD);
        IdcToken(_dcToken).mint(triggerServer, pf);
        _depositsSinceLastCharge = 0;
        _withdrawalsSinceLastCharge = 0;
        emit PFCharged(triggerServer, pf);
    }
    ...
    function getTotalUSDCValue() public view returns (uint256) {
        ...
        getReserve() +
        FullMath.mulDiv(IERC20(_wToken).balanceOf(address(this)),
    ...
    * @return price in USDC (1e6)
    ...
    function getDCTokenPrice() public view returns (uint256) {
        ...
        return FullMath.mulDiv(getTotalUSDCValue(), 1e18, IdcToken(_dcToken).totalSupply());
    }
    ...
    * @return fundingRate_10_6
    ...
    function getCurrentFundingRate() public view returns (uint256) {
        ...
        return FullMath.mulDiv(dailyMarketTwap, 1e6, dailyIndexTwap);
    }
    ...
    /**
       * @dev Charge subscription fee from amount and return leftover
    */
    function _chargeSubscriptionFee(uint256 amount) private returns(uint256) {
        if (_subscriptionFee == 0) {
            return amount;
        }
        uint256 fee = FullMath.mulDiv(amount, _subscriptionFee, 1e6);
        _buyNativeWTokenToTrigger(_usdToken, fee);
        return amount - fee;
    }
    ...
    function _closePositionsForWithdrawal(uint256 reserve, uint256 toWithdraw) private {
        ...
        uint256 wTokenAmount = FullMath.mulDiv(toClose, 1e6, _nativeStrategyTokenPrice(0));
        _openPosition(wTokenAmount, false);
        _buyTokensToUniswap(_wToken, _usdToken, wTokenAmount);
    }
  ...
  */
    ...
    function allowDeposit(address user) external onlyOwner {
        _allowList[user] = true;
        emit DepositAllowed(user);
    }


    function denyDeposit(address user) external onlyOwner {
        _allowList[user] = false;
    }


    /// DEBUG
    function availableDeposit(address user) external view returns(uint256) {
        return _depositLimit - _depositAmount[user];
    }


    function isAllowedToDeposit(address user) external view returns(bool) {
        return _allowList[user];
    }


    function getDailyMarketTwap() public onlyTrigger() view returns(uint256) {
        uint160 dailyMarketTwap160X96 = IExchange(_perp.getExchange()).getSqrtMarkTwapX96(_wToken, 60 * 60 * 1);
        uint256 dailyMarketTwapX96 = FullMath.formatSqrtPriceX96ToPriceX96(dailyMarketTwap160X96);
        return FullMath.formatX96ToX10_18(dailyMarketTwapX96);
    }
}
...
import "../integrations/uniswap/ISwapRouter.sol";
...
contract StrategyStorage is OwnableUpgradeable{
    ...
    modifier onlyTrigger() {
        require(triggerServer == _msgSender(), "dcStrategyStorage: caller is not trigger");
        _;
    }


    function setTriggerServer(address _newTriggerServer) public onlyOwner {
        require(_newTriggerServer != (address(0x0)), "dcStrategyStorage: zero address");
        triggerServer = _newTriggerServer;
    }


    function setEscrow(address _newEscrow) public onlyOwner {
        require(_newEscrow != address(0x0), "dcStrategyStorage: zero address");
        escrowServer = _newEscrow;
    }


    function emergencyWithdraw(IERC20 _token) public onlyOwner {
        require(escrowServer != address(0x0), "dcStrategyStorage: zero address");
        _token.transfer(escrowServer, _token.balanceOf(address(this)));
    }
    ...
    function _buyTokensToUniswap(address _tokenA, address _tokenB, uint256 amount) internal {
        ISwapRouter(_router).exactInputSingle(ISwapRouter.ExactInputSingleParams({
            tokenIn: _tokenA,
            tokenOut: _tokenB,
            fee: _feeToPair,
    ...
    function _sellTokensToUniswap(address _tokenA, address _tokenB, uint256 amount, uint256 amountInMaximum) internal {
        ISwapRouter(_router).exactOutputSingle(ISwapRouter.ExactOutputSingleParams({
        tokenIn: _tokenA,
        tokenOut: _tokenB,
        fee: _feeToPair,
        recipient: address(this),
    ...
    function _buyNativeWTokenToTrigger(address _tokenA, uint256 amount) internal {
        ISwapRouter(_router).exactInputSingle(ISwapRouter.ExactInputSingleParams({
        tokenIn: _tokenA,
        tokenOut: address(0),
        fee: _feeToPair,
        recipient: triggerServer,
        deadline: block.timestamp + _deadlineTime,
        amountIn: amount,
        amountOutMinimum: 0,
        sqrtPriceLimitX96: _sqrtPriceLimitX96toUni
