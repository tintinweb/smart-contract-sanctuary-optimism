...
contract VelodromeOptimismUsdcMim is DefiiWithCustomEnter, DefiiWithCustomExit {
    IERC20 constant MIM = IERC20(0xB153FB3d196A8eB25522705560ac152eeEc57901);
    IERC20 constant USDC = IERC20(0x7F5c764cBc14f9669B88837ca1490cCa17c31607);
    IERC20 constant lpToken =
        IERC20(0x29A20cA4968d756C7D0922a7064a1A56f4624e9a);

    IERC20 constant VELO = IERC20(0x3c8B650257cFb5f272f799F5e2b4e65093a11a05);

    IRouter constant router =
        IRouter(0x9c12939390052919aF3155f41Bf4160Fd3666A6f);
    IGauge constant gauge = IGauge(0x330C3C1E493a95bf45bC0147c5F68d983A9d4E00);
        ...
        returns (bytes memory)
    {
        ...
        usdcToSwap = usdcToSwap / 1e12;

        usdcToSwap = (usdcToSwap * 999) / 1000;
        (uint256 mimMinAmount, ) = router.getAmountOut(usdcToSwap, USDC, MIM);
        return abi.encode(usdcToSwap, (mimMinAmount * slippage) / 1000);
    }
    ...
    function exitParams(uint256 slippage) external view returns (bytes memory) {
        ...
        // estimate price for 1,000 MIM
        (uint256 mimPrice, ) = router.getAmountOut(1000e18, MIM, USDC);
        mimPrice = mimPrice / 1000;
        return abi.encode((mimPrice * slippage) / 1000);
    }

    function _enterWithParams(bytes memory params) internal override {
        (uint256 usdcToSwap, uint256 minMimFromSwap) = abi.decode(
            params,
            (uint256, uint256)
        );
        USDC.approve(address(router), usdcToSwap);
        ...
        _enter(USDC.balanceOf(address(this)), amounts[1]);
    }

    function _enter() internal override(Defii, DefiiWithCustomEnter) {
        uint256 usdcBalance = USDC.balanceOf(address(this));
        uint256 mimBalance = MIM.balanceOf(address(this));

        _enter(usdcBalance, mimBalance);
    }

    function _enter(uint256 usdcBalance, uint256 mimBalance) internal {
        USDC.approve(address(router), usdcBalance);
        MIM.approve(address(router), mimBalance);
    ...
    function _exitWithParams(bytes memory params) internal override {
        ...
        uint256 mimAmount = MIM.balanceOf(address(this));
        uint256 minUsdcAmount = (mimAmount * minPrice) / 1e18;

        MIM.approve(address(router), mimAmount);
    ...
    function _exit() internal override(Defii, DefiiWithCustomExit) {
        ...
        uint256 lpAmount = gauge.balanceOf(address(this));
