...
contract RewardDeriver is TwoStepOwnable, IRewardDeriver {
        ...
        returns (RewardInfo[] memory)
    {
        ...
        if (campaignTracker.isOptimismOGHolder(_recipient)) {
            uint256 rewardAmountInOP = campaignTracker.getRewardAmountInOP(
                OPTIMISM_OG_CAMPAIGN_STRING,
                considerationAmountInOP
            );
            rewardInfoArray[0] = RewardInfo({
        ...
        uint256 baselineRewardAmountInOP = campaignTracker.getRewardAmountInOP(
            BASELINE_CAMPAIGN_STRING,
            considerationAmountInOP
        );
        rewardInfoArray[1] = RewardInfo({
        ...
        uint256 collectionBoostRewardAmountInOP = campaignTracker.getRewardAmountInOP(
            collectionBoostCampaignString,
            considerationAmountInOP
        );
        rewardInfoArray[2] = RewardInfo({
