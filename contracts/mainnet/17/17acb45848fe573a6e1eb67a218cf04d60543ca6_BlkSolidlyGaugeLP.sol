...
// File contracts/BIFI/strategies/Common/StratManagerBlk.sol
...
// File contracts/BIFI/strategies/Common/FeeManagerBlk.sol
...
abstract contract FeeManager is StratManager {
    ...
    uint constant public MAX_CALL_FEE = 200;
    ...
    uint public platformFee = MAX_FEE - callFee;

    function setCallFee(uint256 _fee) public onlyManager {
        require(_fee <= MAX_CALL_FEE, "!cap");
        
        callFee = _fee;
        platformFee = MAX_FEE - callFee;
    }
...
// File contracts/BIFI/strategies/Common/BlkSolidlyGaugeLP.sol
...
contract BlkSolidlyGaugeLP is StratManager, FeeManager, GasThrottler {
    ...
    event ChargedFees(uint256 callFees, uint256 platformFees);
    ...
    function chargeFees(address callFeeRecipient) internal {
        uint256 toNative = IERC20(output).balanceOf(address(this)).mul(50).div(1000);
        ...
        uint256 feeAmount = nativeBal.mul(platformFee).div(MAX_FEE);
        IERC20(native).safeTransfer(feeRecipient, feeAmount);

        emit ChargedFees(callFeeAmount, feeAmount);
    }
