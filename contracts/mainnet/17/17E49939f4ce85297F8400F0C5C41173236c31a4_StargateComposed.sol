...
contract StargateComposed is IStargateReceiver, WardedLiving {
    ...
    struct Destination {
        ...
        uint16 destinationPool;
    }

    mapping(uint16 => Destination) public supportedDestinations; //destination stargate_chainId => Destination struct

    function setSources(address bridgeToken_, uint16 poolId_) external auth {
        bridgeToken = bridgeToken_;
        poolId = poolId_;

        IERC20(bridgeToken).approve(address(stargateRouter),
            115792089237316195423570985008687907853269984665640564039457584007913129639935);
    }

    function setSupportedDestination(uint16 destChainId, address receiver, uint16 destPoolId) external auth {
        supportedDestinations[destChainId] = Destination(receiver, destPoolId);
    }

    function test(address srcToken, uint256 amountIn) public {
        IERC20(srcToken).approve(address(ammRouter), amountIn);
        console.log("UNI PARAMS:", amountIn);
        console.log("path: ", srcToken, bridgeToken);
        console.log("Sender: ", msg.sender);
        console.log("Deadline: ", block.timestamp + 300);

        uint256 dai = IERC20(srcToken).balanceOf(address(this));
        ...
        IV3SwapRouter(ammRouter).exactInputSingle(params);
    }

    function toBridgeAsset(address srcToken, uint256 amountIn) public returns (uint256) {
        IERC20(srcToken).approve(address(ammRouter), amountIn);
        //        address factory = UniRouterV2(ammRouter).factory();
        //        require(UniFactoryV2(factory).getPair(srcToken, bridgeToken) != address(0x0), "SgBridge/not-supported-token");
...
//
        ...
        uint256 r = IV3SwapRouter(ammRouter).exactInputSingle(params);

        //        (uint256[] memory amounts) = UniRouterV2(ammRouter).swapExactTokensForTokens(
        //            amountIn,
        //            0, //amountOutMin
        //            path,
        //            msg.sender,
    ...
    function bridge(
        ...
        require(supportedDestinations[dstChainId].receiveContract != address(0x0), "SgBridge/not-supported-dest");

        IERC20(srcToken).transferFrom(msg.sender, address(this), amountIn);

        uint256 bridgeTokenAmount = toBridgeAsset(srcToken, amountIn);

        uint256 deadline = block.timestamp + 300;
        uint256 amountOutMinDest = 0;
        bytes memory data;
        {
            data = abi.encode(dstToken, deadline, amountOutMinDest, to);
        }
        address bridgeReceiver = supportedDestinations[dstChainId].receiveContract;

        // Stargate's Router.swap() function sends the tokens to the destination chain.
        IStargateRouter(stargateRouter).swap{value : msg.value}(
            dstChainId, // the destination chain id
            poolId, // the source Stargate poolId
            supportedDestinations[dstChainId].destinationPool, // the destination Stargate poolId
            payable(msg.sender), // refund adddress. if msg.sender pays too much gas, return extra eth
            bridgeTokenAmount, // total tokens to send to destination chain
            0, // minimum
            IStargateRouter.lzTxObj(200000, 0, "0x"), // 200,000 for the sgReceive()
            abi.encodePacked(bridgeReceiver), // destination address, the sgReceive() implementer
            data                                            // bytes payload
        );

        emit Bridge(msg.sender, dstChainId, amountIn);
    }

    function quickSwapNative(
        ...
        swapNativeForNative(
            ...
            bridgeToken,
            poolId,
            supportedDestinations[dstChainId].destinationPool,
            nativeAmountIn,
            to,
            0,
            0,
            0,
            block.timestamp + 300,
            supportedDestinations[dstChainId].receiveContract
        );
    }
    //-----------------------------------------------------------------------------------------------------------------------
    ...
    function swapNativeForNative(
        ...
        // using the amm router, swap native into the Stargate pool token, sending the output token to this contract
        {
            ...
            address[] memory path = new address[](2);
            path[0] = UniRouterV2(ammRouter).WETH();
            // native IN requires that we specify the WETH in path[0]
            path[1] = bridgeToken;
            // the bridge token,
        ...
        emit Bridge(msg.sender, dstChainId, nativeAmountIn);
    }
