...
// File: contracts/pool/OWLiquidityPool.sol


pragma solidity =0.8.19;

contract OWLiquidityPool is Ownable {
    ...
    function set(uint256 _pid, uint256 _allocPoint) public onlyOwner {
        require(_pid <= poolInfo.length - 1, "OWLiquidityPool: Can not find this pool");
    ...
    function setStandardRewardInterval(uint256 interval) public onlyOwner {
        require(interval > 0, "OWLiquidityPool: interval must be greater than 0");
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if(_amount > 0) {
            ...
            require(_amount.add(user.amount) <= maxStakingPerUser, 'OWLiquidityPool: exceed max stake');
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        require(user.amount >= _amount, "OWLiquidityPool: Withdraw with insufficient balance");
    ...
    function emergencyRewardWithdraw(uint256 _amount) public onlyOwner {
        require(_amount <= rewardToken.balanceOf(address(this)), 'OWLiquidityPool: not enough token');
        rewardToken.safeTransfer(address(msg.sender), _amount);
    }

    function getPoolView(uint256 pid) public view returns (PoolView memory) {
        require(pid < poolInfo.length, "OWLiquidityPool: pid out of range");
    ...
    function addCaller(address val) public onlyOwner() {
        require(val != address(0), "OWLiquidityPool: val is the zero address");
        _callers.add(val);
    }

    function delCaller(address caller) public onlyOwner returns (bool) {
        require(caller != address(0), "OWLiquidityPool: caller is the zero address");
