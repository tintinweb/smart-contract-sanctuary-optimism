...
contract AuraSrcSwapper is NonblockingLzApp {
    ...
    function _returnNative(address _strategy, uint256 _amount) public {
        if (msg.sender != address(this)) revert NotAuthorized();
        IERC20(native).transfer(_strategy, _amount);
    }
     ...
     function sgReceive(
        ...
        if (_token != native) revert WrongToken();
        require(_srcAddress.length == auraSwapper.length && keccak256(_srcAddress) == keccak256(auraSwapper), "NotAuthorized");
        address strategy = abi.decode(payload, (address));
        try this._returnNative(strategy, amountLD) {
            // Do Nothing
        } catch {
            emit Error();
        }
     ...
     function swapAura(uint256 _amount) external payable {
        ...
        uint256 totalGas = transferGas + messageGas;
        require(totalGas > msg.value, "Not Enough Gas");
        ...
        lzEndpoint.send{ value: messageGas}(
            dstChainId,
            auraSwapper,
            payload,
            payable(address(this)),
