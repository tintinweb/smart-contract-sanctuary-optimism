    ...
    ILeechRouter
{
        ...
        checkDepositAmount(depositToken, amount)
    {
        ...
        // Set withdraw delay
        pool.user[msg.sender].delayToBlock = block.number + withdrawDelay;
        // Notify watchers
        emit Deposited(
            msg.sender,
            poolId,
        ...
        checkDelay(poolId, msg.sender)
    {
        ...
        if (chainId != pool.chainId) revert CrosschainError();
        // Get strategy
        IStrategyMasterchefFarmV2 strategy =
            IStrategyMasterchefFarmV2(strategies[pool.strategyId].addr);
        // Convert to the strategy shares
        uint256 withdrawAmount = shares * strategy.allocationOf(poolId) / pool.totalAllocation;
        // Withdraw from strategy
        uint256 amount = strategy.withdraw(poolId, withdrawAmount, tokenOut, data);
        // Minus fee if needed
        uint256 withdrawFee;
        ...
        console.log(shares);
        console.log(pool.user[msg.sender].allocation);
