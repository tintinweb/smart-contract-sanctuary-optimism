...
contract LadyFoxy is ERC721A, Ownable, ReentrancyGuard {
  ...
  uint256 public maxFreeMint = 128;
  mapping(address => uint256) public addressMintedBalance;     
  mapping(address => uint256) public addressGetFreeMint;
  uint256 public whitelistEndDate = 1659974400;

  bool public paused = true;
  bool public revealed = false;
  bytes32 public whitelistSAP;
  bytes32 public whitelistFoxy;


  constructor(
  ) ERC721A("Lady Foxy", "LADYFOXY") {
    setHiddenMetadataUri("ipfs://QmPbXr4K9xHRFnAittjfsSEWNYJU97nz4CJc2dsGXgpQa4/hidden.json");
  ...
  function Mint(uint256 _mintAmount, bytes32[] calldata merkleProof) public payable nonReentrant notSmartContract mintCompliance(_mintAmount) {
    require(!paused, 'The contract is paused!');
    if(block.timestamp < whitelistEndDate) {
      if(!isValidMerkleProof(merkleProof, whitelistSAP) && 
         !isValidMerkleProof(merkleProof, whitelistFoxy)){
        revert("This is SAP & Whitelist round! Please wait for the public mint");
      }
    }
    ...
    if (addressGetFreeMint[_msgSender()] == 0 && block.timestamp < whitelistEndDate){
        if(maxFreeMint == 0){
            maxFreeMint = 0;
        }else{
  ...
  function setwhitelistFoxy(bytes32 merkleRoot) external onlyOwner {
    whitelistFoxy = merkleRoot;
  }
