...
contract SeedGenesisRewardPool {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. SEED to distribute.
        uint256 lastRewardTime; // Last time that SEED distribution occurs.
        uint256 accSeedPerShare; // Accumulated SEED per share, times 1e18. See below.
    ...
    // The time when SEED mining ends.
    uint256 public poolEndTime;

    // MAINNET
    uint256 public seedPerSecond = 0.11574 ether; // 30000 SEED / (72h * 60min * 60s)
    ...
    modifier onlyOperator() {
        require(
            operator == msg.sender,
            "SeedGenesisPool: caller is not the operator"
        );
        _;
    }
        ...
        returns (uint256)
    {
        ...
        if (_toTime >= poolEndTime) {
            ...
            if (_fromTime <= poolStartTime)
                return poolEndTime.sub(poolStartTime).mul(seedPerSecond);
            return poolEndTime.sub(_fromTime).mul(seedPerSecond);
        } else {
            if (_toTime <= poolStartTime) return 0;
            if (_fromTime <= poolStartTime)
                return _toTime.sub(poolStartTime).mul(seedPerSecond);
            return _toTime.sub(_fromTime).mul(seedPerSecond);
        }
    }

    // View function to see pending SEED on frontend.
    ...
    function safeSeedTransfer(address _to, uint256 _amount) internal {
        uint256 _seedBalance = seed.balanceOf(address(this));
        if (_seedBalance > 0) {
            if (_amount > _seedBalance) {
                seed.safeTransfer(_to, _seedBalance);
            } else {
    ...
    function governanceRecoverUnsupported(
        ...
        if (block.timestamp < poolEndTime + 90 days) {
            // do not allow to drain core token (SEED or lps) if less than 90 days after pool ends
