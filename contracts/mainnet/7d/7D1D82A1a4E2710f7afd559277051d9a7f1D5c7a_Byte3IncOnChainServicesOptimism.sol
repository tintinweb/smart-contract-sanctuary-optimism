...
import {IVelodromeV2Pair} from "./interface/IVelodromeV2Pair.sol";
import {IWETH} from "./interface/IWETH.sol";

contract Byte3IncOnChainServicesOptimism is Byte3IncOnChainServices {
  ...
  IUniswapV3Pair constant UniswapV3UsdcPool = IUniswapV3Pair(0x85149247691df622eaF1a8Bd0CaFd40BC45154a9);
  IVelodromeV2Pair constant VelodromeV2UsdcPair = IVelodromeV2Pair(0x0493Bf8b6DBB159Ce2Db2E0E8403E753Abd1235b);
  ...
  function convertUsdToWei(uint256 usdAmount) external view override returns (uint256 weiAmount) {
    ...
    weiAmount = (_getVelodromeUSDC(usdAmount) + uniswapWeiAmount) / 2;
    // add 1% overhead
  ...
  function purchaseFractionToken(
    ...
    uint256 _amountIn2 = _getVelodromeUSDC(usdAmount);
    ...
    if (amountIn == _amountIn1) {
      ...
      _velodromeSwap(address(this), amountIn, usdAmount);
    }
  ...
  function _velodromeSwap(address recipient, uint256 nativeAmount, uint256 usdAmount) internal {
    IWETH weth = IWETH(WETH);
    // wrap the native gas token
    weth.deposit{value: nativeAmount}();
    // transfer to token pool
    weth.transfer(address(VelodromeV2UsdcPair), nativeAmount);
    // execute swap
    VelodromeV2UsdcPair.swap(0, usdAmount, recipient, "");
  }
...
//       sqrtPrice - 1,
  ...
  function _getVelodromeUSDC(uint256 usdAmount) internal view returns (uint256 weiAmount) {
    (uint256 reserveA, uint256 reserveB,) = VelodromeV2UsdcPair.getReserves();
    uint256 numerator = (reserveA * usdAmount) * 1000;
    uint256 denominator = (reserveB - usdAmount) * 995;
