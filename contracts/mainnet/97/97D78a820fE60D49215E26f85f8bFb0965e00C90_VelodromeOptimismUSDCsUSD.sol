...
contract VelodromeOptimismUSDCsUSD is DefiiWithCustomEnter {
    ...
    IERC20 constant lpToken =
        IERC20(0xd16232ad60188B68076a235c65d692090caba155);
    ...
    IGauge constant gauge = IGauge(0xb03f52D2DB3e758DD49982Defd6AeEFEa9454e80);

    function hasAllocation() external view override returns (bool) {
        return gauge.balanceOf(address(this)) > 0;
    }

    function encodeParams(uint256 startUsdcAmount)
        external
        view
        returns (bytes memory encodedParams)
    {
        if (startUsdcAmount == 0) {
            startUsdcAmount = USDC.balanceOf(address(this));
        }
        (uint256 r0, uint256 r1) = router.getReserves(USDC, sUSD, true);
        ...
        for (uint16 i = 0; i < 10000; i++) {
            usdcAmount = r0 > r1
                ? (startUsdcAmount * (10000 - 2 * i)) / 20000
                : (startUsdcAmount * (10000 + 2 * i)) / 20000;
            liquidity = _getLiquidity(usdcAmount, startUsdcAmount - usdcAmount);

            if (liquidity < prevLiquidity) break;
            prevLiquidity = liquidity;
        }
        encodedParams = abi.encode(startUsdcAmount - usdcAmount);
    }

    function _getLiquidity(uint256 usdcToAdd, uint256 usdcToSwap)
        internal
        view
        returns (uint256 liquidity)
    {
        (uint256 susdToAdd, ) = router.getAmountOut(usdcToSwap, USDC, sUSD);
        (, , liquidity) = router.quoteAddLiquidity(
            USDC,
            sUSD,
            true,
            usdcToAdd,
    ...
    function _enterWithParams(bytes memory params) internal override {
        uint256 usdcToSwap = abi.decode(params, (uint256));
    ...
    function _enter() internal override {
        ...
        sUSD.approve(address(router), susdBalance);
