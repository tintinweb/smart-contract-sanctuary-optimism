...
contract ShowerRoom is IShowerRoom, OwnerPausable, ReentrancyGuardUpgradeable, ShowerRoomStorageV1 {
    ...
    function pushToKantaban(uint256 maxSkipped) external override whenNotPaused nonReentrant returns (uint256) {
        ...
        // because it will always break in the if block inside this for loop
     ...
     * @dev 2. this IS a state changing function; Yearn-related storage states ARE updated here
     */
        ...
        returns (uint256 pendingStakingRewards, uint256 stakingRewardPerToken)
    {
        ...
        if (user.stakingRewardPerToken > 0 && !_isZeroAddress(_yearnStakingRewards)) {
            ...
            // sub 1 wei from pendingStakingRewards to avoid rounding inconsistency between ShowerRoom & StakingRewards
            pendingStakingRewards = pendingStakingRewards == 0 ? pendingStakingRewards : pendingStakingRewards.sub(1);
        }
