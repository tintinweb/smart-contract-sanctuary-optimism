...
contract SwapperWithCompensationUniV2 is SwapperWithCompensationAbs {
    ...
    constructor(
        ...
        address _strategy,
        address _exchange
    ) SwapperWithCompensationAbs(_want, _lp, false, _strategy, _exchange) { }
...
abstract contract SwapperWithCompensationAbs is Swappable, ReentrancyGuard {
    ...
    uint public reserveSwapRatio = 2; // 0.02% (0.3% of swap fee + a little more to get the more LP as possible
    ...
    function _wantAmountToLpTokensAmount(uint _amount) internal view returns (uint _amount0, uint _amount1) {
        ...
        if (stable) {
            ...
            uint _amountIn0 = _amount * 1e18 / decimals0;
            // uint _amountIn1 = _amount * 1e18 / decimals1;

            uint _totalReserves = _reserve0 + _reserve1;
            uint y0 = _reserve1 - _get_y(_amountIn0 + _reserve0, xy, _reserve1);

            _amount0 = y0 * decimals1 * _reserve0 * (RATIO_PRECISION + reserveSwapRatio) / 1e18 / _totalReserves / RATIO_PRECISION;

            // uint y1 = _reserve0 - _get_y(_amountIn1 + _reserve1, xy, _reserve0);

            _amount1 = _amount - _amount0; // y1 * decimals0 / 1e18 * _reserve1 / _totalReserves;
        ...
        // }
            ...
            uint _totalReserves = _reserve0;
            _totalReserves += _reserve1 / _reservePrecision();
    ...
    function _get_y(uint _x0, uint _xy, uint y) internal pure returns (uint) {
        for (uint i = 0; i < 255; i++) {
            uint _y_prev = y;
            uint k = _f(_x0, y);

            if (k < _xy) {
                uint dy = (_xy - k) * 1e18 / _d(_x0, y);
                y = y + dy;
            } else {
                uint dy = (k - _xy) * 1e18 / _d(_x0, y);
                y = y - dy;
            }
    ...
    function _f(uint _x0, uint _y) internal pure returns (uint) {
        return _x0 * (_y * _y / 1e18 * _y / 1e18) / 1e18 + (_x0 * _x0 / 1e18 * _x0 / 1e18) * _y / 1e18;
    }

    function _d(uint _x0, uint _y) internal pure returns (uint) {
        return 3 * _x0 * (_y * _y / 1e18) / 1e18 + (_x0 * _x0 / 1e18 * _x0 / 1e18);
    }
}
...
contract SwapperWithCompensationSolidly is SwapperWithCompensationAbs {
    ...
    constructor(
        ...
        address _strategy,
        address _exchange
    ) SwapperWithCompensationAbs(_want, _lp, _stable, _strategy, _exchange) { }

    function setRoute(address _from, route[] calldata _routes) external onlyAdmin {
        require(_from != address(0), "!ZeroAddress");
    ...
    function _swap(address _from, uint _amount, address _to) internal override returns (uint) {
        route[] memory _route = _getRoute(_from, _to);
        route[] memory _route2 = _getRoute(_to, _from);

        // console.log("Amounts OUT", ISolidlyRouter(exchange).getAmountsOut(_amount, _route)[0]);
        console.log("From", _from, _to);
        console.log("Amount", _amount);
        console.log("Amounts OUT", ISolidlyRouter(exchange).getAmountsOut(_amount, _route)[1]);
        console.log("Amounts OUT", ISolidlyRouter(exchange).getAmountsOut(_amount, _route2)[1]);
    ...
    function _getAmountsOut(IERC20Metadata _from, IERC20Metadata _to, uint _amount) internal override view returns (uint) {
        if (_from == _to) {
            ...
            route[] memory _route = _getRoute(address(_from), address(_to));
    ...
    function _getRoute(address _from, address _to) internal view returns (route[] memory) {
        route[] memory _route = routes[_from][_to];
...
contract ControllerSolidlyLPStrat is ControllerStratAbs {
    ...
    mapping(address => route[]) public rewardToWantSolidlyRoute;
    ...
    function setRewardToWantSolidlyRoute(address _reward, route[] calldata _routes) external onlyAdmin {
        require(_reward != address(0), "!ZeroAddress");
    ...
    function _addLiquidity(uint _amount0a, uint _amount1a) internal {
        // Let's save some gas
        address _exchange = exchange;
        uint _amount0 = IERC20Metadata(token0).balanceOf(address(this));
        uint _amount1 = IERC20Metadata(token1).balanceOf(address(this));

        // Approve only needed amounts
        _approveToken(token0, _exchange, _amount0);
        _approveToken(token1, _exchange, _amount1);

        console.log("amount0", _amount0, _amount0a);
        console.log("amount1", _amount1, _amount1a);

        // Add liquidity to the LP
        (, , uint _liquidity) = ISolidlyRouter(_exchange).addLiquidity(
            token0,
            token1,
            stable,
            _amount0,
        ...
        if (_liquidity > 0) {
            uint _lpLiquidity = lp.balanceOf(address(this));

            _approveToken(address(lp), address(gauge), _lpLiquidity);

            gauge.deposit(_lpLiquidity, tokenId);
        }
...
import "./ControllerStratAbs.sol";
import "../interfaces/IRocket.sol";
import "../interfaces/IWNative.sol";

contract ControllerRocketStrat is ControllerStratAbs {
    using SafeERC20 for IERC20;
    using SafeERC20 for IERC20Metadata;

    IWNative public constant WETH = IWNative(address(0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2));
    ...
    constructor(
        ...
        require(_rocketStorage != address(0), "Rocket storage !ZeroAddress");

        rocketStorage = RocketStorageInterface(_rocketStorage);
    }

    function identifier() external view returns (string memory) {
        return string(abi.encodePacked(want.symbol(), "@Rocket#1.0.0"));
    }
    ...
    function _deposit() internal override {
        uint _wethBalance = WETH.balanceOf(address(this));

        WETH.withdraw(_wethBalance);

        // Load contracts
        address rocketDepositPoolAddress = rocketStorage.getAddress(_address("rocketDepositPool"));
        RocketDepositPoolInterface rocketDepositPool = RocketDepositPoolInterface(rocketDepositPoolAddress);

        address rocketETHTokenAddress = rocketStorage.getAddress(_address("rocketETHToken"));
        IERC20 rocketETHToken = IERC20(rocketETHTokenAddress);
    ...
    function _withdraw(uint _amount) internal override returns (uint) {
        ...
        require(rocketETHToken.transfer(msg.sender, _amount), "rETH was not transferred to caller");
    ...
    function _address(string memory _name) internal pure returns (bytes32) {
...
contract ControllerCurveUniV3bStrat is ControllerStratAbs {
    using SafeERC20 for IERC20;
    using SafeERC20 for IERC20Metadata;
