...
contract DhedgeSuperSwapper is IUniswapV2RouterSwapOnly {
  ...
  constructor(
    IUniswapV2Router[] memory _uniV2Routers,
    ICurveCryptoSwap[] memory _curvePools,
    RouteHint[] memory hints
  ) {
  ...
  function swapExactTokensForTokens(
    ...
    // If there is no amountOutMin we look for a better route by default if there's an intermediary
    if (amountOutMin == 0 || (curveBestAmountOut < amountOutMin && uniV2BestAmountOut < amountOutMin)) {
      ...
      if (intermediary != address(0)) {
        ...
        path2[2] = path[1];
        (IUniswapV2Router router2, uint256 uniV2BestAmountOut2) = getBestAmountOutUniV2Router(amountIn, path2);
        if (uniV2BestAmountOut2 > curveBestAmountOut && uniV2BestAmountOut2 > uniV2BestAmountOut) {
          uniV2BestAmountOut = uniV2BestAmountOut2;
          router = router2;
          path = path2;
    ...
    if (curveBestAmountOut > uniV2BestAmountOut) {
      // Use Curve pool
      require(curveBestAmountOut > 0, encodeError("SwapRouter: invalid routing 03", path[0], path[1])); // invalid routing with Curve
      ...
      // use Uni v2 router
      require(uniV2BestAmountOut > 0, encodeError("SwapRouter: invalid routing 01", path[0], path[1])); // invalid routing with Uni v2 swapExactTokensForTokens
  ...
  function swapTokensForExactTokens(
    ...
    // If there is no amountInMax we look for a better route by default
    if (uniBestAmountIn > amountInMax || amountInMax == uint256(-1)) {
      address intermediary = getRouteHint(path);
      if (intermediary != address(0)) {
        address[] memory path2 = new address[](3);
        path2[0] = path[0];
        path2[1] = intermediary;
        path2[2] = path[1];
        (IUniswapV2Router router2, uint256 uniBestAmountIn2) = getBestAmountInUniV2Router(expectedAmountOut, path2);
        if (uniBestAmountIn2 < uniBestAmountIn) {
          router = router2;
          uniBestAmountIn = uniBestAmountIn2;
          path = path2;
          emit Interpolate(intermediary);
        }
      }
    ...
    require(uniBestAmountIn > 0, encodeError("SwapRouter: invalid routing 02", path[0], path[1])); // invalid routing with Uni v2 swapTokensForExactTokens
    require(uniBestAmountIn < uint256(-1), encodeError("SwapRouter: invalid routing 021", path[0], path[1])); // invalid routing with Uni v2 swapTokensForExactTokens
    require(uniBestAmountIn < amountInMax, encodeError("SwapRouter: invalid routing 022", path[0], path[1])); // invalid routing with Uni v2 swapTokensForExactTokens
