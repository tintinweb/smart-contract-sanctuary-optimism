...
@external
@view
def rewardPairs(_limit: uint256, _offset: uint256, _venft_id: uint256) \
    -> DynArray[address, MAX_RESULTS]:
  """
  @notice Returns a list of pair addresses with rewards for a veNFT
  @param _limit The max amount of pairs to check for rewards
  @param _offset The amount of pairs to skip checking for rewards
  @param _venft_id The veNFT ID to get rewards for
  @return Array for Pair addresses with rewards
  """
  psugar: IPairsSugar = IPairsSugar(self.pairs_sugar)
  pairs_count: uint256 = IPairFactory(self.pair_factory).allPairsLength()
  with_rewards: DynArray[address, MAX_RESULTS] = \
    empty(DynArray[address, MAX_RESULTS])

  for pindex in range(_offset, _offset + MAX_RESULTS):
    if len(with_rewards) == _limit or pindex >= pairs_count:
      break

    # Do not send the `msg.sender` to save gas...
    pair: Pair = psugar.byIndex(pindex, empty(address))

    if self._pairHasRewards(_venft_id, pair):
      with_rewards.append(pair.pair_address)
...
@internal
@view
def _pairHasRewards(_venft_id: uint256, _pair: Pair) -> bool:
  """
  @notice Returns a collection with veNFT pair rewards
  @param _venft_id The veNFT ID to get rewards for
  @param _pair The `Pair` sturct to work with
  ...
  if (
    _pair.gauge == empty(address) or \
    _pair.fee == empty(address) or \
    _pair.wrapped_bribe == empty(address)
  ):
    return False

  fees: IBribe = IBribe(_pair.fee)

  if (
    fees.earned(_pair.token0, _venft_id) > 0 or
    fees.earned(_pair.token1, _venft_id) > 0
   ):
    return True
