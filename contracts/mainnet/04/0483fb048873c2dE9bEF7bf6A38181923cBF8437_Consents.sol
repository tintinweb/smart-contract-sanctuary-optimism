...
contract Consents is IConsents, ConsentRegistry, Managers {
    ...
    /**
     * @dev Event is triggered when points contract address is set
     * @param _pointContractAddress address of points contract
     */
    event PointsContractSet(address _pointContractAddress);
    ...
    /**
     * @dev Sets points contract address
     * @param _pointsContractAddress address of points contract
     */
    function setPointsContract(address _pointsContractAddress) external onlyOwner {
        if (_pointsContractAddress == address(0x0)) {
            revert Consents_AddressIsZero();
        }
        points = Points(_pointsContractAddress);
        emit PointsContractSet(_pointsContractAddress);
    }

    /**
     * @dev Gets points contract address
     * @return address of points contract
     */
    function getPointsContract() external view returns (address) {
     ...
     */
    function consentPersons(uint32 id) public view returns (bytes32[] memory) {
    ...
    /**
     * @dev Returns total amount of consents
     * @return uint256 amount of consents
     */
    function totalConsents() public view returns (uint256) {
        uint256 count;
        for (uint32 i = 1; i <= _consentsCount; i++) {
            count += consentPersons(i).length;
        }
        return count;
    }

    /**
     * @dev Returns users count
     * @return uint256 users count
     */
    function countUsers() external view returns (uint256) {
        bytes32[] memory usersArr = new bytes32[](totalConsents());
        uint256 counter;
        bool isUnique;
        for (uint32 i = 1; i <= _consentsCount; i++) {
            bytes32[] memory currentConsents = consentPersons(i);

            for (uint32 j = 0; j < currentConsents.length; j++) {
                isUnique = true;

                for (uint32 k = 0; k < counter; k++) {
                    if (usersArr[k] == currentConsents[j]) {
                        isUnique = false;
                        break;
                    }
                }
                ...
                if (isUnique) {
                    usersArr[counter] = currentConsents[j];
                    counter++;
                }
            }
        }
...
import './access/PointsManager.sol';

error Points_AddressIsZero();
error Points_InvalidAmount();
error Points_Error(string msg);

/**
 * @title Points Contract
 */
contract Points is PointsManager {
    /**
     * @dev Event is triggered when consents contract address is set
     * @param _consentsContractAddress address of consents contract
     */
    event ConsentsContractAddressSet(address _consentsContractAddress);

    /**
     * @dev Event is triggered when benefits contract address is set
     * @param _benefitsContractAddress address of benefits contract
     */
    event BenefitsContractAddressSet(address _benefitsContractAddress);

    /**
     * @dev Event is triggered when points added
     * @param _id bytes32 person id
     * @param _amount uint256 amount of points
     */
    event PointsAdded(bytes32 _id, uint256 _amount);

    /**
     * @dev Event is triggered when points removed
     * @param _id bytes32 person id
     * @param _amount uint256 amount of points
     */
    event PointsRemoved(bytes32 _id, uint256 _amount);

    /**
     * @dev Event is triggered when points set
     * @param _id bytes32 person id
     * @param _prevAmount uint256 previous amount of points
     * @param _currentAmount uint256 current amount of points
     */
    event PointsSet(bytes32 _id, uint256 _prevAmount, uint256 _currentAmount);
    ...
    address private consentsContractAddress;
    address private benefitsContractAddress;
    mapping(bytes32 => uint256) private personPoints;
    mapping(bytes32 => Operation[]) private personHistory;

    /**
     * @dev Throws if called by any account other than consents contract, benefits contract, owner or manager.
     */
    modifier onlyOwnerOrManagerOrContract() {
        if (
            _msgSender() != consentsContractAddress &&
            _msgSender() != benefitsContractAddress &&
            _msgSender() != owner() &&
            _msgSender() != pointsManager()
        ) {
            revert Points_Error('Caller is not the owner, manager, consents contract or benefits contract!');
        }
        _;
    }

    constructor(address _consentsContractAddress) {
        if (_consentsContractAddress == address(0x0)) {
            revert Points_AddressIsZero();
        }
        consentsContractAddress = _consentsContractAddress;
    }

    /**
     * @dev Sets consent contract address
     * @param _consentContractAddress address of consent contract
     */
    function setConsentsContractAddress(address _consentContractAddress) external onlyOwner {
        if (_consentContractAddress == address(0x0)) {
            revert Points_AddressIsZero();
        }
        consentsContractAddress = _consentContractAddress;
        emit ConsentsContractAddressSet(_consentContractAddress);
    }

    /**
     * @dev Gets consent contract address
     * @return address of consent contract
     */
    function getConsentsContractAddress() external view returns (address) {
        return consentsContractAddress;
    }

    /**
     * @dev Sets benefits contract address
     * @param _benefitsContractAddress address of benefits contract
     */
    function setBenefitsContractAddress(address _benefitsContractAddress) external onlyOwner {
        if (_benefitsContractAddress == address(0x0)) {
            revert Points_AddressIsZero();
        }
        benefitsContractAddress = _benefitsContractAddress;
        emit BenefitsContractAddressSet(_benefitsContractAddress);
    }

    /**
     * @dev Gets benefits contract address
     * @return address of benefits contract
     */
    function getBenefitsContractAddress() external view returns (address) {
        return benefitsContractAddress;
    }

    /**
     * @dev Adds points for selected user
     * @param _id bytes32 id of user
     * @param _amount unt256 amount of tokens
     */
    function addPoints(bytes32 _id, uint256 _amount) external onlyOwnerOrManagerOrContract {
        if (_amount <= 0) {
            revert Points_InvalidAmount();
        }
        personPoints[_id] += _amount;
        addOperation('added', _id, _amount);
        emit PointsAdded(_id, _amount);
    }

    /**
     * @dev Removes points for selected user
     * @param _id bytes32 id of user
     * @param _amount unt256 amount of tokens
     */
    function removePoints(bytes32 _id, uint256 _amount) external onlyOwnerOrManagerOrContract {
        if (_amount <= 0) {
            revert Points_InvalidAmount();
        }
        if (_amount > getPoints(_id)) {
            revert Points_Error('Points: removing more than total');
        }
        personPoints[_id] -= _amount;
        addOperation('removed', _id, _amount);
        emit PointsRemoved(_id, _amount);
    }

    /**
     * @dev Sets points for selected user
     * @param _id bytes32 id of user
     * @param _amount unt256 amount of tokens
     */
    function setPoints(bytes32 _id, uint256 _amount) external onlyOwnerOrManagerOrContract {
        if (_amount == getPoints(_id)) {
            revert Points_Error('Points: amount matches current balance');
        }
        uint256 _oldAmount = getPoints(_id);
        personPoints[_id] = _amount;
        addOperation('set', _id, _amount);
        emit PointsSet(_id, _oldAmount, _amount);
    }

    /**
     * @dev Gets user history
     * @param _id bytes32 id of user
     * @return OperationsHistory[] structure
     */
    function getUserHistory(bytes32 _id) external view returns (Operation[] memory) {
        return personHistory[_id];
    }

    /**
     * @dev Gets user points
     * @param _id bytes32 id of user
     * @return uint256 amount of tokens
     */
    function getPoints(bytes32 _id) public view returns (uint256) {
        return personPoints[_id];
    }

    /**
     * @dev Adds operation
     * @param _operation string name of operation
     * @param _id bytes32 id of user
     * @param _amount uint256 amount of tokens
     */
    function addOperation(string memory _operation, bytes32 _id, uint256 _amount) internal {
        Operation memory operation = Operation({
            timestamp: getCurrentTimestamp(),
            amount: _amount,
            operation: _operation
        });
        personHistory[_id].push(operation);
    }
...
error PointsManager_Error(string msg);

abstract contract PointsManager is Ownable2Step {
    /**
     * @dev Event is triggered when manager address is changed
     * @param _address manager address
     */

    event PointsManagerChanged(address _address);

    address manager;

    /**
     * @dev Throws if called by any account other than the points manager.
     */
    modifier onlyPointsManager() {
        if (_msgSender() != manager) {
            revert PointsManager_Error('Caller is not the points manager!');
        }
        _;
    }

    /**
     * @dev Returns the address of the current points manager.
     */
    function pointsManager() public view returns (address) {
        return manager;
    }
     ...
     */
    function setPointsManager(address _manager) external onlyOwner {
        if (_manager == address(0x0)) {
            revert PointsManager_Error('Address is zero!');
        }
        manager = _manager;
        emit PointsManagerChanged(_manager);
    }
}
