...
//    uint256 immutable DEFAULT_LOCKUP_PERIOD = 60 * 1; // 1 minute
//    uint256 immutable DEFAULT_SF_PERCENT = 5000; // 5000 / 100000 = 0.05 = 5%
//    uint256 immutable DEFAULT_MF_PERIOD = 30 * 1; // 1 minute
//    uint256 immutable DEFAULT_MF_PERCENT = 10000; // 10000 / 100000 = 0.1 = 10%
//    uint256 immutable DEFAULT_PF_PERIOD = 30 * 1; // 1 minute
//    uint256 immutable DEFAULT_PF_PERCENT = 10000; // 10000 / 100000 = 0.1 = 10%
//    uint256 immutable DEFAULT_AF_PERCENT = 2000; // 2000 / 100000 = 0.02 = 2%
//    uint256 immutable DEFAULT_MIN_STAKING_AMOUNT = 100; //100$ minimum deposit
//    uint256 immutable DEFAULT_MIN_WITHDRAW_AMOUNT = 100; //100$ minimum withdraw
//    uint256 immutable DEFAULT_INVEST_PERIOD = 60 * 1;
    ...
    function gatherSf(uint256 fundId, uint256 pending, address token) external override auth returns(uint256) {
        ...
        uint256 sf = Math.mulDiv(funds[fundId].sf + serviceSf, pending, 1e18);
        FundFees memory gatheredFees = gatheredFees[fundId];
        gatheredFees.sf += sf;

        IERC20Upgradeable(token).safeTransferFrom(msg.sender, address(this), sf);

        emit SfCharged(fundId, sf);
    ...
    function gatherPf(uint256 fundId, uint256 pending, address token) external override auth {
        ...
        uint256 pf = Math.mulDiv(funds[fundId].pf + servicePf, pending, 1e18);
        FundFees memory gatheredFees = gatheredFees[fundId];
        gatheredFees.pf += pf;

        IERC20Upgradeable(token).safeTransferFrom(msg.sender, address(this), pf);

        emit PfCharged(fundId, pf);
    }
    ...
    function gatherMf(uint256 fundId, uint256 pending, address token, address manager) external override auth {
        ...
        uint256 mf = Math.mulDiv(funds[fundId].mf + serviceMf, pending, 1e18);
        FundFees memory gatheredFees = gatheredFees[fundId];
        gatheredFees.mf += mf;

        IERC20Upgradeable(token).safeTransferFrom(msg.sender, manager, mf);

        emit MfCharged(fundId, mf);
    }

    function withdraw(address user, address token, uint256 amount) external auth {
