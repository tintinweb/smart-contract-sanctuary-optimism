...
contract Interaction is Initializable, UUPSUpgradeable, OwnableUpgradeable, WardedLivingUpgradeable, IInteraction {
    ...
    function tvl(uint256 fundId, uint256 tradeTvl) public override view returns (int256) {
        ...
        return feeder.tvl(fundId, tradeTvl) + int256(trader.usdtAmount());
    }
    ...
    function hide(uint256 fundId) external override live {
        ...
        require(funds[fundId].live != 0, "I/FNL"); // Fund not live

        funds[fundId].live = 0;

        emit FundStateChanged(fundId, funds[fundId].live);
    }

    function show(uint256 fundId) external override live {
        require(fundExist(fundId), "I/FNE");
        require(feeder.managers(fundId) == msg.sender, "I/NAM");//not a manager
        require(funds[fundId].live == 0, "I/FL"); // Fund already live
    ...
    function drip(uint256 fundId, uint256 tradeTvl) external override live auth {
        ...
        if (pending > 0) {
            feeder.drip(fundId, funds[fundId].trade, tradeTvl);
        }
        funds[fundId].nextPeriod = block.timestamp + funds[fundId].period;
