/**
 *Submitted for verification at optimistic.etherscan.io on 2022-07-01
*/
...
// File: contracts/BIFI/interfaces/common/IUniswapRouterSolidly.sol
...
contract StrategyCommonSolidlyGaugeLP is StratManager, FeeManager, GasThrottler {
    ...
    Routes[] public outputToNativeRoute;
    Routes[] public outputToLp0Route;
    Routes[] public outputToLp1Route;
    ...
    function _initOutputToNativeRoute(address[][] memory tokens, bool[] memory stables) internal {
        for (uint i; i < tokens.length; ++i) {
            outputToNativeRoute.push(Routes({
                from: tokens[i][0],
                to: tokens[i][1],
                stable: stables[i]
            }));
    ...
    function chargeFees(address callFeeRecipient) internal {
        ...
        for (uint i; i < outputToNativeRoute.length; ++i) {
            IUniswapRouterSolidly(unirouter).swapExactTokensForTokensSimple(toNative, 0, outputToNativeRoute[i].from, outputToNativeRoute[i].to, outputToNativeRoute[i].stable, address(this), now);
            toNative = IERC20(outputToNativeRoute[i].to).balanceOf(address(this));
        }
    ...
    function addLiquidity() internal {
        ...
        if (lpToken0 != output) {
            uint256 lp0SwapBal = outputHalf;
            for (uint i; i < outputToLp0Route.length; ++i) {
            IUniswapRouterSolidly(unirouter).swapExactTokensForTokensSimple(lp0SwapBal, 0, outputToLp0Route[i].from, outputToLp0Route[i].to, outputToLp0Route[i].stable, address(this), now);
            lp0SwapBal = IERC20(outputToLp0Route[i].to).balanceOf(address(this));
            }
        }

        if (lpToken1 != output) {
            uint256 lp1SwapBal = outputHalf;
            for (uint i; i < outputToLp1Route.length; ++i) {
            IUniswapRouterSolidly(unirouter).swapExactTokensForTokensSimple(lp1SwapBal, 0, outputToLp1Route[i].from, outputToLp1Route[i].to, outputToLp1Route[i].stable, address(this), now);
            lp1SwapBal = IERC20(outputToLp0Route[i].to).balanceOf(address(this));
            }
        }
    ...
    function _giveAllowances() internal {
        ...
        for (uint i; i < outputToNativeRoute.length; ++i) {
            IERC20(outputToNativeRoute[i].from).safeApprove(unirouter, 0);
            IERC20(outputToNativeRoute[i].from).safeApprove(unirouter, uint256(-1));
        }

        for (uint i; i < outputToLp0Route.length; ++i) {
            IERC20(outputToLp0Route[i].from).safeApprove(unirouter, 0);
            IERC20(outputToLp0Route[i].from).safeApprove(unirouter, uint256(-1));
        }

         for (uint i; i < outputToLp1Route.length; ++i) {
            IERC20(outputToLp1Route[i].from).safeApprove(unirouter, 0);
            IERC20(outputToLp1Route[i].from).safeApprove(unirouter, uint256(-1));
        }
