...
/// @title This contract is intended to be use by contracts via delegatecall.
contract CapsuleProxy is ICapsuleProxy {
    using SafeERC20 for IERC20;

    error AddressIsNull();
    error UnsupportedCapsuleType();

    ICapsuleMinter public immutable capsuleMinter;

    constructor(ICapsuleMinter capsuleMinter_) {
        if (address(capsuleMinter_) == address(0)) revert AddressIsNull();
        capsuleMinter = capsuleMinter_;
    }

    /// @inheritdoc ICapsuleProxy
    function burnCapsule(
        ...
        if (capsuleType_ == CapsuleData.CapsuleType.SIMPLE) {
            capsuleMinter.burnSimpleCapsule(collection_, capsuleId_, burnFrom_);
        } else if (capsuleType_ == CapsuleData.CapsuleType.ERC20) {
            if (capsuleMinter.singleERC20Capsule(collection_, capsuleId_).tokenAmount > 0) {
                capsuleMinter.burnSingleERC20Capsule(collection_, capsuleId_, burnFrom_, receiver_);
            } else {
                capsuleMinter.burnMultiERC20Capsule(collection_, capsuleId_, burnFrom_, receiver_);
            }
        } else if (capsuleType_ == CapsuleData.CapsuleType.ERC721) {
            if (capsuleMinter.singleERC721Capsule(collection_, capsuleId_).tokenAddress != address(0)) {
                capsuleMinter.burnSingleERC721Capsule(collection_, capsuleId_, burnFrom_, receiver_);
            } else {
                capsuleMinter.burnMultiERC721Capsule(collection_, capsuleId_, burnFrom_, receiver_);
            }
        } else if (capsuleType_ == CapsuleData.CapsuleType.ERC1155) {
            capsuleMinter.burnMultiERC1155Capsule(collection_, capsuleId_, burnFrom_, receiver_);
        } else {
            revert UnsupportedCapsuleType();
        }
    }
    ...
    function mintCapsule(
        ...
        _capsuleId = ICapsule(collection_).counter();
        if (capsuleContent_.capsuleType == CapsuleData.CapsuleType.SIMPLE) {
            capsuleMinter.mintSimpleCapsule{value: mintFee_}(collection_, capsuleContent_.tokenURI, receiver_);
        } else if (capsuleContent_.capsuleType == CapsuleData.CapsuleType.ERC20) {
            uint256 _len = capsuleContent_.tokenAddresses.length;
            if (_len == 1) {
                address _tokenAddress = capsuleContent_.tokenAddresses[0];
                uint256 _actualAmount = _handleIncomingERC20Tokens(_tokenAddress, capsuleContent_.amounts[0]);
                capsuleMinter.mintSingleERC20Capsule{value: mintFee_}(
                    collection_,
                    _tokenAddress,
                    _actualAmount,
                    capsuleContent_.tokenURI,
                    receiver_
                );
            } else {
                uint256[] memory _actualAmounts = new uint256[](_len);
                for (uint8 i; i < _len; i++) {
                    _actualAmounts[i] = _handleIncomingERC20Tokens(
                        capsuleContent_.tokenAddresses[i],
                        capsuleContent_.amounts[i]
                    );
                }
                capsuleMinter.mintMultiERC20Capsule{value: mintFee_}(
                    collection_,
                    capsuleContent_.tokenAddresses,
                    _actualAmounts,
                    capsuleContent_.tokenURI,
                    receiver_
                );
            }
        } else if (capsuleContent_.capsuleType == CapsuleData.CapsuleType.ERC721) {
            uint256 _len = capsuleContent_.tokenAddresses.length;
            if (_len == 1) {
                address _tokenAddress = capsuleContent_.tokenAddresses[0];
                uint256 _tokenId = capsuleContent_.tokenIds[0];
                _handleIncomingERC721Tokens(_tokenAddress, _tokenId);
                capsuleMinter.mintSingleERC721Capsule{value: mintFee_}(
                    collection_,
                    _tokenAddress,
                    _tokenId,
                    capsuleContent_.tokenURI,
                ...
                for (uint8 i; i < _len; i++) {
                    _handleIncomingERC721Tokens(capsuleContent_.tokenAddresses[i], capsuleContent_.tokenIds[i]);
                }
                capsuleMinter.mintMultiERC721Capsule{value: mintFee_}(
                    collection_,
                    capsuleContent_.tokenAddresses,
                    capsuleContent_.tokenIds,
                    capsuleContent_.tokenURI,
                    receiver_
                );
            }
        } else if (capsuleContent_.capsuleType == CapsuleData.CapsuleType.ERC1155) {
            uint256 _len = capsuleContent_.tokenAddresses.length;
            uint256[] memory _actualAmounts = new uint256[](_len);
            for (uint8 i; i < _len; i++) {
                _actualAmounts[i] = _handleIncomingERC1155Tokens(
                    capsuleContent_.tokenAddresses[i],
                    capsuleContent_.tokenIds[i],
                    capsuleContent_.amounts[i]
                );
            }
            capsuleMinter.mintMultiERC1155Capsule{value: mintFee_}(
                collection_,
                capsuleContent_.tokenAddresses,
                capsuleContent_.tokenIds,
                _actualAmounts,
    ...
    function _handleIncomingERC20Tokens(
        address tokenAddress_,
        uint256 amount_
    ) internal returns (uint256 _actualAmount) {
        // Handle incoming ERC20 tokens
        uint256 _balanceBefore = IERC20(tokenAddress_).balanceOf(address(capsuleMinter));
        IERC20(tokenAddress_).safeTransferFrom(msg.sender, address(capsuleMinter), amount_);
        _actualAmount = IERC20(tokenAddress_).balanceOf(address(capsuleMinter)) - _balanceBefore;
    }

    function _handleIncomingERC721Tokens(address tokenAddress_, uint256 tokenId_) internal {
        // Handle incoming ERC721 tokens
        IERC721(tokenAddress_).safeTransferFrom(msg.sender, address(capsuleMinter), tokenId_);
        // check the contract owns NFT with tokenId_
        require(IERC721(tokenAddress_).ownerOf(tokenId_) == address(capsuleMinter), "not-nft-owner");
    }

    function _handleIncomingERC1155Tokens(
        address tokenAddress_,
        uint256 tokenId_,
        uint256 amount_
    ) internal returns (uint256 _actualAmount) {
        // Handle incoming ERC1155 tokens
        uint256 _balanceBefore = IERC1155(tokenAddress_).balanceOf(address(capsuleMinter), tokenId_);
        IERC1155(tokenAddress_).safeTransferFrom(msg.sender, address(capsuleMinter), tokenId_, amount_, "");
        _actualAmount = IERC1155(tokenAddress_).balanceOf(address(capsuleMinter), tokenId_) - _balanceBefore;
    }
}
