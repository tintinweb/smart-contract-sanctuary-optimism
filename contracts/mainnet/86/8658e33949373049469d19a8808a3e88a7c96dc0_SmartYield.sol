...
contract SmartYield is ISmartYield, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    function addLiquidity(address _bondProvider, uint256 _tokenAmount) external override onlyVault {
        ...
        emit AddLiquidity(msg.sender, pool.liquidityProviderBalance, _tokenAmount);
    }
    ...
    function removeLiquidity(address _bondProvider, uint256 _tokenAmount) external override onlyVault {
        ...
        emit RemoveLiquidity(msg.sender, pool.liquidityProviderBalance, _tokenAmount);
    }
     ...
     */
    function buyBond(address _bond, uint256 _tokenAmount) external override nonReentrant whenNotPaused defaultCheck(_bond) {
        address bondProvider = providerByBond[_bond];
        Pool storage pool = poolByProvider[bondProvider];
        require(_tokenAmount > 0, "Amount must be > 0");
        ...
        require((start_ - pool.withdrawWindow) > block.timestamp, "cannot buy now");
     ...
     * @param _tokenAmount the amount of the underlying token
     */
    function redeemBond(address _bond, uint256 _tokenAmount) external override nonReentrant whenNotPaused defaultCheck(_bond) {
        address bondProvider = providerByBond[_bond];
        Pool storage pool = poolByProvider[bondProvider];
        require(_tokenAmount > 0, "Amount must be > 0");
        TermInfo memory termInfo = pool.bondData[_bond];
     ...
     * @param _tokenAmount the amount of the underlying token
     */
    function rolloverBond(address _bond, uint256 _tokenAmount) external override nonReentrant whenNotPaused defaultCheck(_bond) {
        address bondProvider = providerByBond[_bond];
        Pool storage pool = poolByProvider[bondProvider];
        require(_tokenAmount > 0, "Amount must be > 0");
        TermInfo memory termInfo = pool.bondData[_bond];
        ...
        require((block.timestamp < (nextTermInfo.start - pool.withdrawWindow)), "next term has started");
     ...
     */
    function withdraw(address _bond, uint256 _withdrawAmount) external override nonReentrant whenNotPaused defaultCheck(_bond) {
        address bondProvider = providerByBond[_bond];
        Pool storage pool = poolByProvider[bondProvider];
        ...
        uint256 balanceBefore = IERC20(pool.underlying).balanceOf(address(this));
        IProvider(bondProvider).withdraw(_withdrawAmount);
        uint256 balanceAfter = IERC20(pool.underlying).balanceOf(address(this));
        IERC20(pool.underlying).safeTransfer(msg.sender, balanceAfter - balanceBefore);
    ...
    ) external override nonReentrant whenNotPaused defaultCheck(_bond) {
        address bondProvider = providerByBond[_bond];
        Pool storage pool = poolByProvider[bondProvider];
     ...
     */
    function liquidateTerm(address _bond) external nonReentrant whenNotPaused defaultCheck(_bond) {
        address bondProvider = providerByBond[_bond];
        Pool storage pool = poolByProvider[bondProvider];
        require(pool.underlying != address(0), "invalid provider");
        TermInfo memory termInfo = pool.bondData[_bond];
    ...
    function createPool(
        ...
        pool.withdrawWindow = _withdrawWindow;
        pool.nextDebtId = 1;
        pool.healthFactorGuard = _healthFactorGuard;
    ...
    function setNextTermFor(
        ...
        Pool storage pool = poolByProvider[_bondProvider];
        require(pool.underlying != address(0), "invalid provider");
        require(_start > (block.timestamp + pool.withdrawWindow), "invalid start");
