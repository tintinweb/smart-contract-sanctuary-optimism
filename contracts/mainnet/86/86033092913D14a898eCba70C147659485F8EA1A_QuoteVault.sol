...
contract QuoteVault is CommonVault {
    ...
    function deposit(uint256 assets) external override whenNotPaused nonReentrant returns (uint256) {
        ...
        // QV_ILP: invalid liquidity provider
        require(IVaultConfig(_vaultConfig).isWhitelistedLiquidityProvider(msgSender), "QV_ILP");

        uint256 _totalSupply = IVaultToken(_vaultToken).totalSupply();

        // calculate shares first as the below transferFrom() influences totalAssets()
        uint256 shares = _convertToShares(assets, totalAssets(), _totalSupply, IERC20Metadata(_quoteToken).decimals());

        IVaultToken(_vaultToken).mint(msgSender, shares);
        emit Deposit(msgSender, assets, shares);

        SafeERC20Upgradeable.safeTransferFrom(IERC20Metadata(_quoteToken), msgSender, address(this), assets);
        IPerpPositionManager(_perpPositionManager).deposit(_quoteToken, assets);
        return shares;
    }
        ...
        returns (uint256)
    {
        ...
        // TODO: rename totalSupply_6 which is not 6 decimals, it's in vaultToken's decimals
        // cache totalSupply before calling _burn()
        uint256 totalSupply_6 = IVaultToken(_vaultToken).totalSupply();
        IVaultToken(_vaultToken).burn(msgSender, shares);

        // get current quote token balance * share ratio
        uint256 usdcHere_6 = IERC20Metadata(_quoteToken).balanceOf(address(this)).mulDiv(shares, totalSupply_6);

        uint256 baseHere = IERC20Metadata(_baseToken).balanceOf(address(this));
        uint256 usdcOnUni_6;
        if (baseHere > 0) {
            // get current base token balance * share ratio
            uint256 reducedBase = baseHere.mulDiv(shares, totalSupply_6);

            // NOTE: it's possible that positionSize.abs() < baseHere if
            // someone (including arb contract) sends some WETH directly to QuoteVault
            // however, the extra WETH can be treated as arb profit, though it's WETH instead USDC
            // we don't need to fully-hedge the extra WETH
            int256 positionSize = IPerpPositionManager(_perpPositionManager).getTakerPositionSize();
            _requireShortOrZeroPosition(positionSize);

            if (positionSize != 0) {
                // reduce position on perp
                uint256 reducedPositionSizeAbs = positionSize.abs().mulDiv(shares, totalSupply_6);
            ...
            usdcOnUni_6 = UniswapV3Facade.exactInputSingle(
                _uniswapV3Router,
        ...
        // stack too deep
        {
            ...
            // redeemAmount = arb profit +
            //                sell eth to usdc on uniswap +
            //                reduce position and withdraw on perp
            redeemedUsdc_6 = usdcHere_6.add(usdcOnUni_6).add(usdcOnPerp_6);
        }

        // CV_RALTMRA: RedeemedAmount is Less Than MinRedeemedAmount
        require(redeemedUsdc_6 >= minRedeemedAmount, "CV_RALTMRA");

        SafeERC20Upgradeable.safeTransfer(IERC20Metadata(_quoteToken), msgSender, redeemedUsdc_6);

        emit Redeem(msgSender, redeemedUsdc_6, shares);
        ...
        returns (uint256)
    {
        ...
        bool isQuoteToBase = params.tokenIn == _quoteToken;
        uint256 amountIn_18;

        // quoteVault will only have short position
        // only QuoteToken will be deposited to Curie Vault contract; BaseToken is simply held here (in QuoteVault)
        // thus, deposit QuoteToken if tokenIn == QuoteToken
        if (isQuoteToBase) {
            // long on perp => quoteVault decrease position
            amountIn_18 = _formatDecimals(params.amountIn, _quoteTokenDecimals, 18);
            IPerpPositionManager(_perpPositionManager).deposit(_quoteToken, params.amountIn);
        } else {
            // short on perp => quoteVault increase position
            // QV_MIBS : margin insufficient before swap
            require(
        ...
        // if buy eth, "QV_PSL" means insufficient liquidity: baseToken
        ...
        // NOTE: ensure quoteVault only has short position
        int256 perpPositionSize = IPerpPositionManager(_perpPositionManager).getTakerPositionSize();

        _requireShortOrZeroPosition(perpPositionSize);
        ...
        if (isQuoteToBase) {
            ...
            // TODO: it seems we don't need "QV_ILB" since "QV_PSL" will always trigger first?
            // though I guess keeping it here won't hurt
            // require there is enough amountOut as QuoteVault can run out of BaseToken
            // QV_ILB: insufficient liquidity: baseToken
            require(amountOut <= IERC20Metadata(params.tokenOut).balanceOf(address(this)), "QV_ILB");
        } else {
            // short on perp => quoteVault increase position
            amountOut = _formatDecimals(quote, 18, _quoteTokenDecimals);

            // withdraw QuoteToken if tokenOut == QuoteToken
            // 1. if QuoteVault has no enough freeCollateral for the specified amountOut of usdc for _withdraw(),
            ...
            // QV_MIAS : margin insufficient after swap
            require(
                IPerpPositionManager(_perpPositionManager).isMarginSufficientByRatio(
    ...
    // TODO reducedPositionSize should <= getDeleveragedPositionSize
    function deleverage(uint256 reducedPositionSize) external override whenNotPaused nonReentrant {
        _requireIsWhitelistedArbitrageur(_msgSender());

        uint24 deleverageRatio = IVaultConfig(_vaultConfig).getDeleverageMarginRatio();
        ...
        _deleverage(
            reducedPositionSize == 0
                ? IPerpPositionManager(_perpPositionManager).getDeleveragedPositionSize(deleverageRatio)
                : reducedPositionSize
        );
    }
    ...
    /// @dev in notional of quoteToken
    function totalAssets() public view override returns (uint256) {
        uint256 quoteTokenBalance_6 = IERC20Metadata(_quoteToken).balanceOf(address(this));

        uint256 baseTokenNotional_6 = _getBaseTokenNotional();

        // _getAccountValue revert if bankrupt
        uint256 perpTotalAccountValue_6 = IPerpPositionManager(_perpPositionManager).getAccountValue_6();

        return quoteTokenBalance_6.add(baseTokenNotional_6).add(perpTotalAccountValue_6);
    }
    ...
    function _deleverage(uint256 reducedPositionSizeAbs) internal {
        ...
        uint256 base_bd = _formatDecimals(base, 18, _baseTokenDecimals);
        uint256 uniQuoteTokenAmountOut_6 = UniswapV3Facade.exactInputSingle(
            _uniswapV3Router,
            ISwapRouter.ExactInputSingleParams({
                tokenIn: _baseToken,
                tokenOut: _quoteToken,
    ...
    function _getBaseTokenNotional() internal view returns (uint256) {
        ...
        uint256 baseTokenNotional_qd = _formatDecimals(
            baseTokenBalance_bd.mulDiv(indexTwap_18, 10**18),
            _baseTokenDecimals,
            _quoteTokenDecimals
        );

        return baseTokenNotional_qd;
    }
    ...
    function _requireShortOrZeroPosition(int256 perpPositionSize) internal pure {
        // QV_PSL: Position Size is Long
        require(perpPositionSize <= 0, "QV_PSL");
    }
}
