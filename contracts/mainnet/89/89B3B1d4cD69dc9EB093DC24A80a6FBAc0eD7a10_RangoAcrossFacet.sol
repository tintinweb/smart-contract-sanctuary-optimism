...
interface IRango {
    ...
    event RangoBridgeCompleted(
        ...
        address receiver,
        uint amount,
        bool success // todo:  use LibInterchain status instead of success
    );
...
library LibSwapper {
    ...
    function onChainSwapsPreBridge(
        ...
        uint minimumRequiredValue = (isNative ? request.feeIn + request.affiliateIn + request.amountIn + request.destinationExecutorFee : 0) + extraFee;
    ...
    function callSwapsAndFees(SwapRequest memory request, Call[] calldata calls) private returns (bytes[] memory) {
        ...
        // Get Platform fee
        if (request.feeIn > 0 || request.destinationExecutorFee >0) {
            require(baseSwapperStorage.feeContractAddress != ETH, "Fee contract address not set");
            _sendToken(request.fromToken, request.feeIn + request.destinationExecutorFee, baseSwapperStorage.feeContractAddress, isSourceNative, false);
