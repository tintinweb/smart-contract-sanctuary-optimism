...
contract Treasury is ReentrancyGuard {
    ...
    event RetrieveETH(uint256 amount);
    ...
    function retrieveTokenExact(address _token, uint256 _amount)
        public
        onlyGovernance
    {
    ...
    function retrieveETHExact(uint256 _amount)
        public
        onlyGovernance
        nonReentrant
    {
        (bool success, bytes memory returnData) =
            governance.call{value: _amount}("");
        if (!success) {
            emit FailedETHSend(returnData);
        }
