/**
 *Submitted for verification at Optimistic.Etherscan.io on 2022-09-07
*/
...
interface IVault {
    ...
    function buy(address _token, address _receiver) external returns (uint256);
    function sell(address _token, address _receiver, uint256 _usdAmount) external returns (uint256);
...
contract Vault is ReentrancyGuard, IVault {
    ...
    event Buy(address account, address token, uint256 tokenAmount, uint256 usdAmount, uint256 feeBasisPoints);
    event Sell(address account, address token, uint256 usdAmount, uint256 tokenAmount, uint256 feeBasisPoints);
    ...
    function sell(address _token, address _receiver, uint256 _usdAmount) external override nonReentrant returns (uint256) {
        ...
        uint256 feeBasisPoints = vaultUtils.getSellUsdFeeBasisPoints(_token, _usdAmount);
    ...
    function swap(address _tokenIn, address _tokenOut, address _receiver) external override nonReentrant returns (uint256) {
        ...
        amountOut = amountOut.mul(10 ** tokenDecimals[_tokenOut]).div(10 ** tokenDecimals[_tokenIn]);

        uint256 usdAmount = amountIn.mul(priceIn).div(PRICE_PRECISION);
        usdAmount = usdAmount.mul(PRICE_PRECISION).div(10 ** tokenDecimals[_tokenIn]);
    ...
    function getRedemptionAmount(address _token, uint256 _usdAmount) public override view returns (uint256) {
        uint256 price = getMaxPrice(_token);
        uint256 redemptionAmount = _usdAmount.mul(PRICE_PRECISION).div(price);
        redemptionAmount = redemptionAmount.mul(10 ** tokenDecimals[_token]).div(PRICE_PRECISION);
    ...
    function getFeeBasisPoints(address _token, uint256 _usdDelta, uint256 _feeBasisPoints, uint256 _taxBasisPoints, bool _increment) public override view returns (uint256) {
        return vaultUtils.getFeeBasisPoints(_token, _usdDelta, _feeBasisPoints, _taxBasisPoints, _increment);
    }

    function getTargetUsdAmount(address _token) public override view returns (uint256) {
    ...
    // calculate current token aum in Vault 
    function getTokenAum(address _token) public override view returns (uint256) {
