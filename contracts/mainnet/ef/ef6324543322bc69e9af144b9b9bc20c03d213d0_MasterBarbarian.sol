...
contract Gauge is ReentrancyGuard {
    ...
    constructor(address _brb, address _inBrb, address _token) public {
        BRB = IERC20(_brb);
        inBRB = IERC20(_inBrb);
        TOKEN = IERC20(_token);
    ...
    function derivedBalance(address account) public view returns (uint) {
        ...
        uint _adjusted = (_totalSupply.mul(inBRB.balanceOf(account)).div(inBRB.totalSupply())).mul(60).div(100);
    ...
    function notifyRewardAmount(uint256 reward) external onlyDistribution updateReward(address(0)) {
        ...
        uint balance = BRB.balanceOf(address(this));
...
contract GaugeProxy is ProtocolGovernance {
    ...
    function addGauge(address _token) external returns(address){
        ...
        gauges[_token] = address(new Gauge(address(BRB), address(inBRB), _token));
        _tokens.push(_token);
        return gauges[_token];
    }
    ...
    function distribute() external {
        collect();
        uint _balance = BRB.balanceOf(address(this));
