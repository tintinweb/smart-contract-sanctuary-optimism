...
contract Pool {
	...
	function deposit(uint256 amount) external payable {
        ...
        uint256 rlpAmountToMint = lastBalance == 0 || totalSupply == 0 ? amount : amount * totalSupply / lastBalance;

        lastDeposited[msg.sender] = block.timestamp;

        IRewards(rewards).updateRewards(msg.sender);

        totalSupply += rlpAmountToMint;
        balances[msg.sender] += rlpAmountToMint;
	...
	function withdraw(uint256 currencyAmount) external {
		...
		// Determine corresponding RLP amount
