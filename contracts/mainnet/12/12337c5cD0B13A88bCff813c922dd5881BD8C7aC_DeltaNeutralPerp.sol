...
contract DeltaNeutralPerp is StrategyStorage, IStrategy {
        ...
        if (operationType) {// openPosition
            ...
            if (positionType) {
                ...
                if (Math.mulDiv(IERC20(_wToken).balanceOf(address(this)),
                    nativeStrategyTokenPrice(0), 1e18) < usdcAmount) {
                    _fullClose();
                } else {
    ...
    */
    function getTotalUSDCValue() public view override returns (uint256) {
        ...
        getReserve() +
        Math.mulDiv(IERC20(_wToken).balanceOf(address(this)), nativeStrategyTokenPrice(0), 1e18);
    }
    ...
    * @notice Current funding rate for the perpV2
    ...
    function getCurrentFundingRate() public view override returns (uint256) {
        ...
        uint256 dailyIndexTwap = nativeStrategyTokenPrice(60 * 60);
        // 1 hour
   ...
   * @dev returns user profit in USDC  (1e6)
    ...
    * @notice Total deposit amount
    ...
    function nativeStrategyTokenPrice(uint256 interval) public view returns (uint256) {// wBTC, wETH or other
    ...
    function _closePositionsForWithdrawal(uint256 reserve, uint256 toWithdraw) private {
        ...
        uint256 wTokenAmount = Math.mulDiv(toClose, 1e18, nativeStrategyTokenPrice(0));
