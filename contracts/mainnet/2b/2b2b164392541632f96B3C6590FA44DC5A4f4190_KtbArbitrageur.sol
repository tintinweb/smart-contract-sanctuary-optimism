...
contract KtbArbitrageur is ReentrancyGuard {
    ...
    constructor(address uniswapV3RouterArg, address oneInchAggregationRouterV5Arg) {
    ...
    function arbitrage(
        ...
        // KTB_CAC: caller is a contract
        require(tx.origin == msgSender, "KTB_CAC");

        _transferTokenIn(tokenIn, msgSender, amountIn);

        // TODO:
        // if swap on 1inch first, we can call 1inch http api offchain to generate tx data,
        // however, if swap on ktb first, we cannot call 1inch http api
        // since we don't know the amount that returns by ktb swap
        uint256 amountOut;
        if (isToBuyOnKtb) {
        ...
        // KOA_NP: no profit
        require(amountOut > amountIn, "KOA_NP");
    ...
    function _swapOnOneInch(
        ...
        // TODO: It could be dangerous if we accept oneInchTxData directly from off-chain
        // parse oneInchTxData off-chain?
        (bool success, bytes memory data) = _oneInchAggregationRouterV5.call{ value: 0 }(oneInchTxData);
        if (!success) {
            // TODO: parse failure code?
            // KOA_SEO: 1inch Swap Fail, for instance, due to slippage
            revert("KOA_OSF");
        }

        // if protocols:all
        // 1inchAggregator.swap()
        (uint256 returnAmount, uint256 spentAmount) = abi.decode(data, (uint256, uint256));

        // if protocols:OPTIMISM_UNISWAP_V3
        // 1inchAggregator.uniswapV3Swap()
