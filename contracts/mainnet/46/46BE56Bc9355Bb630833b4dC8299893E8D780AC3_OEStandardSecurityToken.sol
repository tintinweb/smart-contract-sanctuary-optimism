/**
 *Submitted for verification at Optimistic.Etherscan.io on 2022-11-18
*/

/**
 *Submitted for verification at Optimistic.Etherscan.io on 2022-08-08
*/
...
// File: contracts/optimism/standardTokens/IL2StandardERC20.sol
...
interface IL2StandardERC20 is IERC20, IERC165 {
    ...
    function setL1Token(address l1TokenAddr) external;

    function setL2Bridge(address l2BridgeAddr) external;
    ...
    */
    function detectTransferRestriction(address from, address to, address initiator, uint256 value) external view returns (uint256);
    ...
    */
    function messageForTransferRestriction(uint256 restrictionCode) external view returns (string memory);
}
...
// File: contracts/optimism/standardTokens/L2SecuritiesERC20.sol
 ...
 */
abstract contract L2SecuritiesERC20 is Context, IL2StandardERC20 {
...
// File: contracts/optimism/standardTokens/OEStandardSecurityToken.sol
...
/**
 * @title OEStandardSecurityToken
 * @dev This is the baseline for standard security token to be issued by TokenFunder on Optimism
*/
contract OEStandardSecurityToken is L2SecuritiesERC20, WhitelistedRole {
    using SafeMath for uint256;
    ...
    bytes32 public _eip712DomainHash;
    ...
    ) public L2SecuritiesERC20(name, symbol) {
    ...
    modifier onlyWhitelistedAndL2Bridge {
        require(isWhitelisted(_msgSender()) || _msgSender() == l2Bridge, "Only Whitelisted and L2 Bridge can mint and burn");
        _;
    }
    ...
    /**
    * @dev admin of the security token contract (can be another contract) and l2 bridge can mint to a specific address
    */
    function mint(address to, uint256 value) public virtual override onlyWhitelistedAndL2Bridge {
        _mint(to, value);
        emit Mint(to, value);
    }

    /**
    * @dev admin of the security token contract (can be another contract) and l2 bridge can burn balance from a specific address
    */
    function burn(address from, uint256 value) public virtual override onlyWhitelistedAndL2Bridge {
        _burn(from, value);
        emit Burn(from, value);
    }

    /**
    * @dev allows change of L1 token mapping by admin, expected to be used for delayed L1 token deployment
    * @param l1TokenAddr the address of corresponding token on L1
    */
    function setL1Token(address l1TokenAddr) external override onlyWhitelisted {
        l1Token = l1TokenAddr;
    }

    /**
    * @dev allows upgradability of L2 bridge, custom bridge may be required for compliance in the future
    * @param l2BridgeAddr the address of L2 bridge
    */
    function setL2Bridge(address l2BridgeAddr) external override onlyWhitelisted {
        l2Bridge = l2BridgeAddr;
    }
    ...
    /**
    *
    * Below are standard security token functions exact same as L1 except mint and burn
    *
    */
    ...
    */
    ...
    /**
    * @dev perform an approve where initiator approves on signer's behalf, signer produces signature detailing the approve request to be taken
    * The signature in this cases conforms to EIP712 standard, eg. signed with eth_signTypedData_v3
    * @param v recovery id, part of signature
    * @param r part of signature
    * @param s part of signature
    * @param from address of the signer
    * @param settlementContract address to be approved, a.k.a spender
    * @param approveAmount amount to be approved
    * @param nonce replay nonce of the request
    */
    function metaApproveWithEIP712Signature(
        uint8 v,
        bytes32 r,
        bytes32 s,
        address from,
        ...
        bytes32 hashStruct = keccak256(
            abi.encode(
                keccak256("EIP712MetaApproval(string action,address settlementContract,address token,uint256 approveAmount,uint256 replayNonce)"),
                keccak256(bytes("Authorize Settlement")),
                settlementContract,
                address(this),
                approveAmount,
                nonce
        ...
        bytes32 metaHash = keccak256(abi.encodePacked("\x19\x01", _eip712DomainHash, hashStruct));
        address signer = ecrecover(metaHash, v, r, s);
        ...
        super._approve(signer, settlementContract, approveAmount);
