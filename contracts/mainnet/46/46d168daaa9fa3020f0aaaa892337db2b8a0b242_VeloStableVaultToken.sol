...
contract VeloStableVaultToken is IVeloStableVaultToken, PoolToken {
    ...
    function _optimalDeposit(
        ...
        uint256 den;
        {
            ...
            uint256 p = y.mul(x2.mul(3).add(y2).mul(1e18).div(y2.mul(3).add(x2))).div(x);
            num = a.mul(y);
            den = a.add(x).mul(p).div(1e18).add(y);
        }
    ...
    function _expectedLiquidity(
        uint256 amountA,
        uint256 amountB,
        bool tokenAIsToken0
    ) internal view returns (uint256 expectedLiquidity) {
        (uint256 _reserve0, uint256 _reserve1, ) = IUniswapV2Pair(underlying).getReserves();
        (uint256 reserveA, uint256 reserveB) = tokenAIsToken0 ? (_reserve0, _reserve1) : (_reserve1, _reserve0);
        uint256 lpTotalSupply = IUniswapV2Pair(underlying).totalSupply();
        expectedLiquidity = Math.min(amountA.mul(lpTotalSupply).div(reserveA), amountB.mul(lpTotalSupply).div(reserveB));
    }
    ...
    function _optimalLiquidity(
        ...
        (uint256 _reserve0, uint256 _reserve1, ) = IUniswapV2Pair(underlying)
            .getReserves();
        (uint256 reserveA, uint256 reserveB) = tokenAIsToken0 ? (_reserve0, _reserve1) : (_reserve1, _reserve0);
    ...
    function _quote(
        ...
        require(amountA > 0, "VaultToken: INSUFFICIENT_AMOUNT");
        require(
            reserveA > 0 && reserveB > 0,
            "VaultToken: INSUFFICIENT_LIQUIDITY"
        );
    ...
    function reinvest() external nonReentrant update {
        ...
        if (totalAmountA > 0 && totalAmountB > 0) {
            (uint256 liqAmountA, uint256 liqAmountB) = _optimalLiquidity(
                totalAmountA,
                totalAmountB,
                tokenA == token0
            );

            if (liqAmountA > 0 && liqAmountB > 0) {
                uint256 expectedLiquidity = _expectedLiquidity(liqAmountA, liqAmountB, tokenA == token0);
                if (expectedLiquidity > 0) {
                    liquidity = addLiquidity(tokenA, tokenB, liqAmountA, liqAmountB);
                    totalAmountA = totalAmountA.sub(liqAmountA);
                    totalAmountB = totalAmountB.sub(liqAmountB);
                }
            }
        }
        if (totalAmountA > 0 || totalAmountB > 0) {
            ...
            if (totalAmountA.mul(reserveB) > totalAmountB.mul(reserveA)) {
                ...
                //Swap B for A
                uint256 swapAmount = _optimalDeposit(totalAmountB, reserveB, reserveA, decimalsB, decimalsA);
                if (swapAmount > 0) {
            ...
            if (totalAmountA > 0 && totalAmountB > 0) {
                ...
                if (liqAmountA > 0 && liqAmountB > 0) {
                    uint256 expectedLiquidity = _expectedLiquidity(liqAmountA, liqAmountB, tokenA == token0);
                    if (expectedLiquidity > 0) {
                        liquidity = liquidity.add(addLiquidity(tokenA, tokenB, liqAmountA, liqAmountB));
                    }
                }
            }
        }
