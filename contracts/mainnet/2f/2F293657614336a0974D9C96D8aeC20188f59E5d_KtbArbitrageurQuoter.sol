...
abstract contract CommonArbitrageur is SafeOwnableNonUpgradable, ReentrancyGuard {
    using PerpMath for int256;

    address internal immutable _router;
    address internal immutable _usdcUsdChainlinkAggregator;
    ...
    constructor(address routerArg, address usdcUsdChainlinkAggregatorArg) {
        _router = routerArg;
        _usdcUsdChainlinkAggregator = usdcUsdChainlinkAggregatorArg;
    }

    // If we consider USDC/USD in the range of [1.003, 0.997] is pegged,
    ...
    function setMaxUsdcDepeggedDeviation(uint256 maxUsdcDepeggedDeviationArg) external onlyOwner {
        // CA_IDDU: invalid depegged deviation USDC
        require(maxUsdcDepeggedDeviationArg < 1e8, "CA_IDDU");
    ...
    function deleverage(address kantabanVault, uint256 reducedPositionSizeAbs)
        ...
        returns (int256 pnl)
    {
        // CA_IC: Invalid Caller
        require(_whitelistedCallerMap[_msgSender()], "CA_IC");
    ...
    function setWhitelistedCaller(address whitelistedCallerArg, bool enable) external onlyOwner {
        // CA_SV: same value
        require(_whitelistedCallerMap[whitelistedCallerArg] != enable, "CA_SV");
    ...
    function getUsdcUsdChainlinkAggregator() external view returns (address) {
        return _usdcUsdChainlinkAggregator;
    }
    ...
    function _transferTokenIn(
        ...
        // CA_IBA: inconsistent balance amount, to prevent from deflationary tokens
        require(balanceNow > balanceBefore && (balanceNow - balanceBefore) == amount, "CA_IBA");
    }

    // INTERNAL VIEW

    function _requireUsdcIsPegged() internal view {
        AggregatorV3Interface chainlinkAggregator = AggregatorV3Interface(_usdcUsdChainlinkAggregator);
        uint8 decimals = chainlinkAggregator.decimals();

        // CA_DC: Decimals Changed
        require(decimals == 8, "CA_DC");

        (, int256 price, , , ) = chainlinkAggregator.latestRoundData();

        // CA_IP: invalid price
        require(price >= 0, "CA_IP");

        // we expect USDC/USD = 1 = 1e8 (Chainlink aggregator decimals is 8)
        // CA_UD: USDC Depegged
        require((1e8 - price).abs() <= _maxUsdcDepeggedDeviation, "CA_UD");
    }
}
...
import { CommonArbitrageur } from "./CommonArbitrageur.sol";
import { ICommonVault } from "./interface/ICommonVault.sol";

// 1inch version
contract KtbArbitrageur is CommonArbitrageur {
    using SafeMath for uint256;

    enum ArbitrageType {
        BUY_ON_KTB_SELL_ON_UNI,
        BUY_ON_1INCH_SELL_ON_KTB,
        BUY_ON_UNI_SELL_ON_KTB,
        BUY_ON_KTB_SELL_ON_1INCH
    }

    address internal immutable _oneInchAggregationRouterV5;
    ...
    constructor(
        address routerArg,
        address usdcUsdChainlinkAggregatorArg,
        address oneInchAggregationRouterV5Arg
    ) CommonArbitrageur(routerArg, usdcUsdChainlinkAggregatorArg) {
        _oneInchAggregationRouterV5 = oneInchAggregationRouterV5Arg;
    }
    ...
    function arbitrage(
        ...
        require(tx.origin == msgSender, "CA_CAC");

        return _arbitrageFrom(msgSender, kantabanVault, tokenIn, tokenOut, amountIn, arbitrageType, oneInchTxData);
    }

    /// @dev this function is for quoter using, will always revert, do not interact it on-chain
    function revertSwapOnKtb(
        address kantabanVault,
        address tokenIn,
        address tokenOut,
        uint256 amountIn,
        ...
        _transferTokenIn(tokenIn, approver, amountIn);
    ...
    function getOneInchAggregationRouterV5() external view returns (address) {
        return _oneInchAggregationRouterV5;
    }
    ...
    function _arbitrageFrom(
        ...
        uint256 amountOut;
        if (arbitrageType == ArbitrageType.BUY_ON_KTB_SELL_ON_UNI) {
            uint256 amountOutFromKtb = _swapOnKtb(kantabanVault, tokenIn, tokenOut, amountIn);
            amountOut = _swapOnUni(tokenOut, tokenIn, amountOutFromKtb);
        } else if (arbitrageType == ArbitrageType.BUY_ON_1INCH_SELL_ON_KTB) {
            uint256 amountOutFromOneInch = _swapOnOneInch(tokenIn, tokenOut, amountIn, oneInchTxData);
            amountOut = _swapOnKtb(kantabanVault, tokenOut, tokenIn, amountOutFromOneInch);
        } else if (arbitrageType == ArbitrageType.BUY_ON_UNI_SELL_ON_KTB) {
            uint256 amountOutFromUni = _swapOnUni(tokenIn, tokenOut, amountIn);
            amountOut = _swapOnKtb(kantabanVault, tokenOut, tokenIn, amountOutFromUni);
        } else if (arbitrageType == ArbitrageType.BUY_ON_KTB_SELL_ON_1INCH) {
            uint256 amountOutFromKtb = _swapOnKtb(kantabanVault, tokenIn, tokenOut, amountIn);
            amountOut = _swapOnOneInch(tokenOut, tokenIn, amountOutFromKtb, oneInchTxData);
        } else {
            // CA_UAT: unknown arbitrage type
            revert("CA_UAT");
        }

        // CA_NP: no profit
        require(amountOut > amountIn, "CA_NP");
    ...
    function _swapOnOneInch(
        ...
        (bool success, ) = _oneInchAggregationRouterV5.call{ value: 0 }(oneInchTxData);
        if (!success) {
...
contract KtbArbitrageurQuoter {
    ...
    function quoteSwapOnKtb(
        ...
        try
            ktbArbitrageur.revertSwapOnKtb(kantabanVault, tokenIn, tokenOut, amountIn, msg.sender)
...
// copy from https://github.com/perpetual-protocol/perp-curie-periphery-contract
