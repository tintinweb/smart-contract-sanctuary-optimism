    ...
    ReentrancyGuardUpgradeable
{
     ...
     */
    function fulfillOrder(Order calldata _order) external payable whenNotPaused nonReentrant {
        require(
            _order.parameters.offer.length == 1,
            "SeaportWrapper: order must have one offer item"
        );

        uint256 reward = rewardDeriver.getRewardInOP(_order, msg.sender);

        if (reward > 0) {
            _sendReward(reward);
        }

        bool fulfilled = seaport.fulfillOrder{ value: msg.value }(_order, bytes32(0));
        require(fulfilled, "SeaportWrapper: order cannot be fulfilled");

        OfferItem memory offerItem = _order.parameters.offer[0];
    ...
    function fulfillAvailableOrders(
        ...
        uint256 reward = 0;
        for (uint256 i = 0; i < availableOrders.length; i++) {
            // Skip orders that were not fulfilled.
     ...
     *
     * @param _order   Seaport order struct.
     * @param _address Address to receive the reward.
     *
     * @return OP token reward amount.
     */
    function getRewardInOP(Order memory _order, address _address) external view returns (uint256) {
        return rewardDeriver.getRewardInOP(_order, _address);
    }
