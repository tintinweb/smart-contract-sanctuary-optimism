...
contract VAYC is ERC721A, Ownable {
    ...
    mapping(uint256 => bool) public isClaimMint; // if the NFT was freely minted by whitelist
    mapping(address => bool) public isClaim; // if the user already claimed

    string private baseURI;

    constructor() ERC721A("Ape's Self-Portraits", "ASP") {
        refundAddress = msg.sender;
        toggleRefundCountdown();
    }

    function checkPreClaimMint(address userAddress, bytes32[] calldata proof) public view {
        require(presaleActive, "Presale is not active");
        require(!isClaim[userAddress], "Claimed");
        require(
            _isAllowlisted(userAddress, proof, merkleRoot),
            "Not on allow list"
        );
        require(
            _numberMinted(userAddress) + 1 <= maxUserMintAmount,
            "Max amount"
        );
        require(_totalMinted() + 1 <= maxSupply, "Max mint supply");
    }

    function preClaimMint(bytes32[] calldata proof)
        external
        payable
    {
        checkPreClaimMint(msg.sender, proof);
        isClaim[msg.sender] = true;
        _safeMint(msg.sender, 1);
        isClaimMint[_currentIndex - 1] = true;
    }

    function numberMinted(address userAddress) external view returns(uint) {
    ...
    function _checkRefund(address userAddress, uint256 tokenId) private view {
        require(_exists(tokenId), "Query for non existent Token");
        require(userAddress == ownerOf(tokenId), "Not token owner");
        require(!hasRefunded[tokenId], "Already refunded");
        require(!isOwnerMint[tokenId], "Freely minted NFTs cannot be refunded");
        require(!isClaimMint[tokenId], "Claim minted NFTs cannot be refunded");
}

    function checkRefund(address userAddress, uint256[] calldata tokenIds) external view {
        require(isRefundGuaranteeActive(), "Refund expired");

        for (uint256 i = 0; i < tokenIds.length; i++) {
            uint256 tokenId = tokenIds[i];
            _checkRefund(userAddress, tokenId);
        }
    }
    ...
    function refund(uint256[] calldata tokenIds) external {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            uint256 tokenId = tokenIds[i];
            _checkRefund(msg.sender, tokenId);
