...
contract IrBankMonitor  {
    ...
    function getIrBankUnderlyingPrice2(address  pToken) internal  view returns (uint256) {
        uint256 price;
        price = IrBankOracleInterface(irbankOracle).getUnderlyingPrice(pToken);
        return price;
    }
    ...
    function notSupportSymbol2(address [] memory pTokens) internal view returns (bool) {
    ...
    function isCompareOracleDeviate(address [] memory pTokens) internal  view returns (bool) {
        ...
        uint256 _irbankPrice;
        uint256 _bandPrice;
        uint256 _priceGap;
        uint256 _ratio;

        for(uint256 i=0; i<pTokens.length; i++) {
            _irbankPrice = getIrBankUnderlyingPrice(pTokens[i]);
            _bandPrice = getBandPrice(tokenConfigs[pTokens[i]].underlyingSymbol, 'USD', pTokens[i]);
            if ( !(_bandPrice > 0) ) {
                return  true;
            }
            _ratio = tokenConfigs[pTokens[i]].ratio;

            if (_irbankPrice -  _bandPrice > 0 ) {

                _priceGap = _irbankPrice - _bandPrice;
                if (_bandPrice.mul(_ratio).div(100) < _priceGap){
                    return true;
                } 

            } else {
                _priceGap =_bandPrice - _irbankPrice ;
                if (_bandPrice.mul(_ratio).div(100) < _priceGap){
                    return true;
                }  
            }
    ...
    function isCompareOracleDeviate2(address [] memory pTokens) internal  view returns (bool) {
    ...
    function isLiquidityInsufficient2(address [] memory pTokens) internal view returns (bool) {
    ...
    function isPoolPaused2(address [] memory stakerRewards) internal view returns (bool) {
        bool pause;
        for(uint256 i=0; i<stakerRewards.length; i++ ){      
            pause = StakingRewardsInterface(stakerRewards[i]).paused();
            if (pause == true) {
