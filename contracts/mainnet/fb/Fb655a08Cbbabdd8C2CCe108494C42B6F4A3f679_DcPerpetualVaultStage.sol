 ...
 */
/* solhint-disable */
contract DcPerpetualVaultStage is Guarded {
    ...
    event PositionAdjusted(bool operationType,bool positionType,uint256 amount);
  ...
  /**
    ...
    */
    ...
    function deposit(
        ...
        if (baseAsset == address(0x0)) {
            require(
                msg.value == amount,
                "PERP BB: message value mismatch amount"
            );
        } else {
            require(
                ...
                "PERP BB: Desposit transfer failed"
            );
        }
    ...
    function _depositToVault(uint256 _amount) private {
            require(_amount != 0,"PERP BB:: zero amount");
    ...
    */
    function adjustPosition(bytes memory _data) external auth  {
     ...
     */
    function emergencyClose() external returns(bool) {
        _closePosition(0);
        uint256 collateral = _perpVault.getFreeCollateral(address(this));
        _withdrawFromPerp(collateral);
        emit PositionAdjusted(false, false, collateral);
    }

    function withdraw(address asset, uint256 amount, address payable receiver) external auth returns(bool){
        if(asset == address(0x0)){
            require(address(this).balance >= amount, "GMX BB: Insufficient funds");
            (bool success,) = address(receiver).call{value:amount}("");
            require(success, "GMX BB: Withdraw transfer failled");
        }else{
            require(IERC20(asset).balanceOf(address(this)) >= amount, "GMX BB: Insufficient funds");
            require(IERC20(asset).transfer(receiver,amount),"GMX BB: Withdraw transfer failed");
        }
        return true;
    }
    ...
    function _withdrawFromPerp(uint256 amount) internal returns(uint256 _amount){
        uint256 freeCollateral = _perpVault.getFreeCollateral(address(this));
        if (amount > freeCollateral) {
            _perpVault.withdraw(_usdcToken, freeCollateral);
    ...
    */
    ...
    function setPoolLimit(uint256 _newLimit) public {
        require(_newLimit != 0, "PERP BB:: zero amount");
