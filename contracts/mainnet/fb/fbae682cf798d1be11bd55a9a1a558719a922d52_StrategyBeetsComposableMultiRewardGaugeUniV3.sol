...
contract StrategyBeetsComposableMultiRewardGaugeUniV3 is StratFeeManager {
    ...
    function swapRewardsToNative() internal {
        ...
        if (outputBal > 0) {
            ...
            balancerSwap(_swaps, outputToNativeAssets);
        }
        // extras
        for (uint i; i < rewardTokens.length; i++) {
            ...
            if (bal >= rewards[rewardTokens[i]].minAmount) {
                if (rewards[rewardTokens[i]].assets[0] != address(0)) {
                    IBalancerVault.BatchSwapStep[] memory _swaps = buildSwapStructArray(rewards[rewardTokens[i]].swapInfo, bal);
                    balancerSwap(_swaps, rewards[rewardTokens[i]].assets);
                } else {
    ...
    function addLiquidity() internal {
        ...
        balancerSwap(_swaps, nativeToWantAssets);
    }
    ...
    function balancerSwap(IBalancerVault.BatchSwapStep[] memory _swaps, address[] memory _route) internal returns (int256[] memory) {
    ...
    function callReward() public pure returns (uint256) {
     ...
     function addRewardToken(address _token, BeefyBalancerStructs.BatchSwapStruct[] memory _swapInfo, address[] memory _assets, bytes calldata _routeToNative, uint _minAmount) external onlyOwner {
        ...
        for (uint i; i < _swapInfo.length; ){
            rewards[_token].swapInfo[i] = _swapInfo[i];
            unchecked {
                ++i;
            }
        }
