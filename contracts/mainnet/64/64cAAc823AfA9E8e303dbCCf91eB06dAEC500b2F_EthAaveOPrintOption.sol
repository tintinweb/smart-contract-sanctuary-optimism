...
contract EthAaveOPrintOption is OPrintOption {
    using SafeERC20 for IERC20;

    IL2Pool public pool;
    IL2Encoder public encoder;
    ...
    function initialize(
        ...
        __OPrintOption_init(_reward, _stake, _buyWith, _treasury, _oracle, _chef, _pid);
        ...
        stake.approve(address(pool), type(uint).max);
    }

    receive() external payable {
        require(msg.sender == address(stake), 'Receive not allowed');
    }
    ...
    function depositEth() external whenNotPaused payable {
        IWETH(address(stake)).deposit{value : msg.value}();
        pool.supply(encoder.encodeSupplyParams(address(stake), msg.value, 0));
        pendingReward[msg.sender] += chef.deposit(msg.sender, pid, msg.value);
    ...
    function withdrawEth(uint amount) external whenNotPaused {
        pool.withdraw(encoder.encodeWithdrawParams(address(stake), amount));

        pendingReward[msg.sender] += chef.withdraw(msg.sender, pid, amount);

        uint fee = amount * withdrawnFee / withdrawnFeeBase;
        stake.safeTransfer(treasury, fee);
        uint remaining = amount - fee;
        IWETH(address(stake)).withdraw(remaining);
        safeTransferETH(msg.sender, remaining);

        userTotal -= amount;

        emit Withdraw(msg.sender, remaining, fee);
    }
    ...
    function deposit(uint) external pure override {
    ...
    function claimRewards() external {
        //in case claim before anyone stake
        if (userTotal > 0) {
        ...
        //claim interest
        uint claimable = aToken.balanceOf(address(this)) - userTotal;
        if (claimable > 0) {
            pool.withdraw(encoder.encodeWithdrawParams(address(stake), claimable));
            stake.safeTransfer(treasury, stake.balanceOf(address(this)));
        }
    }
}
...
import "../interfaces/IHolyChef.sol";
import "../interfaces/IOPrintOracle.sol";

abstract contract OPrintOption is Initializable, PausableUpgradeable, OwnableUpgradeable, UUPSUpgradeable {
    using SafeERC20 for IERC20;

    IERC20 public reward;
    IERC20 public stake;
    IERC20 public buyWith;
    address public treasury;

    IHolyChef public chef;
    uint public pid;

    IOPrintOracle public oracle;
    ...
    uint public expiry;
    uint public discountBase;

    uint public withdrawnFee;
    uint public withdrawnFeeBase;

    mapping(address => uint) public pendingReward;

    event Deposit(address indexed from, uint amount);
    event Withdraw(address indexed to, uint amount, uint fee);
    event Created(address indexed owner, uint amount, uint strike, uint expiry, uint id);
    event Redeem(address indexed owner, uint amount, uint strike, uint id);

    function __OPrintOption_init(
        IERC20 _reward,
        IERC20 _stake,
        IERC20 _buyWith,
        address _treasury,
    ...
    function updateWithdrawnFee(uint _fee, uint _base) external onlyOwner {
        withdrawnFee = _fee;
        withdrawnFeeBase = _base;
    }

    function updateDiscount(uint _discount, uint _base) external onlyOwner {
        discount = _discount;
        discountBase = _base;
    }

    function updateExpiry(uint _expiry) external onlyOwner {
        expiry = _expiry;
    }

    function updateOracle(IOPrintOracle _oracle) external onlyOwner {
        oracle = _oracle;
    }

    function updatePid(uint _pid) external onlyOwner {
        pid = _pid;
    }
    ...
    function _deposit(uint amount, address user) internal whenNotPaused {
        stake.safeTransferFrom(user, address(this), amount);
        pendingReward[user] += chef.deposit(user, pid, amount);
    ...
    function _withdraw(uint amount, address user) internal whenNotPaused {
        pendingReward[user] += chef.withdraw(user, pid, amount);
        uint fee = amount * withdrawnFee / withdrawnFeeBase;
        stake.safeTransfer(treasury, fee);
        uint remaining = amount - fee;
        stake.safeTransfer(user, remaining);

        emit Withdraw(user, remaining, fee);
    }

    function rewardForUser(address user) external view returns (uint) {
        return pendingReward[msg.sender] + chef.pendingOPrint(user, pid);
    }

    function balanceOf(address user) public view returns (uint amount) {
        (amount,) = chef.userInfo(pid, user);
    }

    function optionPrice(uint amount) public view returns (uint) {
        return oracle.consult(address(reward), amount) * discount / discountBase;
    }

    function claim(uint amount) external whenNotPaused {
        pendingReward[msg.sender] += chef.deposit(msg.sender, pid, 0);
        pendingReward[msg.sender] -= amount;

        uint _strike = optionPrice(amount);
        require(_strike > 0, "invalid oracle price");
        uint _expiry = block.timestamp + expiry;
        options.push(option(amount, _strike, _expiry, false, msg.sender));
        emit Created(msg.sender, amount, _strike, _expiry, nextIndex);
        nextIndex++;
    }

    function redeem(uint id) external whenNotPaused {
        option storage _opt = options[id];
        require(_opt.owner == msg.sender, "!owner");
        require(_opt.expiry >= block.timestamp && !_opt.exercised, "invalid option");
        _opt.exercised = true;
        buyWith.safeTransferFrom(msg.sender, treasury, _opt.strike);
        reward.safeTransfer(msg.sender, _opt.amount);
        emit Redeem(msg.sender, _opt.amount, _opt.strike, id);
    }
...
interface IHolyChef {
    function deposit(address _user, uint256 _pid, uint256 _amount) external returns (uint256);

    function withdraw(address _user, uint256 _pid, uint256 _amount) external returns (uint256);

    function pendingOPrint(address _user, uint256 _pid) external view returns (uint256);
