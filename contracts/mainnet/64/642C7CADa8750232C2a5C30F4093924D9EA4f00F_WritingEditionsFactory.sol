    ...
    IOwnableEvents
{
    ...
    bytes32 public constant override CREATE_TYPEHASH =
        keccak256(
            "Create(address owner,string name,string symbol,uint256 limit,uint256 price)"
        );
    ...
    constructor(address implementation_, address tributaryRegistry_) {
    ...
    function _isValid(
        ...
        // If the owner is a contract, we attempt to validate the signature
        // using EIP-1271
        if (owner_.code.length != 0) {
            ...
            return
                IERC1271(owner_).isValidSignature(salt, signature) ==
                MAGICVALUE;
        }
...
interface IWritingEditionEvents {
    ...
    event ApprovedMinter(address indexed account, bool approved);
}
