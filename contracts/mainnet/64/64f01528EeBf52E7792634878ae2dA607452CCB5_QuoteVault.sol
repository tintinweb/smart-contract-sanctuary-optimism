...
abstract contract CommonVault is ICommonVault, OwnerPausable, ReentrancyGuardUpgradeable, CommonVaultStorageV1 {
        ...
        returns (uint256 amountOut)
    {
        ...
        _requireMarginRatioBeforeOpenPosition(isBaseToQuote);
        ...
        // however, we still check margin ratio if decreasing position anyway
        _requireEnoughMarginRatioAfterSwap();
    ...
    function _deposit(uint256 assets) internal returns (uint256 shares) {
        ...
        // CV_ILP: invalid liquidity provider
        require(IVaultConfig(_vaultConfig).isWhitelistedLiquidityProvider(msgSender), "CV_ILP");
    ...
    function _requireMarginRatioBeforeOpenPosition(bool isBaseToQuote) internal view virtual;
    ...
    function _getSpotTwap() internal view returns (uint256) {
        return
            UniswapV3Facade.getTwap(
                _uniswapV3Factory,
                _baseToken,
                _quoteToken,
                _uniswapV3DefaultFeeTier,
                IPerpPositionManager(_perpPositionManager).getTwapInterval()
            );
    }
    ...
    function _requireEnoughMarginRatioAfterSwap() internal view {
...
contract QuoteVault is CommonVault {
    ...
    function deleverage(uint256 reducedPositionSize) external override whenNotPaused nonReentrant {
        ...
        require(reducedPositionSize <= maxDeleveragePositionSize, "QV_RPSTM");

        _deleverage(reducedPositionSize == 0 ? maxDeleveragePositionSize : reducedPositionSize);
    }
    ...
    function totalAssets() public view override returns (uint256) {
        ...
        uint256 spotTwap_qd = _getSpotTwap();
        // scale down baseToken's decimals _bd to quoteToken's decimals _qd
        uint256 baseNotional_qd = base_bd.mulDiv(spotTwap_qd, 10**baseToken.decimals());
        ...
        return quote_qd.add(baseNotional_qd).add(perpTotalAccountValue_qd);
    }
    ...
    function _requireMarginRatioBeforeOpenPosition(bool isBaseToQuote) internal view override {
        if (isBaseToQuote) {
            // short on perp => quoteVault increase position, check margin ratio
