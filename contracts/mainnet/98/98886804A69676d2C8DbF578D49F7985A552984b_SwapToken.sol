...
there was no way to native currency come in to contract except NativeTokenToMultiSwap function so address(this).balance was the fee collected.
...
@important - always try to use ERC20ToMultiSwapWithReceiver and NativeTokenToMultiSwapWithReceiver function because it is cheaper and more gas efficient.
*/
...
contract SwapToken is ReentrancyGuard {
    ...
    address public immutable _wNativeTokenAddress;
    address public immutable walletImplementation;

    ISwapRouter public constant swapRouter = ISwapRouter(routerAddress);
    IWNATIVETOKEN public immutable _wNativeToken;
    ...
    address[] public definedTokensKeys;
    address[] public walletsKeys;
    address[] public tokenToAggregatorKeys;
    ...
    modifier onlyOwner() {
        require(msg.sender == owner, "CLAMP-V2: NOT OWNER");
        _;
    }

    constructor(address _owner, uint8 _fee, address payable _WNativeTokenAddress) {
        owner = _owner;
        fee = _fee;
        _wNativeTokenAddress = _WNativeTokenAddress;
        _wNativeToken = IWNATIVETOKEN(payable(_WNativeTokenAddress));
    ...
    function getDefinedToken( address _tokenAddress ) public view returns (bool) {
        return definedTokens[_tokenAddress];
    }
    function getTokenToAggregator( address _tokenAddress ) public view returns (address) {
        return tokenToAggregator[_tokenAddress];
    }
    function getWallet(address _userAddress) external view returns (address) {
        return wallets[_userAddress];
    }
    ...
    function addInDefinedTokenWithAggregator(
        ...
        require(!definedTokens[_tokenAddress], "CLAMP-V2: TOKEN ADDRESS EXISTS"); // Check that the address is not already in the mapping
        definedTokensKeys.push(_tokenAddress);
        definedTokens[_tokenAddress] = true; // Add the address to the mapping
        tokenToAggregatorKeys.push(_tokenAggregatorAddress);
    ...
    function removeFromDefinedTokenWithAggregator(
        ...
        require(definedTokens[_tokenAddress],"CLAMP-V2: TOKEN ADDRESS DOES NOT EXISTS"); // Check that the address is in the mapping
        ...
        // Remove the address from definedTokensKeys array
        for (uint256 i = 0; i < definedTokensKeys.length; i++) {
            if (definedTokensKeys[i] == _tokenAddress) {
                definedTokensKeys[i] = definedTokensKeys[definedTokensKeys.length - 1];
                definedTokensKeys.pop();
                break;
            }
        }
        // Remove the tokenAggregator address from tokenToAggregatorKeys array
        address _tokenAggregatorAddress = getTokenToAggregator(_tokenAddress);
        for (uint256 i = 0; i < tokenToAggregatorKeys.length; i++) {
            if (tokenToAggregatorKeys[i] == _tokenAggregatorAddress) {
                tokenToAggregatorKeys[i] = tokenToAggregatorKeys[tokenToAggregatorKeys.length - 1];
                tokenToAggregatorKeys.pop();
                break;
            }
        }
    }
    ...
    function ownerWithdraw() external onlyOwner {
        ...
        require(sent, "CLAMP-V2: WITHDRAWAL FAILED!");
    }
    ...
    function NativeTokenToMultiSwap(
        address[] memory _tokenAddresses,
        uint256[] memory _tokenAmounts,
        uint256 _amountTotal
    ) external payable {
        NativeTokenToMultiSwapWithReceiver(
            _tokenAddresses,
            _tokenAmounts,
            _amountTotal,
            msg.sender
    ...
    function NativeTokenToMultiSwapWithReceiver( address[] memory _tokenAddresses, uint256[] memory _tokenAmounts, uint256 _amountTotal, address _receiver ) public payable nonReentrant {
        require(
            _tokenAddresses.length > 1 && _tokenAddresses.length < 16,
            "CLAMP-V2: INDEX-LENGTH-INVALID"
        );
        require(
            _tokenAddresses.length == _tokenAmounts.length,
            "CLAMP-V2: Mismatched token addresses and amounts"
        );
        require(msg.value > 0, "CLAMP-V2: MSG.VALUE CONDITION FAILED - 1"); //will change msg.value in future
        require(
            msg.value == amountWithFee(_amountTotal),
            "CLAMP-V2: MSG.VALUE CONDITION FAILED - 2"
        );
        ...
        require(
            ...
            "CLAMP-V2: NOT DEFINED AND UNIQUE"
        );

        // Convert received nativeToken to WNativeToken
        _wNativeToken.deposit{value: _amountTotal}();
        _wNativeToken.approve(address(routerAddress), _amountTotal);
    ...
    // internal functions
    function createWallet(address _user) private {
    ...
    receive() external payable {
        ...
        require(sent, "CLAMP-V2: TRANSACTION FAILED");
    }

    function getDefinedTokensKeys() external view returns (address[] memory) {
        return definedTokensKeys;
    }

    function getWalletsKeys() external view returns (address[] memory) {
        return walletsKeys;
    }

    function getTokenToAggregatorKeys() external view returns (address[] memory) {
        return tokenToAggregatorKeys;
    }
}
...
contract Wallet is Initializable {
    ...
    */
    ...
    address public CLAMP_V2_SWAPTOKEN;
    address constant wNativeToken = 0x4200000000000000000000000000000000000006;
    ...
    */
    ...
    /**************************************************************************************** MODIFIERS *********************************************************************************************/
    modifier onlyOwner() {
        require(msg.sender == Owner, "CLAMP-V2: NOT-WALLET-OWNER");
        _;
    }
    modifier onlyClampContract() {
        require(msg.sender == CLAMP_V2_SWAPTOKEN, "CLAMP-V2: ERROR-UPDATE");
        _;
    }
   ...
   */
    function update(
     ...
     function updateTransfer(
        ...
        uint n = _tokenAddresses.length;
        if (userIndexes[_indexIdentifier].Exists == true) {
            for (uint i = 0; i < _tokenAddresses.length; i++) {
                userIndexes[_indexIdentifier].TokenHoldings[i] += _tokenAmounts[i];
            }
        } else {
            userIndexes[_indexIdentifier] = Index(
                true,
        ...
        for (uint i = 0; i < n; ) {
            _priceOfWithdrawTokens[i] = getLatestPrice(_tokenAddresses[i]);
            unchecked {
                ++i;
            }
        }
        ...
        emit ReceivedIndex(
            Transaction.TransactionStatus.ReceivedIndex,
            _tokenAddresses,
            _tokenAmounts,
            sender,
    ...
    */
    ...
    function internalSwapToNativeToken(
        ...
        IWNATIVETOKEN(payable(wNativeToken)).withdraw(sum(amountsOut));
    }
    ...
    function internalSwap(
        ...
        require(
            ...
            "CLAMP-V2: INDEX-NOT-FOUND"
        );
        require(
            _percentage > 0 && _percentage <= 1000,
            "CLAMP-V2: INVALID-PERCENTAGE"
        );
        require(
            SWAPTOKEN.isDefinedToken(_returnBaseToken),
            "CLAMP-V2: INVALID-BASE-TOKEN"
        );
    ...
    */
    // note : pass the owner address in _address in frontend to withdraw native token to owner address
    ...
    // note : pass the owner address in _address in frontend to withdraw native token to owner address
    function withdrawNativeTokenToAddress(
        uint256 _amount,
        address _address
    ) external onlyOwner {
        require(
    ...
    function swapNativeTokenToIndex(
        address[] memory _tokenAddresses,
        uint256[] memory _tokenAmounts,
        uint256 _amountTotal,
        uint256 _amountWithFee
    ) external payable onlyOwner {
        SWAPTOKEN.NativeTokenToMultiSwapWithReceiver{value: _amountWithFee}(
            _tokenAddresses,
            _tokenAmounts,
            _amountTotal,
            msg.sender
    ...
    function tranferIndex(
        uint _indexIdentifier,
        address _recipient
    ) external onlyOwner{
        Index storage indexToUpdate = userIndexes[_indexIdentifier];
        address[] memory _tokenAddresses = indexToUpdate.TokenAddresses;
        uint256[] memory _tokenAmounts = indexToUpdate.TokenHoldings;
        uint n = indexToUpdate.TokenHoldings.length;
        int[] memory _priceOfWithdrawTokens = new int[](n);

        address _recipientWallet = SWAPTOKEN.getWallet(_recipient);

        for (uint i = 0; i < n; ) {
            IERC20 token = IERC20(_tokenAddresses[i]);
            token.safeTransfer(_recipientWallet, _tokenAmounts[i]);
            _priceOfWithdrawTokens[i] = getLatestPrice(_tokenAddresses[i]);
            unchecked {
                ++i;
            }
        ...
        IWallet walletInstance = IWallet(_recipientWallet);
        walletInstance.updateTransfer(_tokenAddresses, _tokenAmounts);

        emit SentIndex(
            Transaction.TransactionStatus.SentIndex,
            _tokenAddresses,
            _tokenAmounts,
            _recipient,
            _recipientWallet,
    ...
    function getAmountOutMin(
        ...
        int256 price = getDerivedPrice(_tokenIn, _tokenOut, 8);
        if (price <= 0) {
            revert("CLAMP-V2: INVALID-ORACLE-PRICE");
        }
        uint256 tokenIndecimal = IERC20(_tokenIn).decimals();
        uint256 tokenOutdecimal = IERC20(_tokenOut).decimals();
        uint256 amountOut;
...
interface ISwapToken {
    ...
    function NativeTokenToMultiSwapWithReceiver(
        address[] memory _tokenAddresses,
        uint256[] memory _tokenAmounts,
        uint256 _amountTotal,
        address _receiver
