...
contract VelodromeOptimismUsdtMai is DefiiWithCustomEnter, DefiiWithCustomExit {
    ...
    IERC20 constant lpToken =
        IERC20(0x4AB66eED0b39F9d229A1281468a4482515dB0709);
    ...
    IGauge constant gauge = IGauge(0x221aB70Fa01f635ef24B39795228fccC919fAFF3);
        ...
        returns (bytes memory)
    {
        ...
        (uint256 maiMinAmount, ) = router.getAmountOut(usdtToSwap, USDT, MAI);
        return abi.encode(usdtToSwap, (maiMinAmount * slippage) / 1000);
    }
    ...
    function exitParams(uint256 slippage) external view returns (bytes memory) {
        ...
        (uint256 maiPrice, ) = router.getAmountOut(1000e18, MAI, USDT);
    ...
    function _enterWithParams(bytes memory params) internal override {
        (uint256 usdtToSwap, uint256 minMaiFromSwap) = abi.decode(
            params,
            (uint256, uint256)
        );
        USDT.approve(address(router), usdtToSwap);
        ...
        _enter(USDT.balanceOf(address(this)), amounts[1]);
    }
    ...
    function _enter() internal override(Defii, DefiiWithCustomEnter) {
        ...
        _enter(usdtBalance, maiBalance);
    }

    function _enter(uint256 usdtBalance, uint256 maiBalance) internal {
