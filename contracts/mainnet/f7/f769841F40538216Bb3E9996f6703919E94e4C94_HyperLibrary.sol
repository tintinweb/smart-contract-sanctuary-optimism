...
contract HyperLibrary {
    ...
    {
        IHyperLPoolStorage hyperPool = IHyperLPoolStorage(_hyperPool);

        preBalance0 = hyperPool.token0().safeBalanceOf(_hyperPool);
        preBalance1 = hyperPool.token1().safeBalanceOf(_hyperPool);

        leftover0 = preBalance0 - managerBalance0 - hyperpoolsBalance0;
        leftover1 = preBalance1 - managerBalance1 - hyperpoolsBalance1;
    }

    function getBurnAmountsForRebalance(address _hyperPool)
        public
        view
        returns (uint256 burn0, uint256 burn1)
    {
        IHyperLPoolStorage hyperPool = IHyperLPoolStorage(_hyperPool);
        IUniswapV3Pool uniPool = hyperPool.pool();
        ...
        (uint128 liquidity, , , , ) = uniPool.safePositions(positionID);

        (uint160 sqrtRatioX96, , , , , , ) = uniPool.safeSlot0();
        (burn0, burn1) = LiquidityAmounts.getAmountsForLiquidity(
            sqrtRatioX96,
            hyperPool.lowerTick().getSqrtRatioAtTick(),
            hyperPool.upperTick().getSqrtRatioAtTick(),
            liquidity
        );
    ...
    function getFeeAmountsForRebalance(
        address _hyperPool,
        uint256 preBalance0,
        uint256 preBalance1,
        uint256 burn0,
        ...
        fee0 =
            hyperPool.token0().safeBalanceOf(_hyperPool) -
            preBalance0 -
            burn0;
        fee1 =
            hyperPool.token1().safeBalanceOf(_hyperPool) -
            preBalance1 -
            burn1;
    }

    function getNewFeeAmountsForRebalance(
        uint256 fee0,
        uint256 fee1,
        uint256 managerBalance0,
        uint256 managerBalance1,
        ...
        uint16 hyperpoolsFeeBPS,
        uint256 leftover0,
        uint256 leftover1
    ) public view returns (uint256 newFee0, uint256 newFee1) {
        hyperpoolsBalance0 += FullMath.mulDiv(fee0, hyperpoolsFeeBPS, 10000);
        hyperpoolsBalance1 += FullMath.mulDiv(fee1, hyperpoolsFeeBPS, 10000);

        managerBalance0 += FullMath.mulDiv(fee0, managerFeeBPS, 10000);
        managerBalance1 += FullMath.mulDiv(fee1, managerFeeBPS, 10000);

        newFee0 =
            fee0 -
            FullMath.mulDiv(fee0, hyperpoolsFeeBPS + managerFeeBPS, 10000);
        newFee1 =
            fee1 -
            FullMath.mulDiv(fee1, hyperpoolsFeeBPS + managerFeeBPS, 10000);

        newFee0 += leftover0;
        newFee1 += leftover1;
    }

    function getLeftOversForRebalance(
        address _hyperPool,
        ...
        uint256 managerBalance1,
        uint256 feeAmount
    ) public view returns (uint256 leftover0, uint256 leftover1) {
        IHyperLPoolStorage hyperPool = IHyperLPoolStorage(_hyperPool);

        if (_paymentToken == address(hyperPool.token0())) {
            // require((fee0 * hyperpoolsRebalanceBPS) / 10000 >= feeAmount, "high fee");
            leftover0 =
                hyperPool.token0().safeBalanceOf(_hyperPool) -
                managerBalance0 -
                hyperpoolsBalance0 -
            ...
            leftover1 =
                ...
                managerBalance1 -
                hyperpoolsBalance1;
        } else if (_paymentToken == address(hyperPool.token1())) {
            // require((fee1 * hyperpoolsRebalanceBPS) / 10000 >= feeAmount, "high fee");
            leftover0 =
                hyperPool.token0().safeBalanceOf(_hyperPool) -
                managerBalance0 -
                hyperpoolsBalance0;
    ...
    function getSwapAmountForRebalance(
        address _hyperPool,
        uint256 leftover0,
        uint256 leftover1,
        uint256 swapAmountBPS,
        bool zeroForOne
    ) public view returns (uint256 swapAmount) {
        IHyperLPoolStorage hyperPool = IHyperLPoolStorage(_hyperPool);
        IUniswapV3Pool uniPool = hyperPool.pool();

        (uint160 sqrtRatioX96New, , , , , , ) = uniPool.safeSlot0();
        ...
        if (baseLiquidity > 0) {
            ...
            leftover0 -= amountDeposited0;
            leftover1 -= amountDeposited1;
        }

        swapAmount = FullMath.mulDiv(
            (zeroForOne ? leftover0 : leftover1),
            swapAmountBPS,
            10000
        );
    }
