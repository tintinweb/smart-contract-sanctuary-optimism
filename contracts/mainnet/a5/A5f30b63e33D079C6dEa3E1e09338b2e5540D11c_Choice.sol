...
contract Choice {
    ...
    function choose(bytes32 seed, uint32 amount) external view returns (bytes32[N] memory result) {
        ...
        while (successes < N) {
            ...
            entropy >>= 8; // an overestimate on the _log_ (!) of the blackheight of the _firn. blackheight <= 256.
            uint32 cursor = _firn.root();
            ...
            (,uint32 candidate,) = _firn.info(account); // what is the total amount this person has deposited?
