...
import "./libs/OptimismZipSwapLibrary.sol";
...
contract O3OptimismZipSwapAggregator is Ownable {
    ...
    address public factory = 0x8BCeDD62DD46F1A76F8A1633d4f5B76e0CDa521E;
    ...
    function _swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        if (pull) {
            IERC20(path[0]).safeTransferFrom(
                msg.sender, OptimismZipSwapLibrary.pairFor(factory, path[0], path[1]), amountIn
            );
        } else {
            IERC20(path[0]).safeTransfer(OptimismZipSwapLibrary.pairFor(factory, path[0], path[1]), amountIn);
        }
    ...
    function _swapExactETHForTokensSupportingFeeOnTransferTokens(
        ...
        assert(IWETH(WETH).transfer(OptimismZipSwapLibrary.pairFor(factory, path[0], path[1]), amountIn));
        uint balanceBefore = IERC20(path[path.length - 1]).balanceOf(address(this));
        _swapSupportingFeeOnTransferTokens(path, address(this));
        uint amountOut = IERC20(path[path.length - 1]).balanceOf(address(this)).sub(balanceBefore);
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            (address token0,) = OptimismZipSwapLibrary.sortTokens(input, output);
            require(IUniswapV2Factory(factory).getPair(input, output) != address(0), "O3Aggregator: PAIR_NOT_EXIST");
            IUniswapV2Pair pair = IUniswapV2Pair(OptimismZipSwapLibrary.pairFor(factory, input, output));
            uint amountInput;
            uint amountOutput;
            { // scope to avoid stack too deep errors
                ...
                amountOutput = OptimismZipSwapLibrary.getAmountOut(amountInput, reserveInput, reserveOutput);
            }
            (uint amount0Out, uint amount1Out) = input == token0 ? (uint(0), amountOutput) : (amountOutput, uint(0));
            address to = i < path.length - 2 ? OptimismZipSwapLibrary.pairFor(factory, output, path[i + 2]) : _to;
...
library OptimismZipSwapLibrary {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint160(uint(keccak256(abi.encodePacked(
                ...
                hex'1a76b7e7272f6187014e23f04d1b2e543eed2fd1f76481149008cc6eacb05c22' // init code hash
            )))));
    }
    ...
    function quote(uint amountA, uint reserveA, uint reserveB) internal pure returns (uint amountB) {
        require(amountA > 0, 'Zipswap: INSUFFICIENT_AMOUNT');
        require(reserveA > 0 && reserveB > 0, 'Zipswap: INSUFFICIENT_LIQUIDITY');
        amountB = amountA.mul(reserveB) / reserveA;
    }

    // given an input amount of an asset and pair reserves, returns the maximum output amount of the other asset
    function getAmountOut(uint amountIn, uint reserveIn, uint reserveOut) internal pure returns (uint amountOut) {
        require(amountIn > 0, 'Zipswap: INSUFFICIENT_INPUT_AMOUNT');
        require(reserveIn > 0 && reserveOut > 0, 'Zipswap: INSUFFICIENT_LIQUIDITY');
    ...
    function getAmountIn(uint amountOut, uint reserveIn, uint reserveOut) internal pure returns (uint amountIn) {
        require(amountOut > 0, 'Zipswap: INSUFFICIENT_OUTPUT_AMOUNT');
        require(reserveIn > 0 && reserveOut > 0, 'Zipswap: INSUFFICIENT_LIQUIDITY');
    ...
    function getAmountsOut(address factory, uint amountIn, address[] memory path) internal view returns (uint[] memory amounts) {
        require(path.length >= 2, 'Zipswap: INVALID_PATH');
