...
contract BeefyUniV2ZapSolidly {
    ...
    function _getSwapAmount(uint256 investmentA, uint256 reserveA, uint256 reserveB, address tokenA, address tokenB) private view returns (uint256 swapAmount) {
        ...
        (uint256 nominator,) = router.getAmountOut(halfInvestment, tokenA, tokenB);
        uint256 denominator = router.quoteLiquidity(halfInvestment, reserveA.add(halfInvestment), reserveB.sub(nominator));
    ...
    function estimateSwap(address beefyVault, address tokenIn, uint256 fullInvestmentIn) public view returns(uint256 swapAmountIn, uint256 swapAmountOut, address swapTokenOut) {
        ...
        (swapAmountOut, ) = router.getAmountOut(swapAmountIn, tokenIn, swapTokenOut);
