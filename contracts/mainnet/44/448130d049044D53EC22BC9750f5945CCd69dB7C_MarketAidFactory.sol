...
contract MarketAid is Multicall {
    ...
    function strategistRebalanceFunds(
    ...
    ) public onlyApprovedStrategist returns (uint256 fill_amt) {
        ...
        try swapRouter.exactInputSingle(params) returns (uint256 amountOut) {
            ...
            // If the external call fails with a known error, log the error message and return 0
            // emit LogError(error);
            return 0;
        } catch Panic(
            uint256 /*panicCode*/ // Catch panic
        ) {
            // If the external call fails with a panic, revert with the panic code
            // revert(panicCode);
            return 0;
        } catch (bytes memory /* data */) {
            // If the external call fails with an unknown error, revert with the data
            // revert(string(data));
            return 0;
        }
    }
