...
contract VeloVoteHelper {
    ...
    constructor(address _manager) {
        manager = IVeloVoteManager(_manager);
    }
    ...
    function getAPRForLog(uint256 log) external view returns (uint256 apr) {
        ...
        for(uint256 i; i < strategies.length; i++) {
            if (IVeloVoteFarmer(strategies[i]).harvestLogLength() >= log) {
                apr += IVeloVoteFarmer(strategies[i]).calculateAPRForLog(log);
                strategiesProcessed++;
            }
        }
    ...
    function getMaxLogLength() public view returns (uint256 maxLength) {
        address[] memory strategies = _getStrategies();
        for(uint256 i; i < strategies.length; i++) {
            if(IVeloVoteFarmer(strategies[i]).harvestLogLength() > maxLength) {
                maxLength = IVeloVoteFarmer(strategies[i]).harvestLogLength();
            }
        }
    }

    function getTotalVeloLocked() external view returns (uint256 total) {
        address[] memory strategies = _getStrategies();
        for (uint256 i; i < strategies.length; i++) {
            total += _getTotalVeloLockedOfStrat(strategies[i]);
        }
    }
    ...
    function getTotalVeloLockedForUser(address user) external view returns (uint256 total) {
        uint256[] memory userTokens = getManagedTokensOfUser(user);
        for (uint256 i; i < userTokens.length; i++) {
            total += uint128(VEVELO.locked(userTokens[i]).amount);
        }
    }
    ...
    function getTotalVotingPowerForUser(address user) external view returns (uint256 total) {
        uint256[] memory userTokens = getManagedTokensOfUser(user);
        for (uint256 i; i < userTokens.length; i++) {
            total += VEVELO.balanceOfNFT(userTokens[i]);
        }
    }

    function getManagedTokensOfUser(address user) public view returns (uint256[] memory) {
        uint256 max = VEVELO.balanceOf(user);
        uint256[] memory tokens = new uint256[](max);

        for(uint256 i; i < max; i++) {
            uint256 tokenId = VEVELO.tokenOfOwnerByIndex(user, i);
    ...
    function _getTotalVeloLockedOfStrat(address strategy) internal view returns (uint256 total) {
        IVeloVoteFarmer strat = IVeloVoteFarmer(strategy);
        uint256 tokenIdsLen = strat.tokenIdsLength();
        for(uint256 i; i < tokenIdsLen; i++) {
            uint256 tokenId = strat.tokenIds(i);
            total += uint128(VEVELO.locked(tokenId).amount);
        }
    }
    ...
    function _getTotalVotingPowerOfStrat(address strategy) internal view returns (uint256 total) {
        ...
        for(uint256 i; i < tokenIdsLen; i++) {
            uint256 tokenId = strat.tokenIds(i);
            total += VEVELO.balanceOfNFT(tokenId);
        }
    }
    ...
    function _isManaged(uint256 tokenId) internal view returns (bool) {
        address[] memory strategies = _getStrategies();
        for (uint256 i; i < strategies.length; i++) {
            uint256 tokenIdsLen = IVeloVoteFarmer(strategies[i]).tokenIdsLength();
            for (uint256 j; j < tokenIdsLen; j++) {
                if (IVeloVoteFarmer(strategies[i]).tokenIds(j) == tokenId) {
                    return true;
                }
            }
        }
