...
contract Origins is Ownable {
    ...
    function setMerkleDropMinter(
        MerkleDropMinter _merkleDropMinter
    ) public onlyOwner {
        merkleDropMinter = _merkleDropMinter;
    }

    function setPricePerSong(uint256 _pricePerSong) public onlyOwner {
        pricePerSong = _pricePerSong;
    }

    // Update the contract at index
    ...
    // Function to call mint on all contracts and send to caller
    function mintAll(uint32 quantity) public payable {
        uint totalSongPrice = quantity * pricePerSong; // As there are 7 contracts
        uint txCost = totalSongPrice * 7;
        ...
        for (uint8 i = 0; i < 7; i++) {
            ...
            merkleDropMinter.mintTo{value: totalSongPrice}(
                ...
                proofs[i],
                address(0),
                new bytes32[](0), // empty bytes32 array
                0
            );
        }
    }
