...
contract SwapPlusv1 is Ownable {
  ...
  constructor(
    ...
    routers["VelodromeV2"] = 0xa062aE8A9c5e11aaA026fc2670B0D65cCc8B2858;
    routers["VelodromeV1"] = 0xa062aE8A9c5e11aaA026fc2670B0D65cCc8B2858;
  ...
  function _swap(swapLine memory line, uint256 amount) internal returns(uint256) {
    ...
    for (uint256 x=0; x<swLen; x++) {
      ...
      if (_compareStrings(line.swaps[x].platform, "UniswapV3")) {
      ...
      else if (_compareStrings(line.swaps[x].platform, "VelodromeV2")) {
        IVelodromeRouter.Route[] memory pRoute = new IVelodromeRouter.Route[](1);
        pRoute[0] = IVelodromeRouter.Route({
          from: line.swaps[x].tokenIn,
          to: line.swaps[x].tokenOut,
          stable: (line.swaps[x].meta == 1 ? true : false),
          factory: 0xF1046053aa5682b4F9a81b5481394DA16BE5FF5a
        });
        uint256[] memory amounts = IVelodromeRouter(routers["VelodromeV2"]).swapExactTokensForTokens(
      ...
      else if (_compareStrings(line.swaps[x].platform, "VelodromeV1")) {
        IVelodromeRouter.Route[] memory pRoute = new IVelodromeRouter.Route[](1);
        pRoute[0] = IVelodromeRouter.Route({
          from: line.swaps[x].tokenIn,
          to: line.swaps[x].tokenOut,
        ...
        uint256[] memory amounts = IVelodromeRouter(routers["VelodromeV1"]).swapExactTokensForTokens(
          inAmount * line.swaps[x].percent / managerDecimal,
          line.swaps[x].amountOutMin,
          pRoute,
          address(this),
