...
contract DappadToken is IERC20, Context {
    ...
    event SetLotteryWallet(address LotteryWallet, address NFTHouse);
    ...
    modifier onlyModerator() {
        require(isModerator[msg.sender], "Ownable: caller is not the moderator");
        _;
    }
    ...
    unchecked {
        ...
        if (isExcludedFromFee[sender] || isExcludedFromFee[recipient] || feePercentage == 0) {
            ...
            uint256 secondBeneficiaryAmount = feeAmount - firstBeneficiaryAmount;
            _balances[LotteryWallet] += firstBeneficiaryAmount;
            _balances[NFTHouse] += secondBeneficiaryAmount;
            receiveAmount = amount - feeAmount;
            _balances[recipient] += receiveAmount;
            emit TransferFee(sender, LotteryWallet, firstBeneficiaryAmount);
            emit TransferFee(sender, NFTHouse, secondBeneficiaryAmount);
        }
    ...
    function setFeePercentage(uint8 feePercentage_) external onlyModerator {
        require(feePercentage_ <= 3, "transaction fee percentage exceeds 3%");
        feePercentage = feePercentage_;
        emit SetFeePercentage(feePercentage);
    }

    function setLotteryWallet(address LotteryWallet_, address NFTHouse_) external onlyModerator {
        LotteryWallet = LotteryWallet_;
        NFTHouse = NFTHouse_;
        emit SetLotteryWallet(LotteryWallet, NFTHouse);
    }

    function excludeFromFee(address address_, bool isExcluded) external onlyModerator {
