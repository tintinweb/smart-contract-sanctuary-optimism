...
contract DcSwapperV3 is BaseBuildingBlock, IDcSwapperV3 {
    ...
    function initialize(bytes memory _data) public initializer {
        ...
        IERC20(weth).approve(address(routerAddress), type(uint256).max);
        fees = feeLevels;
    }

    function setDex(bytes memory _data) public override {
        (address _factory, address ammAddress) = abi.decode(
            _data,
            (address, address)
        );
        amm = ISwapRouter(ammAddress);
        factory = IUniswapV3Factory(_factory);
        IERC20(weth).approve(ammAddress, type(uint256).max);
    }
    ...
    function swap(
        ...
        return amm.exactInputSingle{value: msg.value}(params);
    }
        ...
        )
    {
        ...
        for (uint256 i = 0; i < fees.length; i++) {
            ...
            //
            if (pool != address(0x0)) {
                return (pool, 300, swapTokenA, swapTokenB);
            }
        } */
        // return (address(0x0), 0, swapTokenA, swapTokenB);
        return (address(0x0), 100, swapTokenA, swapTokenB);
    }
