...
contract DefySwapERC20 is IDefySwapERC20 {
    using SafeMath for uint;

    string public constant name = 'DefySwap LPs';
    string public constant symbol = 'DFY-LP';
    ...
    function permit(address owner, address spender, uint value, uint deadline, uint8 v, bytes32 r, bytes32 s) external {
        require(deadline >= block.timestamp, 'DefySwap: EXPIRED');
        ...
        require(recoveredAddress != address(0) && recoveredAddress == owner, 'DefySwap: INVALID_SIGNATURE');
...
interface IDefySwapCallee {
    function defyswapCall(address sender, uint amount0, uint amount1, bytes calldata data) external;
}

contract DefySwapPair is IDefySwapPair, DefySwapERC20 {
    using SafeMath  for uint;
    ...
    uint public lpFee = 0 ; //trading fee ratio for lp providers 
    uint public sFee = 25 ; //secondary trading fee
    ...
    modifier lock() {
        require(unlocked == 1, 'DefySwap: LOCKED');
        unlocked = 0;
        _;
        unlocked = 1;
    }
    ...
    function _safeTransfer(address token, address to, uint value) private {
        ...
        require(success && (data.length == 0 || abi.decode(data, (bool))), 'DefySwap: TRANSFER_FAILED');
    }
    ...
    function _update(uint balance0, uint balance1, uint112 _reserve0, uint112 _reserve1) private {
        require(balance0 <= uint112(-1) && balance1 <= uint112(-1), 'DefySwap: OVERFLOW');
    ...
    function mint(address to) external lock returns (uint liquidity) {
        ...
        require(liquidity > 0, 'DefySwap: INSUFFICIENT_LIQUIDITY_MINTED');
        _mint(to, liquidity);
    ...
    function burn(address to) external lock returns (uint amount0, uint amount1) {
        ...
        require(amount0 > 0 && amount1 > 0, 'DefySwap: INSUFFICIENT_LIQUIDITY_BURNED');
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external lock {
        require(amount0Out > 0 || amount1Out > 0, 'DefySwap: INSUFFICIENT_OUTPUT_AMOUNT');
        (uint112 _reserve0, uint112 _reserve1,) = getReserves(); // gas savings
        require(amount0Out < _reserve0 && amount1Out < _reserve1, 'DefySwap: INSUFFICIENT_LIQUIDITY');
        ...
        require(to != _token0 && to != _token1, 'DefySwap: INVALID_TO');
        if (amount0Out > 0) _safeTransfer(_token0, to, amount0Out); // optimistically transfer tokens
        if (amount1Out > 0) _safeTransfer(_token1, to, amount1Out); // optimistically transfer tokens
        if (data.length > 0) IDefySwapCallee(to).defyswapCall(msg.sender, amount0Out, amount1Out, data);
        balance0 = IERC20(_token0).balanceOf(address(this));
        balance1 = IERC20(_token1).balanceOf(address(this));
        }
...
contract DefySwapFactory is IDefySwapFactory {
    bytes32 public constant INIT_CODE_PAIR_HASH = keccak256(abi.encodePacked(type(DefySwapPair).creationCode));
    ...
    function createPair(address tokenA, address tokenB) external returns (address pair) {
        require(tokenA != tokenB, 'DefySwap: IDENTICAL_ADDRESSES');
        (address token0, address token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'DefySwap: ZERO_ADDRESS');
        require(getPair[token0][token1] == address(0), 'DefySwap: PAIR_EXISTS'); // single check is sufficient
        bytes memory bytecode = type(DefySwapPair).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(token0, token1));
        assembly {
            pair := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        IDefySwapPair(pair).initialize(token0, token1);
