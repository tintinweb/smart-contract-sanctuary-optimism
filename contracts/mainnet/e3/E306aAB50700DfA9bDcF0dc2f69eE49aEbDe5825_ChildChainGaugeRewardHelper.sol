...
contract ChildChainGaugeRewardHelper {
        ...
        returns (uint256)
    {
        ...
        if (lastUpdateTime + CLAIM_FREQUENCY < block.timestamp) {
            ...
            uint256 pendingOnStreamer = lastUpdateTime * rewardToken.rate;

            uint256 rewardPerShare = (1e18 * pendingOnStreamer) /
                gauge.totalSupply();

            return (gauge.balanceOf(msg.sender) * rewardPerShare) / 1e18;
        }
    }
}
...
interface IChildChainStreamer {
    ...
    function reward_data(address _addr)
        external
        view
        returns (RewardToken memory);

    function last_update_time() external returns (uint256);
