...
contract PerpsV2DynamicFeesModule is Ownable {
  ...
  function getMinKeeperFee() public view returns (uint256) {
    ...
    uint256 costOfExecutionGrossEth = ((((_gasUnitsL1 + overhead) * l1BaseFee * scalar) /
      10 ** decimals) + (_gasUnitsL2 * gasPriceL2));
    uint256 costOfExecutionGross = costOfExecutionGrossEth.mulDiv(price, UNIT);
