...
contract StrategyCommonSolidlyGaugeLP is StratManager, FeeManager, GasThrottler {
    ...
    function addLiquidity() internal {
        ...
        if (stable) {
            uint256 outputBal = IERC20(output).balanceOf(address(this));
            lp0Amt = outputBal.mul(getRatio()).div(1**18);
    ...
    function getRatio() public view returns (uint256) {
        ...
        uint256 lp0Amt = opLp0.div(10**IERC20Extended(lpToken0).decimals()).mul(10**18);
        uint256 lp1Amt = opLp1.div(10**IERC20Extended(lpToken1).decimals()).mul(10**18);         
        return lp0Amt.div(lp1Amt);
    }
