 ...
 * @notice SeaportWrapper is a contract that fulfills orders on behalf of users and rewards eligible
 *         orders with OP tokens. This contract cannot fulfill any order that is not a sell order
 *         paid in ETH.
 */
contract SeaportWrapper is
    PausableUpgradeable,
    ...
    IERC1155Receiver
{
    ...
    /**
     * @notice Emitted when a buyer receives OP tokens for a fulfilled order.
     *
     * @param rewardReceiver Buyer who is receiving the OP tokens.
     * @param reward Amount of OP token rewarded.
     */
    event RewardSent(address indexed rewardReceiver, uint256 reward);
    ...
    function fulfillOrder(Order calldata order) external payable whenNotPaused nonReentrant {
        require(
            ...
            "SeaportWrapper: order must have one offer item"
        );

        uint256 reward = rewardDeriver.getRewardInOP(order, msg.sender);
        ...
        require(fulfilled, "SeaportWrapper: order cannot be fulfilled");
        ...
        if (offerItem.itemType == ItemType.ERC721) {
            ...
            require(
                ...
                "SeaportWrapper: ERC1155 start amount must equal end amount"
            );
    ...
    function fulfillAvailableOrders(
        ...
        require(
            ...
            "SeaportWrapper: order length must equal maximum fulfilled"
        );
        ...
        for (uint256 i = 0; i < orders.length; i++) {
            ...
            if (availableOrders[i] == true) {
                ...
                reward += rewardDeriver.getRewardInOP(order, msg.sender);
        ...
        if (refund > 0) {
            ...
            require(success, "SeaportWrapper: call to refund buyer failed");
        }
    }
    ...
    function getRewardInOP(Order memory _order, address _recipient)
        ...
        returns (uint256)
    {
        return rewardDeriver.getRewardInOP(_order, _recipient);
    }
    ...
    function withdrawETH() external onlyOwner {
        ...
        require(success, "SeaportWrapper: call to ChugSplashManager failed");
    ...
    /**
     * @notice Send the OP token reward to the buyer. This call should never revert because we check
     ...
     *         2. The SeaportWrapper has a sufficient allowance of OP token.
     *
     * @param _reward Amount of OP token to reward to the buyer.
     */
    function _sendReward(uint256 _reward) internal {
        IERC20Upgradeable(OP_TOKEN).safeTransferFrom(owner(), msg.sender, _reward);
        emit RewardSent(msg.sender, _reward);
    }
}
