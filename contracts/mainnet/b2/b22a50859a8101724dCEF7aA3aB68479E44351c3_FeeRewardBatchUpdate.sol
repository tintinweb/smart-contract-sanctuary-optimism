...
contract FeeRewardBatchUpdate  is Ownable {

    address public pikaFeeReward;
    address public token = 0x7F5c764cBc14f9669B88837ca1490cCa17c31607;
    ...
    function update(address[] memory _addresses) external {
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            uint256 reward = IPikaFeeReward(pikaFeeReward).getClaimableReward(userAddress);
            IPikaFeeReward(pikaFeeReward).updateReward(userAddress);
            IERC20(token).transfer(userAddress, reward);
        }
    }

    function getRewards(address[] memory _addresses) external view returns(uint256[] memory) {
        uint256 length = _addresses.length;
    ...
    function getAllRewards(address[] memory _addresses) external view returns(uint256) {
        uint256 length = _addresses.length;
