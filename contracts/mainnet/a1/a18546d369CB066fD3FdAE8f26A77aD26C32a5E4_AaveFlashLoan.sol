...
contract AaveFlashLoan is FlashLoanReceiverBase, Ownable {
    ...
    function flashLoan(
        ...
        bytes memory params = abi.encode(_openPosition, _closePosition); // extra data to pass abi.encode(...)
    ...
    function executeOperation(
        ...
        for (uint i = 0; i < assets.length; i++) {
            ...
            console.log(premiums[i], "premiums fee");
            console.log(_amount, "borrowed amount");
            uint amountOwing = _amount.add(premiums[i]);

            IERC20(_asset).approve(Vault, amountOwing);
            IVault(Vault).deposit(_asset, _amount);

            OpenPositions(_openPosition);
            ClosePositions(_closePosition);

            IVault(Vault).withdraw(_asset, _amount);
            uint256 balance = IERC20(_asset).balanceOf(initiator);
    ...
    function OpenPositions(IClearingHouse.OpenPositionParams memory params)
        ...
        public
    {
        ...
        IClearingHouse clearingHouse = IClearingHouse(ClearingHouse);
        (uint256 base, uint256 quote) = clearingHouse.openPosition(updateParams);
        console.log("open position", base, quote);
    }

    function ClosePositions(IClearingHouse.ClosePositionParams memory params)
        public
    {
        IClearingHouse.ClosePositionParams memory updateParams = params;
        updateParams.deadline = block.timestamp + 900;
        IClearingHouse clearingHouse = IClearingHouse(ClearingHouse);
        (uint256 base, uint256 quote) = clearingHouse.closePosition(updateParams);
        console.log("close position", base, quote);
    }
