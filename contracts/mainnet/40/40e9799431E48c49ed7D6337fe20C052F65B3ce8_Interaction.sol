...
contract Interaction is Initializable, UUPSUpgradeable, OwnableUpgradeable, WardedLivingUpgradeable, IInteraction {
    ...
    function setTrigger(address trigger_) external auth {
        require(trigger_ != address(0), "Interaction/invalid-trigger");
    ...
    function newFund(uint256 fundId,
        ...
        address manager,
        IToken itoken) external override auth live {
        require(!fundExist(fundId), "Interaction/fund-exists");

        QHTrade trade = new QHTrade(triggerServer, address(usdt), manager);
    ...
    function tvl(uint256 fundId) public override view returns (int256) {
    ...
    function stake(uint256 fundId, uint256 amount) external override live returns (uint256) {
        ...
        usdt.transferFrom(msg.sender, address(feeder), amount);
...
//        uint256 tvl = uint256(tvl(fundId)); //FIXME
//        uint256 amountOut = Math.mulDiv(amount, tvl, tokenSupply);
    ...
    function cancelDeposit(uint256 fundId) external live {
        feeder.cancelDeposit(fundId, msg.sender);
    }

    function cancelWithdraw(uint256 fundId) external live {
        uint256 tokens = feeder.cancelWithdrawal(fundId, msg.sender);

        funds[fundId].itoken.mint(msg.sender, tokens);
    }

    function drip(uint256 fundId, uint256 tradeTvl) external override live auth {
        require(fundExist(fundId), "Interaction/fund-not-exists");
        ...
        if (pending > 0) {
            ...
            feeder.drip(fundId, funds[fundId].trade, traderTvl);
        }
    ...
    function withdrawMultiple(uint256 fundId, address[] calldata users) external override auth {
        require(fundExist(fundId), "Interaction/fund-not-exists");
    ...
    ///@return FundAddress, AUM, Investors amount, Next period ts
    function fundInfo(uint256 fundId) external override view returns (address, uint256, uint256, uint256) {
        int256 tvl = tvl(fundId);
        if (tvl < 0) {
            tvl = 0;
        }
        return (
    ...
    function tokenRate(uint256 fundId) public override view returns (uint256) {
        int256 tvl = tvl(fundId);
        if (tvl < 0) {
            tvl = 0;
        }
        return uint256(tvl) * 1e18 / funds[fundId].itoken.totalSupply();
    }
    ...
    function quoteTokens(uint256 fundId, uint256 usdt) external override view returns (uint256) {
        return usdt * 1e18 / tokenRate(fundId);
    }

    function userTVL(uint256 fundId, address user) external override view returns (uint256) {
        return Math.mulDiv(funds[fundId].itoken.balanceOf(user), tokenRate(fundId), 1e18);
    }

    function pendingTvl(uint256 fundId) external view returns (int256) {
        return feeder.tvl(fundId);
    }

    function setFees(address fees_) external onlyOwner {
        require(fees_ != address(0), "Interaction/invalid-fees");
        fees = IFees(fees_);
    }
    ...
    function addTradeManager(uint256 fundId, address _newManager) external auth {
        QHTrade(funds[fundId].trade).setManager(_newManager, true);
    }
...
interface IFeeder {
    ...
    */
    ...
    function withdraw(uint256 fundId, address user) external;

    function withdrawMultiple(uint256 fundId, address[] calldata users) external;
    ...
    */
    ...
    function tvl(uint256 fundId) external view returns (int256);
...
interface IInteraction {
    ...
    function fundInfo(uint256 fundId) external view returns (address, uint256, uint256, uint256);
    function tokenRate(uint256 fundId) external view returns (uint256);
    function tokenSupply(uint256 fundId) external view returns (uint256);
    function quoteTokens(uint256 fundId, uint256 usdt) external view returns (uint256);
    function userTVL(uint256 fundId, address user) external view returns (uint256);
    function pendingTvl(uint256 fundId) external view returns (int256);
    function totalFees(uint256 fundId) external view returns (uint256);
