...
contract StrategyStargateOp is StratManager, FeeManager, GasThrottler {
    ...
    constructor(
        ...
        uint256[] memory _outputToNativeFee,
        uint256[] memory _outputToDepositFee,
    ...
    function setNativePath(address[] memory _route, uint256[] memory _fee) external onlyOwner {
    ...
    function setDepositPath(address[] memory _route, uint256[] memory _fee) external onlyOwner {
        require(_route[0] == output, '!output');
...
library UniswapV3Utils {
    ...
    function pathToRoute(bytes memory _path) internal pure returns (address[] memory route) {
