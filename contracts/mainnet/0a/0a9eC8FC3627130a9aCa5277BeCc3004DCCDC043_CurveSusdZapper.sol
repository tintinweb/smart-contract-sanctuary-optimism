...
contract CurveSusdZapper is ZapperBase {
    ...
    function doApprovals() public override onlyGovernance {
        for (uint256 i = 0; i < 4; i++) {
            IERC20(tokensToDeposit[i]).safeApprove(
                CURVE_FACTORY_DEPOSIT_ADDRESS,
                type(uint256).max
            );
        }
        OptimismWETH(WETH_ADDRESS).approve(UNISWAPROUTER_V3_ADDRESS, type(uint256).max);
        super.doApprovals();
    }
        ...
        returns (uint256 shares)
    {
        ...
        if (msg.value > 0) {
            ...
            // user sent eth so we shall swap that for the selected currency or if there is none, just any currency

            // The call to `exactInputSingle` executes the swap.
            uint256 amountOut = ISwapRouter03(UNISWAPROUTER_V3_ADDRESS).exactInputSingle(
        ...
        for (uint256 i = 0; i < 4; i++) {
            if (tokensToDeposit[i] == _token) {
                IERC20(_token).safeTransferFrom(
                    msg.sender,
                    address(this),
                    _amount
                );
                fundsToDeposit[i] += _amount;
                break; // remove if you want to change this to that it accepts multiple tokens at once
            }
        }

        uint256 accapetableReturnAmount = calculateAcceptableReturnAmount(
            _amount,
            acceptableAddLiqReturnAmount
        );

        CurveFactoryDeposit(CURVE_FACTORY_DEPOSIT_ADDRESS).add_liquidity(
            vaultWant,
            fundsToDeposit,
