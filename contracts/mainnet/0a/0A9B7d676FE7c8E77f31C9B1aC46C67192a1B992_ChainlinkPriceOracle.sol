...
contract ChainlinkPriceOracle is PriceOracle {
    mapping(string => IAggregatorV3) internal priceFeeds;

    constructor(string[] memory symbols, IAggregatorV3[] memory feeds) {
    ...
    function getUnderlyingPrice(CToken cToken)
        ...
        returns (uint256)
    {
        (uint256 price, ) = _getLatestPrice(cToken.symbol());
        return price;
    }
        ...
        returns (uint256, uint256)
    {
        ...
        uint256 uPriceMantissa = uPrice *
            10**(18 - priceFeeds[symbol].decimals());

        return (uPriceMantissa, timeStamp);
    }
}