...
import { VaultConfigStorageV1 } from "./storage/VaultConfigStorage.sol";
import { IVaultConfigEvent } from "./interface/IVaultConfigEvent.sol";

contract VaultConfig is IVaultConfig, IVaultConfigEvent, SafeOwnable, VaultConfigStorageV1 {
    // TODO should fetch from chConfig
    ...
    function setWhitelistForLiquidityProvider(address account, bool enable) external onlyOwner {
        _requireNonZeroAddress(account);
        // VC_SV: same value
        require(_whitelistedLiquidityProviderMap[account] != enable, "VC_SV");
