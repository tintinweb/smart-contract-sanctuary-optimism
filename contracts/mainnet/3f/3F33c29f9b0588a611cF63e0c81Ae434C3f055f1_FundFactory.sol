...
contract FundFactory is Initializable, UUPSUpgradeable, OwnableUpgradeable, IFundFactory {
    ...
    //returns next fund id
    function newFund(FundInfo calldata fundInfo) public override returns(uint256) {
        address manager = msg.sender;

        QHToken itoken = new QHToken(address(interaction));
        interaction.newFund(fundInfo.id, fundInfo.hwm, fundInfo.investPeriod, 100, 100, manager, itoken);
...
contract QHToken is ERC20, IToken {
   ...
   // ITKN stands for Invest Token
    constructor (address manager_) ERC20("Defunds Token", "ITKN") {
     ...
     */
    function mint(address investor, uint256 amount) public override onlyManager {
     ...
     * `amount`.
     */
    function burnFrom(address account, uint256 amount) public override onlyManager {
