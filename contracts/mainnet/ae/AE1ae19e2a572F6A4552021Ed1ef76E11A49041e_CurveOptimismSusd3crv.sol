...
contract CurveOptimismSusd3crv is DefiiWithCustomEnter, DefiiWithCustomExit {
    // base tokens
    ...
    // rewards
    IERC20 constant CRV = IERC20(0x0994206dfE8De6Ec6920FF4D779B0d950605Fb53);
    IERC20 constant OP = IERC20(0x4200000000000000000000000000000000000042);

    // pools
    IPlainPool constant plainPool =
        IPlainPool(0x1337BedC9D22ecbe766dF105c9623922A27963EC);
    IMetaPool constant metaPool =
        IMetaPool(0x061b87122Ed14b9526A813209C8a59a633257bAb);

    // farming
    ILiquidityGauge constant liquidityGauge =
        ILiquidityGauge(0xc5aE4B5F86332e70f3205a8151Ee9eD9F71e0797);
    IRewardMinter constant rewardMinter =
        IRewardMinter(0xabC000d88f23Bb45525E447528DBF656A9D55bf5);
    ...
    function exitParams(int128 index, uint256 slippage)
        external
        view
        returns (bytes memory)
    {
        ...
        if (index == 0) {
            for (int128 t = 0; t < 3; t++) {
                uint256 a = plainPool.calc_withdraw_one_coin(1e18, t);
                if (tokenPerLp < a) {
                    tokenPerLp = a;
                    index = t + 1;
                }
            ...
            tokenPerLp = plainPool.calc_withdraw_one_coin(1e18, index - 1);
        }
    ...
    function _enterWithParams(bytes memory params) internal override {
        ...
        USDC.approve(address(plainPool), usdcAmount);
        plainPool.add_liquidity([0, usdcAmount, 0], 0);

        uint256 plainPoolLpAmount = plainPool.balanceOf(address(this));
        plainPool.approve(address(metaPool), plainPoolLpAmount);
        uint256 lpAmount = metaPool.add_liquidity(
            [0, plainPoolLpAmount],
            ((usdcAmount * 1e12) / minPrice)
        );
        metaPool.approve(address(liquidityGauge), lpAmount);
    ...
    function _commonExit() internal returns (uint256) {
        ...
        liquidityGauge.withdraw(lpAmount);
        return lpAmount;
    }
    ...
    function _exitWithParams(bytes memory params) internal override {
        ...
        uint256 lpAmount = _commonExit();
        uint256 plainPoolLpAmount = metaPool.remove_liquidity_one_coin(
            lpAmount,
            1,
            0
        );
        plainPool.remove_liquidity_one_coin(
            plainPoolLpAmount,
            index - 1,
            (plainPoolLpAmount * tokenPerLp) / 1e18
        );
    }
    ...
    function _harvest() internal override {
        ...
        rewardMinter.mint(address(liquidityGauge));
        _claimIncentive(CRV);
    }
...
interface IPlainPool is IERC20 {
