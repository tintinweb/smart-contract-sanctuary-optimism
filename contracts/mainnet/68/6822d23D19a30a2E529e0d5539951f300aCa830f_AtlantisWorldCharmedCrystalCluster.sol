    ...
    ERC2981Upgradeable
{
    ...
    /// @notice The merkle root created from the first 11,111 registered wallets in Atlantis World
    ...
    event MintWithProof(address indexed sender);
    event MintTo(address indexed recipient, address indexed admin);
    ...
    /**
     * @notice Allows only whitelisted arbitrary addresses.
     * @param proof A proof generated from server
     */
    modifier onlyWhitelisted(bytes32[] calldata proof) {
        require(
            whitelisted(msg.sender, proof),
            'AtlantisWorldCharmedCrystalCluster: Account not whitelisted.'
        );
        _;
    }
     ...
     * @param account An arbitrary address
     * @param proof A proof generated from server
     */
    function whitelisted(address account, bytes32[] calldata proof)
        public
        view
        returns (bool)
    {
    ...
    function mintTo(address to) external onlyAdmin {
        _mint(to, 1);
        emit MintTo(to, msg.sender);
    }

    /**
     * @notice Mints a token to only whitelisted arbitrary addresses.
     * @param proof A proof generated from server
     */
    function mintWithProof(bytes32[] calldata proof)
        external
        onlyWhitelisted(proof)
    {
        _mint(msg.sender, 1);
        emit MintWithProof(msg.sender);
    }
    ...
    function _mint(address to, uint256 quantity) internal virtual override {
        ...
        require(
            _ownerToTokenId[to] == 0,
            'AtlantisWorldCharmedCrystalCluster: Cannot mint more than once.'
        );

        _ownerToTokenId[to] = getCurrentIndex();
