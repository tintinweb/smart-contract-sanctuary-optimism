...
contract MirrorRendererV2 {
        ...
        return
            string(
                abi.encodePacked(
                    ...
                    _getTokenURIJson(tokenId, editionNumber)
                )
            );
    }

    function contractURI() public view returns (string memory) {
        // Pass in 10_000 as mock "price" to get royalty BPS instead
        // of royalty amount.
        (address receiver, uint256 royaltyBPS) = IWritingEditions(msg.sender)
            .royaltyInfo(1, 10_000);

        return
            string(
                abi.encodePacked(
                    "data:application/json;base64,",
                    _getContractURIJson(royaltyBPS, receiver)
                )
            );
    }
    ...
    function _getMedia() internal view returns (string memory) {
        return
            string(
                abi.encodePacked(
                    '", "content": "ar://',
