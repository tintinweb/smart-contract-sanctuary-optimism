...
// DUNEToken.
contract CORG is BEP20("CORG", "CORG") {
...
contract MasterChefV2 is Ownable, ReentrancyGuard {
    ...
    uint256 public maxEmissionRate = 5 ether;
    // Dev address.
    ...
    function set(uint256 _pid, uint256 _allocPoint, uint16 _withdrawFeeBP, bool _withUpdate) external onlyOwner {
        require(_withdrawFeeBP <= 800, "add: invalid withdrawal fee basis points");
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    function updateEmissionRate(uint256 _tokenPerSecond) external onlyOwner {
        require(_tokenPerSecond <= 5 ether, "Maximum exceeded");
        massUpdatePools();
        tokenPerSecond = _tokenPerSecond;
    ...
    function setTokenAddress(CORG _token) external onlyOwner {
