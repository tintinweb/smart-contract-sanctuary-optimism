...
MAX_NFTS: constant(uint256) = 1000
...
@external
@view
def all(_limit: uint256, _offset: uint256) -> \
    DynArray[VeNFT, MAX_NFTS]:
  """
  ...
  col: DynArray[VeNFT, MAX_NFTS] = empty(DynArray[VeNFT, MAX_NFTS])

  for index in range(MAX_NFTS):
    if _offset > index:
      continue
...
@external
@view
def byAccount(_account: address) -> DynArray[VeNFT, MAX_NFTS]:
  """
