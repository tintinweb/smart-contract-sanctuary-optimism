...
// Built to swap a token to BIFI at a one hour cadence. 
contract BeefyBifiSwapper is Ownable {
    ...
    uint256 private constant swapDelay = 1 hours;
    ...
    function swap(address _token, bytes calldata _data) external onlyGelato {
        require(block.timestamp > (lastSwap + swapDelay), "Cant swap yet");
        lastSwap = block.timestamp;
        emit TokenSwapped(_token, IERC20(_token).balanceOf(address(this)));
        _swapViaOneInch(_token, _data);
        
        uint256 amount = IERC20(bifi).balanceOf(address(this));
        IERC20(bifi).safeTransfer(treasury, amount);
        emit Harvest(amount);
    }
