...
contract ReaperStrategyGranarySupplyOnly is ReaperBaseStrategyv4, VeloSolidMixin {
    ...
    function _harvestCore(uint256 _debt) internal virtual override returns (int256 roi, uint256 repayment) {
        ...
        for (uint256 i = 0; i < numSteps; i = i.uncheckedInc()) {
            ...
            _swapVelo(step[0], step[1], amount, 0, VELO_ROUTER);
        }
    ...
    function balanceOfPool() public view returns (uint256) {
        (uint256 supply,,,,,,,,) =
            IAaveProtocolDataProvider(DATA_PROVIDER).getUserReserveData(address(want), address(this));
        return supply;
    }
}
...
contract ReaperStrategyGranarySupplyOnlyV2 is ReaperStrategyGranarySupplyOnly, BalMixin, UniV3Mixin {
    using ReaperMathUtils for uint256;

    address public constant BEET_VAULT = 0xBA12222222228d8Ba445958a75a0704d566BF2C8;
    ...
    function _harvestCore(uint256 _debt) internal virtual override returns (int256 roi, uint256 repayment) {
        ...
        for (uint256 i = 0; i < numSteps; i = i.uncheckedInc()) {
            ...
            if (step.dex == Exchange.Velodrome) {
                ...
                _swapUniV3(step.start, step.end, amount, 0, UNIV3_ROUTER, UNIV3_QUOTER);
            }
        }
