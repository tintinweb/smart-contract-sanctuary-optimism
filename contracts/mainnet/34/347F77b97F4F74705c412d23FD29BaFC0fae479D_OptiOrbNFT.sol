...
contract OptiOrbNFT is ERC721AQueryable, ERC2981, Ownable, Pausable, ReentrancyGuard, Apetimism {
  ...
  string private constant TOKEN_NAME = "OptiOrb";
  string private constant TOKEN_SYMBOL = "ORB";
  ...
  mapping(int16 => uint256) public roleAllocations;
  ...
  function initialize(
    ...
    addAllowedRolesInRound(rolesInRound_);
    setRoundsAllocation(roundAllocations_);
    setRolesAllocation(roleAllocations_);
  }
  ...
  function addAllowedRolesInRound(RoleInRoundParams[] memory params) public onlyOwner {
  ...
  function setRoundsAllocation(RoundAllocationParams[] memory params) public onlyOwner {
    for (uint i = 0; i < params.length; i++)
      setRoundAllocation(params[i].round, params[i].allocation);
  }
  ...
  function setRolesAllocation(RoleAllocationParams[] memory params) public onlyOwner {
    for (uint i = 0; i < params.length; i++)
      setRoleAllocation(params[i].role, params[i].allocation);
  }

  function setRoleAllocation(int16 role, uint256 allocation) public onlyOwner {
    roleAllocations[role] = allocation;
  }
  ...
  function mint(uint256 quantity, int16 role, uint256 nonce, uint8 v, bytes32 r, bytes32 s) external payable whenNotPaused nonReentrant {
    ...
    // uint256 to_team = msg.value - to_apetimism;
  ...
  function mintPriceForCurrentRound(address) public view returns (uint256) {
    return allowedRolesInRound[currentRound][0].mint_price;
  }
  ...
  function maxMintable(address addr) public view virtual returns (uint256) {
    return maxMintableForRole(addr, 0);
  }
  ...
  function maxMintableForRole(address addr, int16 role) public view virtual returns (uint256) {
    ...
    if (totalMintedInRound[currentRound] >= roundAllocations[currentRound])
      return 0;
    if (_addressTokenMintedInRole[addr][selected_role] >= roleAllocations[selected_role])
      return 0;
    ...
    uint256 role_quota_left = max_mint - _addressTokenMintedInRole[addr][selected_role];
