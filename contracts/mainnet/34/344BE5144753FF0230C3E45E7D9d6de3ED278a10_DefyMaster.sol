   ...
   //  @notice Destroys `amount` tokens from `account`, reducing the total supply.
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override {
        ...
        if (_isSExcludedFromFee[sender] || _isRExcludedFromFee[recipient]) {
            ...
            _moveDelegates(_delegates[sender], _delegates[BURN_VAULT] , burnAmount);
            _moveDelegates(_delegates[sender], _delegates[ILP_VAULT] , ilpAmount);
            _moveDelegates(_delegates[sender], _delegates[dev] , devAmount);
...
contract DefyMaster is Ownable , ReentrancyGuard {
  ...
  PoolInfo[] public poolInfo;
    ...
    constructor(
        ...
        require(_secondaryReward != IERC20(0), 'DEFY: SecondaryReward cannot be the zero address');
        
        defy = _defy;
        secondR = _secondaryReward;
        burn_vault = _burnvault;
        devaddr = _devaddr;
        emDev = _emDev;
        ...
        returns (uint256)
    {
		...
		if(currentVal < user.depVal){
            ...
            if(difference > (user.depVal).div(10)){
               difference =  (user.depVal).div(10);
            }
    ...
    function setStartTimestamp(uint256 sTimestamp) public onlyDev{
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            ...
            pool.lastRewardTimestampDR = sTimestamp;
        }
    ...
    uint256 _rewardEndTimestamp
        ...
        address lpToken = address(poolInfo[_pid].lpToken);
    ...
    function emergencyWithdraw(uint256 _pid) public {
        ...
        UserInfo storage user = userInfo[_pid][msg.sender];
        if (pool.issueStub){
                require(pool.stubToken.balanceOf(msg.sender) >= user.amount , "withdraw : No enough STUB tokens!");
                pool.stubToken.burn(msg.sender, user.amount);
            }
