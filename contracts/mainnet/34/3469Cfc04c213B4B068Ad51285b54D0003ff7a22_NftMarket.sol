...
contract NftMarket is Ownable {
	...
	event nftListed(address indexed seller, uint256 tokenId, address fundingToken, uint256 price, uint256 startDate, uint256 endDate);

	function listNFT(address _nft, uint256 _tokenId, address _fundingToken, uint256 _price, uint256 _startDate, uint256 _endDate) public {
		...
		emit nftListed(msg.sender, _tokenId, _fundingToken, _price, _startDate, _endDate);
	}
	...
	function buyNFT(uint256 _listingID) public {
		...
		uint256 feeAmount = Listings[_listingID].price * feePerK / 10000;

		bool success = IERC20(Listings[_listingID].fundingToken).transferFrom(msg.sender, Listings[_listingID].seller, Listings[_listingID].price - feeAmount);
		require(success, "Transfer From Failed.");
		success = IERC20(Listings[_listingID].fundingToken).transferFrom(msg.sender, address(this), feeAmount);
		require(success, "Transfer From Failed.");
	...
	function setFee(uint256 _feePerK) public onlyOwner {
		feePerK = _feePerK;
	}
