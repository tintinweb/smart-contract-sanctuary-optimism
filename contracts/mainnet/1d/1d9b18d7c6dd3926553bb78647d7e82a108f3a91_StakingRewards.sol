...
contract StakingRewards is ReentrancyGuard, Pausable, Minion {
    ...
    //should check how much sUSD is in the contract and start distribuiting it
    function swapAndRefreshRewards() external onlyMinion updateReward(address(0)) {
        ...
        //only use sUSD accured between the last notify and now;
        uint256 rewardAfter = rewardsToken.balanceOf(address(this));
        uint256 reward = rewardAfter.sub(lastRewardBalance);
