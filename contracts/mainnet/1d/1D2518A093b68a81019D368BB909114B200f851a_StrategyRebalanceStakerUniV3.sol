...
contract StrategyRebalanceStakerUniV3 is Initializable, OwnableUpgradeable {
    ...
    )
  {
    ...
    return _readRebalanceProportion(_tickLower, _tickUpper);
  }

  function _readRebalanceProportion(int24 _tickLower, int24 _tickUpper)
    internal
    view
    returns (
      uint256,
  ...
  function _balanceProportionOneInch(
    ...
    (uint256 _amountSpecified, address _inputToken, ) = _readRebalanceProportion(_tickLower, _tickUpper);
   ...
   * @return  _tokenId  new token id
   */
  function rebalanceVia1inch(address _oneInchRouter, bytes calldata _data)
    external
    onlyBenevolent
    returns (uint256 _tokenId)
  {
    ...
    _balanceProportionOneInch(_tickLower, _tickUpper, _oneInchRouter, _data);
    //Need to do this again after the swap to cover any slippage.
    uint256 _amount0Desired = token0.balanceOf(address(this));
    uint256 _amount1Desired = token1.balanceOf(address(this));
