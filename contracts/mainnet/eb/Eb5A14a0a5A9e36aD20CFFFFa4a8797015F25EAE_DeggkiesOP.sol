...
contract DeggkiesOP is ERC721, Ownable {
  // Setup
  ...
  constructor(string memory _uri) ERC721("Deggkies", "DEGGKIES") {
    _baseTokenURI = _uri;
  }
  
  // mint migration
  function mintMigration(address[] memory _addresses, uint[] memory _arrayNo)
    external
    onlyOwner
  {
    for(uint i = 0; i < _addresses.length; i++ ){
      _mintBatch(_addresses[i], _arrayNo[i]);
    }
  }
