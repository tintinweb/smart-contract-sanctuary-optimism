...
contract HyphenFacet is ReentrancyGuard, Signers, VerifySigEIP712 {
    ...
    function bridgeToHyphen(HyphenDescription calldata hDesc) external payable nonReentrant {
        LibPlexusUtil._isTokenDeposit(hDesc.token, hDesc.amount);
        _hyphenStart(hDesc.amount, hDesc);
    }

    function swapAndBridgeToHyphen(SwapData calldata _swap, HyphenDescription calldata hDesc) external payable nonReentrant {
        SwapData calldata swapData = _swap;
        uint256 dstAmount = LibPlexusUtil._tokenDepositAndSwap(swapData);
        _hyphenStart(dstAmount, hDesc);
    }

    function _hyphenStart(uint256 amount, HyphenDescription memory hDesc) internal {
        amount = LibPlexusUtil._fee(hDesc.token, amount);
        bool isNotNative = !LibPlexusUtil._isNative(hDesc.token);

        if (isNotNative) {
            IERC20(hDesc.token).safeApprove(address(HYPHEN), amount);
            HYPHEN.depositErc20(hDesc.toChainId, hDesc.token, hDesc.receiver, amount, "PLEXUS");
        } else {
            HYPHEN.depositNative{value: amount}(hDesc.receiver, hDesc.toChainId, "PLEXUS");
        }

        bytes32 transferId = keccak256(
            abi.encodePacked(address(this), hDesc.receiver, hDesc.token, amount, hDesc.toChainId, hDesc.nonce, block.chainid)
        );

        emit LibData.Bridge(hDesc.receiver, uint64(hDesc.toChainId), hDesc.token, hDesc.toDstToken, amount, transferId, "Hyphen");
    }
}
