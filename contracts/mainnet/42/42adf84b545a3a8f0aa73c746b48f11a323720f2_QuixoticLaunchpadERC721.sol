...
// File @openzeppelin/contracts/token/ERC721/IERC721Receiver.s[email protected]
...
contract QuixoticLaunchpadERC721 is ERC721("Optimism X TAKASHI MURAKAMI", "TakashiX"), ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    function mintToken(uint numberOfTokens) public payable nonReentrant {
        ...
        require(numberOfTokens + addressToNumberMinted[_msgSender()] <= 100, "Exceeded max token purchase");
    ...
    function greenListMintToken(uint numberOfTokens, bytes memory signature) public payable nonReentrant {
        ...
        require(numberOfTokens + addressToGreenListNumberMinted[_msgSender()] <= 100, "Exceeded max token purchase");
        require(totalSupply() + numberOfTokens <= MAX_TOKENS, "Purchase would exceed max supply of tokens");
        require(0 * numberOfTokens <= msg.value, "Ether value sent is not correct");
