...
import { UniswapV3CallbackBridge } from "@perp/curie-contract/contracts/base/UniswapV3CallbackBridge.sol";
...
contract PerpPositionManager is IPerpPositionManager, ContextUpgradeable, PerpPositionManagerStorageV1 {
    ...
    function initialize(address clearingHouseArg, address vBaseTokenArg) external initializer {
        ...
        address marketRegistry = UniswapV3CallbackBridge(exchange).getMarketRegistry();
    ...
    function getAccountValue_6() external view override returns (uint256) {
    ...
    /// @dev (totalPositionValue * targetMarginRatio - accountValue) / (indexTwap * targetMarginRatio)
    ...
    function getDeleveragedPositionSize(uint24 targetMarginRatio) external view override returns (uint256) {
        ...
        uint256 indexTwap = getIndexTwap();
        uint256 divisor = indexTwap.mulRatio(targetMarginRatio);
        ...
        return
            totalPositionValueMultipliedByTargetMarginRatio < accountValueAbs
                ? 0
                : totalPositionValueMultipliedByTargetMarginRatio.sub(accountValueAbs).mulDiv(1 ether, divisor).div(2);
    }
    ...
    function getIndexTwap() public view override returns (uint256) {
        uint256 indexTwap = IBaseToken(_vBaseToken).isClosed()
            ? IBaseToken(_vBaseToken).getClosedPrice()
            : IIndexPrice(_vBaseToken).getIndexPrice(IClearingHouseConfig(_clearingHouseConfig).getTwapInterval());
        // PPM_ZIT: zero indexTwap
        require(indexTwap > 0, "PPM_ZIT");
        return indexTwap;
    }
