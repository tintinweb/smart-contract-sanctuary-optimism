...
import { IYieldToken } from "@koyofinance/contracts-interfaces/contracts/perpetuals/core/IYieldToken.sol";
...
contract YieldBaseToken is IERC20, IYieldToken {
    ...
    modifier onlyGov() {
        require(msg.sender == gov, "YieldBaseToken: forbidden");
        _;
    }

    modifier onlyAdmin() {
        require(admins[msg.sender], "YieldBaseToken: forbidden");
        _;
    }
    ...
    function addNonStakingAccount(address _account) external onlyAdmin {
        require(!nonStakingAccounts[_account], "YieldBaseToken: _account already marked");
    ...
    function removeNonStakingAccount(address _account) external onlyAdmin {
        require(nonStakingAccounts[_account], "YieldBaseToken: _account not marked");
        _updateRewards(_account);
    ...
    function _mint(address _account, uint256 _amount) internal {
        require(_account != address(0), "YieldBaseToken: mint to the zero address");
    ...
    function _burn(address _account, uint256 _amount) internal {
        require(_account != address(0), "YieldBaseToken: burn from the zero address");
    ...
    function _transfer(
        ...
        require(_sender != address(0), "YieldBaseToken: transfer from the zero address");
        require(_recipient != address(0), "YieldBaseToken: transfer to the zero address");

        if (inWhitelistMode) {
            require(whitelistedHandlers[msg.sender], "YieldBaseToken: msg.sender not whitelisted");
        }
    ...
    function _approve(
        ...
        require(_owner != address(0), "YieldBaseToken: approve from the zero address");
        require(_spender != address(0), "YieldBaseToken: approve to the zero address");
