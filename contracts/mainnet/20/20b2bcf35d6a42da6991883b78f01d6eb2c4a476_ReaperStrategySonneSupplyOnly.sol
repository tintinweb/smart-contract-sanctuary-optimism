 ...
 */
contract ReaperStrategySonneSupplyOnly is ReaperBaseStrategyv4, VeloSolidMixin, UniV3Mixin {
    using ReaperMathUtils for uint256;
    using SafeERC20Upgradeable for IERC20Upgradeable;
    ...
    // Internal balance for amount of tokens deposited in Sonne
    ...
    function initialize(
        ...
        cWant = _cWant;
        __ReaperBaseStrategy_init(_vault, cWant.underlying(), _strategists, _multisigRoles, _keepers);
        comptroller = IComptroller(cWant.comptroller());
        balanceOfPool = 0;

        address[] memory markets = new address[](1);
        markets[0] = address(_cWant);
    ...
    function _harvestCore(uint256 _debt) internal override returns (int256 roi, uint256 repayment) {
        ...
        for (uint256 i = 0; i < numSteps; i = i.uncheckedInc()) {
            ...
            if (step.dex == Exchange.Velodrome) {
                _swapVelo(step.start, step.end, amount, 0, veloRouter);
            } else if (step.dex == Exchange.UniV3) {
                _swapUniV3(step.start, step.end, amount, 0, uniV3Router, uniV3Quoter);
            }
        }
     ...
     */
    function _deposit(uint256 toReinvest) internal doUpdateBalance {
        if (toReinvest != 0) {
            IERC20Upgradeable(want).safeIncreaseAllowance(address(cWant), toReinvest);
            cWant.mint(toReinvest);
        }
    }
    ...
    function _withdrawUnderlying(uint256 _withdrawAmount) internal doUpdateBalance {
        uint256 withdrawable = CErc20I(cWant).balanceOfUnderlying(address(this));
    ...
    function updateBalance() public {
        balanceOfPool = CErc20I(cWant).balanceOfUnderlying(address(this));
    }
