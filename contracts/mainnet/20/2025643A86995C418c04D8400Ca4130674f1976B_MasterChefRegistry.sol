...
import './interfaces/IMasterChefRegistry.sol';

contract MasterChefRegistry is OwnableUpgradeable, IMasterChefRegistry {
    mapping(address => mapping(uint256 => bool)) public deployed;
    ...
    function markAsDeployed(address _masterChef, uint256 _pid) external onlyOperator {
        deployed[_masterChef][_pid] = true;
    }
...
interface IMasterChefRegistry {
    function deployed(address _masterChef, uint256 _pid) external view returns (bool);

    function markAsDeployed(address _masterChef, uint256 _pid) external;
}
