...
/**
 * @title AutoCompound Stargate pool
 * @notice vault for auto-compounding Stargate tokens on pools using a standard MasterChef contract
 * @author YieldWolf
 */
contract ACMasterChefStargateV2WithVelodromeSwap is AutoCompoundVault {
    uint[] amounts;
    address public immutable compoundToken;
    uint256 public immutable poolId;
    StargateRouter public immutable liquidityRouter;
    ...
    constructor(
        ...
        uint256 _poolId,
        address _compoundTokenAddress
    ) ERC20(_name, _symbol) AutoCompoundVault(_pid, _addresses) {
        compoundToken = _compoundTokenAddress;
        poolId = _poolId;
        liquidityRouter = StargateRouter(_liquidityRouter);
    }

    function _earnToStake(uint256 _earnAmount) internal override {
        if (compoundToken != address(earnToken)) {
            _safeSwap(_earnAmount, address(earnToken), compoundToken); // OP -> USDC
        }

        uint256 liquidityInAmount = IERC20(compoundToken).balanceOf(address(this));
        IERC20(compoundToken).safeIncreaseAllowance(address(liquidityRouter), liquidityInAmount);
    ...
    function _safeSwap(
        ...
        address router = 0xa132DAB612dB5cB9fC9Ac426A0Cc215A3423F9c9;
        address nextToken = address(0);
        route[] memory routes = new route[](1);
        routes[0].from = _tokenFrom;
        routes[0].to = _tokenTo;
        ...
        swapInterface(router).swapExactTokensForTokens(_amountIn,
            1,
            routes,
            address(this),
            block.timestamp);
