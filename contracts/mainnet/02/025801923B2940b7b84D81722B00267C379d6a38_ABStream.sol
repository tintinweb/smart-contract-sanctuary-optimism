 ...
 * @notice Anotherblock Token Streaming Contract on Optimism
 **/
...
/// @dev Thrown when trying to activate an already active stream
error STREAM_ALREADY_ACTIVE();

contract ABStream is
    SuperAppBase,
    ERC2771Recipient,
    Initializable,
    AccessControlUpgradeable
{
    ...
    function updateStream(
        ...
        int96 flowUpdate = int96(int256(_quantity)) * baseFlowPerDrop[_dropId];

        // Reduce the previous holder flow
        _reduceFlow(_previousReceiver, flowUpdate);

        // Increase the new holder flow
        _increaseFlow(_newReceiver, flowUpdate);
    }
    ...
    /**
     * @notice
     *  Recalculate flow and activate streams to `_receivers` addresses
     *
     * @param _receivers addresses to be refreshed
    ...
    function refreshStreams(
        ...
        for (uint256 i = 0; i < _receivers.length; ++i) {
            if (_receivers[i] != address(0)) {
                // Get the user boost
                int96 newUserBoost = this.getUserBoost(_receivers[i]);
                ...
                for (uint256 j = 0; j < _dropIds.length; ++j) {
                    ...
                    if (balance > 0) {
                        int96 dropBaseFlow = baseFlowPerDrop[_dropIds[j]];
                        dropsFlow += dropBaseFlow * int96(int256(balance));
                    }
                }

                // Calculate the new flow
                int96 newFlow = dropsFlow +
                    newUserBoost -
                    currentUserBoost[_receivers[i]];

                // Get the current flow to the user
                int96 currentFlow = _getOutflow(_receivers[i]);
                if (currentFlow == 0 && newFlow > 0) {
                    // Allow the new holder to be streamed AB Tokens
                    AB_TOKEN.setAllowedId(_receivers[i]);
                ...
                currentUserBoost[_receivers[i]] = newUserBoost;
            }
        }
    }
    ...
    /**
     * @notice
     *  Cancel the streams of `_receivers` addresses
     *
     * @param _receivers addresses to be refreshed
     */
    function cancelStreams(address[] memory _receivers)
        external
     ...
     *
     * @param _ABToken Anotherblock Token contract address
     */
    function setABToken(address _ABToken)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
     ...
     */
    function setABRegistry(address _ABRegistry)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        // Ensure the given address is not the zero-address
     ...
     *  Only the contract owner can perform this operation
     *
     * @param _ABRelay Anotherblock Relay contract address
     */
    function setABRelay(address _ABRelay)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        // Ensure the given address is not the zero-address
     ...
     * @param _increase amount of AB Token increased (in token/seconds) to be granted by the boost
     */
    function updateBoost(uint256 _boostId, int96 _increase)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
    ...
    function grantSpecialBoost(address _to, int96 _specialBoost)
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        _grantSpecialBoost(_to, _specialBoost);
    }

    /**
     * @notice
     *  Grant a special boost to all the given users
     *
     * @dev
     *  Only the contract owner can perform this operation
     *
     * @param _toUsers users addresses to receive the special boost
     * @param _specialBoost amount of AB Token increased (in token/seconds) to be granted by the special boost
     */
    function grantSpecialBoostBatch(
        address[] memory _toUsers,
        int96 _specialBoost
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        for (uint256 i = 0; i < _toUsers.length; ++i) {
            _grantSpecialBoost(_toUsers[i], _specialBoost);
        }
    }
    ...
    function revokeSpecialBoost(address _from, int96 _specialBoost)
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        _revokeSpecialBoost(_from, _specialBoost);
    }

    /**
     * @notice
     *  Revoke a special boost to all given users
     *
     * @dev
     *  Only the contract owner can perform this operation
     *
     * @param _fromUsers users addresses to reduce the special boost from
     * @param _specialBoost amount of AB Token flow to be decreased (in token/seconds)
     */
    function revokeSpecialBoostBatch(
        address[] memory _fromUsers,
        int96 _specialBoost
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        for (uint256 i = 0; i < _fromUsers.length; ++i) {
            _revokeSpecialBoost(_fromUsers[i], _specialBoost);
        }
    }
    ...
    /**
     * @notice
     *  Get the base flow for the given `_dropId`
     *
     * @param _dropId drop identifier
     *
     * @return : base flow for the given drop
     */
    function getDropBaseFlow(uint256 _dropId) external view returns (int96) {
        return baseFlowPerDrop[_dropId];
    }
     ...
     * @param _baseFlowAmount flow amount to be added
     *
     */
    function _increaseFlow(address _to, int96 _baseFlowAmount) internal {
     ...
     * @param _baseFlowAmount flow amount to be reduced
     *
     */
    function _reduceFlow(address _from, int96 _baseFlowAmount) internal {
     ...
     * @param _specialBoost amount of AB Token increased (in token/seconds) to be granted by the special boost
     */
    function _grantSpecialBoost(address _to, int96 _specialBoost) internal {
        ...
        if (currentFlow == 0) {
            // Allow the recipient to be streamed AB Tokens
            AB_TOKEN.setAllowedId(_to);
            cfaV1Lib.createFlow(_to, ISuperToken(address(AB_TOKEN)), newFlow);
        } else {
            cfaV1Lib.updateFlow(_to, ISuperToken(address(AB_TOKEN)), newFlow);
     ...
     * @param _specialBoost amount of AB Token flow to be decreased (in token/seconds)
     */
    function _revokeSpecialBoost(address _from, int96 _specialBoost) internal {
        if (
            _from == address(0) ||
            _specialBoost <= 0 ||
        ...
        returns (bool)
    {
        for (uint256 i = 0; i < _condition.dropIds.length; ++i) {
            if (
                AB_REGISTRY.getUserBalancePerDrop(
                    _user,
                    _condition.dropIds[i]
