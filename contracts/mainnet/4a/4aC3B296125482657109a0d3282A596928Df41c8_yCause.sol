...
contract yCause {
    ...
    address public _causeAddress;

    mapping (address => uint256) public _staked;
    mapping (address => uint256) public _PPSWhenStaked;

    event Staked(address indexed user, uint256 amount);
    event Unstaked(address indexed user, uint256 amountUnstaked, uint256 amountDonated);

    constructor (address stakedTokenAddress, address yVaultAddress, address causeAddress) {
        ...
        _causeAddress = causeAddress;
        _stakedToken.approve(yVaultAddress, 2**256 - 1);
    }
    ...
    function stake(uint256 amount) public {
        ...
        _PPSWhenStaked[msg.sender] = _yVault.pricePerShare();
        _yVault.deposit(amount);
    ...
    function unstake() public {
        ...
        uint256 totalShares = _yVault.balanceOf(msg.sender);
        uint256 donatorShares = (staked * _PPSWhenStaked[msg.sender]) / _yVault.pricePerShare();
        uint256 donatedShares = totalShares - donatorShares;
        
        _staked[msg.sender] = 0;

        uint256 totalRedeemed = _yVault.withdraw(totalShares);
        _stakedToken.transfer(msg.sender, staked);
        _stakedToken.transfer(_causeAddress, totalRedeemed - staked);

        emit Unstaked(msg.sender, staked, totalRedeemed - staked);
    }
