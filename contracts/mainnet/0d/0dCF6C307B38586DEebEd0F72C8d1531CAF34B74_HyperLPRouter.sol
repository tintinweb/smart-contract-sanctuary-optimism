...
contract HyperLPRouter is IUniswapV3SwapCallback, GasStationRecipient, Ownable {
        ...
        )
    {
        ...
        if (paymentToken != token0) { 
            amount0 = _swap(paymentToken, token0, fee, amount0, 0);
        }

        if (paymentToken != token1) { 
            amount1 = _swap(paymentToken, token1, fee, amount1, 0);
        }
        ...
        )
    {
        ...
        uint256 price0 = calculatePrice(paymentToken, token0, fee);
        uint256 price1 = calculatePrice(paymentToken, token1, fee);
