...
contract DfyToken is ERC20('DefySwap', 'DFY') {
    ...
    function setMaxTxLimit(uint256 maxTx) external onlyOwner() {
        require(maxTx >= 100*1e18 , "Error : Minimum maxTxLimit is 100 DFY");
        require(maxTx <= _maxSupply , "Error : Maximum maxTxLimit is 10M DFY");
        _maxTxAmount = maxTx;
    }
   ...
   //  @notice Destroys `amount` tokens from `account`, reducing the total supply.
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override {
        ...
        if (_isSExcludedFromFee[sender] || _isRExcludedFromFee[recipient]) {
            ...
            require(amount <= _maxTxAmount , "DEFY Transfer: Transfer amount is above the limit!");
