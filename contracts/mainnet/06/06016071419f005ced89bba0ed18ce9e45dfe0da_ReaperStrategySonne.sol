...
interface CErc20I is CTokenI {
    ...
    function liquidateBorrow(address borrower, uint256 repayAmount, CTokenI cTokenCollateral)
        external
        returns (uint256);
...
contract ReaperStrategySonne is ReaperBaseStrategyv4, ILeverageable {
    ...
    function initialize(
        ...
        __ReaperBaseStrategy_init(_vault, _swapper, cWant.underlying(), _strategists, _multisigRoles, _keepers);
        markets = [_cWant];
    ...
    function _deposit(uint256 _amount) internal override doUpdateBalance {
        if (_amount != 0) {
            IERC20Upgradeable(want).safeIncreaseAllowance(address(cWant), _amount);
            cWant.mint(_amount);
        }
    ...
    function _leverMax() internal {
        ...
        for (uint256 i = 0; i < borrowDepth && totalAmountToBorrow > minWantToLeverage; i = i.uncheckedInc()) {
     ...
     */
    function _getMaxBorrowFromSupplied(uint256 wantSupplied, uint256 collateralFactor)
        internal
        pure
        returns (uint256)
    {
     ...
     */
    function _withdrawUnderlying(uint256 _withdrawAmount) internal doUpdateBalance {
    ...
    function _deleverage(uint256 _withdrawAmount) internal {
        ...
        for (uint256 i = 0; newBorrow > minWantToLeverage && i < borrowDepth; i = i.uncheckedInc()) {
    ...
    function _leverDownStep(uint256 maxDeleverage) internal returns (uint256 deleveragedAmount) {
        ...
        maxAllowedDeleverageAmount = maxAllowedDeleverageAmount * 0.999 ether / MANTISSA;
    ...
    function manualDeleverage(uint256 amount) external doUpdateBalance {
        _atLeastRole(STRATEGIST);
        require(cWant.redeemUnderlying(amount) == 0, "Cannot redeem underlying");
    ...
    function setTargetLtv(uint256 _ltv) public {
        ...
        (, uint256 collateralFactorMantissa,) = comptroller.markets(address(cWant));
        require(collateralFactorMantissa > _ltv + allowedLtvDrift, "LTV too high to be safe");
        require(
            _ltv + allowedLtvDrift <= (collateralFactorMantissa * LTV_SAFETY_ZONE) / MANTISSA,
     ...
     * @param triggerHarvest whether to call the harvest function at the end
     */
    function setLeverage(uint256 targetLeverage, uint256, bool triggerHarvest) external {
    ...
    function setAllowedLtvDrift(uint256 _drift) external {
        ...
        (, uint256 collateralFactorMantissa,) = comptroller.markets(address(cWant));
        require(
            targetLtv + _drift <= (collateralFactorMantissa * LTV_SAFETY_ZONE) / MANTISSA,
            "LTV+drift too high to be safe"
        );
