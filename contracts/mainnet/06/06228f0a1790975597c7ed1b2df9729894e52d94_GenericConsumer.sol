...
import { IExternalFulfillment } from "./interfaces/IExternalFulfillment.sol";
import { Entry, EntryLibrary } from "./libraries/internal/EntryLibrary.sol";
    ...
    KeeperCompatible
{
    ...
    uint96 private s_minGasLimitPerformUpkeep;
    ...
    error GenericConsumer__LinkAllowanceIsInsufficient(uint256 allowance, uint96 payment);
    error GenericConsumer__LinkBalanceIsInsufficient(uint256 balance, uint96 payment);
    ...
    error GenericConsumer__PaymentIsGreaterThanLinkTotalSupply();
    error GenericConsumer__PaymentIsZero();
    error GenericConsumer__SpecIdIsZero();

    event GenericConsumer__EntryRemoved(uint256 indexed lot, bytes32 indexed key);
    event GenericConsumer__EntryRequested(
        uint256 roundId,
        uint256 indexed lot,
        bytes32 indexed key,
    ...
    event GenericConsumer__EntrySet(uint256 indexed lot, bytes32 indexed key, Entry entry);
    event GenericConsumer__FundsWithdrawn(address payee, uint256 amount);
    event GenericConsumer__LastRequestTimestampSet(
        uint256 indexed lot,
        bytes32 indexed key,
        uint256 lastRequestTimestamp
    );
    event GenericConsumer__LotRemoved(uint256 indexed lot);
    event GenericConsumer__RequestFulfilled(
        bytes32 indexed requestId,
        bool success,
        bool isForwarded,
    ...
    event GenericConsumer__SetChainlinkExternalRequestFailed(
        address indexed callbackAddr,
        bytes32 indexed requestId,
        bytes32 indexed key
    );
    ...
    fallback() external whenNotPaused nonReentrant {
        ...
        if (callbackAddr == NO_CALLBACK_ADDR) {
            emit GenericConsumer__RequestFulfilled(
                requestId,
                true,
                false,
                address(this),
    ...
    function performUpkeep(bytes calldata _performData) external override whenNotPaused nonReentrant {
        ...
        for (uint256 i = 0; i < keysLength; ) {
            ...
            emit GenericConsumer__EntryRequested(roundId, lot, key, requestId, blockTimestamp);
    ...
    function removeLot(uint256 _lot) external onlyRole(DEFAULT_ADMIN_ROLE) whenNotPaused {
        ...
        emit GenericConsumer__LotRemoved(_lot);
    }
    ...
    function setMinGasLimitPerformUpkeep(uint96 _minGasLimit) external onlyRole(DEFAULT_ADMIN_ROLE) whenNotPaused {
        s_minGasLimitPerformUpkeep = _minGasLimit;
    }
    ...
    function withdraw(address payable _payee, uint256 _amount) external onlyOwner {
        emit GenericConsumer__FundsWithdrawn(_payee, _amount);
    ...
    function _buildAndSendOperatorRequest(
        ...
        if (_callbackAddr != address(this)) {
            ...
            try fulfillmentContract.setChainlinkExternalRequest(address(this), requestId) {} catch {
                ...
                emit GenericConsumer__SetChainlinkExternalRequestFailed(_callbackAddr, requestId, _key);
            }
        }
        return requestId;
    }
    ...
    function _removeEntry(
        ...
        emit GenericConsumer__EntryRemoved(_lot, _key);
    }
    ...
    function _setEntry(
        ...
        emit GenericConsumer__EntrySet(_lot, _key, _entry);
    }
    ...
    function _setLastRequestTimestamp(
        ...
        emit GenericConsumer__LastRequestTimestampSet(_lot, _key, _lastRequestTimestamp);
    }
    ...
    function _requireInterval(uint96 _interval) private pure {
        if (_interval == 0) {
            revert GenericConsumer__IntervalIsZero();
        }
    }

    function _requireLinkAllowance(uint256 _allowance, uint96 _payment) private pure {
        if (_allowance < _payment) {
            revert GenericConsumer__LinkAllowanceIsInsufficient(_allowance, _payment);
        }
    }

    function _requireLinkBalance(uint256 _balance, uint96 _payment) private pure {
    ...
    function _requirePayment(uint96 _payment) private pure {
        if (_payment == 0) {
            revert GenericConsumer__PaymentIsZero();
        }
        if (_payment > LINK_TOTAL_SUPPLY) {
            revert GenericConsumer__PaymentIsGreaterThanLinkTotalSupply();
        }
    }
...
struct Entry {
    bytes32 specId; // 32 bytes - 1st 32 bytes
    address oracle; // 20 bytes
    uint96 payment; // 12 bytes - 2nd 32 bytes
    address callbackAddr; // 20 bytes
    uint96 startAt; // 12 bytes - 3rd 32 bytes
    uint96 interval; // 12 bytes
    bytes4 callbackFunctionSignature; // 4 bytes
    bool inactive; // 1 byte - 4rc 17 bytes
    bytes buffer;
}
