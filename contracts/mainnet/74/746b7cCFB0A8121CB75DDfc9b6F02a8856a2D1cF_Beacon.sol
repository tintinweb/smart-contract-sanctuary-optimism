...
import "./IBeacon.sol";
import "../library/OperatorRole.sol";
...
contract Beacon is OperatorRole, IBeacon {
    ...
    constructor(address _initialCopy) initializer {
...
contract OperatorRole is OwnableUpgradeable {
    ...
    function addOperators(address[] calldata accounts) public onlyOwner {
        for(uint256 i = 0; i<accounts.length; i++) {
            _addOperator(accounts[i]);
        }
    }
    ...
    function removeOperators(address[] calldata accounts) public onlyOwner {
        for(uint256 i = 0; i<accounts.length; i++) {
            _removeOperator(accounts[i]);
        }
    }
