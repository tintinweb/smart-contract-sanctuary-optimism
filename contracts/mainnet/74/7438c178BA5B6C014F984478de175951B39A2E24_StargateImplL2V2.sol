...
contract StargateImplL2V2 is BridgeImplBase {
    ...
    function bridgeAfterSwap(
        ...
        if (stargateBridgeData.token == NATIVE_TOKEN_ADDRESS) {
            ...
            (bool nativeSuccess, bytes memory nativeResult) = socketRoute
                .getRoute(stargateBridgeData.swapId)
                .delegatecall(stargateBridgeData.swapData);

            if (!nativeSuccess) {
                assembly {
                    revert(add(nativeResult, 32), mload(nativeResult))
                }
            }

            (uint256 valueReceived, ) = abi.decode(
                nativeResult,
                (uint256, address)
            );

            uint256 amountToTransfer = amount -
                stargateBridgeData.optionalValue;

            if (valueReceived > stargateBridgeData.value) {
                msg.sender.call{
                    value: valueReceived - stargateBridgeData.value
                }("");
            }
    ...
    function bridgeERC20To(
        ...
        _performNativeSwap(
            ...
            stargateBridgeExtraData.optionalValue
        );
        // token address might not be indication thats why passed through extraData
        ERC20 tokenInstance = ERC20(token);
        tokenInstance.safeTransferFrom(msg.sender, socketGateway, amount);
        ...
        {
            router.swap{value: stargateBridgeExtraData.optionalValue}(
                stargateBridgeExtraData.stargateDstChainId,
                stargateBridgeExtraData.srcPoolId,
                stargateBridgeExtraData.dstPoolId,
                payable(senderAddress), // default to refund to main contract
