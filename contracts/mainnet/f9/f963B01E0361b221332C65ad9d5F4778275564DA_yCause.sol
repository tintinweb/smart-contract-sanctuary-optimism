...
contract yCause {
    ...
    function stake(uint256 amount) public {
        ...
        uint shares = _yVault.deposit(amount);
        _stakedShares[msg.sender] = shares;
    ...
    function unstake() public returns (uint256 stakedAmount) {
        ...
        uint256 totalRedeemed = _yVault.withdraw();
        _stakedToken.transfer(msg.sender, stakedShares);
        if (totalRedeemed - stakedShares > 0) {
            _stakedToken.transfer(_causeAddress, totalRedeemed - stakedShares);
        }
        emit Unstaked(msg.sender, stakedShares, totalRedeemed - stakedShares);
        return stakedShares;
    }
