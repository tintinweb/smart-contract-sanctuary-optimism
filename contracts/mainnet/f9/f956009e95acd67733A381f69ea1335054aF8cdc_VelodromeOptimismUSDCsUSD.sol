...
contract VelodromeOptimismUSDCsUSD is DefiiWithCustomEnter {
    ...
    function encodeParams(uint256 slippage)
        external
        view
        returns (bytes memory)
    {
        ...
        (uint256 susdMinAmount, ) = router.getAmountOut(usdcToSwap, USDC, sUSD);
        return abi.encode(usdcToSwap, (susdMinAmount * slippage) / 1000);
    }

    function _enterWithParams(bytes memory params) internal override {
        (uint256 usdcToSwap, uint256 susdFromSwap) = abi.decode(
            params,
            (uint256, uint256)
        );
    ...
    function _enter() internal override {
        ...
        _enter(usdcBalance, susdBalance);
    }

    function _enter(uint256 usdcBalance, uint256 susdBalance) internal {
