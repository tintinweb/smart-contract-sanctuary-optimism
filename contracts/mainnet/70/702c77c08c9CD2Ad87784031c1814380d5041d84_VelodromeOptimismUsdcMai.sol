...
contract VelodromeOptimismUsdcMai is DefiiWithCustomEnter, DefiiWithCustomExit {
    IERC20 constant MAI = IERC20(0xdFA46478F9e5EA86d57387849598dbFB2e964b02);
    IERC20 constant USDC = IERC20(0x7F5c764cBc14f9669B88837ca1490cCa17c31607);
    IERC20 constant lpToken =
        IERC20(0xd62C9D8a3D4fd98b27CaaEfE3571782a3aF0a737);
    ...
    IGauge constant gauge = IGauge(0xDF479E13E71ce207CE1e58D6f342c039c3D90b7D);
        ...
        returns (bytes memory)
    {
        ...
        (uint256 maiMinAmount, ) = router.getAmountOut(usdcToSwap, USDC, MAI);
        return abi.encode(usdcToSwap, (maiMinAmount * slippage) / 1000);
    }
    ...
    function exitParams(uint256 slippage) external view returns (bytes memory) {
        ...
        // estimate price for 1,000 MAI
        (uint256 maiPrice, ) = router.getAmountOut(1000e18, MAI, USDC);
        maiPrice = maiPrice / 1000;
        return abi.encode((maiPrice * slippage) / 1000);
    }

    function _enterWithParams(bytes memory params) internal override {
        (uint256 usdcToSwap, uint256 minMaiFromSwap) = abi.decode(
            params,
            (uint256, uint256)
        );
    ...
    function _enter() internal override(Defii, DefiiWithCustomEnter) {
        ...
        _enter(usdcBalance, maiBalance);
    }

    function _enter(uint256 usdcBalance, uint256 maiBalance) internal {
        USDC.approve(address(router), usdcBalance);
        MAI.approve(address(router), maiBalance);
    ...
    function _exitWithParams(bytes memory params) internal override {
        ...
        uint256 minUsdcAmount = (maiAmount * minPrice) / 1e18;
