...
contract Account {
    ...
    function exitLong(uint256 price) public {
        ...
        do {
            ...
            if (totalToRepay > baseToken.balanceOf(address(this))) {
                pool.repay(USDC, totalToRepay, 1, address(this));
            } else {
                pool.repay(USDC, baseToken.balanceOf(address(this)), 1, address(this));
            }
            (, debtBase, , , , ) = aaveView.getUserAccountData(address(this));
        } while (debtBase > 0);
        uint256 available = aaveView.getMaxAvailableToWithdraw(address(this), WETH);
    ...
    function repay() public {
        ...
        if (availableWETH > 0) {
            ...
            pool.repay(USDC, availableUSDC, 1, address(this));
        }
        if (availableUSDC > 0) {
            IERC20(WETH).approve(address(pool), availableWETH);
            pool.repay(WETH, availableWETH, 1, address(this));
        }
    }
