...
contract HopFacet is ReentrancyGuard {
    ...
    function bridgeToHop(HopDescription calldata hDesc) external payable nonReentrant {
        LibPlexusUtil._isTokenDeposit(hDesc.srcToken, hDesc.amount);
        _hopStart(hDesc.amount, hDesc);
    }

    function swapAndBridgeToHop(SwapData calldata _swap, HopDescription calldata hDesc) external nonReentrant {
        uint256 dstAmount = LibPlexusUtil._tokenDepositAndSwap(_swap);
        _hopStart(dstAmount, hDesc);
    }

    function _hopStart(uint256 amount, HopDescription calldata hDesc) internal {
        amount = LibPlexusUtil._fee(hDesc.srcToken, amount);
        ...
        if (isNotNative) {
            ...
            IERC20(hDesc.srcToken).safeIncreaseAllowance(bridge, amount);
        }

        if (block.chainid == 1) {
            IHop(bridge).sendToL2{value: isNotNative ? 0 : amount}(
                hDesc.dstChainId,
                hDesc.recipient,
                amount,
                getSlippage(amount, hDesc.slippage),
                hDesc.deadline,
                hopRelayer(hDesc.srcToken),
                hDesc.bonderFee
            );
        } else {
            IHop(bridge).swapAndSend{value: isNotNative ? 0 : amount}(
                hDesc.dstChainId,
                hDesc.recipient,
                amount,
                hDesc.bonderFee,
        ...
        bytes32 transferId = keccak256(
            abi.encodePacked(address(this), hDesc.recipient, hDesc.srcToken, amount, hDesc.dstChainId, block.timestamp, uint64(block.chainid))
        );

        emit LibData.Bridge(hDesc.recipient, uint64(hDesc.dstChainId), hDesc.srcToken, hDesc.toDstToken, amount, transferId, "Hop");
    }
