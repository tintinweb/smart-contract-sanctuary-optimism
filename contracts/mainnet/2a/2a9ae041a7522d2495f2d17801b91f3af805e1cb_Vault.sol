...
contract Vault is Ownable {
    ...
    // TODO make vault reusable instead of having 3 different vault.sol's. set constants as immutable.
    ...
    // TODO: risks: make sure: the `DAO` address on the L2 is the same as L1.
    function _moveOutFromL2(
        ...
        token.safeIncreaseAllowance(HOP_ADDRESS, _amount);
    ...
    function _calculateAmountToMove(IERC20 token, uint256 amount) internal view returns (uint256) {
    ...
    function moveBackToDAOL1toL2(
        ...
        uint256 amountToMove = _calculateAmountToMove(token, amount);
        token.safeIncreaseAllowance(HOP_ADDRESS, amountToMove);
    ...
    function moveBackToDAOL1toL1(IERC20 token, uint256 amount) external onlyOwner {
        uint256 amountToMove = _calculateAmountToMove(token, amount);
        token.safeTransfer(DAO, amountToMove);
        emit MoveBackToDAO(msg.sender, DAO, amountToMove);
    }
    ...
    function moveBackToDAOFromL2(
        ...
        _moveOutFromL2(token, amountToMove, _bonderFee, _amountOutMin, _deadline);
        emit MoveBackToDAO(msg.sender, DAO, amountToMove);
    }

    function addLiquidity(address provider, uint256 amount) external onlyOwner {
        IERC20 _underlying = IERC20(_smartYield.underlying(provider));
        _underlying.safeApprove(address(_smartYield), amount);
        _smartYield.addLiquidity(provider, amount);
        emit Spent(msg.sender, address(_smartYield), amount);
    }

    function removeLiquidity(address provider, uint256 amount) external onlyOwner {
        _smartYield.removeLiquidity(provider, amount);
        emit Received(msg.sender, amount);
    }

    function provideRealizedYield(address bond, uint256 amount) external onlyOwner {
        address provider = _smartYield.providerByBond(bond);
        IERC20 _underlying = IERC20(_smartYield.underlying(provider));
        _underlying.safeApprove(address(_smartYield), amount);
