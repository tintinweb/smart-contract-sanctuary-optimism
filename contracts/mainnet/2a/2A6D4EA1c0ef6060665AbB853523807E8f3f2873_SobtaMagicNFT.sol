...
contract SobtaMagicNFT is ERC721AQueryable, ERC2981, Ownable, Pausable, ReentrancyGuard, Apetimism {
  ...
  uint256 public MAX_SUPPLY = 1554;
  uint256 public TEAM_NFT_COUNT = 0;
  uint256 public START_TOKEN_ID = 1;
  string private constant TOKEN_NAME = "Sobta Magic";
  string private constant TOKEN_SYMBOL = "SOBTAMAGIC";
  ...
  function maxMintableForRole(address addr, int16 role) public view virtual returns (uint256) {
    ...
    if (totalMintedInRound[currentRound] >= roundAllocations[currentRound])
      return 0;
    if (_addressTokenMintedInRole[addr][role] >= roleAllocations[role])
      return 0;
    ...
    uint256 role_quota_left = max_mint - _addressTokenMintedInRole[addr][role];
