...
import "./interfaces/IVeloVoteManager.sol";
import "./interfaces/IVeloVotingEscrow.sol";
import "./interfaces/IVeloVoter.sol";
import "./interfaces/IVeloInternalBribe.sol";
import "./interfaces/IVeloExternalBribe.sol";
import "./interfaces/IVeloRewardsDistributor.sol";
...
error VeloVoteFarmer__UpgradeCooldown();
...
contract VeloVoteFarmer is UUPSUpgradeable, AccessControlEnumerableUpgradeable, PausableUpgradeable {
    ...
    function __VeloVoteFarmer_init(address[] memory _strategists, address _multisig) internal onlyInitializing {
    ...
    function initialize(
        ...
        __VeloVoteFarmer_init(_strategists, _multisig);
    ...
    function averageAPRAcrossLastNHarvests(uint256 _n) external view returns (uint256) {
        ...
        for (uint256 i = harvestLog.length - 1; i > 0 && numLogsProcessed < _n; i--) {
            runningAPRSum += calculateAPRForLog(i-1);
            numLogsProcessed++;
        }
    ...
    function deposit(uint256 _tokenId, address _owner) external whenNotPaused {
        _onlyManager();
    ...
    function withdraw(uint256 _withdrawTokenId, address _owner) external {
        _onlyManager();
    ...
    function increaseDurationAll() public {
        _onlyStrategist();
    ...
    function claimFees() public whenNotPaused {
        _onlyStrategist();
        for (uint256 i; i < tokenIds.length; i = _uncheckedInc(i)) {
    ...
    function resetBalances(uint256[] memory _tokenIds) external {
        _onlyStrategist();
        for (uint256 i; i < rewards.length(); i = _uncheckedInc(i)) {
            for (uint256 j; j < _tokenIds.length; j = _uncheckedInc(j)) {
                tokenToRewardInfo[rewards.at(i)].totalReceived -= tokenIdToInfo[_tokenIds[j]].tokenToRewardAmount[
                    rewards.at(i)
                ];
                tokenIdToInfo[_tokenIds[j]].tokenToRewardAmount[rewards.at(i)] = 0;
            }
        }
    }
    ...
    function updateSwapPath(address[] memory _path) external {
        ...
        if (_path.length < 2 || _path[0] == _path[_path.length - 1]) {
            revert VeloVoteFarmer__WrongInput();
        }
    ...
    function synchronize() external {
        ...
        for(uint256 i; i < gaugesLength; i = _uncheckedInc(i)) {
            gauges[i] = manager.gauges(i);
            weights[i] = manager.weights(i);
        }
    ...
    function _claimFees(uint256 _tokenId) internal {
        ...
        for (uint256 i; i < gauges.length; i = _uncheckedInc(i)) {
            ...
            address eBribe = VELODROME_VOTER.external_bribes(gauges[i]); //bribes
    ...
    function _increaseDuration(uint256 _tokenId) internal {
        uint256 newUnlockTime = IVeloVotingEscrow(VEVELO).locked__end(_tokenId) + EPOCH;
    ...
    function _removeRevokedNfts() internal {
        for (uint256 i; i < tokenIds.length;  i = _uncheckedInc(i)) {
            if (!IVeloVotingEscrow(VEVELO).isApprovedOrOwner(address(this), tokenIds[i])) {
    ...
    function _requireCooldownOver() internal view {
        if (upgradeProposalTime + UPGRADE_TIMELOCK > block.timestamp) {
            revert VeloVoteFarmer__UpgradeCooldown();
        }
    }

    function _onlyManager() internal view {
        if (!hasRole(MANAGER, msg.sender) && !hasRole(DEFAULT_ADMIN_ROLE, msg.sender)) {
            revert VeloVoteFarmer__Unauthorized();
        }
    }

    function _onlyStrategist() internal view {
        if (!hasRole(STRATEGIST, msg.sender) && !hasRole(DEFAULT_ADMIN_ROLE, msg.sender)) {
            revert VeloVoteFarmer__Unauthorized();
        }
    }
