    ...
    ReentrancyGuardUpgradeable
{
    ...
    function fulfillOrder(Order calldata _order) external payable whenNotPaused nonReentrant {
        ...
        uint256 reward = rewardDeriver.getRewardInOP(_order);
    ...
    function fulfillAvailableOrders(
        ...
        for (uint256 i = 0; i < availableOrders.length; i++) {
            ...
            if (availableOrders[i] == true) {
                ...
                reward += rewardDeriver.getRewardInOP(order);
    ...
    /**
     * @notice Calls the RewardDeriver to get the OP token reward amount for the given order.
     *
     * @param _order Seaport order struct.
     *
     * @return OP token reward amount.
     */
    function getRewardInOP(Order memory _order) external view returns (uint256) {
        return rewardDeriver.getRewardInOP(_order);
    }
