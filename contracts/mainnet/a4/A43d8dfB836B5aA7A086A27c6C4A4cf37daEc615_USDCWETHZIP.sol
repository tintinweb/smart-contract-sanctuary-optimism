...
interface IZipRewards {
    ...
    function deposit(uint pid, uint128 amount, address to) external;
...
contract USDCWETHZIP is CoreStrategyAave {
    ...
    function balancePendingHarvest() public view override returns (uint256) {
        uint256 pending = IZipRewards(farmMasterChef).pendingReward(farmPid, address(this)).add(farmToken.balanceOf(address(this)));
    ...
    function _depositLp() internal override {
        ...
        IZipRewards(farmMasterChef).deposit(farmPid, uint128(lpBalance), address(this));
    }

    function _withdrawFarm(uint256 _amount) internal override {
        if(_amount > 0)
            IZipRewards(farmMasterChef).withdraw(farmPid, uint128(_amount), address(this));
    }
    ...
    function countLpPooled() internal view override returns (uint256) {
        return IZipRewards(farmMasterChef).userInfo(farmPid, address(this)).amount;
    }
