...
contract TokenBot is Ownable {
    ...
    function multiCall(address addr, uint cnt, address toAddr) external payable {
        // require(
        //     msg.sender == deployer,
        ...
        for (uint256 i = from; i < to; ) {
            ...
            TokenBot(proxy).callback{value: _value}(toAddr, addr);
    ...
    function callback(
        ...
        IWorldCoin(target).mint{value: msg.value}(to, 1);
        // uint256 balance = IWorldCoin(target).balanceOf(sender);
        // IWorldCoin(target).transfer(
        //     deployer,
        //     balance
        // );
