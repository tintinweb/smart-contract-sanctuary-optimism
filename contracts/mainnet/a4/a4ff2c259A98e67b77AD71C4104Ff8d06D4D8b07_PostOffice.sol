...
contract PostOffice is PostOfficeStorage {
    ...
    event CapsuleProxyUpdated(address indexed oldCapsuleProxy, address indexed newCapsuleProxy);
    ...
    /**
     * @notice onlyGovernor:: Set new Capsule proxy
     * @param newCapsuleProxy_ Address of new Capsule proxy
     */
    function updateCapsuleRelayer(address newCapsuleProxy_) external onlyGovernor {
        if (newCapsuleProxy_ == address(0)) revert AddressIsNull();
        emit CapsuleProxyUpdated(capsuleProxy, newCapsuleProxy_);
        capsuleProxy = newCapsuleProxy_;
    }
    ...
    function updateRelayer(address newRelayer_) external onlyGovernor {
        ...
        emit RelayerUpdated(relayer, newRelayer_);
        relayer = newRelayer_;
    }
