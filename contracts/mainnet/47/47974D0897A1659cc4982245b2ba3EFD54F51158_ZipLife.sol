...
contract ZipLife is BaseStrategy {
    ...
    uint128 private minZipToSell;
    uint128 private maxSingleInvest;

    constructor(address _vault, uint256 _poolId) public BaseStrategy(_vault) {
        // Zipswap router
        zipRouter = 0xE6Df0BB08e5A97b40B21950a0A51b94c4DbA0Ff6;
        poolId = _poolId;
        ...
        want.approve(zipChef, uint256(-1));

        minWant = 1000; // TODO: set minWant
        maxSingleInvest = 100000000000000000000;
        minZipToSell = 10000000000000;
    ...
    function balanceOfPool() internal view returns (uint256) {
        (uint256 _amount, ) = IZipChef(zipChef).userInfo(poolId, address(this));
        return _amount;
    }
    ...
    function getTokenOutPathV2(address _tokenIn, address _tokenOut) internal view returns (address[] memory _path) {
        // expect zip in, tokenOut make not be liquid trading partner. max path size is 3.

        bool isLiquid = _tokenOut == weth;
    ...
    function harvester() public {
        ...
        uint256 _zip = balanceOfToken(zip);
        ...
        if (_zip > 0) {
            ...
            if (zip != token0) {
                IUniswapV2Router02(zipRouter).swapExactTokensForTokens(
                    toToken0,
                    1,
                    getTokenOutPathV2(zip, token0),
                    address(this),
                    block.timestamp
                );
            }
            ...
            uint256 _token0 = balanceOfToken(token0);
            uint256 _token1 = balanceOfToken(token1);

            if (_token0 > 0 && _token1 > 0) {
                IUniswapV2Router02(zipRouter).addLiquidity(token0, token1, _token0, _token1, 0, 0, address(this), now + 60);

                // Donates DUST
                if (balanceOfToken(token0) > 0) IERC20(token0).transfer(rewards, balanceOfToken(token0));
                if (balanceOfToken(token1) > 0) IERC20(token1).safeTransfer(rewards, balanceOfToken(token1));
            }
        }
    }
    ...
    function depositSome(uint128 _amount) internal {
        uint256 bal = balanceOfToken(address(want));
        if (bal >= _amount) {
            IZipChef(zipChef).deposit(poolId, _amount, address(this));
        }
    }
    ...
    function adjustPosition(uint256 _debtOutstanding) internal override {
        ...
        // totally safe im pretty sure...
        uint128 toInvest = uint128(_wantBal.sub(_debtOutstanding));

        uint128 _wantToInvest = uint128(Math.min(toInvest, maxSingleInvest));
        // deposit and stake
    ...
    function liquidatePosition(uint256 _amountNeeded) internal override returns (uint256 _liquidatedAmount, uint256 _loss) {
        ...
        if (_liquidatedAmount > 0) {
            uint256 wantBal = balanceOfToken(address(want));
            if (_liquidatedAmount > wantBal) withdrawSome(_liquidatedAmount.sub(wantBal));
        }

        _liquidatedAmount = balanceOfToken(address(want)); // TODO: withdrawSome occationally leaves dust if trying to withdraw 100% liquidity
    ...
    function priceCheck(
        ...
        uint256[] memory amounts = IUniswapV2Router02(zipRouter).getAmountsOut(_amount, getTokenOutPathV2(start, end));
...
interface IZipChef {
    ...
    function userInfo(uint256, address) external view returns (uint128 amount, int128 rewardDebt);
