...
contract DarkCryptoNFT is Context, AccessControlEnumerable, ERC721Enumerable {
    using Counters for Counters.Counter;

    uint256 public constant MIN_ID = 1000000;
    uint256 public constant MAX_ID = 6000000;
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");

    Counters.Counter private _tokenIdTracker;

    uint256[6] public currentIdLevel;
    ...
    constructor(string memory name, string memory symbol, string memory baseTokenURI) ERC721(name, symbol) {
        ...
        currentIdLevel = [0, 1000000, 2000000, 3000000, 4000000, 5000000];
    }
    ...
    function getTokenLevel(uint256 tokenId) external view returns (uint256) {
        ...
        return currentNFTDetails.level;
    }

    function getCurrentIdLevel(uint256 level) external view returns (uint256) {
        return currentIdLevel[level];
    }
    ...
    function mint(
        ...
        require(tokenId >= MIN_ID && tokenId < MAX_ID, "invalid id");
        uint256 mintedLevel = tokenId / MIN_ID;
        require(level == mintedLevel, "invalid level");
        currentIdLevel[mintedLevel]++;
        _tokenIdTracker.increment();
        _mint(to, tokenId);
        getNFTDetailsById[tokenId] = NFTDetails(level);
    }
