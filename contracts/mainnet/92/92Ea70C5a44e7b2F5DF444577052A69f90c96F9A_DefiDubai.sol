...
contract DefiDubai {
    ...
    uint256 public hasNextMeetupTimestamp;
    ...
    constructor() payable {
        ...
        nextMeetupTimestamp = 1643494842;
        maxAttendees = 15;
    ...
    function setNextMeetupAll(uint256 max, string memory url) public {
        setMaxAttendees(max);
        resetAttendees();
        setMeetupUrl(url);
        attendees.reset();
    ...
    function whereNextMeetupAgainSir() public view returns (string memory) {
        if (attendeesAddr.contains(address(msg.sender))) {
    ...
    function rsvp() public returns (string memory) {
        if (attendees.current() == maxAttendees) {
            return "sorry habibi, meetup is already full, maybe next time :)";
        }
        if (attendeesAddr.contains(address(msg.sender))) {
            return "hold on habibi, you've already rsvp'ed";
        }
