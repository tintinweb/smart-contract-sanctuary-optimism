...
@title Curve aPool
@author Curve.Fi
@license Copyright (c) Curve.Fi, 2022 - all rights reserved
...
interface RewardsController:
    def claimAllRewards(_atokens: DynArray[address, N_COINS], _receiver: address): nonpayable

interface LendingPool:
    def supply(_underlying_asset: address, _amount: uint256, _on_behalf_of: address, _referral_code: uint16): nonpayable
...
AAVE_REWARDS: constant(address) = 0x929EC64c34a17401F460460D4B9390518E5B473e
AAVE_LENDING_POOL: constant(address) = 0x794a61358D6845594F94dc1DB02A252b5b4814aD
...
@internal
def _claim_rewards():
    # push rewards into the reward receiver
    reward_receiver: address = self.reward_receiver
    if reward_receiver != ZERO_ADDRESS:
        coins: DynArray[address, N_COINS] = []
        for coin in self.coins:
            coins.append(coin)
        RewardsController(AAVE_REWARDS).claimAllRewards(coins, reward_receiver)
    ...
    if _use_underlying:
        ...
        for i in range(N_COINS):
            ...
            if amount != 0:
                ...
                LendingPool(AAVE_LENDING_POOL).supply(coin, amount, self, self.aave_referral)
    else:
        for i in range(N_COINS):
            amount: uint256 = _amounts[i]
            if amount != 0:
...
def get_y(i: int128, j: int128, x: uint256, xp: uint256[N_COINS]) -> uint256:
    ...
    for __i in range(N_COINS):
        _i: int128 = convert(__i, int128)
        if _i == i:
            _x = x
        elif _i != j:
            _x = xp[_i]
...
def exchange_underlying(i: int128, j: int128, dx: uint256, min_dy: uint256) -> uint256:
    ...
    # deposit to aave lending pool
    LendingPool(AAVE_LENDING_POOL).supply(u_coin_i, dx, self, self.aave_referral)
...
@external
def set_aave_referral(referral_code: uint16):
    assert msg.sender == self.owner  # dev: only owner
