...
abstract contract CommonArbitrageur is SafeOwnableNonUpgradable, ReentrancyGuard {
    ...
    address internal immutable _uniswapV3Router;
    ...
    function _swapOnUni(
        ...
        return
            UniswapV3Facade.exactInputSingle(
                _uniswapV3Router,
...
contract KtbArbitrageur is CommonArbitrageur {
    ...
    ) CommonArbitrageur(uniswapV3RouterArg, usdcUsdChainlinkAggregatorArg, whitelistedCallerArg) {
    ...
    function arbitrage(
        ...
        emit Arbitrage(kantabanVault, tokenIn, tokenOut, amountIn, isToBuyOnKtb, profit, uniswapV3PoolFeeTier);

        return profit;
    }

    function deleverage(address kantabanVault, uint256 reducedPositionSizeAbs) external nonReentrant {
        // CA_IC: Invalid Caller
        require(_msgSender() == _whitelistedCaller, "CA_IC");
        ICommonVault(kantabanVault).deleverage(reducedPositionSizeAbs);
    }
    ...
    function _swapOnOneInch(
        ...
        (bool success, bytes memory data) = _oneInchAggregationRouterV5.call{ value: 0 }(oneInchTxData);
        if (!success) {
...
library UniswapV3Facade {
    ...
    // base TWAP in quote and quote's decimals
    function getTwap(
        address factory,
        address baseToken,
        address quoteToken,
        ...
        uint160 sqrtMarketTwapX96 = UniswapV3Broker.getSqrtMarketTwapX96(pool, twapInterval);
        uint8 baseTokenDecimal = IERC20Metadata(baseToken).decimals();
        if (baseToken < quoteToken) {
            // baseToken is token0
            return token0Price(sqrtMarketTwapX96, baseTokenDecimal);
        }
        // baseToken is token1
        return token1Price(sqrtMarketTwapX96, baseTokenDecimal);
    }
