...
contract FloorDropMintpass is ERC1155Drop {
    ...
    address public constant SECOND_ADMIN = 0x48CD130949880D951C3846F653DdCbDd28a7A6f1;
    ...
    /// TODO: verify this function works (it was auto-genereated)
    function airdrop(
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            ...
            _beforeClaim(tokenId, receiver, quantity, _currency, _pricePerToken, _allowlistProof, _data);

            // ClaimCondition memory condition = claimCondition[tokenId];
            // bytes32 activeConditionId = conditionId[tokenId];

            // verifyClaim(tokenId, _dropMsgSender(), quantity, _currency, _pricePerToken, _allowlistProof);

            // // Update contract state.
            // condition.supplyClaimed += quantity;
            // supplyClaimedByWallet[activeConditionId][_dropMsgSender()] += quantity;
            // claimCondition[tokenId] = condition;

            // // If there's a price, collect price.
            // address artistRecipient = primarySaleRecipient[tokenId];
            // _collectPriceOnClaim(artistRecipient, quantity, _currency, _pricePerToken);
            ...
            _afterClaim(tokenId, receiver, quantity, _currency, _pricePerToken, _allowlistProof, _data);
        }
    }
