...
// File: contracts/NFTELaunchpad.sol
...
contract NFTELaunchpad is ERC721A, Ownable, ReentrancyGuard {
    enum MintType{ RESERVED, PRESALE, PUBLIC }
    ...
    mapping(MintType => bool) public activeSale;
    ...
    constructor(InitialConfig memory config) ERC721A(config.name, config.symbol) {
        _URI = config.URI;
        signer = config.signer;
        maxPerWallet = config.maxPerWallet;
        maxSupply = config.maxSupply;
        supply[MintType.RESERVED] = config.reservedSupply;
        supply[MintType.PRESALE] = config.presaleSupply;
        supply[MintType.PUBLIC] = config.publicSupply;
        presalePrice = config.presalePrice;
        publicPrice = config.publicPrice;
    }

    function setActiveSale(bool enablePresale, bool enablePublic) external onlyOwner {
        activeSale[MintType.PRESALE] = enablePresale;
        activeSale[MintType.PUBLIC] = enablePublic;
    }
    ...
    function adjustSupply(MintType _type, uint256 _newSupply) external onlyOwner {
        ...
        maxSupply = supply[MintType.RESERVED] + supply[MintType.PRESALE] + supply[MintType.PUBLIC];
    }
    ...
    function reservedMint(address receiver, uint256 amount)
    external
    validateSupply(MintType.RESERVED, amount)
    onlyOwner
    {
        minted[MintType.RESERVED] += amount;
        _mint(receiver, amount);
        emit Minted(receiver, amount, MintType.RESERVED);
    }

    function preSaleMint(bytes calldata signature, uint256 amount)
    external
    payable
    validateMint(MintType.PRESALE, amount)
    ...
    modifier validateSupply(MintType _type, uint256 amount) {
        ...
        require(totalSupply() + amount < maxSupply + 1, "Mint would exceed max supply");
        _;
    }

    modifier validateMint(MintType _type, uint256 amount) {
        uint256 price = _type == MintType.PRESALE ? presalePrice : publicPrice;
        require(activeSale[_type], "Sale is not active");
        require(walletMint[msg.sender] + amount < maxPerWallet + 1, "Reached max mint per wallet");
