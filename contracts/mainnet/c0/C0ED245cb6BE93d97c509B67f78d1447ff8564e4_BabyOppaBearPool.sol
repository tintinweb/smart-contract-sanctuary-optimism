...
contract BabyOppaBearPool is Ownable, ReentrancyGuard {
    ...
    uint256 public COMMON_BOOST = 0.5 * 10**18;
    uint256 public RARE_BOOST = 1 * 10**18;
    uint256 public SUPER_RARE_BOOST = 2 * 10**18;
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            ...
            if (typeNFT == TypeNFT.COMMON) {
                vault[tokenId] = Staker({
                    ...
                    typeNFT: typeNFT,
                    rewardDebt: commonReward
                });
            } else if (typeNFT == TypeNFT.RARE) {
                vault[tokenId] = Staker({
                    tokenId: uint256(tokenId),
                    owner: msg.sender,
                    stakeAt: uint256(block.timestamp),
    ...
    function unstake(uint256[] calldata _tokenIds) external nonReentrant {
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            ...
            if (staked.typeNFT == TypeNFT.COMMON) {
                if (reward > 0) {
                    safeOPBTokenTransfer(msg.sender, reward);
                }
                staked.rewardDebt = commonReward;
            } else if (staked.typeNFT == TypeNFT.RARE) {
                if (reward > 0) {
                    safeOPBTokenTransfer(msg.sender, reward);
                }
                staked.rewardDebt = rareReward;
            } else {
                if (reward > 0) {
                    safeOPBTokenTransfer(msg.sender, reward);
                }
                staked.rewardDebt = superRareReward;
            }
    ...
    //Admin call
    function emergencyWithdrawAdmin(uint256[] calldata _tokenIds)
        external
        nonReentrant
        onlyOwner
    {
    ...
    function updatePool() public {
        ...
        (uint256 cm, uint256 r, uint256 sr) = calculateReward(
            common,
            rare,
            superRare
        );

        commonReward = commonReward.add(cm);
        rareReward = rareReward.add(r);
        superRareReward = superRareReward.add(sr);
        ...
        )
    {
        ...
        uint256 boostCommon = cm.mul(COMMON_BOOST).div(1e18);
        uint256 boostRare = r.mul(RARE_BOOST).div(1e18);
        uint256 boostSuperRare = sr.mul(SUPER_RARE_BOOST).div(1e18);

        uint256 totalBoost = boostCommon + boostRare + boostSuperRare;

        rewardCommonPerNFT = reward.div(totalBoost);

        rewardRarePerNFT = rewardCommonPerNFT * RARE_BOOST.div(1e18);

        rewardSuperRarePerNFT = rewardCommonPerNFT * SUPER_RARE_BOOST.div(1e18);
    }
        ...
        returns (uint256 totalReward, uint256 avaliableReward)
    {
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            ...
            if (staked.typeNFT == TypeNFT.COMMON) {
                uint256 pending = commonReward.sub(staked.rewardDebt);

                if (staked.timeCanUnstake <= block.timestamp) {
                    totalReward += pending;
                    avaliableReward += pending;
                } else {
                    totalReward += pending;
                }
            } else if (staked.typeNFT == TypeNFT.RARE) {
                uint256 pending = rareReward.sub(staked.rewardDebt);
                ...
                uint256 pending = superRareReward.sub(staked.rewardDebt);
        ...
        returns (uint256 totalReward, uint256 avaliableReward)
    {
        ...
        if (user.typeNFT == TypeNFT.COMMON) {
            uint256 pending = commonReward.sub(user.rewardDebt);
        ...
        } else if (user.typeNFT == TypeNFT.RARE) {
            uint256 pending = rareReward.sub(user.rewardDebt);
            if (user.timeCanUnstake <= block.timestamp) {
                totalReward += pending;
                avaliableReward += pending;
            } else {
            ...
            uint256 pending = superRareReward.sub(user.rewardDebt);
    ...
    // Admin Function
    function getBackReward() external onlyOwner {
