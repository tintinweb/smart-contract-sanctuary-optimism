...
contract StrategyExactlySupply is StratFeeManagerInitializable {
    ...
    bytes[] public rewardToNativePaths;
    ...
    function chargeFees(address callFeeRecipient) internal {
        ...
        for (uint i; i < rewards.length;) {
            ...
            if (tokenBal > 0) {
                UniswapV3Utils.swap(unirouter, rewardToNativePaths[i], tokenBal);
                unchecked { ++i; }
            }
        }
    ...
    function addReward(address[] calldata _route, uint24[] calldata _fees) external onlyOwner {
        address token = _route[0];
        require(token != want && token != output, "Reward is want or output");
        IERC20(token).safeApprove(unirouter, type(uint).max);
        rewards.push(token);
        rewardToNativePaths.push(UniswapV3Utils.routeToPath(_route, _fees));
    }
