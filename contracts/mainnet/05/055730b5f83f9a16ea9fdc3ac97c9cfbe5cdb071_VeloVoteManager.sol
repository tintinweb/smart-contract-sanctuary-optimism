...
// File contracts/interfaces/IVeloVoteFarmer.sol
...
interface IVeloVoteFarmer {
    ...
    function removeRevokedNfts() external returns(bool);
    function voteFor(uint256, address[] memory pairs) external;
}
...
// File contracts/VeloVoteManager.sol
...
error VeloVoteManager__UpgradeCooldown();
error VeloVoteManager__Unauthorized();
error VeloVoteManager__WrongInput();

contract VeloVoteManager is UUPSUpgradeable, AccessControlEnumerableUpgradeable, PausableUpgradeable {
    using EnumerableSetUpgradeable for EnumerableSetUpgradeable.AddressSet;
    /* ========== STATE VARIABLES ========== */
    /// @dev Role
    ...
    IVeloVotingEscrow public constant VEVELO = IVeloVotingEscrow(0x9c7305eb78a432ced5C4D14Cac27E8Ed569A2e26);
    ...
    /// {HarvestedFarmers} Event that is fired each time harvests are processed.
    /// {VotedFarmers} Event that is fired each time a vote goes through farmers.
    /// {VotedNfts} Event that is fired each time a vote goes through nfts directly.
    event TotalFeeUpdated(uint256 newFee);
    event SwapPathUpdated(address from, address to);
    event HarvestedFarmers(uint256 start, uint256 end);
    event VotedFarmers(uint256 start, uint256 end);
    event VotedNfts(uint256[] tokenIds, address[] pairs);

    /* ========== CONSTRUCTOR ========== */
    /// @custom:oz-upgrades-unsafe-allow constructor
    constructor() initializer {}

    function __VeloVoteManager_init(address[] memory _strategists) internal onlyInitializing {
    ...
    function deposit(uint256 _tokenId) external whenNotPaused {
        ...
        //     revert VeloVoteManager__WrongInput();
        // }
        if (managingStrategyFor(_tokenId) != address(0)) {
            revert VeloVoteManager__WrongInput();
        }
        ...
        IVeloVoteFarmer(strategy).deposit(_tokenId, msg.sender);
        // tokenIdToStrat[_tokenId] = strategy;
    }

    function withdraw(uint256 _tokenId) external {
        // address strategy = tokenIdToStrat[_tokenId];
        address strategy = managingStrategyFor(_tokenId);
        IVeloVoteFarmer(strategy).withdraw(_tokenId, msg.sender);
    ...
    function autoLock(uint256 _tokenId, bool _enable) external {
        // address strategy = tokenIdToStrat[_tokenId];
        address strategy = managingStrategyFor(_tokenId);
        IVeloVoteFarmer(strategy).autoLock(_tokenId, _enable, msg.sender);
    }
    ...
    function vote(uint256 start, uint256 end) external {
        ...
        for (uint256 i = start; i < end; i = _uncheckedInc(i)) {
            IVeloVoteFarmer(strategies.at(i)).vote();
        }
        emit VotedFarmers(start, end);
    }
    ...
    function harvest(uint256 start, uint256 end) external {
        ...
        // Harvest
        for (uint256 i = start; i < end; i = _uncheckedInc(i)) {
            IVeloVoteFarmer(strategies.at(i)).harvest();
        }
        emit HarvestedFarmers(start, end);
    }
    ...
    function resetBalancesAll(uint256 start, uint256 end) external {
        ...
        // Reset balances
        for (uint256 i = start; i < end; i = _uncheckedInc(i)) {
            IVeloVoteFarmer(strategies.at(i)).resetBalancesAll();
        }
    }

    function refreshTokenIdToStrat() external {
        _atLeastRole(KEEPER);
        for (uint256 i; i < strategies.length(); i = _uncheckedInc(i)) {
            uint256 tokenIdsLen = IVeloVoteFarmer(strategies.at(i)).tokenIdsLength();
            for (uint256 j; j < tokenIdsLen; j = _uncheckedInc(j)) {
                uint256 tokenId = IVeloVoteFarmer(strategies.at(i)).tokenIds(j);
                tokenIdToStrat[tokenId] = strategies.at(i);
            }
        }
    }

    function voteForTokens(uint256[] memory tokenIds, address[] memory pairs) external {
        _atLeastRole(KEEPER);
        for (uint256 i; i < tokenIds.length; i = _uncheckedInc(i)) {
            IVeloVoteFarmer(tokenIdToStrat[tokenIds[i]]).voteFor(tokenIds[i], pairs);
        }
        emit VotedNfts(tokenIds, pairs);
    }
    ...
    function removeRevokedNfts(uint256 start, uint256 end) external {
        _atLeastRole(DEFAULT_ADMIN_ROLE);
        _verifySlice(start, end);

        for (uint256 i = start; i < end; i = _uncheckedInc(i)) {
            IVeloVoteFarmer(strategies.at(i)).removeRevokedNfts();
        }
    ...
    function selectDepositStrategy() public view returns (address depositStrategy, bool strategiesFull) {
        ...
        for (uint256 i; i < stratLen; i = _uncheckedInc(i)) {
            if (IVeloVoteFarmer(strategies.at(i)).tokenIdsLength() < nftCap) {
                depositStrategy = strategies.at(i);
                if ((i == stratLen - 1) && (IVeloVoteFarmer(strategies.at(i)).tokenIdsLength() == nftCap - 1)) {
                    strategiesFull = true;
                }
                break;
            }
    ...
    function _deployStrategy() internal returns (address strategy) {
        ...
        IVeloVoteFarmer(strategy).initialize();
    ...
    function _synchronizeStrategies(uint256 start, uint256 end) internal {
        _verifySlice(start, end);
        for(uint256 i = start; i < end; i = _uncheckedInc(i)) {
            IVeloVoteFarmer(strategies.at(i)).synchronize();
        }
    }
    ...
    function managingStrategyFor(uint256 tokenId) internal view returns (address strategy) {
        for (uint256 i; i < strategies.length(); i = _uncheckedInc(i)) {
            uint256 tokenIdsLen = IVeloVoteFarmer(strategies.at(i)).tokenIdsLength();
            for (uint256 j; j < tokenIdsLen; j = _uncheckedInc(j)) {
                if (IVeloVoteFarmer(strategies.at(i)).tokenIds(j) == tokenId) {
    ...
    function averageAPRAcrossLastNHarvests(uint256 _n) public view returns (uint256 yearly) {
        ...
        for (uint256 i; i < strategies.length(); i = _uncheckedInc(i)) {
            uint256 hLen = IVeloVoteFarmer(strategies.at(i)).harvestLogLength();
            if (hLen >= _n) {
                yearly += IVeloVoteFarmer(strategies.at(i)).averageAPRAcrossLastNHarvests(_n);
    ...
    function _requireCooldownOver() internal view {
        if (upgradeProposalTime + UPGRADE_TIMELOCK > block.timestamp) {
            revert VeloVoteManager__UpgradeCooldown();
        }
    }
    ...
    function _atLeastRole(bytes32 role) internal view {
        ...
        if (!specifiedRoleFound || !senderHighestRoleFound) {
            revert VeloVoteManager__Unauthorized();
        }
    }
    ...
    function pause(uint256 start, uint256 end) external {
        ...
        _pause();
        for (uint256 i = start; i < end; i = _uncheckedInc(i)) {
            IVeloVoteFarmer(strategies.at(i)).pause();
        }
    }
    ...
    function unpause(uint256 start, uint256 end) external {
        ...
        _unpause();
        for (uint256 i = start; i < end; i = _uncheckedInc(i)) {
            IVeloVoteFarmer(strategies.at(i)).unpause();
        }
    }
}