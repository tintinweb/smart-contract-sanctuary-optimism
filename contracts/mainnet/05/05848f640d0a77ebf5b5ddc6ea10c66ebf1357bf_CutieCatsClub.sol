...
//-------------------- PublicMint
    function publicMint(uint256 _mintAmount) external payable callerIsUser {
        ...
        require(mintPhase == 5, "public sale has not begun yet");
...
//-------------------- WhitelistMint
    ...
    function mintWhiteList(uint256 _mintAmount , bytes32[] memory _Proof) external payable {
        require(mintPhase == 1, "Whitelist status, must be active to mint");
...
//-------------------- AllowlistMint
    ...
    function mintAllowList(uint256 _mintAmount , bytes32[] memory _Proof) external payable {
        require(mintPhase == 3, "Allowlist status, must be active to mint");
        ...
        require(numberMinted(msg.sender) + _mintAmount <= maxPerAllowlist, "It's max per address for allowlist. Please try lower amount to mint");
...
//------------------------- Withdraw Money
    ...
    function withdrawMoney() external payable nonReentrant { 
        ...
        uint256 _paytoW1 = address(this).balance*40/100 ; // K.pim
        uint256 _paytoW2 = address(this).balance*30/100 ; // K.Chris
        uint256 _paytoW3 = address(this).balance*30/100 ; // K.Kung
