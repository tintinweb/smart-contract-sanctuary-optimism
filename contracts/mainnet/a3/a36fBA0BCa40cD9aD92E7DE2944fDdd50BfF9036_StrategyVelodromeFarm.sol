...
contract StrategyVelodromeFarm is BaseFarmStrategy {
    ...
    function setRoutes(
        IERC20 tokenIn,
        IERC20 tokenOut,
        IRouterVelodrome.route[] calldata routesList
    ) external onlyOwner {
    ...
    function _deposit(
        ...
        (uint256 amount0, uint256 amount1, ) = VELODROME_ROUTER
            .quoteAddLiquidity(
                address(token0),
                address(token1),
                true,
        ...
        // Rebalance tokens shares
        if (amount0 == tokenBal / 2) {
            amount0 *= 10 ** 12;
            amount0 = (amount0 + ((amount0 - amount1) / 2)) / (10 ** 12);
            (amount0, amount1, ) = VELODROME_ROUTER.quoteAddLiquidity(
                address(token0),
                address(token1),
                true,
                amount0,
            ...
            amount1 = (amount1 + ((amount1 - (amount0 * 10 ** 12)) / 2));
            (amount0, amount1, ) = VELODROME_ROUTER.quoteAddLiquidity(
                address(token0),
                address(token1),
                true,
