...
import "../strategy/Math.sol";

contract PerpTest is Initializable, UUPSUpgradeable, OwnableUpgradeable {

    IClearingHouse public _perp;
    address public _wToken; // wrapped Native Token
    address public _vToken; /// Native Strategy Perp Token
    ...
    function getDailyMarketTwap(uint32 interval) public view returns (uint256) {
    ...
    // Price of underlying perp asset [1e18]
    function nativeStrategyTokenPrice() public view returns (uint256) {// wBTC, wETH or other
        ...
        return IIndexPrice(_vToken).getIndexPrice(config.getTwapInterval());
    }

    function fundingRate() public view returns (uint256) {// wBTC, wETH or other
        uint32 interval = IClearingHouseConfig(_perp.getClearingHouseConfig()).getTwapInterval();

        return Math.mulDiv(getDailyMarketTwap(interval) - nativeStrategyTokenPrice(), 1e18, nativeStrategyTokenPrice());
    }
}
