...
contract SecureExecutor is Ownable {
	...
	function setPeripheralPool(address _peripheralPool) external onlyOwner {
	...
	function isValidSignature(bytes32 messageHash, bytes memory signature) internal view {
	...
	function getEthSignedParamsHash(bytes32 _paramsHash) public pure returns (bytes32) {
	...
	function executeTransfer(Utils.Iparams memory params, bytes memory signature) external {
		...
		isValidSignature(getParamsHash(params), signature);

		signatureOracle[signature] = true;

		intentExecuted[params.txHash] = true;

		IPeripheralPool(peripheralPool).transferTo(params.tokenAddress, params.toAddress, params.amount);
	}
}
