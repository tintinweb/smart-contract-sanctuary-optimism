/**
 *Submitted for verification at Optimistic.Etherscan.io on 2022-10-10
*/
...
// File: contracts/BIFI/strategies/Beethovenx/StrategyBeetsComposableMultiRewardGaugeUniV3.sol
...
contract StrategyBeetsComposableMultiRewardGaugeUniV3 is StratFeeManager {
    ...
    // Tokens used
    address public want;
    address public output = address(0xFE8B128bA8C78aabC59d4c64cEE7fF28e9379921);
    address public native = address(0x4200000000000000000000000000000000000006);
    address public input;
    bool public inputIsComposable;
    ...
    constructor(
        ...
        inputIsComposable = _inputIsComposable;
        ...
        for (uint i; i < rewards.length; i++) {
            ...
            if (bal >= rewards[i].minAmount) {
                if (rewards[i].rewardSwapPoolId != bytes32(0)) {
                    if (rewards[i].hopPoolId != bytes32(0)) {
                        balancerSwap(rewards[i].rewardSwapPoolId, rewards[i].token, rewards[i].extraHop, bal);
                        uint256 hopBal = IERC20(rewards[i].extraHop).balanceOf(address(this));
                        balancerSwap(rewards[i].hopPoolId, rewards[i].extraHop, native, hopBal);
                    } else {
                        balancerSwap(rewards[i].rewardSwapPoolId, rewards[i].token, native, bal);
                    }
                } else {
                    _swapV3(rewards[i].routeToNative, bal);
                }
            }
        }
    }
    ...
    function callReward() public returns (uint256) {
     ...
     function addRewardToken(address _token, bytes32 _rewardSwapPoolId, address _hopToken, bytes32 _extraHop, bytes calldata _routeToNative, uint _minAmount) external onlyOwner {
        ...
        if (_rewardSwapPoolId != bytes32(0)) {
            ...
            if (_extraHop != bytes32(0)) {
                IERC20(_hopToken).safeApprove(unirouter, 0);
                IERC20(_hopToken).safeApprove(unirouter, type(uint).max);
            }
        } else {
        ...
        rewards.push(Reward(_token, _rewardSwapPoolId, _hopToken, _extraHop, _routeToNative, _minAmount));
    }
            ...
            for (uint i; i < rewards.length; ++i) {
                if (rewards[i].rewardSwapPoolId != bytes32(0)) {
                    IERC20(rewards[i].token).safeApprove(unirouter, type(uint).max);
                     if (rewards[i].hopPoolId != bytes32(0)){
                        IERC20(rewards[i].extraHop).safeApprove(unirouter, type(uint).max);
                    }
                } else {
                    IERC20(rewards[i].token).safeApprove(uniswapRouter, type(uint).max);
                }
            }
        }

        if (!inputIsComposable) {
        ...
        if (rewards.length != 0) { 
            for (uint i; i < rewards.length; ++i) {
                if (rewards[i].rewardSwapPoolId != bytes32(0)) {
                    ...
                    if (rewards[i].hopPoolId != bytes32(0)){
                        IERC20(rewards[i].extraHop).safeApprove(unirouter, 0);
                    }
                } else {
                    IERC20(rewards[i].token).safeApprove(uniswapRouter, 0);
                }
            }
        }
