  ...
  ERC677ReceiverInterface
{
   ...
   */
  function getMinBalanceForUpkeep(uint256 id) external view returns (uint96 minBalance, uint256 l1CostWei) {
   ...
   */
  function getMaxPaymentForGas(uint256 gasLimit) public view returns (uint96 maxPayment, uint256 l1CostWei) {
  ...
  function _performUpkeepWithParams(PerformParams memory params) private nonReentrant returns (bool success) {
    ...
    uint256 l1CostWei = 0;
    uint96 payment = 0;
    (payment, l1CostWei) = _calculatePaymentAmount(gasUsed, params.fastGasWei, params.linkEth, true);
...
abstract contract KeeperRegistryBase is ConfirmedOwner, ExecutionPrevention, ReentrancyGuard, Pausable {
  ...
  function _calculatePaymentAmount(
    ...
    uint256 linkEth,
    bool isExecution
  ) internal view returns (uint96 payment, uint256 l1CostWei) {
    Storage memory store = s_storage;
    ...
    return (uint96(total), l1CostWei); // LINK_TOTAL_SUPPLY < UINT96_MAX
  }
  ...
  function _generatePerformParams(
    ...
    (uint256 fastGasWei, uint256 linkEth) = _getFeedData();
    (uint96 maxLinkPayment, uint256 l1CostWei) = _calculatePaymentAmount(gasLimit, fastGasWei, linkEth, isExecution);
