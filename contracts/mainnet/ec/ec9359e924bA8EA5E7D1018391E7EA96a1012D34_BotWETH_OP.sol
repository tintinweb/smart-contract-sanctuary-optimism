...
contract BotWETH_OP is IERC721Receiver {
    ...
    struct AdjustPositionParams {
        ...
        uint256 swapMaxAmountInOP;
        uint256 swapForOPAmount;
...

        {
            if (params.swapForWethAmount > 0) {
                _swapExactOutputSingle(
                    ...
                    params.swapMaxAmountInOP
                );
            }
        }

        {
            if (params.swapForOPAmount > 0) {
                _swapExactOutputSingle(
                    WETH,
                    OP,
                    params.swapForOPAmount,
    ...
    function collectAllFees(uint256 tokenID) external onlyWL {
        ...
        TransferHelper.safeTransfer(OP, msg.sender, amount1);
    }
    ...
    function withdrawAll() public onlyWL {
        ...
        if (IERC20(OP).balanceOf(address(this)) > 0) {
            TransferHelper.safeTransfer(
                OP,
                msg.sender,
                IERC20(OP).balanceOf(address(this))
            );
        }
    }
