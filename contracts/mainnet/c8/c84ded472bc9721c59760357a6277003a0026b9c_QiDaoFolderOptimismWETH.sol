...
contract QiDaoFolderOptimismWETH is QiDaoFolder {
  using SafeERC20 for IERC20;

  address internal constant MAI_ADDRESS = 0xdFA46478F9e5EA86d57387849598dbFB2e964b02;
  address internal constant MAI_PAIR_ADDRESS = address(0); // CAUTION: no emergency unfolding!
  address internal constant WETH_VAULT_ADDRESS = 0x062016Cd29Fabb26c52BAB646878987fC9B0Bc55; // TODO: replace with real vault address
  address internal constant WETH_ADDRESS = 0x4200000000000000000000000000000000000006;
  address internal constant QI_ADDRESS = 0x3F56e0c36d275367b8C502090EDF38289b3dEa0d;
  address internal constant GELATO_OPS_ADDRESS = 0x340759c8346A1E6Ed92035FB8B6ec57cE1D82c2c;

  address internal constant USDC_ADDRESS = 0x7F5c764cBc14f9669B88837ca1490cCa17c31607;
  address internal constant MAI_3POOL_ADDRESS = 0x6645237fb81fe7227AA2Fa2A244D23A5AC0DA1cd; // 0=mai, 1=3crv
  address internal constant USDC_WETH_UNIV3_POOL_ADDRESS = 0x85149247691df622eaF1a8Bd0CaFd40BC45154a9;

  address expectedSenderAddress;

  constructor() QiDaoFolder(MAI_ADDRESS, MAI_PAIR_ADDRESS, WETH_VAULT_ADDRESS, WETH_ADDRESS, QI_ADDRESS, GELATO_OPS_ADDRESS) {
  }

  function _approveSwaps() internal override {
    IERC20(MAI_ADDRESS).safeApprove(MAI_3POOL_ADDRESS, type(uint).max);
    IERC20(USDC_ADDRESS).safeApprove(MAI_3POOL_ADDRESS, type(uint).max);
    IERC20(USDC_ADDRESS).safeApprove(USDC_WETH_UNIV3_POOL_ADDRESS, type(uint).max);
    IERC20(WETH_ADDRESS).safeApprove(USDC_WETH_UNIV3_POOL_ADDRESS, type(uint).max);
  }

  function uniswapV3SwapCallback(int256 amount0Delta, int256 amount1Delta, bytes calldata data) external {
    require(msg.sender == expectedSenderAddress, "not authorized");
    IERC20(abi.decode(data, (address))).safeTransfer(msg.sender, uint(amount0Delta > 0 ? amount0Delta : amount1Delta));
  }
  ...
  function _swapMaiForToken(uint _amountMai, uint _minAmountTokenReceived) internal override {
    ...
    IAPool(MAI_3POOL_ADDRESS).exchange_underlying(0, 2, _amountMai, 0);
    gotUsdc = IERC20(USDC_ADDRESS).balanceOf(address(this)) - gotUsdc;

    // USDC -> WETH
    uint gotWeth = IERC20(WETH_ADDRESS).balanceOf(address(this));
    expectedSenderAddress = USDC_WETH_UNIV3_POOL_ADDRESS;
    IUniswapV3Pool(expectedSenderAddress).swap(
      address(this),
      false,
      int(gotUsdc),
      MAX_SQRT_RATIO_MINUS_ONE,
      abi.encode(USDC_ADDRESS)
    );
    gotWeth = IERC20(WETH_ADDRESS).balanceOf(address(this)) - gotWeth;

    require(
      gotWeth >= _minAmountTokenReceived,
      "too much slippage"
    );
    emit SwappedMaiForToken(_amountMai, gotWeth);
  }
  ...
  function _swapTokenForMai(uint _amountToken, uint _minAmountMaiReceived) internal override {
    ...
    IAPool(MAI_3POOL_ADDRESS).exchange_underlying(2, 0, gotUsdc, 0);
