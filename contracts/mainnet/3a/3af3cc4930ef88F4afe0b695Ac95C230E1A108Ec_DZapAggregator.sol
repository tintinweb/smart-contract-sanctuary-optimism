...
contract DZapAggregator is SwapHandler, IDZapAggregator {
    ...
    constructor(uint256[3] memory fees_, address[] memory routers_, Router[] memory routerDetails_, address governor_, address aggregationRouter_, address wNative_, address protocolFeeVault_, address feeDiscountNft_) SwapHandler(routers_, routerDetails_, aggregationRouter_, wNative_) FeeModule(fees_, governor_, protocolFeeVault_, feeDiscountNft_) {}

    function rescueFunds(IERC20 token_, address to_, uint256 amount_) external onlyGovernance {
...
abstract contract FeeModule is Governable, IFeeModule {
    ...
    constructor(uint256[3] memory fees_, address governor_, address protocolFeeVault_, address feeDiscountNft_) Governable(governor_) {
    ...
    function updateProtocolFee(FeeType[] calldata feeTypes_, uint256[] calldata fees_) external onlyGovernance {
        ...
        emit ProtocolFeeUpdated(feeTypes_, fees_);
    }
    ...
    function updateProtocolFeeVault(address newProtocolFeeVault_) external onlyGovernance {
        ...
        emit ProtocolFeeVaultUpdated(newProtocolFeeVault_);
    }
    ...
    function updateProjectFee(uint256 projectId_, FeeType[] memory feeTypes_, uint256[] memory fees_) external onlyGovernance {
        require(projectId_ < nextProjectId, "DZF003");
    ...
    // return protocolFeeBps, projectFeeBps, projectFeeVault
    function _getFeeDetail(uint256 projectId_, uint256 nftId_, FeeType feeType_) internal view returns (uint256, uint256, address) {
    ...
    // return returnAmount, protocolFee, projectFee
    function _calculateFeeAmount(uint256 amount_, uint256 protocolFeeBps_, uint256 projectFeeBps_) internal pure returns (uint256, uint256, uint256){
...
import { Math as Math2 } from "../libraries/Math.sol";
...
abstract contract SwapHandler is FeeModule, Permitable, ISwapHandler {
    ...
    constructor(address[] memory routers_, Router[] memory routerDetails_, address aggregationRouter_, address wNative_) {
    ...
    function calculateOptimalSwapAmount(uint256 amountA_, uint256 amountB_, uint256 reserveA_, uint256 reserveB_, address router_) public view returns (uint256) {
    ...
    function swapTokensToTokens(SwapDetails[] calldata data_, address recipient_, uint256 projectId_, uint256 nftId_) external payable {
        require(recipient_ != address(0), "DZS001");
        SwapInfo[] memory swapInfo = new SwapInfo[](data_.length);
        (uint256 tempProtocolFeeBps, uint256 tempProjectFeeBps, address projectFeeVault) = _getFeeDetail(projectId_, nftId_, FeeType.BATCH_SWAP);
        ...
        for (uint256 i; i < data_.length; ++i) {
            ...
            try IAggregationRouterV5(AGGREGATION_ROUTER).swap{ value: value }(data.executor, data.desc, "", data.routeData) returns (uint256 returnAmount, uint256) {
                ...
                _swapTransferDstTokens(data.desc.dstToken, recipient_, projectFeeVault, returnAmount, tempProtocolFeeBps, tempProjectFeeBps);
    ...
    function swapToken(SwapDetails calldata data_, address recipient_, uint256 projectId_, uint256 nftId_) external payable {
        require(recipient_ != address(0), "DZS001");
        (uint256 tempProtocolFeeBps, uint256 tempProjectFeeBps, address projectFeeVault) = _getFeeDetail(projectId_, nftId_, FeeType.BATCH_SWAP);

        require(data_.desc.dstReceiver == address(0), "DZS002");
        require(data_.desc.flags & _PARTIAL_FILL == 0, "DZS003");

        uint256 value;

        if (_isNative(data_.desc.srcToken)) {
            value = data_.desc.amount;
        } else {
            _transferAndApprove(data_.permit, data_.desc.srcToken, AGGREGATION_ROUTER, data_.desc.amount);
        }

        (uint256 returnAmount, ) = IAggregationRouterV5(AGGREGATION_ROUTER).swap{ value: value }(data_.executor, data_.desc, "", data_.routeData);

        require(returnAmount >= data_.desc.minReturnAmount, "DZS004");

        SwapInfo memory swapInfo = SwapInfo(data_.desc.srcToken, data_.desc.dstToken, data_.desc.amount, returnAmount);

        _swapTransferDstTokens(data_.desc.dstToken, recipient_, projectFeeVault, returnAmount, tempProtocolFeeBps, tempProjectFeeBps);

        emit TokenSwapped(_msgSender(), recipient_, swapInfo, [tempProtocolFeeBps, tempProjectFeeBps]);
    }

    // can return only native when dest is wNative
    function unoSwapTokensToTokens(UnoSwapDetails[] calldata swapData_, address recipient_, uint256 projectId_, uint256 nftId_) external payable {
        require(recipient_ != address(0), "DZS001");
        ...
        for (uint256 i; i < swapData_.length; ++i) {
            ...
            try IUniswapV2Router02(data.router).swapExactTokensForTokens(data.amount, data.minReturnAmount, data.path, address(this), block.timestamp + 60) returns (uint256[] memory amountOuts) {
                ...
                _unoSwapTransferDstTokens(dstToken, recipient_, projectFeeVault, returnAmount, tempProtocolFeeBps, tempProjectFeeBps);
            } catch Error(string memory) {
    ...
    // can return both native and wNative
    function swapLpToTokens(LpSwapDetails[] calldata lpSwapDetails_, WNativeSwapDetails[] calldata wEthSwapDetails_, address recipient_, uint256 projectId_, uint256 nftId_) external {
        require(recipient_ != address(0), "DZS001");
        ...
        (uint256 tempProtocolFeeBps, uint256 tempProjectFeeBps, address projectFeeVault) = _getFeeDetail(projectId_, nftId_, FeeType.BATCH_SWAP_LP);

        // swap lp to weth
        swapInfo.lpInput = _swapLpToWNative(lpSwapDetails_);

        // swap weth to tokens
        swapInfo.lpOutput = _swapWNativeToDstTokens(wEthSwapDetails_, recipient_, projectFeeVault, IWNATIVE(wNative).balanceOf(address(this)), tempProtocolFeeBps, tempProjectFeeBps);

        emit LpSwapped(_msgSender(), recipient_, swapInfo, [tempProtocolFeeBps, tempProjectFeeBps]);
    }

    function swapTokensToLp(SwapDetails[] calldata data_, LpSwapDetails[] calldata lpSwapDetails_, OutputLp calldata outputLpDetails_, address recipient_, uint256 projectId_, uint256 nftId_) public payable {
        require(recipient_ != address(0), "DZS001");
        ...
        for (i; i < data_.length; ++i) {
            ...
            if (srcToken != wNative && srcToken != token0 && srcToken != token1) {
                ...
                (uint256 returnAmount, ) = IAggregationRouterV5(AGGREGATION_ROUTER).swap(data.executor, data.desc, data.permit, data.routeData);
        ...
        for (uint256 j; j < lpSwapDetails_.length; ++j) {
            ...
            _swapExactTokensForTokens(tokenA, amountA, details.tokenAToPath, tokenA != wNative && tokenA != token0 && tokenA != token1, details.router);
            _swapExactTokensForTokens(tokenB, amountB, details.tokenBToPath, tokenB != wNative && tokenB != token0 && tokenB != token1, details.router);
        ...
        _transferOutputLP(IERC20(outputLpDetails_.lpToken), recipient_, projectFeeVault, returnAmounts[0], tempProtocolFeeBps, tempProjectFeeBps);
        ...
        emit LiquidityAdded(_msgSender(), recipient_, input, outputLpDetails_.lpToken, returnAmounts, [tempProtocolFeeBps, tempProjectFeeBps]);
    }

    function batchTransfer(TransferDetails[] calldata data_, uint256 projectId_, uint256 nftId_) external payable {
        TransferInfo[] memory transferInfo = new TransferInfo[](data_.length);

        (uint256 tempProtocolFeeBps, uint256 tempProjectFeeBps, address projectFeeVault) = _getFeeDetail(projectId_, nftId_, FeeType.BATCH_TRANSFER);
        ...
        for (uint256 i; i < data_.length; ++i) {
            ...
            for (uint256 j; j < details.data.length; ++j) {
                ...
                (uint256 amountAfterFee, uint256 protocolFee, uint256 projectFee) = _calculateFeeAmount(data.amount, tempProtocolFeeBps, tempProjectFeeBps);
    ...
    function _transferAndApprove(bytes memory permit_, IERC20 srcToken_, address router_, uint256 amount_) private {
        _permit(address(srcToken_), permit_);
        srcToken_.safeTransferFrom(_msgSender(), address(this), amount_);
        srcToken_.approve(router_, amount_);
    }

    function _swapTransferDstTokens(IERC20 token_, address recipient_, address projectFeeVault, uint256 returnAmount, uint256 tempProtocolFeeBps, uint256 tempProjectFeeBps) private {
        (uint256 amountAfterFee, uint256 protocolFee, uint256 projectFee) = _calculateFeeAmount(returnAmount, tempProtocolFeeBps, tempProjectFeeBps);
    ...
    function _unoSwapTransferDstTokens(IERC20 token_, address recipient_, address projectFeeVault, uint256 returnAmount, uint256 tempProtocolFeeBps, uint256 tempProjectFeeBps) private {
        (uint256 amountAfterFee, uint256 protocolFee, uint256 projectFee) = _calculateFeeAmount(returnAmount, tempProtocolFeeBps, tempProjectFeeBps);
    ...
    function _transferOutputLP(IERC20 lpToken, address recipient_, address projectFeeVault, uint256 returnAmount, uint256 tempProtocolFeeBps, uint256 tempProjectFeeBps) private {
        (uint256 amountAfterFee, uint256 protocolFee, uint256 projectFee) = _calculateFeeAmount(returnAmount, tempProtocolFeeBps, tempProjectFeeBps);
    ...
    function _swapWNativeForToken(uint256 amount_, address[] memory path_, address router_) private returns (uint256) {
        IWNATIVE(wNative).approve(router_, amount_);

        uint256[] memory amountOuts = IUniswapV2Router02(router_).swapExactTokensForTokens(amount_, 0, path_, address(this), block.timestamp + 60);
        return amountOuts[amountOuts.length - 1];
    }

    function _swapExactTokensForTokens(address token_, uint256 amount_, address[] memory path_, bool executeSwap_, address router_) private {
        if (executeSwap_) {
            IERC20(token_).approve(router_, amount_);
            IUniswapV2Router02(router_).swapExactTokensForTokens(amount_, 0, path_, address(this), block.timestamp + 60);
        }
    }
    ...
    //  used in swapLpToTokens
    function _swapWNativeToDstTokens(WNativeSwapDetails[] calldata wEthSwapDetails_, address recipient_, address projectFeeVault, uint256 wNativeBalance, uint256 tempProtocolFeeBps, uint256 tempProjectFeeBps) private returns (Token[] memory) {
        ...
        for (uint256 i; i < wEthSwapDetails_.length; ++i) {
            ...
            uint256 wNativeAmount = i != wEthSwapDetails_.length - 1 ? (wNativeBalance * details.sizeBps) / BPS_DENOMINATOR : IWNATIVE(wNative).balanceOf(address(this));
            ...
            if (details.nativeToOutputPath.length == 0) {
                ...
                (uint256 amountAfterFee, uint256 protocolFee, uint256 projectFee) = _calculateFeeAmount(wNativeAmount, tempProtocolFeeBps, tempProjectFeeBps);
                ...
                (uint256 amountAfterFee, uint256 protocolFee, uint256 projectFee) = _calculateFeeAmount(amountOut, tempProtocolFeeBps, tempProjectFeeBps);
    ...
    function _removeLiquidity(LpSwapDetails memory details_, address tokenA_, address tokenB_, address router_) private returns (uint256 amountA, uint256 amountB) {
        _transferAndApprove(details_.permit, IERC20(details_.token), router_, details_.amount);

        (amountA, amountB) = IUniswapV2Router02(router_).removeLiquidity(tokenA_, tokenB_, details_.amount, 0, 0, address(this), block.timestamp + 60);
    }

    function _addOptimalLiquidity(OutputLp calldata lpDetails_, address tokenA_, address tokenB_) private returns (uint256[3] memory) {
        uint256 wNativeBalance = IWNATIVE(wNative).balanceOf(address(this));

        // swap 50-50
        if (wNativeBalance > 0) {
            if (tokenA_ != wNative) _swapWNativeForToken(wNativeBalance / 2, lpDetails_.nativeToToken0, lpDetails_.router);

            if (tokenB_ != wNative) _swapWNativeForToken(wNativeBalance - (wNativeBalance / 2), lpDetails_.nativeToToken1, lpDetails_.router);
        }

        // do optimal swap
        (uint256 amountA, uint256 amountB) = _optimalSwapForAddingLiquidity(lpDetails_.lpToken, tokenA_, tokenB_, IERC20(tokenA_).balanceOf(address(this)), IERC20(tokenB_).balanceOf(address(this)), lpDetails_.router);

        IERC20(tokenA_).approve(lpDetails_.router, amountA);
        IERC20(tokenB_).approve(lpDetails_.router, amountB);

        // add liquidity
        (uint256 addedToken0, uint256 addedToken1, uint256 lpAmount) = IUniswapV2Router02(lpDetails_.router).addLiquidity(tokenA_, tokenB_, amountA, amountB, 0, 0, address(this), block.timestamp + 60);

        return ([lpAmount, amountA - addedToken0, amountB - addedToken1]);
    }

    function _optimalSwapForAddingLiquidity(address lp, address tokenA_, address tokenB_, uint256 amountA_, uint256 amountB_, address router_) private returns (uint256, uint256) {
        ...
        uint256 optimalSwapAmount = reverse ? calculateOptimalSwapAmount(amountB_, amountA_, reserveB, reserveA, router_) : calculateOptimalSwapAmount(amountA_, amountB_, reserveA, reserveB, router_);
        ...
        if (optimalSwapAmount > 0) {
            ...
            uint256[] memory amountOuts = IUniswapV2Router02(router_).swapExactTokensForTokens(optimalSwapAmount, 0, path, address(this), block.timestamp + 60);
...
interface IFeeModule {
    ...
    event ProtocolFeeVaultUpdated(address newProtocolFeeVault);

    event ProtocolFeeUpdated(FeeType[] feeTypes, uint256[] fees);
    ...
    event ProjectNftUpdated(uint256 indexed id);

    event FeeUpdated(FeeType[] feeTypes, uint256[] fees);
...
interface ISwapHandler is IFeeModule {
    ...
    event TokenSwapped(address indexed sender, address indexed recipient, SwapInfo swapInfo, uint256[2] feeBps); // protocolFeeBps, projectFeeBPS
...
contract DZapDiscountNft is ERC1155, Ownable, ERC1155Burnable, ERC1155Supply, IDZapDiscountNft {
    ...
    function createAndMint(NftData[] calldata nftData_, address[] calldata to_, uint256[] calldata ids_, uint256[] calldata amounts_) public onlyOwner {
        createNfts(nftData_);
    ...
    function approveMinter(address[] calldata minters_, uint256[] calldata ids_, uint256[] calldata amounts_) public onlyOwner {
    ...
    function mint(address[] calldata to_, uint256[] calldata ids_, uint256[] calldata amounts_) public {
        for (uint256 i; i < to_.length; ++i) {
            uint256 id = ids_[i];
            _isValidNft(id);
    ...
    function mintBatch(address to_, uint256[] calldata ids_, uint256[] calldata amounts_, bytes calldata data_) public onlyOwner {
    ...
    function _beforeTokenTransfer(address operator_, address from, address to_, uint256[] memory ids_, uint256[] memory amounts_, bytes memory data_) internal override(ERC1155, ERC1155Supply) {
