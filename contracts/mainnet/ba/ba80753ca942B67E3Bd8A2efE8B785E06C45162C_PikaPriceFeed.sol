...
contract PikaPriceFeed is Governable {
    ...
    uint256 public priceDuration = 300; // 5 mins
    ...
    event DisableFastOracle(address guardian);
    event EnableFastOracle(address guardian);
    event MinGuardianCountSet(uint256 minGuardianCount);
    ...
    function getPrice(address token, bool isMax) external view returns (uint256) {
        ...
        if (isSpreadEnabled || isChainlink || disableFastOracleVote >= minVotes) {
    ...
    function shouldHaveSpread(address token) external view returns (bool) {
        ...
        return isSpreadEnabled || isChainlink || disableFastOracleVote >= minVotes;
    }
    ...
    function setMinGuardianCount(uint256 _minGuardianCount) external onlyOwner {
        minVotes = _minGuardianCount;

        emit MinGuardianCountSet(_minGuardianCount);
    }
