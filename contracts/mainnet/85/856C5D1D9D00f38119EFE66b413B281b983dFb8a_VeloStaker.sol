...
contract VeloStaker is ERC20, BeSolidStaker {
    ...
    function addGauge(address _lp, address[] calldata _bribeTokens, address[] calldata _feeTokens ) external onlyManager {
        Gauges storage addedGauge = gauges[_lp];
        addedGauge.bribeGauge = solidVoter.external_bribes(_lp);
        addedGauge.feeGauge = solidVoter.internal_bribes(_lp);
        addedGauge.bribeTokens = _bribeTokens;
        addedGauge.feeTokens = _feeTokens;
        lpIntialized[_lp] = true;
        emit AddedGauge(addedGauge.bribeGauge, addedGauge.feeGauge, _bribeTokens, _feeTokens);
    }

    // Add reward token
    function addRewardToken(address[][] memory _tokens, bool[] memory _stables) external onlyManager {
        for (uint i; i < _tokens.length; ++i) {
            routes[_tokens[0][0]].push(IUniswapRouterSolidlyV2.Routes({
                from: _tokens[i][0],
                to: _tokens[i][1],
                stable: _stables[i]
            }));
        ...
        IERC20(_tokens[0][0]).safeApprove(address(router), 0);
        IERC20(_tokens[0][0]).safeApprove(address(router), type(uint256).max);
        emit AddedRewardToken(_tokens[0][0]);
    }
    ...
    function harvest() external {
       ...
       for (uint i; i < activeVoteLps.length; ++i) {
            ...
            ISolidlyGauge(rewardsGauge.bribeGauge).getReward(address(this), rewardsGauge.bribeTokens);
            ISolidlyGauge(rewardsGauge.feeGauge).getReward(address(this), rewardsGauge.feeTokens);
        ...
        uint256 rewardAmt = balanceOfWant() - before;
        if (rewardAmt > 0) {
...
interface IUniswapRouterSolidlyV2 {
