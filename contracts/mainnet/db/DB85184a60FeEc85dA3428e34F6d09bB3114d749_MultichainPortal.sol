    ...
    IStargateReceiver
{
    ...
    function swapERC20AndSend(
        ...
        if (swapRouter != address(0)) {
            _swapERC20(tokenIn, amountIn, user, swapRouter, swapArguments, true);
        }
    ...
    function send(
        ...
        require(IERC20(usdc).transfer(beneficiary, brydgeFee), "Brydge fee payment failed");
    ...
    function _processRequest(address _token, uint256 amountLD, bytes calldata rawRequestData) internal {
        ...
        try
            this.swapERC20AndCall(_token, amountLD, user, swapRouter, swapArguments, calls)
        {} catch Error(string memory reason) {
            ...
            lastErr = reason;
            require(IERC20(_token).transfer(user, amountLD), "refund failed");
        ...
        if (tokenOut != address(0)) {
            finalBalance = token.balanceOf(address(this)) - initialBalance;
            require(token.transfer(user, finalBalance), "reimbursement failed");
        } else {
            // Native token output
            finalBalance = address(this).balance - initialBalance;
            (bool success, ) = user.call{value:finalBalance}("");
    ...
    function swapERC20AndCall(
        ...
        if (swapRouter != address(0)) {
            ...
            _swapERC20(tokenIn, amountIn, user, swapRouter, swapArguments, stargate);
        }
        _handleCalls(calls);
    }
