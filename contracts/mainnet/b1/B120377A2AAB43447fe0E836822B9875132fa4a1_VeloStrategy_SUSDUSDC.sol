...
contract VeloStrategy_SUSDUSDC is BasicStrategy {
    ...
    function harvestAndReinvest() public override onlyGovernance {
        ...
        if (performanceFee > 0) {
            uint256 _fee = _calculateFee(rewardAmount, performanceFee);
...
//        _swapSolidlyWithRoute(_veloRoute[1], _toToken1);
        ...
        ISolidlyRouter(_veloRouter).swapExactTokensForTokensSimple(_toToken0, 0, _veloToken, _usdcToken, true,  address(this), block.timestamp.add(60));
        ISolidlyRouter(_veloRouter).swapExactTokensForTokensSimple(_toToken1, 0, _veloToken, _susdToken, true,  address(this), block.timestamp.add(60));
