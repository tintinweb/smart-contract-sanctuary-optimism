...
contract ZonicCouponToken is ERC721Psi, Ownable {
  using Strings for uint256;

  uint256 public MAX_SUPPLY = 19000;

  string private __baseURI;

  constructor() ERC721Psi("Zonic Coupon", "ZONICCOUPON") {
  }
  ...
  function adminMint(address to, uint256 quantity) public onlyOwner {
    require(totalSupply() + quantity <= MAX_SUPPLY, "Run out of supply");
    _safeMint(to, quantity);
  }

  function adminMintMultiple(address[] calldata to) public onlyOwner {
    require(totalSupply() + to.length <= MAX_SUPPLY, "Run out of supply");
