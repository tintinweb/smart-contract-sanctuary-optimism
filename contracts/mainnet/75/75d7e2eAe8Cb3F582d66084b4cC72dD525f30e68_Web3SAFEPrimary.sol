...
abstract contract Web3SAFE is Initializable {
    ...
    function emitBuy(uint256 _safeId, address _investor, uint256 _paid, uint256 _received)
        external
        onlyAdmin(_safeId)
    {
        emit Buy(_safeId, _investor, _paid, _received);
    }

    event NewAllocation(
        uint256 indexed safe, uint256 startPrice, uint256 endPrice, uint256 tokenAmount, uint256 commitment
    );

    function emitNewAllocation(uint256 _safeId, uint256 _start, uint256 _end, uint256 _token, uint256 _commitment)
        external
        onlyAdmin(_safeId)
    {
        emit NewAllocation(_safeId, _start, _end, _token, _commitment);
    }

    event Close(uint256 indexed safe, uint256 commitment, SafeType safeType);

    function emitClose(uint256 _safeId, uint256 _commitment, SafeType _type) external onlyAdmin(_safeId) {
        emit Close(_safeId, _commitment, _type);
    }

    event ConvertToSafe(
        uint256 indexed safe, uint256 commitment, uint256 amountInvested, address[] wallets, uint256[] amounts
    );

    function emitConvertToSafe(
        uint256 _safeId,
        uint256 _commitment,
        uint256 _amountInvested,
        address[] calldata _wallets,
        uint256[] calldata _amounts
    ) external onlyAdmin(_safeId) {
        emit ConvertToSafe(_safeId, _commitment, _amountInvested, _wallets, _amounts);
    }

    event UpdatePrice(uint256 indexed safe, uint256 newPrice);

    function emitUpdatePrice(uint256 _safeId, uint256 _price) external onlyAdmin(_safeId) {
        emit UpdatePrice(_safeId, _price);
    }

    event ChangeCommitment(uint256 indexed safe, uint256 commitment, uint256 committed);

    function emitChangeCommitment(uint256 _safeId, uint256 _commitment, uint256 _committed)
        external
        onlyAdmin(_safeId)
    {
        emit ChangeCommitment(_safeId, _commitment, _committed);
    }
    ...
    function emitInstantiate(
        ...
        emit Instantiate(_safeId, _control, _recipient, _minInvestment, _token, _currency, _type);
    }

    event Configure(uint256 indexed safe, address control, address recipient, uint256 minInvestment);

    function emitConfigure(uint256 _safeId, address _control, address _recipient, uint256 _minInvestment)
        external
        onlyAdmin(_safeId)
    {
        emit Configure(_safeId, _control, _recipient, _minInvestment);
    }

    event AddAdmin(uint256 indexed safe, address admin);

    function emitAddAdmin(uint256 _safeId, address _admin) external onlyAdmin(_safeId) {
        emit AddAdmin(_safeId, _admin);
    }

    event RemoveAdmin(uint256 indexed safe, address admin);

    function emitRemoveAdmin(uint256 _safeId, address _admin) external onlyAdmin(_safeId) {
        emit RemoveAdmin(_safeId, _admin);
    }
    ...
    function revokeAdmin(uint256 _safeId) public onlyAdmin(_safeId) {
        ...
        emit RemoveAdmin(_safeId, msg.sender);
    }
    ...
    function _instantiate(
        ...
        emit Instantiate(_safeId, _control, _recipient, _minInvestment, _token, _currency, _safeType);
    }
    ...
    function _newAllocation(
        ...
        emit NewAllocation(_safeId, _startPrice, _endPrice, _tokenAmount, _newCommitment);
    }
    ...
    function buy(uint256 _safeId, uint256 _amount, uint256 _minimum, address _to) external payable onRun(_safeId) {
        _buy(_safeId, msg.sender, _amount, _minimum, _to);
    }
    ...
    function permitBuy(
        ...
        _buy(_safeId, _signer, _amount, _minimum, _to);
    }

    function _buy(uint256 _safeId, address _signer, uint256 _amount, uint256 _minimum, address _to) internal {
        ERC20 token = ERC20(info[_safeId].token);
        _collectInvestment(_signer, _safeId, _amount);
        (uint256 bought,) = _calcPayout(_safeId, _amount, _minimum, false);
    ...
    function manualBuy(
        ...
        _manualBuy(_safeId, _wallets, _amounts, _minimum, _reward);
    }
    ...
    function _manualBuy(
        ...
        (uint256 bought,) = _calcPayout(_safeId, totalAmount, _minimum, _reward);
    ...
    function close(uint256 _safeId, uint256 _newCommitment, SafeType _newType)
        ...
        onRun(_safeId)
    {
        _close(_safeId, _newCommitment, _newType);
    }
    ...
    function _close(uint256 _safeId, uint256 _newCommitment, SafeType _newType) internal {
        ...
        emit Close(_safeId, _newCommitment, _newType);
    }
    ...
    function convertToSafe(
        ...
        _convertToSafe(_safeId, _newCommitment, _amountInvested, _wallets, _amounts);
    }
    ...
    function _convertToSafe(
        ...
        emit ConvertToSafe(_safeId, _newCommitment, _amountInvested, _wallets, _amounts);
    }

    function updatePrice(uint256 _safeId, uint256 _newPrice) external onlyAdmin(_safeId) onRun(_safeId) {
        _updatePrice(_safeId, _newPrice);
    }
    ...
    function changeCommitment(uint256 _safeId, uint256 _commitment, uint256 _committed) external onlyAdmin(_safeId) {
        SafeParams memory param = params[_safeId];
    ...
    function _calcPayout(uint256 _safeId, uint256 _amount, uint256 _minimum, bool _isReward)
        internal
