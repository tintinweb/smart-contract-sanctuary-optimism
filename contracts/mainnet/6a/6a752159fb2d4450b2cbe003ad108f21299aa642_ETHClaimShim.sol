...
contract ETHClaimShim is Ownable {
    address public claimExtensionAddress;
    bytes32 public claimInstanceHash;
    ...
    function _mintProxy(
        ...
        if (address(this).balance >= airdropAmount) {
            payable(mintFor).transfer(airdropAmount);
        }
    }
    ...
    function updateClaimExtension(
        address _claimExtensionAddress
    ) external onlyOwner {
        claimExtensionAddress = _claimExtensionAddress;
    }

    function updateAirdropAmount(uint256 _airdropAmount) external onlyOwner {
    ...
    function updateClaim(
        ...
        claimInstanceHash = keccak256(
            abi.encodePacked(_creatorContractAddress, _instanceId)
        );
    }
