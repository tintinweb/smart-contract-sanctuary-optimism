...
contract YearnStrategy is
    Initializable,
    AccessControlUpgradeable,
    UUPSUpgradeable,
    IFeedStrategy
{
    IYearnLp public lpToken;
    IFeedStrategy public referenceFeed;
    IERC20Metadata public referenceAsset;

    uint8 public lpDecimals;
    uint8 public referenceAssetDecimals;
    ...
    function initialize(
        ...
        lpToken = IYearnLp(_token);
        referenceFeed = IFeedStrategy(_referenceFeed);

        lpDecimals = IYearnLp(_token).decimals();
        referenceAsset = IERC20Metadata(lpToken.token());

        referenceAssetDecimals = referenceAsset.decimals();
    }

    function getPrice() external view returns (int256, uint8) {
        return getPriceOfAmount(10 ** lpDecimals);
    }

    function getPriceOfAmount(
        uint256 amount
    ) public view returns (int256, uint8) {
        uint256 totalOneSideWithdraw = (amount *
            lpToken.pricePerShare()) / (10 ** lpDecimals);

        (int256 usdPrice, uint8 usdDecimals) = referenceFeed.getPrice();

        require(usdPrice > 0, "YearnPriceStrat: feed lte 0");

        return (
            int256(totalOneSideWithdraw) * usdPrice,
            usdDecimals + referenceAssetDecimals
        );
    }
