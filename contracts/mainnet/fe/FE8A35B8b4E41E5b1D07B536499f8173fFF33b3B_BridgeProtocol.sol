...
// File contracts/BridgeProtocol.sol
...
contract BridgeProtocol is ReentrancyGuard, Ownable {
    ...
    constructor(address initialOwner, uint256 _targetChainId) Ownable(initialOwner) {
    ...
    receive() external payable {
        require(paused == false, "paused");
        _bridgeETH(_msgSender(), _msgSender(), msg.value);
    }
    ...
    function bridgeETH(address _to) external payable whenNotPaused {
        _bridgeETH(_msgSender(), _to, msg.value);
    }
    ...
    function recoverWrongTokens(address _token, address _account, uint256 _amount) external onlyOwner nonReentrant {
        IERC20(_token).safeTransfer(_account, _amount);
    }

    function _sendToken(address payable _receiver, uint256 _outAmount, uint256 _depositId) internal {
    ...
    function sendToken(address payable _receiver, uint256 _outAmount, uint256 _depositId) external onlyOwner nonReentrant {
        _sendToken(_receiver, _outAmount, _depositId);
    }

    function sendTokenBatch(address[] calldata addresses,  uint256[] calldata _outAmounts, uint256[] calldata _depositId) external onlyOwner nonReentrant {
        require(addresses.length == _outAmounts.length, "addresses and amounts are not matched");
        require(addresses.length == _depositId.length, "addresses and depositId are not matched");
        ...
        for (uint i = 0; i < count; i++) {
            totalAmount += _outAmounts[i];
        }

        require(balanceOf() >= totalAmount, "insufficient balance");

        for (uint i = 0; i < count; i++) {
            _sendToken(payable(addresses[i]), _outAmounts[i], _depositId[i]);
        }
    }
