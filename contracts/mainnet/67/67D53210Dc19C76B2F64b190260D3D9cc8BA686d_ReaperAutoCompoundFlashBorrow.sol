...
contract ReaperAutoCompoundFlashBorrow is ReaperBaseStrategyv3, IFlashLoanReceiver {
    ...
    function initialize(
        ...
        aWant = _aWant;
        want = IERC20Upgradeable(_aWant.UNDERLYING_ASSET_ADDRESS());
    ...
    function _chargePerformanceFees() internal returns (uint256 callerFee) {
        uint256 opFee = (IERC20Upgradeable(OP).balanceOf(address(this)) * totalFee) / PERCENT_DIVISOR;
        if (opFee == 0) {
            return 0;
        }
    ...
    function _convertOpToWant() internal {
        uint256 opBal = IERC20Upgradeable(OP).balanceOf(address(this));
        if (opBal != 0) {
