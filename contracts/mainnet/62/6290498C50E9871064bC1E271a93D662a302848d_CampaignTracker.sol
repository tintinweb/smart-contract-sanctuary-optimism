...
contract CampaignTracker is TwoStepOwnable, ICampaignTracker, Initializable {
    ...
    function setCampaign(
        ...
        if (campaignId == BASELINE_CAMPAIGN_ID) {
            ...
            require(rewardDeriver.marketplaceFeePerMille() > _rewardPerMille + optimismOGRewardPerMille, "CampaignTracker: OP OG plus baseline reward must be less the marketplace fee");
        } else if (campaignId == OPTIMISM_OG_CAMPAIGN_ID) {
            uint256 baselineRewardPerMille = campaigns[BASELINE_CAMPAIGN_ID].rewardPerMille;
            require(rewardDeriver.marketplaceFeePerMille() > _rewardPerMille + baselineRewardPerMille, "CampaignTracker: OP OG plus baseline reward must be less the marketplace fee");
        } else {
            require(_royaltyPerMille > _rewardPerMille, "CampaignTracker: royalty per mille must be greater than reward per mille");
        }
    ...
    function toggleRewardsForCampaign(string memory _campaignString) external {
        ...
        require(msg.sender == owner() || msg.sender == campaign.manager, "CampaignTracker: caller must be owner or campaign manager");
