...
contract ABRegistry is Initializable, AccessControlUpgradeable {
    //     _____ __        __
    ...
    /// @dev Mapping storing the drop details for a given drop ID
    mapping(uint256 => DropDetails) private drops;
    ...
    function updateBalance(
        ...
        if (_from != address(0))
            userBalancePerDrop[_from][_dropId] -= _quantity;

        // Increment the balance of `_to` if it is not the zero-address
        if (_to != address(0)) userBalancePerDrop[_to][_dropId] += _quantity;
    }
    ...
    function updateDropRightholder(
        ...
        drops[_dropId].rightholder = _rightholder;
    }
    ...
    function updateDropSupply(
        ...
        // Update the drop supply associated to the drop ID
        drops[_dropId].supply = _supply;
    }
     ...
     */
    function incrementDropSold(
        uint256 _dropId,
        uint256 _quantity
    ) external onlyRole(RELAY_ROLE) {
        // Update the drop supply associated to the drop ID
        drops[_dropId].sold += _quantity;
    }
    ...
    function getDropRightholder(
        ...
        return drops[_dropId].rightholder;
    }

    /**
     * @notice
     *  Get the total NFT supply for the given `_dropId`
     *
     * @param _dropId drop ID to be queried
     *
     * @return : the total NFT supply corresponding to `_dropId`
     */
    function getDropSupply(uint256 _dropId) external view returns (uint256) {
        return drops[_dropId].supply;
    }

    /**
     * @notice
     *  Get the total number of NFT sold for the given `_dropId`
     *
     * @param _dropId drop ID to be queried
     *
     * @return : the total number of NFT sold  corresponding to `_dropId`
     */
    function getDropSold(uint256 _dropId) external view returns (uint256) {
        return drops[_dropId].sold;
    }
}