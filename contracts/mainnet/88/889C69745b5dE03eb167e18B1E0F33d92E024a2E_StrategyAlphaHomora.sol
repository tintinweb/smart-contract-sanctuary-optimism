...
import "IUniswapV3Spell.sol";
import "IUniswapV3Factory.sol";
import "ISwapRouter.sol";
import "INonfungiblePositionManager.sol";
import "UniswapV3SpellIntegrationOp.sol";
import "IUniswapv3OptimalSwap.sol";
import "TickMath.sol";
...
contract StrategyAlphaHomora is BaseStrategy {
    ...
    event SwappingStables(address inToken, uint256 amount);
    ...
    IUniswapV3Spell public spell;
    address public homoraBank;
    address public volatileToken;
    address public router;
    IUniswapV3OptimalSwap optimalSwap = IUniswapV3OptimalSwap(0xC781Cf972AB97601efeCFfA53202A410f52FEF92);
    IUniswapV3Factory factory;
    UniswapV3SpellIntegrationOp public integration;
    IUniswapV3Pool public lpToken;
    address[] public rewards;
    uint256 public positionId;
    uint256 public leverage;
    uint256 public minHarvestRequired;
    ...
    uint stableDecimals;
    uint volatileDecimals;
    uint256 public numRebalances;
    uint256 public ammCheckThreshold;
    uint256 public debtRatioThreshold;
    uint256 public volatilityThreshold;
    address public homoraOracle;
    uint[] public gradients;
    uint[] public gradientBreakPoints;
    uint256 PRECISION;
    uint256 MAX_INT;
    bool public isToken0Stable;
    bool public _isStableNotUSDC;
    address public weth;
    ...
    constructor(
        ...
        leverage = _leverage;
        ammCheckThreshold = 5*PRECISION / 100;
        ...
        spell = IUniswapV3Spell(config[0]);
        homoraBank = config[1];
        lpToken = IUniswapV3Pool(config[2]);
        router = config[3];
        homoraOracle=config[4];
        factory = IUniswapV3Factory(ISwapRouter(router).factory());
        fee = lpToken.fee();
        _setTicks(ticks[0], ticks[1]);
        isToken0Stable = _isToken0Stable;
        if(isToken0Stable){
            stableToken = lpToken.token0();            
            volatileToken = lpToken.token1();
        }else{
            stableToken = lpToken.token1();            
            volatileToken = lpToken.token0();
        }
        stableDecimals = uint(10)**ERC20Upgradeable(stableToken).decimals();
        minHarvestRequired = stableDecimals/10; // Require a minimum of 10 cents to harvest
        volatileDecimals = uint(10)**ERC20Upgradeable(volatileToken).decimals();
        rewards = [stableToken, volatileToken];

        depositToken = USDC;
        priceAnchor = pairPrice(volatileToken);

        vault = _vault;
        slippage = PRECISION/50; // TODO: reduce slippage tolerance

        integration = new UniswapV3SpellIntegrationOp(
            IBankOP(homoraBank),
            factory,
            INonfungiblePositionManager(config[5]),
            ISwapRouter(router),
            stableToken
        );

        // Approvals
        // IERC20Upgradeable(stableToken).approve(homoraBank, MAX_INT);
        // IERC20Upgradeable(volatileToken).approve(homoraBank, MAX_INT);
        IERC20Upgradeable(USDC).approve(router, MAX_INT);
        IERC20Upgradeable(stableToken).approve(router, MAX_INT);
        IERC20Upgradeable(volatileToken).approve(router, MAX_INT);
        IERC20Upgradeable(USDC).approve(address(integration), MAX_INT);
        IERC20Upgradeable(stableToken).approve(address(integration), MAX_INT);
        IERC20Upgradeable(volatileToken).approve(address(integration), MAX_INT);

        gradients = [0, 2*PRECISION/10, 4*PRECISION/10, 75*PRECISION/100, PRECISION];
        gradientBreakPoints = [0, (25*PRECISION)/1000, (50*PRECISION)/1000, (75*PRECISION)/1000, (100*PRECISION)/1000];

        _isStableNotUSDC = USDC!=stableToken;
    }
    ...
    /// debt repayment
    function balanceOfPoolInt() public view returns (uint256) {
        return integration._getPositionBalance(positionId);
    }

    function getAmounts() public view returns (uint, uint) {
        return integration._getPositionTokens(positionId);
    }

    function balanceOfPool() public view override returns (uint256) {
        uint stableBalance = balanceOfPoolInt();
        if (stableToken!=USDC) {
            return integration._calculateAmountOut(stableToken, USDC, stableBalance);
        }
        return (stableBalance*uint(10)**6)/stableDecimals;
    }

    function oraclePrice(address token) public view returns (uint) {
        uint tokenPrice = IBaseOracle(homoraOracle).getETHPx(token)*uint(10)**ERC20Upgradeable(token).decimals();
        uint stablePrice = IBaseOracle(homoraOracle).getETHPx(stableToken)*stableDecimals;
        return (tokenPrice*stableDecimals)/stablePrice;
    }

    function pairPrice(address token) public view returns (uint) {
        IUniswapV3Pool.Slot0 memory slot0 = lpToken.slot0();
        uint160 sqrtPriceX96 = slot0.sqrtPriceX96;
        if (token==lpToken.token0()) {
            if (uint(sqrtPriceX96)*uint(sqrtPriceX96)>MAX_INT/1e18) {
                return volatileDecimals*(uint(sqrtPriceX96)*uint(sqrtPriceX96)>>(96 * 2));
            } else {
                return (volatileDecimals*uint(sqrtPriceX96)*uint(sqrtPriceX96)>>(96 * 2));
            }
        } else {
            return (stableDecimals*2**192)/(uint(sqrtPriceX96)**2);
        }
    }
        ...
        )
    {
        ...
        uint256 collateralValue = IBankOP(homoraBank).getCollateralETHValue(positionId);
        if(positionId != 0 && collateralValue>0){
            debtRatio = (IBankOP(homoraBank).getBorrowETHValue(positionId) * PRECISION) / collateralValue;
        }
        
        // Calculate gradient
        currentPrice = pairPrice(volatileToken);
    ...
    /// @notice Returns whether funds should be harvested
    function shouldHarvest() public view returns (bool) {
        return getHarvestable()>minHarvestRequired;
    }

    function getDebts() public view returns (uint256, uint256) {
        (address[] memory tokens, uint256[] memory debts) = IBankOP(homoraBank).getPositionDebts(positionId);
        if (tokens[0]==lpToken.token0()) {
    ...
    function getCollateralSize() public view returns (uint256 collateral) {
        (, , , collateral) = IBankOP(homoraBank).getPositionInfo(positionId);
    }
    ...
    function getHarvestable() public view returns (uint harvestable) {
        (uint feeAmt0, uint feeAmt1) = integration.getPendingFees(positionId);
        if (isToken0Stable) {
            harvestable = feeAmt0+feeAmt1*pairPrice(volatileToken)/volatileDecimals;
        } else {
            harvestable = feeAmt1+feeAmt0*pairPrice(volatileToken)/volatileDecimals;
        }
    }

    function getCollateralBorrowValue() public view returns (uint collateralValue, uint borrowValue) {
        borrowValue = IBankOP(homoraBank).getBorrowETHValue(positionId);
        collateralValue = IBankOP(homoraBank).getCollateralETHValue(positionId);
    }

    function getRatio() public view returns (uint) {
        (uint debt0, uint debt1) = getDebts();
        if (isToken0Stable) {
            return (PRECISION*debt0)/(debt0+debt1*pairPrice(volatileToken)/volatileDecimals);
        } else {
            return (PRECISION*debt1)/(debt1+debt0*pairPrice(volatileToken)/volatileDecimals);
        }
    }

    /// TODO: Removed due to contract size limit
    ...
    function _setTicks(uint multiplier0, uint multiplier1) internal {
        IUniswapV3Pool.Slot0 memory slot0 = lpToken.slot0();
        uint160 sqrtPriceX96 = slot0.sqrtPriceX96;
        ...
        uint24 tickSpacing = uint24(lpToken.tickSpacing());
        absTick -= absTick % tickSpacing;
        currentTick = currentTick < 0 ? -int24(int(absTick)) : int24(int(absTick));
        tickLower = int24(currentTick - int24(int(multiplier0 * tickSpacing)));
        tickUpper = int24(currentTick + int24(int(multiplier1 * tickSpacing)));
    }

    /// @dev Check if router price matches oracle price        
    function _ammCheck() internal view returns (bool) {
        uint256 ratio = (pairPrice(volatileToken) * PRECISION) / oraclePrice(volatileToken);
        if (ratio < PRECISION) {
    ...
    function _checkPositionHealth() internal returns (bool healthy) {
        uint currentPrice = pairPrice(volatileToken);
        uint priceChange;
        if (currentPrice>priceAnchor) {
   ...
   function requireAmmCheckPass() view internal {
        bool ammCheck = _ammCheck();
        require(ammCheck == true, "reason: Oracle Price difference");
    }

    function _setPnl() internal {
        int pnlInt = int(balanceOfPoolInt())+int(stableWithdrawn)-int(stableSupplied);
        pnl = (pnlInt*int(10)**6)/int(10)**ERC20Upgradeable(stableToken).decimals();
    }
    ...
    function _deposit() internal override {
        int collateralBefore = int(getCollateralSize());
        int balanceBefore = int(balanceOfPool());
        requireAmmCheckPass();
        _swapStableUSDC(USDC);       
        uint256 supplied = IERC20Upgradeable(stableToken).balanceOf(address(this));
        if(supplied == 0) {
            return;
        }
        stableSupplied+=supplied;
        uint borrow_stable; uint borrow_volatile;
        {
            uint256 multiplier = leverage - 100;
            uint256 frac_stable = 100*PRECISION / (leverage + 100);
            uint256 frac_volatile = PRECISION - frac_stable;
            borrow_stable =
                ((supplied * frac_stable * multiplier)/100) / PRECISION;
            uint256 borrow_volatile_in_stable =
                (((supplied * frac_volatile * multiplier)/100) / PRECISION);
            borrow_volatile = borrow_volatile_in_stable * pairPrice(stableToken)/stableDecimals;
        }
        {
            address token0; address token1; uint amt0; uint amt1; uint borrow0; uint borrow1;
            if (isToken0Stable) {
                token0 = stableToken;
                token1 = volatileToken;
                amt0 = supplied;
                amt1 = 0;
                borrow0 = borrow_stable;
                borrow1 = borrow_volatile;
            } else {
                token0 = volatileToken;
                token1 = stableToken;
                amt0 = 0;
                amt1 = supplied;
                borrow0 = borrow_volatile;
                borrow1 = borrow_stable;
            }
            (uint amtSwap, uint amtOut, bool isZeroForOne) = optimalSwap.getOptimalSwapAmt(
                IUniswapV3Pool(lpToken),
                amt0+borrow0,
                amt1+borrow1,
                tickLower,
                tickUpper
            ...
            uint min0; uint min1;
            if (isZeroForOne) {
                min0 = (amt0+borrow0-amtSwap)*(PRECISION-slippage)/PRECISION;
                min1 = (amt1+borrow1+amtOut)*(PRECISION-slippage)/PRECISION;
            } else {
                min0 = (amt0+borrow0+amtOut)*(PRECISION-slippage)/PRECISION;
                min1 = (amt1+borrow1-amtSwap)*(PRECISION-slippage)/PRECISION;
            }
            min0 = min0*(PRECISION-slippage)/PRECISION;
            min1 = min1*(PRECISION-slippage)/PRECISION;
            if (positionId==0) {
                positionId = integration.openPosition(
                    ...
                    IUniswapV3Spell.OpenPositionParams(
                        token0, token1, fee,
                        tickLower, tickUpper,
                        amt0, amt1, borrow0, borrow1,
                        min0, min1,
                        amtSwap, amtOut, isZeroForOne,
                        block.timestamp
                    )
                );
            } else {
        ...
        emit Deposit(
            collateralBefore,
            int(getCollateralSize())-collateralBefore,
            balanceBefore,
    ...
    function _decreasePosition(uint256 gradient) internal returns (uint256) {
        ...
        requireAmmCheckPass();
        {
            (, , , uint collateralAmount) = IBankOP(homoraBank).getPositionInfo(positionId);
            if(collateralAmount == 0) return 0;
            uint256 stableBalance0 = IERC20Upgradeable(stableToken).balanceOf(address(this));
            (uint256 debt0, uint256 debt1) = getDebts();
            uint256 tokensToWithdraw = (collateralAmount * gradient) / PRECISION;
            uint256 repay0 = (debt0 * gradient) / PRECISION;
            uint256 repay1 = (debt1 * gradient) / PRECISION;
            if (gradient==PRECISION) {
                repay0 = MAX_INT;
                repay1 = MAX_INT;
            }
            uint withdrawn = uint(balanceBefore)*gradient/PRECISION;
            uint minWithdrawn = withdrawn*(PRECISION-slippage)/PRECISION;

            integration.reducePosition(
                positionId,
                spell,
                IUniswapV3Spell.RemoveLiquidityParams(tokensToWithdraw, repay0, repay1, 0, 0, block.timestamp),
                slippage
            );
            amountWithdrawn = IERC20Upgradeable(stableToken).balanceOf(address(this))-stableBalance0;
    ...
    function _harvest() internal {
        ...
        if (!healthy) return;
        integration.harvestFee(positionId, spell, false, slippage);
    }

    function _swapStableUSDC(address tokenIn) internal {
        uint balance = IERC20Upgradeable(tokenIn).balanceOf(address(this));
        if (USDC==stableToken) return;
        if (balance<100) return;
        address tokenOut = tokenIn==USDC?stableToken:USDC;
        uint expectedOut = balance*uint(10)**ERC20Upgradeable(tokenOut).decimals()/uint(10)**ERC20Upgradeable(tokenIn).decimals();
        uint minOut = expectedOut*(PRECISION-slippage)/PRECISION;

        address poolAddress = factory.getPool(stableToken, USDC, 100);
        if (poolAddress==address(0)) {
            poolAddress = factory.getPool(stableToken, USDC, 500);
        }
        if (poolAddress==address(0)) {
            poolAddress = factory.getPool(stableToken, USDC, 3000);
        }
        if (poolAddress==address(0)) {
            poolAddress = factory.getPool(stableToken, USDC, 10000);
        }

        IUniswapV3Pool swapPool = IUniswapV3Pool(poolAddress);
        ISwapRouter(router).exactInputSingle(
        ISwapRouter.ExactInputSingleParams(tokenIn, tokenOut, swapPool.fee(), address(this), block.timestamp, balance, minOut, 0)
        );
        emit SwappingStables(tokenIn, balance);
    }
    ...
    function harvest() external {
        uint harvestable = getHarvestable();
        if (harvestable<minHarvestRequired) return;
        _onlyAuthorizedActorsOrController();
        uint balanceBefore = IERC20Upgradeable(stableToken).balanceOf(address(this));
        _harvest();
        uint harvested = IERC20Upgradeable(stableToken).balanceOf(address(this)) - balanceBefore;
        _deposit();
        harvestedRewards+=harvested;
    ...
    function setTicks(uint multiplier0, uint multiplier1) external {
        _onlyAuthorizedActors();
        exitLiquidity();
    ...
    function withdrawOther(address _asset) external override whenNotPaused returns (uint256 balance) {
        ...
        integration.withdrawOther(_asset);
    ...
    function setLeverage(uint256 _leverage) external {
        require(_leverage>0 && leverage<=500, "Invalid leverage");
        _onlyAuthorizedActors();
        leverage = _leverage;
        exitLiquidity();
        _deposit();
    ...
    function setThresholds(
        uint _minHarvestRequired,
        uint _ammCheckThreshold,
        uint _debtRatioThreshold,
        uint _volatilityThreshold,
        uint _slippage) external {
        _onlyAuthorizedActors();
        minHarvestRequired = _minHarvestRequired;
        ammCheckThreshold = _ammCheckThreshold;
        debtRatioThreshold = _debtRatioThreshold;
        volatilityThreshold = _volatilityThreshold;
        require (_slippage<=PRECISION && _slippage>=0, "Invalid slippage");
        slippage = _slippage;
    }

    function setMinHarvestRequired(uint256 _minHarvestRequired) external {
        minHarvestRequired = _minHarvestRequired;
    }
}
...
interface IOracle {
  ...
  function tokenFactors(address token) external view returns (TokenFactors memory);
...
import "INonfungiblePositionManager.sol";
import "IBankOP.sol";
import "IWUniswapV3Position.sol";
import "IUniswapV3Spell.sol";
import "ISwapRouter.sol";
import "IWETH.sol";
import "TickMath.sol";
...
contract UniswapV3SpellIntegrationOp is BaseIntegration, Ownable {
  using SafeERC20Upgradeable for IERC20Upgradeable;
  using HomoraMath for uint;

  IBankOP bank; // homora bank
  IUniswapV3Factory factory; // uniswap v3 factory
  INonfungiblePositionManager npm; // uniswap v3 position manager
  ISwapRouter router;
  address stableToken;
    ...
    returns (uint positionId)
  {
    ...
    IERC20Upgradeable(_params.token0).safeTransferFrom(msg.sender, address(this), _params.amt0User);
    IERC20Upgradeable(_params.token1).safeTransferFrom(msg.sender, address(this), _params.amt1User);
  ...
  function increasePosition(
    ...
    IWUniswapV3Position.PositionInfo memory posInfo = _getPositionInfo(_positionId);
    ...
    // transfer tokens from user
    IERC20Upgradeable(posInfo.token0).safeTransferFrom(msg.sender, address(this), _params.amt0User);
    IERC20Upgradeable(posInfo.token1).safeTransferFrom(msg.sender, address(this), _params.amt1User);
  ...
  function reducePosition(
    uint _positionId,
    IUniswapV3Spell _spell,
    IUniswapV3Spell.RemoveLiquidityParams memory _params, uint slippage
  ) external onlyOwner {
    IWUniswapV3Position.PositionInfo memory posInfo = _getPositionInfo(_positionId);
    ...
    _convertToStable(_positionId, slippage);
  ...
  function harvestFee(
    uint _positionId,
    IUniswapV3Spell _spell,
    bool _convertWETH, uint slippage
  ) external onlyOwner {
  ...
  function closePosition(
    ...
    IUniswapV3Spell.ClosePositionParams memory _params, uint slippage
  ) external onlyOwner returns (uint) {
    IUniswapV3Spell _spell = IUniswapV3Spell(_spellAddress);
    IWUniswapV3Position.PositionInfo memory posInfo = _getPositionInfo(_positionId);

    bytes memory executeData = abi.encodeWithSelector(_spell.closePosition.selector, _params);
    bank.execute(_positionId, address(_spell), executeData);

    uint stableBalance = _convertToStable(_positionId, slippage);
  ...
  function wrapETH() external onlyOwner {
    if (address(this).balance>0) {
        IWETH(0x4200000000000000000000000000000000000006).deposit{value:address(this).balance}();
    }
  }

  function withdrawOther(address _asset) external onlyOwner returns (uint256 balance) {
    balance = IERC20Upgradeable(_asset).balanceOf(address(this));
    IERC20Upgradeable(_asset).safeTransfer(owner(), balance);
  }

  function _volatilePrice(uint _positionId) public view returns (uint) {
    IWUniswapV3Position.PositionInfo memory posInfo = _getPositionInfo(_positionId);
    IUniswapV3Pool pool = IUniswapV3Pool(
      factory.getPool(posInfo.token0, posInfo.token1, posInfo.fee)
    );
    uint stableDecimals = uint(10)**ERC20Upgradeable(stableToken).decimals();
    address volatileToken = stableToken==pool.token0()?pool.token1():pool.token0();
    uint volatileDecimals = uint(10)**ERC20Upgradeable(volatileToken).decimals();

    IUniswapV3Pool.Slot0 memory slot0 = pool.slot0();
    uint160 sqrtPriceX96 = slot0.sqrtPriceX96;
    if (stableToken==pool.token1()) {
        if (uint(sqrtPriceX96)*uint(sqrtPriceX96)>uint(2**256-1)/1e18) {
            return volatileDecimals*(uint(sqrtPriceX96)*uint(sqrtPriceX96)>>(96 * 2));
        } else {
            return (volatileDecimals*uint(sqrtPriceX96)*uint(sqrtPriceX96)>>(96 * 2));
        }
  ...
  function _convertToStable(uint _positionId, uint slippage) internal returns (uint) {
    IWUniswapV3Position.PositionInfo memory posInfo = _getPositionInfo(_positionId);
    address volatileToken = stableToken==posInfo.token0?posInfo.token1:posInfo.token0;

    if (address(this).balance>0) {
      IWETH(volatileToken).deposit{value:address(this).balance}();
    }
    uint volatileBalance = IERC20Upgradeable(volatileToken).balanceOf(address(this));
    if (volatileBalance>0) {
      uint price = _volatilePrice(_positionId);
      uint volatileDecimals = uint(10)**ERC20Upgradeable(volatileToken).decimals();
      uint expectedOut = volatileBalance*price/volatileDecimals;
      uint minOut = expectedOut*((uint(10)**12)-slippage)/uint(10)**12;
      ensureApprove(posInfo.token0, address(router));
      ensureApprove(posInfo.token1, address(router));
      router.exactInputSingle(
        ISwapRouter.ExactInputSingleParams(volatileToken, stableToken, posInfo.fee, address(this), block.timestamp, volatileBalance, minOut, 0)
      );
    }
    uint stableBalance = IERC20Upgradeable(stableToken).balanceOf(address(this));
    return stableBalance;
  }

  function _getPositionInfo(uint _positionId) internal view returns (IWUniswapV3Position.PositionInfo memory) {
  ...
  function _getPositionBalance(uint _positionId) public view returns (uint) {
    if (_positionId==0) return 0;
    IWUniswapV3Position.PositionInfo memory posInfo = _getPositionInfo(_positionId);
    (,,, uint collSize) = bank.getPositionInfo(_positionId);
    if (collSize==0) return 0;
    IUniswapV3Pool pool = IUniswapV3Pool(
      factory.getPool(posInfo.token0, posInfo.token1, posInfo.fee)
    );
    uint debtValueUsd; uint volatileDecimals;
    IUniswapV3Pool.Slot0 memory slot0 = pool.slot0();
    uint160 sqrtRatioX96 = slot0.sqrtPriceX96;
    (uint amount0, uint amount1) = LiquidityAmounts.getAmountsForLiquidity(
        sqrtRatioX96, TickMath.getSqrtRatioAtTick(posInfo.tickLower), TickMath.getSqrtRatioAtTick(posInfo.tickUpper), uint128(collSize));
    {(address[] memory tokens, uint256[] memory debts) = bank.getPositionDebts(_positionId);
    address volatileToken = stableToken==posInfo.token0?posInfo.token1:posInfo.token0;
    volatileDecimals = uint(10)**ERC20Upgradeable(volatileToken).decimals();
    tokens[0]==stableToken?debtValueUsd+=debts[0]:debtValueUsd+=debts[0]*_volatilePrice(_positionId)/volatileDecimals;
    tokens[1]==stableToken?debtValueUsd+=debts[1]:debtValueUsd+=debts[1]*_volatilePrice(_positionId)/volatileDecimals;}
    if (stableToken==pool.token0()) {
        return amount0+amount1*_volatilePrice(_positionId)/volatileDecimals-debtValueUsd;
    } else {
        return amount1+amount0*_volatilePrice(_positionId)/volatileDecimals-debtValueUsd;
    }
  }
  ...
  function _calculateAmountOut(address tokenIn, address tokenOut, uint amountSpecified) public view returns (uint) {
    address poolAddress = factory.getPool(tokenIn, tokenOut, 100);
    if (poolAddress==address(0)) {
        poolAddress = factory.getPool(tokenIn, tokenOut, 500);
    }
    if (poolAddress==address(0)) {
        poolAddress = factory.getPool(tokenIn, tokenOut, 3000);
    }
    if (poolAddress==address(0)) {
        poolAddress = factory.getPool(tokenIn, tokenOut, 10000);
    }
    ...
    SwapState memory state = SwapState({
        amountSpecifiedRemaining: int(amountSpecified),
        amountCalculated: 0,
    ...
    while (state.amountSpecifiedRemaining != 0 && state.sqrtPriceX96 != sqrtPriceLimitX96) {
      ...
      state.amountCalculated = int(state.amountCalculated-int(step.amountOut));
      ...
      if (state.sqrtPriceX96 == step.sqrtPriceNextX96) {
        ...
        if (step.initialized) {
          ...
          int128 liquidityNet;
          (uint128 temp,,,,,,,) = pool.ticks(step.tickNext);
          liquidityNet = int128(temp);
  ...
  function _getPositionTokens(uint _positionId) public view returns (uint, uint) {
    if (_positionId==0) return (0, 0);
    IWUniswapV3Position.PositionInfo memory posInfo = _getPositionInfo(_positionId);
    (,,, uint collSize) = bank.getPositionInfo(_positionId);
    if (collSize==0) return (0, 0);
    IUniswapV3Pool pool = IUniswapV3Pool(
      factory.getPool(posInfo.token0, posInfo.token1, posInfo.fee)
    );
  ...
  /// Uniswap-V3 modified library functions
  ...
  /// @param poolAddress Address of pool being used
...
interface IWUniswapV3Position is IERC1155, IGovernable {
  ...
  function positionManager() external view returns (INonfungiblePositionManager positionManager);
...
library SqrtPriceMath {
    ...
    function customMulDiv(uint a, uint b, uint160 c, uint160 d) internal pure returns (uint) {
        uint MAX = 2**255-1;
        uint lowerDenominator = c<d?c:d;
        uint higherDenominator = c<d?d:c;
        uint higherNumerator = a>b?a:b;
        uint lowerNumerator = a>b?b:a;
        if (a>MAX/b) {
            return lowerNumerator*(higherNumerator/lowerDenominator)/higherDenominator;
        } else {
            return (a*b/c)/d;
        }
    }
    ...
    function getAmount0Delta(
        ...
        return
            roundUp
                ...
                : customMulDiv(numerator1, numerator2, sqrtRatioBX96, sqrtRatioAX96);
    }
...
abstract contract BaseStrategy is Pausable, SettAccessControl {
    ...
    address public constant USDC = 0x7F5c764cBc14f9669B88837ca1490cCa17c31607; //MAINNET USDC
