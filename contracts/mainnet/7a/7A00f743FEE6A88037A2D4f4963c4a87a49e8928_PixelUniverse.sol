...
import { OwnedRoles } from "@turbo-eth/solbase-sol/src/auth/OwnedRoles.sol";
import { PixelStore } from "./PixelStore.sol";
import { PixelPooly } from "./PixelPooly.sol";
import { IPixelCharacter } from "./interfaces/IPixelCharacter.sol";

contract PixelUniverse is OwnedRoles {
  address public erc721KInstance;
  address public pixelStoreInstance;

  uint8 constant NUM_LAYERS = 12;

  IPixelCharacter.Layer[] private layers = new IPixelCharacter.Layer[](NUM_LAYERS);

    /// @notice Map to track if address used free mint
  ...
  constructor(
    ...
    address _pixelStoreInstance,
    address _owner
  ) OwnedRoles() {
    erc721KInstance = _erc721KInstance;
    pixelStoreInstance = _pixelStoreInstance;
    _initializeOwner(_owner);
    for (uint256 index = 0; index < NUM_LAYERS; index++) {
      layers[index] = IPixelCharacter.Layer(index);
    }
  }
  ...
  function mintWithFrames(address to, uint8[] memory frames)
    external
    payable
    returns (uint256 tokenId)
  {
    require(msg.sender == to, "PixelUniverse:self-mint-required");
    require(frames.length <= NUM_LAYERS, "PixelUniverse:invalid-frame-count");

    if (msg.value < 0.05 ether){
      // check if they used their free mint
      require(!freeMinted[msg.sender], "PixelUniverse:already-used-free-mint");

      // if this is their free mint then add to the map
      freeMinted[msg.sender] = true;
    }

    tokenId = PixelPooly(erc721KInstance).mint(to);
    PixelStore(pixelStoreInstance).unlockBatch{ value: msg.value }(tokenId, layers, frames);
  }
