...
contract DAONFT is Ownable {
    ...
    function batchMint(address[] memory targets, uint256[] memory tokenIds) public onlyOwner {
        require(targets.length == tokenIds.length, "Invalid array lengths");
    ...
    function batchBurn(address[] memory targets, uint256[] memory tokenIds) public onlyOwner {
        require(targets.length == tokenIds.length, "Invalid array lengths");
