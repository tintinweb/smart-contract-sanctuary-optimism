...
contract Trending is ERC721, Ownable {
    ...
    function mint(uint256 quantity, address recipient) payable external {
        require(msg.value >= 0.0001 ether, 'BAD');

        uint256 _count_ = _count;
        for (uint256 i = 0; i < quantity; i++) {
            _mint(recipient, ++_count_);
        }
        _count = _count_;
    }
    ...
    function tokenURI(uint256 id) public view override returns (string memory) {
        require(ownerOf(id) != address(0), 'NA');

        return string.concat(
            "data:application/json;base64,",
            Base64.encode(
                abi.encodePacked(
                    '{ "name": "!FUN (again)", "description": "From the creators of Trending !FUN comes a brand new NFT: Trending !FUN (again). The background is blue, the text is new, and it costs ETH :D", "image": "' "data:image/svg+xml;base64,",
                    Base64.encode(
                        bytes(
                            '<svg width="500" height="500" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" style="background-color:blue"> <text fill="white" x="2.5" y="12" font-size="1" font-weight="bold">trend it</text></svg>'
                        )
                    ),
                    '"}'
                )
