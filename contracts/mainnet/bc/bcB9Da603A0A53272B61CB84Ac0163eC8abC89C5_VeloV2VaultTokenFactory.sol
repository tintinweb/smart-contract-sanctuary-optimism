...
interface IVeloV2VaultTokenFactory {
...
import "./interfaces/IVeloV2VaultToken.sol";
...
contract VeloV2VaultToken is IVeloV2VaultToken, PoolToken {
...
import "@openzeppelin/contracts/ownership/Ownable.sol";
import "./interfaces/IBaseV1Pair.sol";
import "./VeloV2VaultToken.sol";
import "./interfaces/IVeloV2VaultTokenFactory.sol";

contract VeloV2VaultTokenFactory is Ownable, IVeloV2VaultTokenFactory {
    address public optiSwap;
    address public router;
    address public voter;
    address public pairFactory;
        ...
        returns (address vaultToken)
    {
        ...
        bytes memory bytecode = type(VeloV2VaultToken).creationCode;
        assembly {
            vaultToken := create2(0, add(bytecode, 32), mload(bytecode), _underlying)
        }
        VeloV2VaultToken(vaultToken)._initialize(
            _underlying,
            optiSwap,
            router,
            voter,
