...
contract RewardDeriver is TwoStepOwnable, IRewardDeriver {
     ...
     */
    address[] public maxRewardERC721s;
     ...
     */
    function getRewardInOP(Order memory _order) external view override returns (uint256) {
        if (
    ...
    /**
     * @notice Determines if the caller should earn the base reward or the max reward for this
     *         order. To do this, it checks if the caller owns an NFT from at least one of the
     *         approved ERC721 collections.
     *
     * @return True if the caller owns an NFT from at least one of the ERC721 collections.
     */
    function _isEligibleForMaxReward() internal view returns (bool) {
        for (uint256 i = 0; i < maxRewardERC721s.length; i++) {
            uint256 balance = IERC721(maxRewardERC721s[i]).balanceOf(msg.sender);
            if (balance > 0) {
                return true;
            }
        }
