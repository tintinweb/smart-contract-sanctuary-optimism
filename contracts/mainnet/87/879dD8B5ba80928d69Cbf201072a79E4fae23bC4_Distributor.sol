...
contract Distributor is Policy {
  ...
  address public immutable pegasusNFTAddress; // staking
  ...
  constructor(
    ...
    treasury = _treasury;
    require(_PEGA != address(0), "Zero address: PEGA");
    PEGA = _PEGA;
    require(_pegasusNFT != address(0), "Zero address: PegaPal");
    pegasusNFTAddress = _pegasusNFT;
  ...
  function distribute() external {
    require(msg.sender == staking, "Only staking contract can call distribute");
    for (uint256 i = 0; i < info.length; i++) {
      if (info[i].rate > 0) {
        ITreasury(treasury).mintRewards( // mint and send from treasury to staking contract
          info[i].recipient,
  ...
  function nextRewardAt(uint256 _rate) public view returns (uint256) {
    //Geto total supply of $Pega = All NFT value + total $Pega tokens
    uint256 pegaSupply = INFT(pegasusNFTAddress).totalPegaValue().add(
      IERC20(PEGA).totalSupply()
    );
    return pegaSupply.mul(_rate).div(rateDenominator); //used to be regular ohm
  }
