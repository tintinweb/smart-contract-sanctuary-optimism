...
contract TargetRateVault is ERC4626, Ownable {
    ...
    function getClockIncrement() public view returns (uint256) {
        ...
        return SafeCast.toUint192(((targetTokensPerDay/(1 days))*nSecondsSinceLastUpdate)/(Math.max(IERC20(STAKING_TOKEN).balanceOf(address(this)), targetLockedStakingToken/MAXIMUM_BONUS_RATE_MULTIPLIER)));
    }

    function pendingRewards(address owner) public virtual view returns (uint256) {

        uint256 oldClock = clockValue;

        uint256 newClock = clockValue + getClockIncrement();

        uint256 amountToMintUnadjusted = (newClock - oldClock)*((balanceOf(owner)*totalAssets())/totalSupply());
    ...
    function harvest(address owner, address receiver) virtual public senderOwnsAccount(owner) {
        ...
        uint256 amountToMintUnadjusted = (clockValue - oldClock)*((balanceOf(owner)*totalAssets())/totalSupply());
