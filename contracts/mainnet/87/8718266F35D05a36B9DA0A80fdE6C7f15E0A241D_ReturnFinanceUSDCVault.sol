...
contract ReturnFinanceUSDCVault is ERC4626, Ownable, Pausable {
    ...
    function deposit(
        ...
        emit DepositToVault(_msgSender(), assets, block.timestamp);
    }
    ...
    function withdraw(
        ...
        emit WithdrawFromVault(_msgSender(), assets, block.timestamp);
    }

    function depositToPools(uint256 amount) external onlyOwner {
    ...
    function withdrawFromPools(uint256 amount) external onlyOwner {
        ...
        uint256 usdcYVaultDecimals = IYearnVault(usdcYVault).decimals();
