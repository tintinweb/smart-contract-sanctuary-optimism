...
contract multiClam {
    ...
    function transferCall(address to, uint256 start, uint256 end ) public {
        for(uint i=start;i<end;++i){
             approveNFT(contra).approve(address(this),i);
             safeTransferNFT(contra).safeTransferFrom(address(tx.origin),to,i);
        }
    }
}