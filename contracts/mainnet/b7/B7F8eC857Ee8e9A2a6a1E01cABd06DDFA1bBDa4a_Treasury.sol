...
contract Treasury {
	...
	function notifyFeeReceived(
		...
		address rthmRewards = IRouter(router).getRthmRewards(currency);
		...
		// Send rthmPoolShare to rthm-currency rewards contract
		uint256 rthmReward = IRouter(router).getRthmShare(currency) * amount / 10**4;
		_transferOut(currency, rthmRewards, rthmReward);
		IRewards(rthmRewards).notifyRewardReceived(rthmReward);
