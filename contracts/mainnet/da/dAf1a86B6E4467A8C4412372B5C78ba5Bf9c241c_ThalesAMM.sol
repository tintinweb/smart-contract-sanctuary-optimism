...
contract ThalesAMM is ProxyOwned, ProxyPausable, ProxyReentrancyGuard, Initializable {
    ...
    // Used for migration of options once the first round of LP starts
    function transferOptions(address market, address destination) external onlyOwner {
        (IPosition up, IPosition down) = IPositionalMarket(market).getOptions();
        IERC20(address(up)).safeTransfer(destination, IERC20(address(up)).balanceOf(address(this)));
        IERC20(address(down)).safeTransfer(destination, IERC20(address(down)).balanceOf(address(this)));
    }
    ...
    function setImpliedVolatilityPerAsset(bytes32 asset, uint _impliedVolatility) external {
        ...
        require(_impliedVolatility > ONE.mul(5) && _impliedVolatility < ONE.mul(300), "IV outside min/max range!");
