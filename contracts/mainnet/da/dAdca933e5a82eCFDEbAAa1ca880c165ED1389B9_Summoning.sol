...
contract Summoning is Ownable, ReentrancyGuard {
    ...
    //token bridges to try in order when swapping, first three are immutably weth, usdc, op
    ...
    function setBridge(address token, address bridge) external onlyAuth {
        // Checks
        require(
            token != axo && token != weth && token != bridge,
    ...
    function convertMultiple(
        ...
        // final step is to swap all weth to AXO and disperse it
        i = IERC20(weth).balanceOf(address(this)); //reuse i for amount to save gas
        bool success;
        if (devCut > 0) {
            uint cut = i.mul(devCut).div(10000);
            IERC20(weth).safeTransfer(devAddr, cut);
            i = i.sub(cut);
        }
        (, success) = _swap(weth, axo, i);
        if(!success)
