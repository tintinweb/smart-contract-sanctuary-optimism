...
/**
 * OptiDogeBot interface for accepting transfer hooks
 */
interface IOptiDogeBot {
    function txHook(address caller, address sender, address receiver, uint256 amount) external;
}

/**
 * Allow external contracts (OptiDogeBots) to hook into OptiDoge transactions
 */
abstract contract OptiDogeBotController is Ownable {
    struct OptiDogeBotInfo {
        bool bot;
        uint256 adrIndex;
    }
    
    mapping (address => OptiDogeBotInfo) _botsInfo;
    address[] _OptiDogeBots;
    uint256 _OptiDogeBotsCount;
    
    /**
     * Returns array of OptiDogeBots
     */
    function getBots() public view returns (address[] memory) {
        return _OptiDogeBots;
    }
    
    /**
     * Returns OptiDogeBot count
     */
    function getBotCount() public view returns (uint256) {
        return _OptiDogeBotsCount;
    }
    
    /**
     * Check if address is registered as OptiDogeBot
     */
     ...
     */
    function addOptiDogeBot(address bot) external owned {
        require(isContract(bot));
        _botsInfo[bot].bot = true;
        _botsInfo[bot].adrIndex = _OptiDogeBots.length;
        _OptiDogeBots.push(bot);
        _OptiDogeBotsCount++;
    }
     ...
     */
    function removeOptiDogeBot(address bot) external owned {
        require(isBot(bot));
        ...
        _OptiDogeBots[i] = _OptiDogeBots[_OptiDogeBots.length-1];
        _botsInfo[_OptiDogeBots[i]].adrIndex = i;
        _OptiDogeBots.pop();
    }
    
    /**
     * Call all OptiDogeBot hooks
     */
    function OptiDogeBotTxHook(address sender, address receiver, uint256 amount) internal {
        if(getBotCount() == 0){ return; }
        for(uint256 i=0; i<_OptiDogeBots.length; i++){ 
            /* 
             * Using try-catch ensures that any errors / fails in one of the OptiDogeBot contracts will not cancel the overall transaction
             */
            try IOptiDogeBot(_OptiDogeBots[i]).txHook(msg.sender, sender, receiver, amount) {} catch {}
        }
    }
...
interface IOptiDogeDistributive {
 ...
 */
abstract contract OptiDogeDistributive is IOptiDogeDistributive, ERC20, Ownable, TracksHolders {
    using SafeMath for uint256;
...
/* > OptiDoge governs exchanges & pools globally */

/**
 * This contract burns tokens on transactions
 */
abstract contract Burnable is OptiDogeDistributive {
    using SafeMath for uint256;
 ...
 */
abstract contract WAGMI20 is IWAGMI20, Burnable, OptiDogeBotController {
    using SafeMath for uint256;
    ...
    /**
    * burns OptiDoge to the dead address 
    */
    ...
    */
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        // call any hooks
        OptiDogeBotTxHook(sender, recipient, amount);
...
contract OptiDoge is WAGMI20, RecordsCreation {
    using SafeMath for uint256;
