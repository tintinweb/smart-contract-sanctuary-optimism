    ...
    IObservabilityEvents
{
    ...
    bytes32 public constant override CREATE_TYPEHASH =
        keccak256(
            "Create(address owner,string name,string symbol,string description,string imageURI,string contentURI,string contractURI,uint256 limit,uint256 price,bool useRenderer,uint256 nonce)"
        );
    ...
    {
        _deployCloneAndInitialize(msg.sender, edition_, address(0));
    }
    ...
    function createWithSig(
        ...
        _deployCloneAndInitialize(owner_, edition_, recipient);
