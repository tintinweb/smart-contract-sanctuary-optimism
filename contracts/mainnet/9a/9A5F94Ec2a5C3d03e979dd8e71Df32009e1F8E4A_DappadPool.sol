...
contract DappadPool is Ownable, Pausable {
    ...
    function getUserStatus(address userAddress, uint256 amount) public view returns (Status) {
        if (paused()) {
            return Status.PAUSED;
        }
        if (block.timestamp < saleStart) {
            return Status.SALE_NOT_STARTED;
        }
        if (block.timestamp >= saleEnd) {
            return Status.SALE_ENDED;
        }
        if (totalReceived.add(amount) > maxCap) {
            return Status.TOTAL_REACHED;
        }
        uint256 userTier = userDetails[userAddress].tier;
        if (userTier == 0 || userTier > tiersLength) {
            return Status.INVALID_TIER;
        }
        uint256 expectedAmount = amount.add(userDetails[userAddress].investedAmount);
        if (expectedAmount < tierDetails[userTier].minUserCap) {
            return Status.MIN_USER_CAP_NOT_REACHED;
        }
        if (expectedAmount > tierDetails[userTier].maxUserCap) {
            return Status.MAX_USER_CAP_REACHED;
        }
        if (expectedAmount > tierDetails[userTier].maxTierCap) {
            return Status.MAX_CAP_REACHED;
        }
