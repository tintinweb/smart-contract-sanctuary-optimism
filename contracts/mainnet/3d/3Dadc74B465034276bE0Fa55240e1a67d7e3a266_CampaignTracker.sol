...
import { IRewardWrapper } from "./IRewardWrapper.sol";
import { IRewardDeriver } from "./IRewardDeriver.sol";
import { Initializable } from "@openzeppelin/contracts-upgradeable/proxy/utils/Initializable.sol";

contract CampaignTracker is TwoStepOwnable, ICampaignTracker, Initializable {
    event CampaignSet(
    ...
    event CampaignRewardToggled(bytes32 indexed campaignId, string campaignString, bool isActive);

    /**
     * @notice Emitted when a OP OG NFT is added to the array.
     *
     * @param erc721 New ERC721 address added.
     */
    event OptimismOGERC721Added(address indexed erc721);

    /**
     * @notice Emitted when a OP OG NFT is removed from the array.
     *
     * @param erc721 ERC721 address removed.
     */
    event OptimismOGERC721Removed(address indexed erc721);
    ...
    bytes32 public constant BASELINE_CAMPAIGN_ID = keccak256(bytes("BASELINE"));

    bytes32 public constant OPTIMISM_OG_CAMPAIGN_ID = keccak256(bytes("OPOG"));

    IRewardWrapper public rewardWrapper;

    /**
     * @notice Maps a campaign ID to a Campaign struct.
     */
    mapping(bytes32 => Campaign) internal campaigns;

    /**
     * @notice Array of OP OG NFT addresses. Buyers are able to earn rewards for this campaign if
     *         their address owns an NFT from one of the collections in this array.
     */
    address[] internal optimismOGERC721s;

    /**
     * @param _owner             Owner of this contract.
     * @param _rewardWrapper     Address of the RewardWrapper.
     * @param _campaigns         Initial campaigns.
     * @param _optimismOGERC721s Array of OP OG NFTs.
     */
    function initialize(
        address _owner,
        IRewardWrapper _rewardWrapper,
        Campaign[] memory _campaigns,
        address[] memory _optimismOGERC721s
        ...
        for (uint256 i = 0; i < _campaigns.length; i++) {
            ...
            setCampaign(
                ...
                campaign.rewardPerMille,
                campaign.manager,
                campaign.royaltyReceiver,
                campaign.royaltyPerMille,
                campaign.maxAllowanceInOP,
                campaign.isActive
            );
        }

        for (uint256 j = 0; j < _optimismOGERC721s.length; j++) {
            addOptimismOGERC721(_optimismOGERC721s[j]);
        }
    ...
    function setCampaign(
        string memory _campaignString,
        uint256 _rewardPerMille,
        address _manager,
        address _royaltyReceiver,
        uint256 _royaltyPerMille,
        uint256 _maxAllowanceInOP,
        bool _isActive
    ) public onlyOwner {
        bytes32 campaignId = keccak256(bytes(_campaignString));
        IRewardDeriver rewardDeriver = rewardWrapper.rewardDeriver();

        // The following checks ensure that someone malicious cannot purchase their own NFT to
        // profitably drain rewards.
        if (campaignId == BASELINE_CAMPAIGN_ID) {
            uint256 optimismOGRewardPerMille = campaigns[OPTIMISM_OG_CAMPAIGN_ID].rewardPerMille;
            require(
                rewardDeriver.marketplaceFeePerMille() > _rewardPerMille + optimismOGRewardPerMille,
                "CampaignTracker: OP OG plus baseline reward must be less the marketplace fee"
            );
        } else if (campaignId == OPTIMISM_OG_CAMPAIGN_ID) {
            uint256 baselineRewardPerMille = campaigns[BASELINE_CAMPAIGN_ID].rewardPerMille;
            require(
                rewardDeriver.marketplaceFeePerMille() > _rewardPerMille + baselineRewardPerMille,
                "CampaignTracker: OP OG plus baseline reward must be less the marketplace fee"
            );
        } else {
            require(
                _royaltyPerMille > _rewardPerMille,
                "CampaignTracker: royalty per mille must be greater than reward per mille"
            );
        }

        campaigns[campaignId] = Campaign({
            campaignString: _campaignString,
            rewardPerMille: _rewardPerMille,
            manager: _manager,
            royaltyReceiver: _royaltyReceiver,
            royaltyPerMille: _royaltyPerMille,
            maxAllowanceInOP: _maxAllowanceInOP,
            isActive: _isActive
        });
     ...
     */
    function setRewardWrapper(IRewardWrapper _rewardWrapper) public onlyOwner {
        rewardWrapper = _rewardWrapper;
        emit RewardWrapperSet(address(_rewardWrapper));
    }
    ...
    function getRewardAmountInOP(string memory _campaignString, uint256 _considerationAmountInOP)
        ...
        returns (uint256)
    {
        Campaign memory campaign = campaigns[keccak256(bytes(_campaignString))];
        if (!campaign.isActive) {
            return 0;
        }

        uint256 prevRewardSent = rewardWrapper.getRewardSentByCampaignInOP(_campaignString);
        uint256 rewardAmountInOP = (campaign.rewardPerMille * _considerationAmountInOP) / 1000;

        // Return 0 if the campaign will exceed its allowance on this call.
        if (rewardAmountInOP + prevRewardSent > campaign.maxAllowanceInOP) {
            return 0;
        }

        return rewardAmountInOP;
    }

    function toggleRewardsForCampaign(string memory _campaignString) external {
        bytes32 campaignId = keccak256(bytes(_campaignString));
        Campaign storage campaign = campaigns[campaignId];
        require(
            msg.sender == owner() || msg.sender == campaign.manager,
            "CampaignTracker: caller must be owner or campaign manager"
        );

        campaign.isActive = !campaign.isActive;

        emit CampaignRewardToggled(campaignId, _campaignString, campaign.isActive);
    }

    function getCampaign(string memory _campaignString) external view returns (Campaign memory) {
        return campaigns[keccak256(bytes(_campaignString))];
    }

    /**
     * @notice Determines if the specified address owns an OP OG NFT from
     *         at least one of the ERC721 collections.
     *
     * @param _address Address in question.
     *
     * @return True if the address owns an NFT from at least one of the ERC721 collections.
     */
    function isOptimismOGHolder(address _address) public view returns (bool) {
        for (uint256 i = 0; i < optimismOGERC721s.length; i++) {
            // slither-disable-next-line calls-loop
            uint256 balance = IERC721(optimismOGERC721s[i]).balanceOf(_address);
            if (balance > 0) {
                return true;
            }
        }
    ...
    /**
     * @notice Returns the array of OP OG NFT addresses.
     *
     * @return The Optimism OG ERC721 array of addresses.
     */
    function getOptimismOGERC721s() external view returns (address[] memory) {
        return optimismOGERC721s;
    }

    /**
     * @notice Allows the contract owner to add a new OP OG NFT to the array.
     */
    function addOptimismOGERC721(address _erc721) public onlyOwner {
        require(
            ERC165Checker.supportsInterface(_erc721, type(IERC721).interfaceId),
            "CampaignTracker: erc721 must support erc-165 interface"
        );
        for (uint256 i = 0; i < optimismOGERC721s.length; i++) {
            require(optimismOGERC721s[i] != _erc721, "CampaignTracker: erc721 already added");
        }
        optimismOGERC721s.push(_erc721);

        emit OptimismOGERC721Added(_erc721);
    }

    /**
     * @notice Allows the owner to remove an Optimism OG NFT from the array.
     */
    function removeOptimismOGERC721(address _erc721) external onlyOwner {
        uint256 lastIndex = optimismOGERC721s.length - 1;
        uint256 targetIndex = type(uint256).max;
        for (uint256 i = 0; i < optimismOGERC721s.length; i++) {
            if (_erc721 == optimismOGERC721s[i]) {
                targetIndex = i;
                break;
            }
        }

        require(targetIndex != type(uint256).max, "CampaignTracker: array does not contain erc721");

        address erc721ToRemove = optimismOGERC721s[targetIndex];
        address lastERC721 = optimismOGERC721s[lastIndex];

        // Move the last element to the slot of the address to delete
        optimismOGERC721s[targetIndex] = lastERC721;

        // Delete the last element of the array
        optimismOGERC721s.pop();

        emit OptimismOGERC721Removed(erc721ToRemove);
    }
}
...
import { Order, FulfillmentComponent } from "../lib/seaport/lib/ConsiderationStructs.sol";
import { Seaport } from "../lib/seaport/Seaport.sol";
import { IRewardDeriver } from "./IRewardDeriver.sol";

/**
 * @title IRewardWrapper
 * @notice RewardDeriver interface.
 */
interface IRewardWrapper {
    /**
     * @notice Seaport contract.
     */
    function seaport() external returns (Seaport);

    /**
     * @notice RewardDeriver contract.
     */
    function rewardDeriver() external returns (IRewardDeriver);

    /**
     * @notice Fulfills an order in Seaport and rewards the buyer with OP tokens if the order is
     *         eligible.
     *
     * @param order Seaport order struct.
     */
    function fulfillOrder(Order calldata order) external payable;

    /**
     * @notice Attempt to fill a group of orders. Eligible orders will reward buyers with OP tokens.
     *         Any order that is not currently active, has already been fully filled, or has been
     *         cancelled will be omitted from the order and not cause it to revert. Note that a
     *         failing item transfer or an issue with order formatting will cause the entire batch
     *         to fail.
     *
     * @param orders                    The orders to fulfill. Note that both the offerer and the
     *                                  fulfiller must first approve Seaport to transfer any
     *                                  relevant tokens on their behalf.
     * @param offerFulfillments         An array of FulfillmentComponent arrays indicating which
     *                                  offer items to attempt to aggregate when preparing
     *                                  executions.
     * @param considerationFulfillments An array of FulfillmentComponent arrays indicating which
     *                                  consideration items to attempt to aggregate when preparing
     *                                  executions.
     * @param maximumFulfilled          Maximum number of orders to fulfill. Must equal the number
     *                                  of items in the `orders` array. Must be included as a
     *                                  parameter to prevent a "stack too deep" error.
     */
    ...
    /**
     * @notice Calls the RewardDeriver to get the OP token reward amount for the given order and
     *         address.
     *
     * @param _order     Seaport order struct.
     ...
     * @return OP token reward amount.
     */
    function getRewardInOP(Order memory _order, address _recipient) external view returns (uint256);

    function getRewardSentByCampaignInOP(string memory _campaignId) external view returns (uint256);
}
