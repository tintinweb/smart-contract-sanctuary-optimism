...
contract Compliance is OwnableUpgradeable, WhitelistOperatorRole {
    ...
    event Halt(address indexed _safe, uint indexed _jurisdictionId);
    event Resume(address indexed _safe, uint indexed _jurisdictionId);
    event MaxInvestorsChanged(address indexed _safe, uint _limit);
    event MaxInvestorsByJurisdictionChanged(address indexed _safe, uint indexed _jurisdictionId, uint _limit);
    ...
    mapping(address => mapping(uint => InvestorLimit)) public jurisdictionInvestorLimit;

    /**
     * @notice Maps Jurisdiction Id to it's halted status
     */
    mapping(address => mapping(uint => bool)) public jurisdictionHalted;

    mapping(address => uint256) public jurisdictionIndex;

    mapping(uint256 => mapping(uint => mapping(uint => uint64))) internal jurisdictionFlows;
    ...
    mapping(address => mapping(address => mapping(uint => Lockup))) internal userIdLockups;
     ...
     */
    function getAuthorizedUserIdInfo(address _userId, address _safe)
        external
        view
        returns (
            uint jurisdictionId,
        ...
        )
    {
        ...
        return (
            info.jurisdictionId,
            lockupInfo.totalTokensLocked,
            lockupInfo.startIndex,
            lockupInfo.endIndex
        );
    }
    ...
    function getJurisdictionInfo(address _safe, uint256 _jurisdictionId) external view returns(bool halted, uint256 maxInvestor, uint256 currentInvestor){
     ...
     */
    function getUserIdLockup(address _safe, address _userId, uint _lockupIndex)
        external
        view
     ...
     */
    function getLockedTokenCount(address _safe, address _userId)
        external
        view
    ...
    function approveNewUsers(
        ...
        for (uint i = 0; i < length; i++) {
            ...
            require(
                walletInfo[_safe][trader].userId == address(0),
            ...
            walletInfo[_safe][trader] = WalletInfo({
                status: Status.Activated,
                userId: trader
            });
            userInfo[_safe][trader] = UserInfo({
                status: Status.Activated,
    ...
    function addApprovedUserWallets(
        ...
        for (uint i = 0; i < length; i++) {
            ...
            require(
                userInfo[_safe][userId].status != Status.Unknown,
                "USER_ID_UNKNOWN"
            );
            ...
            walletInfo[_safe][newWallet] = WalletInfo({
                status: Status.Activated,
                userId: userId
            });
     ...
     */
    function revokeUserWallets(address _safe,address[] calldata _wallets)
        external
    ...
    function _addLockup(
        ...
        userIdLockups[_safe][_userId][info.endIndex] = Lockup(
     ...
     */
    function processLockups(address _safe, address _userId, uint _maxCount) external {
        UserLockupInfo storage info = userLockupInfo[_safe][_userId];
        require(userInfo[_safe][_userId].status != Status.Unknown, "USER_ID_UNKNOWN");
     ...
     */
    function forceUnlockUpTo(address _safe, address _userId, uint _maxLockupIndex)
        external
        onlyWhitelistOperator(_safe)
    {
        UserLockupInfo storage info = userLockupInfo[_safe][_userId];
    ...
    function _isJurisdictionHalted(address _safe, uint _jurisdictionId) internal view returns(bool){
     ...
     **/
    function halt(address _safe, uint[] calldata _jurisdictionIds, uint[] calldata _expirationTimestamps) external onlyWhitelistOperator(_safe) {
    ...
    function _halt(address _safe, uint _jurisdictionId, uint _until) internal {
     ...
     **/
    function resume(address _safe, uint[] calldata _jurisdictionIds) external onlyWhitelistOperator(_safe){
        uint length = _jurisdictionIds.length;
        for(uint i = 0; i < length; i++){
    ...
    function _resume(address _safe, uint _jurisdictionId) internal {
     ...
     */
    function setInvestorLimit(address _safe, uint _limit) external onlyWhitelistOperator(_safe) {
     ...
     */
    function setInvestorLimitForJurisdiction(address _safe, uint[] calldata _jurisdictionIds, uint[] calldata _limits) external onlyWhitelistOperator(_safe) {
        for(uint i = 0; i<_jurisdictionIds.length; i++){
            uint jurisdictionId = _jurisdictionIds[i];
            uint limit = _limits[i];
            require(limit >= jurisdictionInvestorLimit[_safe][jurisdictionId].current, "LIMIT_SHOULD_BE_LARGER_THAN_CURRENT_INVESTORS");
    ...
    function authorizeTransfer(
        ...
        if (from.userId != to.userId) {
            uint fromJurisdictionId = userInfo[msg.sender][from.userId]
            .jurisdictionId;
            uint toJurisdictionId = userInfo[msg.sender][to.userId].jurisdictionId;
            ...
            uint lockupLength = jurisdictionFlows[index][fromJurisdictionId][toJurisdictionId];
            ...
            if(_from != address(0)){
                uint balance = IERC20(msg.sender).balanceOf(_from);
...
contract AdminOperatorRole is OwnableUpgradeable {
    ...
    modifier onlyAdminOperator() {
        require(
            isAdminOperator(msg.sender),
            "AdminOperatorRole: caller does not have the AdminOperator role"
        );
        _;
    }
