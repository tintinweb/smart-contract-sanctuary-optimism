 ...
 * the specified asset in a lending pool such as Granary.
 *
 * Flash loans are utilized during deposit() to achieve max leverage
 * without any loops.
 */
contract ReaperGranaryLoop is ReaperBaseStrategyv3, IFlashLoanReceiver {
    using SafeERC20Upgradeable for IERC20Upgradeable;

    // 3rd-party contract addresses
    address public constant VELO_ROUTER = 0xa132DAB612dB5cB9fC9Ac426A0Cc215A3423F9c9;
    address public constant ADDRESS_PROVIDER = address(0xdDE5dC81e40799750B92079723Da2acAF9e1C6D6);
    address public constant DATA_PROVIDER = address(0x9546F673eF71Ff666ae66d01Fd6E7C6Dae5a9995);
    ...
    /**
     * @dev Tokens Used:
     * {OP} - Reward token for borrowing/lending that is used to rebalance and re-deposit.
     * {OATH} - Reward token ...
     * {GRAIN} - Reward token ...
    ...
    address public constant OATH = 0x39FdE572a18448F8139b7788099F0a0740f51205;
     ...
     * {oathToUsdcPath} - to swap {OATH} to {USDC} for charging fees
     * {grainToUsdcPath} - to swap {GRAIN} to {USDC} for charging fees
     * {usdcToWantPath} - to swap {USDC} to {want} to compound the rewards
     */
    address[] public opToUsdcPath;
    address[] public oathToUsdcPath;
    ...
    function initialize(
        ...
        IAToken _gWant,
        address[] memory _usdcToWantPath,
        uint256 _targetLtv,
        uint256 _maxLtv
    ) public initializer {
        ...
        oathToUsdcPath = [OATH, USDC];
        
        gWant = _gWant;
        want = IERC20Upgradeable(_gWant.UNDERLYING_ASSET_ADDRESS());

        setUsdcToWantPath(_usdcToWantPath);
    ...
    function ADDRESSES_PROVIDER() public pure override returns (ILendingPoolAddressesProvider) {
        return ILendingPoolAddressesProvider(ADDRESS_PROVIDER);
    }
    ...
    function _leverDownStep(uint256 _totalBorrowReduction) internal returns (uint256) {
        (uint256 supply, uint256 borrow) = getSupplyAndBorrow();
        (, , uint256 threshLtv, , , , , , , ) = IAaveProtocolDataProvider(DATA_PROVIDER)
    ...
    function _harvestCore() internal override returns (uint256 callerFee) {
        uint256 opGained = _claimRewards();
        (uint256 usdcGained, uint256 usdcGainedFromOp) = _swapRewards(opGained);
        _chargePerformanceFees(usdcGained, usdcGainedFromOp);
        _swapUsdcToWant();
        deposit();
    }
     ...
     */
    function _claimRewards() internal returns (uint256 opGained) {
        uint256 opBalanceBefore = IERC20Upgradeable(OP).balanceOf(address(this));
        IRewardsController(REWARDER).claimAllRewardsToSelf(rewardClaimingTokens);
        uint256 opBalanceAfter = IERC20Upgradeable(OP).balanceOf(address(this));
        opGained = opBalanceAfter - opBalanceBefore;
    }

    function _swapRewards(uint256 opGained) internal returns (uint256 usdcGained, uint256 usdcGainedFromOp) {
        ...
        if (isOathRewardActive) {
            ...
            _swap(oathToUsdcPath, oathBalance);
        }
        if (isOpRewardActive) {
            uint256 usdcBalanceBeforeOp = IERC20Upgradeable(USDC).balanceOf(address(this));
            uint256 opFee = opGained * totalFee / PERCENT_DIVISOR;
            _swap(opToUsdcPath, opFee);
            uint256 usdcBalanceAfterOp = IERC20Upgradeable(USDC).balanceOf(address(this));
            usdcGainedFromOp = usdcBalanceAfterOp - usdcBalanceBeforeOp;
        }
        if (isGrainRewardActive) {
            uint256 grainBalance = IERC20Upgradeable(GRAIN).balanceOf(address(this));
            _swap(grainToUsdcPath, grainBalance);
        }
     ...
     */
    function _chargePerformanceFees(uint256 usdcGained, uint256 usdcGainedFromOp) internal {
        uint256 usdcFee = (usdcGained - usdcGainedFromOp) * totalFee / PERCENT_DIVISOR;
        usdcFee = usdcFee + usdcGainedFromOp;
        ...
        if (usdcFee != 0) {
            ...
            IERC20Upgradeable(USDC).safeTransfer(strategistRemitter, feeToStrategist);
        }
    }

    /**
     * @dev Converts all of this contract's {USDC} balance into {want}.
     *      Typically called during harvesting to transform assets back into
     *      {want} for re-depositing.
     */
    function _swapUsdcToWant() internal {
        uint256 usdcBalance = IERC20Upgradeable(USDC).balanceOf(address(this));
        _swap(usdcToWantPath, usdcBalance);
    }
    ...
    function setOathToUsdcPath(address[] memory _path) public {
        _atLeastRole(STRATEGIST);
        require(_path[0] == OATH && _path[_path.length - 1] == address(USDC), "Invalid oathToUsdcPath");
        oathToUsdcPath = _path;
    }

    function setGrainToUsdcPath(address[] memory _path) public {
        _atLeastRole(STRATEGIST);
        require(_path[0] == GRAIN && _path[_path.length - 1] == address(USDC), "Invalid grainToUsdcPath");
        grainToUsdcPath = _path;
    }

    function setUsdcToWantPath(address[] memory _path) public {
        _atLeastRole(STRATEGIST);
        require(_path[0] == USDC && _path[_path.length - 1] == address(want), "Invalid usdcToWantPath");
        usdcToWantPath = _path;
    }
    ...
    function toggleIsOpRewardActive() external {
        _atLeastRole(STRATEGIST);
        isOpRewardActive = !isOpRewardActive;
    }
