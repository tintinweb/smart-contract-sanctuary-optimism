...
library SolidlyOracleLibrary {
    ...
    function currentCumulativePrices(
        ...
        price0Cumulative = ISolidlyPair(pair).reserve0CumulativeLast();
        price1Cumulative = ISolidlyPair(pair).reserve1CumulativeLast();

        // if time has elapsed since the last update on the pair, mock the accumulated price values
        (uint112 reserve0, uint112 reserve1, uint32 blockTimestampLast) = ISolidlyPair(pair).getReserves();
...
contract PiOracle {
    ...
    constructor(ISolidlyPair _lp, address _pi) public {
        PiToken = _pi;
        lp = _lp;
        ...
        if (_firstToken) {
            priceCumulativeLast = _lp.reserve0CumulativeLast(); // fetch the current accumulated price value (1 / 0)
        } else {
            priceCumulativeLast = _lp.reserve1CumulativeLast(); // fetch the current accumulated price value (0 / 1)
        }
    ...
    function update() external {
        (uint price0Cumulative, uint price1Cumulative, uint32 blockTimestamp) =
            SolidlyOracleLibrary.currentCumulativePrices(address(lp));
