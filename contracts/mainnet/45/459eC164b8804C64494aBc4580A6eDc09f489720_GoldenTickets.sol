...
import { AugmentedERC20 } from "../libraries/token/AugmentedERC20.sol";
...
/// @title GoldenTickets
/// @notice Your admission tickets to the Circus, painted with real gold by Barnaby's 🤡s!
contract GoldenTickets is AugmentedERC20, CircusAccessControlled, IGoldenTickets {
	/* ========== STATE VARIABLES ========== */

	uint256 public ticketsBurnt;

	/* ========== CONSTRUCTOR ========== */

	constructor(ICircusAuthority _authority)
		AugmentedERC20("Golden Tickets for CircusDAO", unicode"🎟️", 9)
		CircusAccessControlled(_authority)
	{}

	/* ========== MUTATIVE FUNCTIONS ========== */

	/// @notice Note that only the treasury can mint new 🎟️, if and only if excess reserves allow for it
	...
	/// @notice Burns a controlled amount of 🎟️, updating the burn counter
	/// @param amount uint256
	...
	/// @notice Burns a controlled amount of 🎟️ on behalf of a given account, updating the burn counter
	/// @param account address
	/// @param amount uint256
	function burnFrom(address account, uint256 amount) external override {
		(bool positive, uint256 decreasedAllowance) = SafeMath.trySub(allowance(account, msg.sender), amount);
		require(positive, unicode"GoldenTickets::burnFrom: cannot burn those 🎟️!");
	...
	/// @notice Allows governance to update this token metadata (name & symbol) with its purpose or the DAO theme
	/// @param name string
	/// @param symbol string
	function changeMetadata(string memory name, string memory symbol) external onlyGovernor {
		_name = name;
		_symbol = symbol;
	}
}
...
/// @dev ERC20 template contract augmented with permit add-ons and state variables exposed internally. Thread carefully
abstract contract AugmentedERC20 is EIP712, IERC20Metadata, IERC20Permit {
	/* ========== DEPENDENCIES ========== */
	...
	/// @dev Sets the initial values for {name}, {symbol} and {decimals}
	constructor(
		string memory name_,
	...
	/// @dev Returns the symbol for the token, usually a shorter version of the name
	 ...
	 * For example, if `decimals` equals `2`, a balance of `505` tokens should be displayed as `5.05` (`505 / 10 ** 2`).
	 *
	 * Tokens usually opt for a value of 18, imitating the relationship between Ether and Wei.
	 *
	 * NOTE: This information is only used for _display_ purposes: it in no way affects the arithmetic of the contract,
	...
	function transferFrom(
		...
		require(currentAllowance >= amount, "AugmentedERC20::transferFrom: transfer amount exceeds allowance");
		unchecked {
	...
	function decreaseAllowance(address spender, uint256 subtractedValue) public virtual returns (bool) {
		...
		require(currentAllowance >= subtractedValue, "AugmentedERC20::decreaseAllowance: decreased allowance below zero");
		unchecked {
	...
	function permit(
		...
		require(block.timestamp <= deadline, "AugmentedERC20::permit: expired deadline"); // solhint-disable-line not-rely-on-time

		bytes32 structHash = keccak256(abi.encode(_PERMIT_TYPEHASH, owner, spender, value, _useNonce(owner), deadline));
		bytes32 hash = _hashTypedDataV4(structHash);

		address signer = ECDSA.recover(hash, v, r, s);
		require(signer == owner, "AugmentedERC20::permit: invalid signature");
	 ...
	 *
	 * This internal function is equivalent to {transfer}, and can be used to e.g. implement token fees, slashing, etc.
	 *
	...
	function _transfer(
		...
		require(sender != address(0), "AugmentedERC20::_transfer: transfer from the zero address");
		require(recipient != address(0), "AugmentedERC20::_transfer: transfer to the zero address");

		_beforeTokenTransfer(sender, recipient, amount);

		uint256 senderBalance = _balances[sender];
		require(senderBalance >= amount, "AugmentedERC20::_transfer: transfer amount exceeds balance");
		unchecked {
	...
	function _mint(address account, uint256 amount) internal virtual {
		require(account != address(0), "AugmentedERC20::_mint: mint to the zero address");
	...
	function _burn(address account, uint256 amount) internal virtual {
		require(account != address(0), "AugmentedERC20::_burn: burn from the zero address");

		_beforeTokenTransfer(account, address(0), amount);

		uint256 accountBalance = _balances[account];
		require(accountBalance >= amount, "AugmentedERC20::_burn: burn amount exceeds balance");
		unchecked {
	 ...
	 *
	 * This internal function is equivalent to `approve`, and can be used to e.g. set allowances in subsystems, etc.
	 *
	...
	function _approve(
		...
		require(owner != address(0), "AugmentedERC20::_approve: approve from the zero address");
		require(spender != address(0), "AugmentedERC20::_approve: approve to the zero address");
