...
contract SynthSwap is ISynthSwap, Owned, ReentrancyGuard {
    ...
    bytes32 private constant TRACKING_CODE = "CONDUIT";

    event SwapInto(address indexed from, uint256 amountReceived);
    event SwapOutOf(address indexed from, uint256 amountReceived);
    event Received(address from, uint256 amountReceived);
    ...
    function swapInto(bytes32 _destSynthCurrencyKey, bytes calldata _data)
        external
        payable
        override
        returns (uint256)
    {
        (uint256 amountOut,) = swapOn1inch(_data, false);

        // if destination synth is NOT sUSD, swap on Synthetix is necessary
        if (_destSynthCurrencyKey != sUSD_CURRENCY_KEY) {
            amountOut = swapOnSynthetix(amountOut, sUSD_CURRENCY_KEY, _destSynthCurrencyKey);
        }
    ...
    /// @inheritdoc ISynthSwap
    function swapOutOf(bytes32 _sourceSynthCurrencyKey, uint256 _sourceAmount, bytes calldata _data)
        external
        override
        nonReentrant
        returns (uint256)
    {
        ...
        if (_sourceSynthCurrencyKey != sUSD_CURRENCY_KEY) {
            swapOnSynthetix(_sourceAmount, _sourceSynthCurrencyKey, sUSD_CURRENCY_KEY);
        }

        (uint256 amountOut, address dstToken) = swapOn1inch(_data, true);
        ...
        uint256 remainingBalanceSUSD = sUSD.balanceOf(address(this));
    ...
    function uniswapSwapOutOf(
        ...
        uint256 _amountOfSynth,
        uint256 _expectedAmountOfSUSDFromSwap,
        bytes calldata _data
        ...
        // if source synth is NOT sUSD, swap on Synthetix is necessary
        if (_sourceSynthCurrencyKey != sUSD_CURRENCY_KEY) {
            swapOnSynthetix(_amountOfSynth, _sourceSynthCurrencyKey, sUSD_CURRENCY_KEY);
        }
    ...
    function synthetix() internal view returns (ISynthetix) {
        return ISynthetix(addressResolver.requireAndGetAddress(CONTRACT_SYNTHETIX, "Could not get Synthetix"));
    }
    ...
    function swapOn1inch(bytes calldata _data, bool _areTokensInContract) internal returns (uint256, address) {
        // decode _data for 1inch swap
        (IAggregationExecutor executor, IAggregationRouterV4.SwapDescription memory desc, bytes memory routeData) =
            abi.decode(_data, (IAggregationExecutor, IAggregationRouterV4.SwapDescription, bytes));
        ...
        // execute 1inch swap
        (uint256 amountOut,) = router.swap{value: msg.value}(executor, desc, routeData);
    ...
    function swapOnSynthetix(uint256 _amount, bytes32 _sourceSynthCurrencyKey, bytes32 _destSynthCurrencyKey)
        internal
        returns (uint256)
    {
        // execute Synthetix swap
        uint256 amountOut = synthetix().exchangeWithTracking(
            _sourceSynthCurrencyKey, _amount, _destSynthCurrencyKey, volumeRewards, TRACKING_CODE
        );
