...
import "./interfaces/IVeloVotingEscrow.sol";
import "./interfaces/IVeloVoteFarmer.sol";
...
contract VeloVoteManager is UUPSUpgradeable, AccessControlEnumerableUpgradeable, PausableUpgradeable {
    ...
    address public strategyUnderlyingImpl;
    ...
    /* ========== ADMIN ========== */
    function setGaugesAndWeights(address[] calldata _gauges, uint256[] calldata _weights, uint256 start, uint256 end) external {
        _atLeastRole(KEEPER);
        uint256 total;
        ...
        // Update managed strats
        for (uint256 i = start; i <= end; i = _uncheckedInc(i)) {
            IVeloVoteFarmer(strategies.at(i)).setGaugesAndWeights(_gauges, _weights);
        }
    }

    function updateSwapPath(address[] memory _path, uint256 start, uint256 end) external {
        ...
        // Update managed strats
        for (uint256 i = start; i <= end; i = _uncheckedInc(i)) {
            IVeloVoteFarmer(strategies.at(i)).updateSwapPath(_path);
        }
        emit SwapPathUpdated(_path[0], _path[_path.length - 1]);
    }

    function updateTotalFee(uint256 _totalFee, uint256 start, uint256 end) external {
        _atLeastRole(STRATEGIST);
        ...
        // Update managed strats
        for (uint256 i = start; i <= end; i = _uncheckedInc(i)) {
            IVeloVoteFarmer(strategies.at(i)).updateTotalFee(_totalFee);
        }
        emit TotalFeeUpdated(totalFee);
    }

    function addReward(address _reward, uint256 start, uint256 end) external {
        _atLeastRole(STRATEGIST);
        uint256 stratLen = strategies.length();
        if (end >= stratLen) {
            end = stratLen - 1;
        ...
        // Update managed strats
        for (uint256 i = start; i <= end; i = _uncheckedInc(i)) {
            IVeloVoteFarmer(strategies.at(i)).addReward(_reward);
        }
    }

    function removeReward(address _reward, uint256 start, uint256 end) external {
        _atLeastRole(STRATEGIST);
        uint256 stratLen = strategies.length();
        if (end >= stratLen) {
            end = stratLen - 1;
        ...
        // Update managed strats
        for (uint256 i = start; i <= end; i = _uncheckedInc(i)) {
            IVeloVoteFarmer(strategies.at(i)).removeReward(_reward);
        }
    }

    function setNftCap(uint256 _cap, uint256 start, uint256 end) external {
        _atLeastRole(STRATEGIST);
        uint256 stratLen = strategies.length();
        if (end >= stratLen) {
            end = stratLen - 1;
        ...
        // Update managed strats
        for (uint256 i = start; i <= end; i = _uncheckedInc(i)) {
            IVeloVoteFarmer(strategies.at(i)).setNftCap(_cap);
        }
    }
    ...
    function updateImplementation(address _newImplementation, uint256 start, uint256 end) external {
        _atLeastRole(STRATEGIST);
        uint256 stratLen = strategies.length();
        if (end >= stratLen) {
            end = stratLen - 1;
        ...
        strategyUnderlyingImpl = _newImplementation;
        for(uint256 i = start; i < end; i = _uncheckedInc(i)) {
            IERC1967ProxyClone(strategies.at(i)).upgradeTo(strategyUnderlyingImpl);
        }
    }
    ...
    function _deployStrategy() internal returns (address strategy) {
        strategy = Clones.clone(strategyImpl);
        IERC1967ProxyClone(strategy).initializeProxy(strategyUnderlyingImpl, new bytes(0));
        IVeloVoteFarmer(strategy).initialize(strategists, treasury);
        strategies.add(strategy);
        uint256 lastStratId = strategies.length() - 1;
        _synchronizeStrategies(lastStratId, lastStratId);
    }
